...
from chulk import Agent, AgentConfig, AgentEvent, AgentHandle, AsyncAgent, AsyncAgentHandle, AsyncChatAgent, AgentPreset, ChatAgent, Capabilities, MCP, PermissionDecision, PermissionDecisionRecord, PermissionRequest, PlanResult, PlanStepVerification, PlanStepVerificationRequest, RunResult, Skills, Tool, ToolPermissionLevel, Tools, agent, async_agent, skills, tool, tools, __version__
from chulk._version import __version__ as source_version
from chulk.config import DEFAULT_DEEPSEEK_MODEL, DEFAULT_LOCAL_MODEL, DEFAULT_MODEL, DEFAULT_MOONSHOT_MODEL, load_config
from chulk.core.actions import FinalAnswerAction
from chulk.llm import FallbackChain, LLMActionResult, LLMCapabilities, LLMClient, LLMError, LLMModelCapabilities, LocalProvider, MoonshotProvider, PlanningToolAvailability
...
from chulk.tools import PermissionDecision as ToolsPermissionDecision, PermissionDecisionRecord as ToolsPermissionDecisionRecord, PermissionRequest as ToolsPermissionRequest, ToolFailureKind, ToolRegistry, ToolPermissionLevel as ToolsToolPermissionLevel
from tests.support import RepeatingFakeLLMClient

FakeLLMClient = RepeatingFakeLLMClient
...
class HostedMCPRecordingLLM(LLMClient):
    ...
    def complete_action(self, messages: list[dict[str, str]], **kwargs) -> LLMActionResult:
        ...
        return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='hosted mcp captured'), raw_response=json.dumps({'type': 'final_answer', 'content': 'hosted mcp captured'}))
...
def test_governed_skill_usage_requires_host_confirmed_success(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'reviewed'})]), tools=[])
...
def test_public_learning_review_uses_restricted_proposal_flow(tmp_path):
    client = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'done'}), json.dumps({'decision': 'propose', 'rationale': 'The preference is durable.', 'proposals': [{'kind': 'memory_create', 'rationale': 'Explicit preference.', 'content': 'User prefers concise answers.', 'confidence': 1.0, 'verification_steps': []}]})])
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=client, tools=[], skills=[])
...
def test_public_learning_auto_approval_requires_constructor_opt_in(tmp_path):
    client = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'done'}), json.dumps({'decision': 'propose', 'rationale': 'The preference is durable.', 'proposals': [{'kind': 'memory_create', 'rationale': 'Explicit preference.', 'content': 'User prefers concise answers.', 'confidence': 1.0, 'verification_steps': []}]})])
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=client, tools=[], skills=[], automatic_learning_approval=True)
    result = facade.run_result('Remember my preference.')
    review = facade.review_learning(turn_id=result.turn_id)
...
def test_lowercase_factories_return_public_facades(tmp_path):
    sync_agent = agent(config=AgentConfig(project_root=tmp_path / 'sync'), llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'sync'})]), tools=[], skills=[])
    async_facade = async_agent(config=AgentConfig(project_root=tmp_path / 'async'), llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'async'})]), tools=[], skills=[])
...
def test_public_chat_agent_disables_default_tools_and_skills(tmp_path):
    ...
    handle = ChatAgent(config=config, llm=llm)
...
async def test_public_async_chat_agent_disables_default_tools_and_skills(tmp_path):
    handle = AsyncChatAgent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'async chat'})]))
...
def test_public_agent_preset_chat_disables_default_tools_and_skills(tmp_path):
    config = AgentConfig(project_root=tmp_path)
    handle = Agent(config=config, preset=AgentPreset.chat(), llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'preset chat'})]))
    result = handle.run_result('hello')
...
def test_software_engineer_preset_auto_selects_relevant_project_skill(tmp_path):
    write_skill(tmp_path, 'architecture-review', '# Architecture Review Skill\n\nUse this procedure for architecture review work.\n')
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'architecture reviewed'})])
    handle = Agent(config=config, preset=SoftwareEngineer(), llm=llm, tools=[])
...
def test_public_agent_run_accepts_stream_delta_callback(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    handle = Agent(config=config, llm=StreamingFakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'streamed callback'})]), tools=[], skills=[])
    deltas: list[str] = []
...
def test_public_agent_dispatches_events_from_constructor_and_run(tmp_path):
    ...
    deltas: list[str] = []
    handle = Agent(config=config, llm=StreamingFakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'evented answer'})]), tools=[], skills=[], on_event=constructor_events.append)
...
def test_public_agent_run_result_returns_structured_turn_metadata(tmp_path):
    ...
    def echo_label(label: str) -> str:
        """Echo a label."""
        return f'echo: {label}'
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'echo_label', 'arguments_json': json.dumps({'label': 'sdk'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'structured answer'})])
...
def test_public_agent_run_accepts_adapter_context(tmp_path):
    ...
    response = handle.run('answer from docs', context_sections=[{'id': 'doc-1', 'title': 'Employee handbook', 'content': 'Use PTO before year end.'}], prompt_profile='polp-search', locale='es-ES', extension_metadata={'confidence': 0.9})
...
def test_public_agent_run_result_exposes_extension_metadata(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    handle = Agent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'structured'})]), tools=[], skills=[])
...
def test_public_agent_redacts_streamed_and_final_output(tmp_path):
    ...
    handle = Agent(config=config, llm=StreamingFakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'SECRET answer'})]), tools=[], skills=[], on_event=events.append, redaction_callback=redact)
    deltas = []
    response = handle.run('hello', on_delta=deltas.append)
    assert response == '[redacted] answer'
    assert ''.join(deltas) == '[redacted] answer'
    assert any((event.type == 'run.completed' and event.payload.result.content == '[redacted] answer' for event in events))
...
async def test_public_async_agent_awaits_decorated_tool(tmp_path):
    ...
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'async_label', 'arguments_json': json.dumps({'label': 'public'}), 'plan_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'async done'})])
...
def test_public_sync_agent_cooperatively_cancels_active_turn(tmp_path):
    ...
    class BlockingLLM(LLMClient):
        ...
        def close(self) -> None:
            released.set()
    handle = Agent(config=load_config({'CHULK_PROJECT_ROOT': str(tmp_path)}), llm=BlockingLLM(), tools=[], skills=[])
...
async def test_public_async_agent_approve_awaits_decorated_tool(tmp_path):
    ...
    async def async_label(label: str) -> str:
        ...
        calls.append(label)
        return f'async: {label}'
    llm = FakeLLMClient([_plan_response(_plan_payload('Run async approval tool')), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'async_label', 'arguments_json': json.dumps({'label': 'approved'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'Async tool completed.'})}), json.dumps({'type': 'final_answer', 'content': 'async approval done'})])
    handle = AsyncAgent(config=config, llm=llm, tools=[async_label], skills=[])
...
async def test_public_async_agent_uses_external_plan_step_verifier(tmp_path):
    ...
    async def verifier(request: PlanStepVerificationRequest) -> PlanStepVerification:
        requests.append(request)
        return PlanStepVerification(passed=True, evidence='Host assertion passed.')
    llm = FakeLLMClient([_plan_response(_plan_payload('Verify completion externally')), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'Model completion claim.'})}), json.dumps({'type': 'final_answer', 'content': 'verified'})])
    handle = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[], async_plan_step_verifier=verifier)
...
def test_public_agent_runs_decorated_tool(tmp_path):
    ...
    def echo_label(label: str) -> str:
        """Echo a label."""
        return f'echo: {label}'
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'echo_label', 'arguments_json': json.dumps({'label': 'public'}), 'plan_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'echoed'})])
...
def test_public_permission_callback_allows_confirming_tool(tmp_path):
    ...
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'risky_lookup', 'arguments_json': json.dumps({'value': 'ok'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'approved'})])
...
def test_public_permission_callback_denies_confirming_tool(tmp_path):
    ...
    def risky_lookup(value: str) -> str:
        ...
        calls.append(value)
        return f'looked up {value}'
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'risky_lookup', 'arguments_json': json.dumps({'value': 'nope'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'denied'})])
...
def test_public_confirming_tool_is_denied_without_callback(tmp_path):
    ...
    def risky_shell(value: str) -> str:
        ...
        calls.append(value)
        return 'ran'
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'risky_shell', 'arguments_json': json.dumps({'value': 'run'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'blocked'})])
...
def test_public_agent_can_approve_workspace_shell_tool(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'run_cmd', 'arguments_json': json.dumps({'command': 'printf sdk'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'shell approved'})])
    result = Agent(config=config, llm=llm, tools=[Tools.run_cmd], skills=[], permission_callback=lambda _request, _record: PermissionDecision.ALLOW).run_result('run shell')
...
def test_public_agent_can_allowlist_skills_by_catalog_name(tmp_path):
    ...
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'other ignored'}), json.dumps({'type': 'final_answer', 'content': 'review loaded'})])
    ...
    assert second_system_prompt.count('<description>Use this skill when reviewing code.</description>') == 1
...
def test_public_agent_pin_is_additive_to_allowlist(tmp_path):
    ...
    assert system_prompt.count('<description>Use this skill when review work is needed.</description>') == 1
...
def test_public_agent_warns_and_skips_missing_skill_names(tmp_path):
    ...
    with pytest.warns(UserWarning) as warning_records:
        handle = Agent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'missing skipped'})]), tools=[], skills=[Skills.only('missing-only'), Skills.pin('missing-pin'), 'missing-raw'])
...
def test_public_agent_keeps_path_and_directory_skill_refs(tmp_path):
    ...
    (path_skill_dir / 'SKILL.md').write_text('# Path Review Skill\n\nUse this skill when path review work is needed.\n', encoding='utf-8')
    extra_root = tmp_path / 'extra-skills'
    extra_skill_dir = extra_root / 'custom'
    extra_skill_dir.mkdir(parents=True)
    (extra_skill_dir / 'SKILL.md').write_text('# Custom Skill\n\nUse this skill when custom work is needed.\n', encoding='utf-8')
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'path pinned'}), json.dumps({'type': 'final_answer', 'content': 'directory selected'})])
    ...
    assert [skill.name for skill in directory_handle.skill_registry.list_skills()] == ['custom', 'files', 'memory', 'review', 'shell']
...
def test_public_agent_config_supports_programmatic_values_and_env_fallback(monkeypatch, tmp_path):
    ...
    config = AgentConfig(project_root=tmp_path, store_path=tmp_path / 'custom.sqlite', traces_dir=tmp_path / 'custom-traces', skills_dir=tmp_path / 'custom-skills', permission_profile='read-only', local_api_key='local', local_context_window_tokens=65536)
    handle = Agent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'configured'})]), tools=[], skills=[])
    ...
    assert handle.runtime.context_budget.max_prompt_tokens == 65536
...
def test_injected_client_model_capabilities_drive_runtime_budget(tmp_path):
    ...
    client.model_capabilities = LLMModelCapabilities(provider='custom', model='small-context', context_window_tokens=8192, default_response_reserve_tokens=1024, max_input_tokens=4096)
    handle = Agent(config=AgentConfig(project_root=tmp_path), llm=client, tools=[], skills=[])
    result = handle.run_result('hello')
    assert handle.runtime.context_budget.max_prompt_tokens == 8192
    assert handle.runtime.context_budget.max_input_tokens == 4096
    assert handle.runtime.context_budget.input_token_budget == 4096
    assert handle.runtime.context_budget.to_dict()['max_input_tokens'] == 4096
    assert result.context_report is not None
    assert result.context_report.budget.max_input_tokens == 4096
    assert result.context_report.budget.input_token_budget == 4096
...
def test_public_agent_default_config_uses_cwd_runtime_and_read_only(monkeypatch, tmp_path):
    ...
    handle = Agent(llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'default sdk'})]), tools=[], skills=[])
    response = handle.run('hello')
...
def test_public_agent_default_config_uses_project_root_env_override(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv('CHULK_RUNTIME_DIR', raising=False)
    monkeypatch.delenv('CHULK_PERMISSION_PROFILE', raising=False)
    handle = Agent(llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'env project'})]), tools=[], skills=[])
    response = handle.run('hello')
...
def test_public_agent_default_config_uses_runtime_dir_env_override(monkeypatch, tmp_path):
    ...
    handle = Agent(llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'env runtime'})]), tools=[], skills=[])
    response = handle.run('hello')
...
def test_public_agent_config_from_env_and_runtime_dir(monkeypatch, tmp_path):
    ...
    config = AgentConfig.from_env(project_root=tmp_path, runtime_dir=runtime_dir, local_api_key='local').to_config()
...
def test_public_agent_config_provider_constructors_ignore_cross_provider_env_model(monkeypatch, tmp_path):
    ...
    deepseek_config = AgentConfig.deepseek(project_root=tmp_path, runtime_dir=tmp_path / 'deepseek', api_key='deepseek', base_url='https://deepseek.example')
    moonshot_config = AgentConfig.moonshot(project_root=tmp_path, runtime_dir=tmp_path / 'moonshot', api_key='moonshot', base_url='https://moonshot.example/v1')
    local_config = AgentConfig.local(project_root=tmp_path, runtime_dir=tmp_path / 'local', model='local-model', base_url='http://localhost:1234/v1', api_key='local', context_window_tokens=65536)
    ...
    assert local_config.to_config().local_context_window_tokens == 65536
...
def test_local_context_overrides_do_not_shift_public_positional_arguments():
    agent_parameter = inspect.signature(AgentConfig).parameters['local_context_window_tokens']
    provider_parameter = inspect.signature(LocalProvider).parameters['context_window_tokens']
    ...
    provider = LocalProvider('local-model', 'local-key', 'http://localhost:1234/v1', 12.0, 3)
...
def test_moonshot_provider_public_spec_keeps_connection_overrides():
    provider = MoonshotProvider(model='kimi-k3', api_key='moonshot-key', base_url='https://moonshot.example/v1')
...
def test_public_agent_config_with_overrides_for_app_agents(tmp_path):
    ...
    base_config = AgentConfig.openai(project_root=tmp_path, runtime_dir=tmp_path / 'runtime' / 'base', api_key='openai', permission_profile='read-only', max_tool_calls_per_turn=1, llm_fallback_providers=[base_fallback])
    app_agent_config = base_config.with_overrides(model='gpt-4.1', runtime_dir=tmp_path / 'runtime' / 'agent-a', permission_profile='workspace-write', max_tool_calls_per_turn=7, mcp_servers=[server], llm_fallback_providers=[app_fallback])
...
def test_public_agent_config_reuses_programmatic_mcp_iterable(tmp_path):
    server = MCP.streamable_http(label='docs', server_url='https://mcp.example.com')
    config = AgentConfig(project_root=tmp_path, provider='openai', model='gpt-4.1-mini', mcp_servers=(configured_server for configured_server in [server]))
...
def test_public_mcp_builder_uses_hosted_mcp_for_openai(tmp_path):
    ...
    result = Agent(config=config, capabilities=Capabilities(external_services=True), llm=llm, tools=[], skills=[], mcp=[server]).run_result('search docs')
...
def test_public_mcp_builder_resolves_authorization_env(monkeypatch):
    ...
    server = MCP.streamable_http(label='docs', server_url='https://mcp.example.com', authorization_env='DOCS_MCP_TOKEN')
...
def test_public_mcp_builder_rejects_missing_authorization_env(monkeypatch):
    ...
    with pytest.raises(ValueError, match='DOCS_MCP_TOKEN'):
        MCP.streamable_http(label='docs', server_url='https://mcp.example.com', authorization_env='DOCS_MCP_TOKEN')

def test_public_mcp_builder_rejects_string_allowed_tools():
    with pytest.raises(ValueError, match='allowed_tools'):
        MCP.streamable_http(label='docs', server_url='https://mcp.example.com', allowed_tools='search_docs')

def test_public_mcp_builder_accepts_none_allowed_tools():
    server = MCP.streamable_http(label='docs', server_url='https://mcp.example.com', allowed_tools=None)
...
def test_public_mcp_empty_list_disables_configured_servers(monkeypatch, tmp_path):
    ...
    mcp_dir = runtime_dir
    mcp_dir.mkdir()
    (mcp_dir / 'mcp.json').write_text(json.dumps({'servers': [{'label': 'docs', 'server_url': 'https://mcp.example.com', 'allowed_tools': ['search_docs']}]}), encoding='utf-8')
    ...
    config = AgentConfig(project_root=tmp_path, runtime_dir=runtime_dir, provider='local', model='local-model', local_api_key='local')
    handle = Agent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'no mcp'})]), tools=[], skills=[], mcp=[])
    result = handle.run_result('hello')
...
def test_wheel_install_exposes_sdk_defaults_and_bundled_skills(tmp_path):
    ...
    subprocess.run([sys.executable, '-m', 'pip', 'wheel', '--no-deps', '--no-build-isolation', '.', '-w', str(wheelhouse)], cwd=Path(__file__).resolve().parents[1], check=True, capture_output=True, text=True)
    ...
    completed = subprocess.run([str(python), '-c', script], cwd=app_dir, check=True, capture_output=True, text=True)
    assert completed.stdout.strip() == 'ok'

@pytest.mark.parametrize(('provider', 'model', 'extra_config'), [('local', 'local-model', {'local_api_key': 'local'}), ('deepseek', 'deepseek-v4-flash', {'deepseek_api_key': 'deepseek'}), ('moonshot', 'kimi-k3', {'moonshot_api_key': 'moonshot'})])
def test_public_mcp_bridge_registers_for_non_openai_providers(monkeypatch, tmp_path, provider, model, extra_config):
    ...
    server = MCP.streamable_http(label='docs', server_url='https://mcp.example.com')
    config = AgentConfig(project_root=tmp_path, provider=provider, model=model, permission_profile='workspace-write', **extra_config)
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'mcp_docs_search_docs', 'arguments_json': json.dumps({'query': 'sdk'}), 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'bridge ok'})])
    result = Agent(config=config, llm=llm, tools=[], skills=[], mcp=[server], capabilities=Capabilities(external_services=True), permission_callback=lambda _request, _record: True).run_result('search docs')
...
def test_public_plan_result_approve_result_and_reject_result(tmp_path):
    ...
    approve_handle = Agent(config=config, llm=FakeLLMClient([_plan_response(plan_payload), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'Implemented in API.'})}), json.dumps({'type': 'final_answer', 'content': 'plan executed'})]), tools=[], skills=[])
    ...
    reject_root.mkdir()
    reject_handle = Agent(config=load_config({'CHULK_PROJECT_ROOT': str(reject_root)}), llm=FakeLLMClient([_plan_response(_plan_payload('Reject me'))]), tools=[], skills=[])
...
def test_public_approve_and_reject_result_without_pending_plan_use_neutral_metadata(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    handle = Agent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'done'})]), tools=[], skills=[])
...
async def test_async_agent_runs_inside_active_event_loop(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    handle = AsyncAgent(config=config, llm=FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'async ok'}), json.dumps({'type': 'final_answer', 'content': 'async ok again'})]), tools=[], skills=[])
...
def test_public_tool_schema_supports_richer_annotations():
    ...
    @Tool
    def rich_tool(mode: Mode, target: Annotated[str, 'Target name'], maybe_count: int | None=None, level: Literal['low', 'high']='low', profile: Profile | None=None, payload: PydanticLike | None=None, tags: list[str] | None=None, scores: dict[str, int] | None=None, states: list[dict[str, Literal['active', 'paused']]] | None=None) -> str:
    ...
    assert schema['properties']['states']['items']['additionalProperties'] == {'type': ['string'], 'enum': ['active', 'paused']}
...
def test_public_tool_pydantic_schema_uses_enforced_subset():
    ...
    assert calls == []
    assert result.metadata['validation_errors'] == [{'path': 'profile.address', 'message': 'value has the wrong type', 'expected': 'null or object', 'actual': 'integer'}]

def _plan_payload(summary: str) -> dict:
    return {'summary': summary, 'steps': [{'id': '1', 'title': 'Implement', 'description': 'Implement the SDK change.', 'status': 'pending', 'acceptance_criteria': ['The SDK change is implemented.']}]}

def _plan_response(payload: dict) -> str:
    return json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(payload), 'step_update_json': '{}'})
