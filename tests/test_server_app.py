...
def _app(tmp_path, *, max_body_bytes=1000000):
    ...
    def build(profile_id, conversation_id, metadata):
        ...
        return create_agent(resolved.config, llm_client=ServerLLM(), tool_specs=[], skill_specs=[], conversation_id=conversation_id, conversation_metadata=dict(metadata or {}), profile_id=profile_id)
    ...
    app = create_control_app(config, dispatcher=dispatcher, token_store=tokens, max_body_bytes=max_body_bytes, sse_heartbeat_seconds=0.05)
    return (app, tokens)
...
def test_control_token_is_owner_only_and_rotation_invalidates_old_value(tmp_path) -> None:
    ...
    if os.name != 'nt':
        assert store.path.stat().st_mode & 511 == 384
...
def test_http_api_requires_auth_origin_and_browser_csrf(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert client.get('/v1/profiles', headers={**_auth(tokens), 'Origin': 'https://evil.example'}).status_code == 403
        response = client.post('/v1/profiles/default/conversations', headers={**_auth(tokens), 'Origin': 'http://localhost:3000', 'Content-Type': 'application/json'}, json={})
        assert response.status_code == 403
        response = client.post('/v1/profiles/default/conversations', headers={**_auth(tokens), 'Origin': 'http://localhost:3000', 'X-Chulk-CSRF': tokens.csrf_token()}, json={})
        assert response.status_code == 201
    audit = (tmp_path / '.chulk' / 'control-audit.jsonl').read_text(encoding='utf-8')
...
def test_http_api_creates_conversation_and_queues_idempotent_message(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        created = client.post('/v1/profiles/default/conversations', headers=_auth(tokens), json={'metadata': {'client': 'test'}})
        conversation_id = created.json()['conversation']['id']
        first = client.post(f'/v1/profiles/default/conversations/{conversation_id}/messages', headers=_auth(tokens), json={'message': 'hello', 'idempotency_key': 'message-1'})
        replay = client.post(f'/v1/profiles/default/conversations/{conversation_id}/messages', headers=_auth(tokens), json={'message': 'hello', 'idempotency_key': 'message-1'})
...
def test_operator_routes_are_bounded_and_do_not_expose_raw_traces(tmp_path) -> None:
    app, tokens = _app(tmp_path)
    with TestClient(app) as client:
        created = client.post('/v1/profiles/default/conversations', headers=_auth(tokens), json={})
        conversation_id = created.json()['conversation']['id']
        conversations = client.get('/v1/profiles/default/conversations?limit=10', headers=_auth(tokens))
        traces = client.get('/v1/profiles/default/traces', headers=_auth(tokens))
        artifacts = client.get(f'/v1/profiles/default/conversations/{conversation_id}/artifacts', headers=_auth(tokens))
        proposals = client.get('/v1/profiles/default/proposals', headers=_auth(tokens))
        usage = client.get('/v1/profiles/default/usage?limit=10', headers=_auth(tokens))
        ...
        jobs = client.get('/v1/profiles/default/jobs', headers=_auth(tokens))
...
def test_automation_control_and_authenticated_webhook_routes(tmp_path) -> None:
    ...
    job = store.create(adapter='webhook', destination_id='owner', prompt='handle event', next_run_at=datetime.now(timezone.utc) + timedelta(days=1))
    with TestClient(app) as client:
        inspected = client.get(f'/v1/profiles/default/jobs/{job.id}', headers=_auth(tokens))
        assert inspected.status_code == 200
        assert inspected.json()['job']['id'] == job.id
        paused = client.post(f'/v1/profiles/default/jobs/{job.id}/actions', headers=_auth(tokens), json={'action': 'pause', 'revision': job.revision, 'idempotency_key': 'api-pause'})
        assert paused.status_code == 200
        assert paused.json()['job']['status'] == 'paused'
        resumed = store.resume(job.id, expected_revision=paused.json()['job']['revision'], idempotency_key='test-resume')
        webhook = client.post(f'/v1/profiles/default/jobs/{job.id}/webhooks', headers=_auth(tokens), json={})
        assert webhook.status_code == 201
        trigger_id = webhook.json()['trigger']['id']
        credential = webhook.json()['credential']
        denied = client.post(f'/v1/profiles/default/automation-webhooks/{trigger_id}', headers=_auth(tokens), json={'credential': 'wrong', 'event_id': 'evt-api', 'payload': {}})
        assert denied.status_code == 401
        accepted = client.post(f'/v1/profiles/default/automation-webhooks/{trigger_id}', headers=_auth(tokens), json={'credential': credential, 'event_id': 'evt-api', 'payload': {'value': 1}})
        assert accepted.status_code == 202
        replay = client.post(f'/v1/profiles/default/automation-webhooks/{trigger_id}', headers=_auth(tokens), json={'credential': credential, 'event_id': 'evt-api', 'payload': {'value': 2}})
        assert replay.json()['trigger_event']['id'] == accepted.json()['trigger_event']['id']
...
def test_permission_endpoint_is_owned_and_idempotent(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        pending = broker.create(PermissionRequest(tool_name='shell', permission_level=ToolPermissionLevel.SHELL, arguments={'command': 'printf hello'}, requires_confirmation=True, policy_name='workspace-write', reason='shell approval'))
        url = f'/v1/profiles/default/conversations/{conversation_id}/permissions/{pending.id}'
        first = client.post(url, headers=_auth(tokens), json={'decision': 'allow', 'idempotency_key': 'decision-1'})
        replay = client.post(url, headers=_auth(tokens), json={'decision': 'allow', 'idempotency_key': 'decision-1'})
        conflict = client.post(url, headers=_auth(tokens), json={'decision': 'deny', 'idempotency_key': 'decision-2'})
        assert first.status_code == replay.status_code == 200
        assert first.json()['permission']['status'] == 'allowed'
        assert conflict.status_code == 409
        assert client.post(url.replace('/default/', '/missing/'), headers=_auth(tokens), json={'decision': 'allow', 'idempotency_key': 'other'}).status_code == 404

def test_http_api_rejects_malformed_and_oversized_requests(tmp_path) -> None:
    app, tokens = _app(tmp_path, max_body_bytes=64)
    with TestClient(app) as client:
        malformed = client.post('/v1/profiles/default/conversations', headers={**_auth(tokens), 'Content-Type': 'application/json'}, content=b'{')
        oversized = client.post('/v1/profiles/default/conversations', headers=_auth(tokens), json={'metadata': {'value': 'x' * 100}})
        chunked = client.post('/v1/profiles/default/conversations', headers={**_auth(tokens), 'Content-Type': 'application/json'}, content=(part for part in (b'{"metadata":{"value":"', b'x' * 100, b'"}}')))
    ...
    with TestClient(app) as client:
        with pytest.raises(WebSocketDisconnect):
            with client.websocket_connect('/v1/gateway/ws'):
                pass
        with client.websocket_connect('/v1/gateway/ws', subprotocols=[protocol]) as socket:
            socket.send_json({'type': 'hello', 'schema_version': 1, 'profile_id': 'default', 'client_id': 'browser-1'})
            assert socket.receive_json()['type'] == 'ready'
            socket.send_json({'type': 'message', 'schema_version': 1, 'event_id': 'event-1', 'idempotency_key': 'ws-message-1', 'message': 'hello websocket'})
            ...
            socket.send_json({'type': 'message', 'schema_version': 1, 'event_id': 'event-2', 'idempotency_key': 'ws-message-2', 'conversation_id': 'missing-conversation', 'message': 'hello missing'})
...
async def test_sse_stream_orders_events_and_resumes_from_last_event_id(tmp_path) -> None:
    ...
    first = journal.append(AgentEvent(name='model.delta', profile_id='default', conversation_id=conversation_id, payload=ModelDeltaPayload('one')))
    journal.append(AgentEvent(name='model.delta', profile_id='default', conversation_id=conversation_id, payload=ModelDeltaPayload('two')))
    messages = await _asgi_sse_request(app, f'/v1/profiles/default/conversations/{conversation_id}/events', token=tokens.load_or_create(), last_event_id=first.event_id)
...
def test_event_endpoint_supports_bounded_reconnect_pages(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        created = client.post('/v1/profiles/default/conversations', headers=_auth(tokens), json={})
        conversation_id = created.json()['conversation']['id']
        first = app.state.dispatcher.journal('default', conversation_id).append(AgentEvent(name='model.delta', profile_id='default', conversation_id=conversation_id, payload=ModelDeltaPayload('bounded')))
        page = client.get(f'/v1/profiles/default/conversations/{conversation_id}/events?follow=false', headers=_auth(tokens))
        resumed = client.get(f'/v1/profiles/default/conversations/{conversation_id}/events?follow=false&after={first.event_id}', headers=_auth(tokens))
        assert page.status_code == 200
        assert page.json()['events'][0]['id'] == first.event_id
        assert page.json()['next_cursor'] == first.event_id
        assert resumed.json() == {'schema_version': 1, 'events': [], 'next_cursor': first.event_id}

async def _asgi_sse_request(app, path: str, *, token: str, last_event_id: str | None=None) -> list[dict]:
    ...
    scope = {'type': 'http', 'asgi': {'version': '3.0'}, 'http_version': '1.1', 'method': 'GET', 'scheme': 'http', 'path': path, 'raw_path': path.encode(), 'query_string': b'', 'headers': headers, 'client': ('127.0.0.1', 12345), 'server': ('127.0.0.1', 8765)}
