...
def test_tavily_search_returns_bounded_cited_results_without_exposing_key() -> None:
    ...
    def request(url: str, payload: dict[str, object], timeout: float) -> object:
        ...
        return {'results': [{'title': 'Example', 'url': 'https://example.com/news', 'content': 'abcdefgh', 'score': 0.9}, {'title': 'Unsafe', 'url': 'file:///etc/passwd', 'content': 'ignored'}]}
    tool = tavily_search_tool('top-secret', max_results=2, max_result_content_chars=4, request_json=request)
    result = tool.callable({'query': ' current news '})
    assert result.success is True
    assert result.value == {'query': 'current news', 'results': [{'title': 'Example', 'url': 'https://example.com/news', 'content': 'abcd', 'score': 0.9}]}
