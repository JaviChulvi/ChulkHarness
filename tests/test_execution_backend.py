...
import json
import pytest
from chulk import Agent, AgentConfig, AsyncAgent, Capabilities, CommandExecutionRequest, ExecutionSessionRequest, FileReadRequest, FileSearchRequest, FileWriteRequest, HostExecutionBackend
...
class ContainedPolicy:
    ...
    def prepare(self, request):
        return ShellExecutionDecision.allow(request.command, policy_name='test-contained', shell=True, containment_applied=True)

def _tool_call(name: str, **arguments) -> str:
    return json.dumps({'type': 'tool_call', 'content': None, 'tool_name': name, 'arguments_json': json.dumps(arguments)})
...
def test_host_session_preserves_file_and_shell_results_with_execution_evidence(tmp_path):
    ...
    direct_write = write_file({'path': 'direct.txt', 'content': 'shared data', 'overwrite': False}, tmp_path)
    session_write = session.write_file(FileWriteRequest(path='session.txt', content='shared data'))
    ...
    command = session.run_command(CommandExecutionRequest('python -c "import sys;sys.stdout.write(\'host-ok\')"'))
    assert (session_write.success, session_write.observation) == (direct_write.success, direct_write.observation.replace('direct.txt', 'session.txt'))
    assert (session_read.success, session_read.observation) == (direct_read.success, direct_read.observation)
...
def test_host_session_adapts_existing_shell_policy_and_containment(tmp_path):
    backend = HostExecutionBackend(tmp_path, shell_execution_policy=ContainedPolicy(), require_shell_containment=True)
    session = backend.open_session(ExecutionSessionRequest(turn_id='contained'))
    result = session.run_command(CommandExecutionRequest('python -c "import sys;sys.stdout.write(\'safe\')"'))
...
def test_closed_host_session_rejects_operations(tmp_path):
    session = HostExecutionBackend(tmp_path).open_session(ExecutionSessionRequest(turn_id='closed'))
    session.close()
    session.close()
...
def test_runtime_routes_default_tools_through_one_turn_scoped_session(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), capabilities=Capabilities.read_only(), execution_backend=backend, llm=FakeLLM([_tool_call('read_file', path='input.txt'), _final(), _tool_call('read_file', path='input.txt'), _final()]), skills=[])
    ...
    trace_events = [json.loads(line) for line in facade.trace_path.read_text(encoding='utf-8').splitlines()]
    completed = [event['payload'] for event in trace_events if event['type'] == 'tool_call_completed']
...
async def test_async_runtime_closes_turn_session_through_async_lifecycle(tmp_path):
    backend = RecordingHostBackend(tmp_path)
    (tmp_path / 'input.txt').write_text('hello', encoding='utf-8')
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), capabilities=Capabilities.read_only(), execution_backend=backend, llm=FakeLLM([_tool_call('read_file', path='input.txt'), _final()]), skills=[])
...
def test_direct_shell_behavior_matches_host_session(tmp_path):
    ...
    session = HostExecutionBackend(tmp_path).open_session(ExecutionSessionRequest(turn_id='parity'))
    routed = session.run_command(CommandExecutionRequest(command))
    assert (routed.success, routed.observation, routed.stdout, routed.stderr, routed.exit_code, routed.error, routed.failure_kind) == (direct.success, direct.observation, direct.stdout, direct.stderr, direct.exit_code, direct.error, direct.failure_kind)
