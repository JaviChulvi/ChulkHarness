...
from chulk.llm.capabilities import GEMINI_DEFAULT_CONTEXT_WINDOW_TOKENS, GEMINI_DEFAULT_RESPONSE_RESERVE_TOKENS, resolve_model_capabilities
from chulk.llm.factory import LLMProviderConnection, create_llm_client, provider_capabilities, provider_connection_from_config, supported_llm_providers
...
class FakeModels:
    ...
    def __init__(self, *, responses: list[object] | None=None, stream_chunks: list[object] | None=None, error: Exception | None=None) -> None:
...
def _client(models: FakeModels, *, model: str='gemini-test') -> GeminiGenerateContentClient:
    return GeminiGenerateContentClient(model=model, client=SimpleNamespace(models=models))

def _response(*, text: str | None=None, function_calls: list[object] | None=None, usage: object=None, finish_reason: str | None=None) -> object:
    candidates = []
    if finish_reason is not None:
        candidates = [SimpleNamespace(finish_reason=finish_reason)]
    return SimpleNamespace(text=text, function_calls=function_calls or [], usage_metadata=usage, candidates=candidates)
...
def test_gemini_strips_api_key_and_shapes_sdk_client_options(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    GeminiGenerateContentClient(model=' gemini-test ', api_key='  test-key  ', base_url=' https://gemini.example ', timeout_seconds=12.5, max_retries=3)
    assert captured == {'api_key': 'test-key', 'http_options': {'base_url': 'https://gemini.example', 'timeout': 12500, 'retry_options': {'attempts': 4}}}

def test_gemini_text_request_uses_native_message_shape_and_dynamic_output_limit() -> None:
    models = FakeModels(responses=[_response(text='first'), _response(text='second')])
    client = _client(models)
    assert client.complete(MESSAGES) == 'first'
    assert models.generate_calls[0] == {'model': 'gemini-test', 'contents': [{'role': 'user', 'parts': [{'text': 'Hello'}]}], 'config': {'system_instruction': 'Be concise.'}}
    assert client.complete(MESSAGES, max_output_tokens=987654) == 'second'
    assert models.generate_calls[1]['config']['max_output_tokens'] == 987654

def test_gemini_normalizes_provider_usage() -> None:
    usage = SimpleNamespace(prompt_token_count=30, candidates_token_count=9, total_token_count=41, cached_content_token_count=12, thoughts_token_count=2)
    ...
    assert response.usage == LLMUsage(input_tokens=30, output_tokens=11, total_tokens=41, cached_input_tokens=12, cache_hit_input_tokens=12, cache_miss_input_tokens=18, reasoning_tokens=2, estimated=False, source='provider', raw={'cached_content_token_count': 12, 'candidates_token_count': 9, 'prompt_token_count': 30, 'thoughts_token_count': 2, 'total_token_count': 41})

def test_gemini_bills_tool_use_and_thinking_tokens_in_the_right_buckets() -> None:
    usage = SimpleNamespace(prompt_token_count=199999, tool_use_prompt_token_count=2, candidates_token_count=1, thoughts_token_count=1, total_token_count=200003, cached_content_token_count=0)
    models = FakeModels(responses=[_response(text='answer', usage=usage)])
    response = _client(models, model='gemini-3.1-pro-preview').complete_response(MESSAGES)
    assert response.usage is not None
    assert response.usage.input_tokens == 200001
    assert response.usage.cache_miss_input_tokens == 200001
    assert response.usage.output_tokens == 2
    assert response.usage.reasoning_tokens == 1
    assert response.usage.total_tokens == 200003
...
def test_gemini_native_streaming_yields_text_and_final_usage() -> None:
    usage = SimpleNamespace(prompt_token_count=4, candidates_token_count=2, total_token_count=6, cached_content_token_count=0, thoughts_token_count=0)
    models = FakeModels(stream_chunks=[_response(text='hello '), _response(text='world', usage=usage, finish_reason='STOP')])
...
def test_gemini_json_action_mode_uses_schema_without_native_tools() -> None:
    payload = json.dumps({'type': 'final_answer', 'content': 'done', 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': '{}'})
...
async def test_gemini_async_empty_json_action_response_is_fallback_eligible() -> None:
    ...
    client = GeminiGenerateContentClient(model='gemini-test', client=SimpleNamespace(models=sync_models), async_client=SimpleNamespace(models=async_models))
    with pytest.raises(LLMError, match='did not include JSON text') as raised:
...
def test_gemini_native_single_function_call_is_normalized_to_one_action() -> None:
    models = FakeModels(responses=[_response(function_calls=[_function_call('calculator', {'expression': '2 + 2'})])])
...
def test_gemini_native_text_with_no_effective_tools_omits_tool_config() -> None:
    models = FakeModels(responses=[_response(text='A direct answer')])
    result = _client(models).complete_action(MESSAGES, tools=[], planning_tools=PlanningToolAvailability())
    assert result.action == FinalAnswerAction(type='final_answer', content='A direct answer')
    config = models.generate_calls[0]['config']
    assert 'tools' not in config
...
def test_gemini_planning_policy_enables_native_transport_without_regular_tools() -> None:
    ...
    result = _client(models).complete_action(MESSAGES, planning_tools=PlanningToolAvailability(propose_plan=True))
    assert result.action == FinalAnswerAction(type='final_answer', content='native final')
    declarations = models.generate_calls[0]['config']['tools'][0]['function_declarations']
    assert [item['name'] for item in declarations] == [PLAN_TOOL_NAME]
    assert models.generate_calls[0]['config']['tool_config'] == {'function_calling_config': {'mode': 'ANY'}}
    assert result.metadata['action_transport'] == 'provider_native'
...
def test_gemini_rejects_multiple_function_calls_and_uses_json_fallback() -> None:
    ...
    models = FakeModels(responses=[_response(function_calls=[_function_call('calculator', {'expression': '2 + 2'}), _function_call('calculator', {'expression': '3 + 3'})]), _response(text=fallback_payload)])
    result = _client(models).complete_action(MESSAGES, tools=[_calculator_tool()])
...
@pytest.mark.parametrize(('error', 'code', 'retryable', 'fallback_eligible'), [(GeminiSDKError('API key not valid', code=400), 'authentication_error', False, False), (GeminiSDKError('quota exceeded', code=429), 'rate_limit', True, True), (GeminiSDKError('backend unavailable', code=503), 'server_error', True, True)])
def test_gemini_sdk_failures_have_typed_provider_metadata(error: Exception, code: str, retryable: bool, fallback_eligible: bool) -> None:
...
def test_gemini_http_400_invalid_key_overrides_generic_classification() -> None:
    models = FakeModels(error=GeminiHTTPError('API key not valid', status_code=400))
    with pytest.raises(LLMError) as raised:
        _client(models).complete(MESSAGES)
...
def test_gemini_http_400_unsupported_function_calling_uses_json_fallback() -> None:
    fallback_payload = json.dumps({'type': 'final_answer', 'content': 'safe fallback', 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': '{}'})
    models = FailOnceModels(error=GeminiHTTPError('Function calling is not supported for this model', status_code=400), responses=[_response(text=fallback_payload)])
    result = _client(models).complete_action(MESSAGES, tools=[_calculator_tool()])
    assert result.action == FinalAnswerAction(type='final_answer', content='safe fallback')
    assert result.metadata['action_transport'] == 'chulk_json_fallback'
    assert 'function calling is not supported' in result.metadata['native_tool_call_error'].lower()
    assert 'tools' in models.generate_calls[0]['config']
    assert models.generate_calls[1]['config']['response_mime_type'] == 'application/json'
...
def test_gemini_environment_uses_documented_key_precedence_and_optional_base_url(tmp_path: Path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'gemini', 'CHULK_MODEL': 'gemini-test', 'CHULK_GEMINI_API_KEY': ' canonical-key ', 'GEMINI_API_KEY': 'gemini-key', 'GOOGLE_API_KEY': 'google-key', 'CHULK_GEMINI_BASE_URL': ' https://gemini.example '})
    assert config.gemini_api_key == 'canonical-key'
    assert config.gemini_base_url == 'https://gemini.example'
    assert provider_connection_from_config('gemini', config) == LLMProviderConnection(api_key='canonical-key', base_url='https://gemini.example')

@pytest.mark.parametrize(('environment', 'expected_key'), [({'CHULK_GEMINI_API_KEY': '   ', 'GEMINI_API_KEY': 'gemini-key', 'GOOGLE_API_KEY': 'google-key'}, 'gemini-key'), ({'GOOGLE_API_KEY': 'google-key'}, 'google-key')])
def test_gemini_environment_supports_standard_key_aliases_in_order(tmp_path: Path, environment: dict[str, str], expected_key: str) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'gemini', 'CHULK_MODEL': 'gemini-test', **environment})
...
def test_gemini_environment_requires_explicit_model(tmp_path: Path) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_MODEL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'gemini', 'CHULK_GEMINI_API_KEY': 'test-key'})

def test_gemini_fallback_requires_explicit_model(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match='explicit model'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_FALLBACK_PROVIDERS': 'gemini'})

def test_gemini_factory_builds_native_client_with_bound_capabilities(monkeypatch: pytest.MonkeyPatch) -> None:
    sdk_client = object()
    captured: dict[str, object] = {}
    ...
    fake_genai = SimpleNamespace(Client=fake_constructor)
    monkeypatch.setitem(sys.modules, 'google', SimpleNamespace(genai=fake_genai))
    client = create_llm_client(provider='gemini', model='gemini-test', connection=LLMProviderConnection(api_key='test-key', base_url='https://gemini.example'), timeout_seconds=12.5, max_retries=3)
    ...
    assert client.model_capabilities.default_response_reserve_tokens == GEMINI_DEFAULT_RESPONSE_RESERVE_TOKENS
    assert captured['api_key'] == 'test-key'
    assert captured['http_options'] == {'base_url': 'https://gemini.example', 'timeout': 12500, 'retry_options': {'attempts': 4}}

def test_gemini_agent_config_cli_and_safe_config_output(tmp_path: Path) -> None:
    secret = 'gemini-secret-that-must-not-be-rendered'
    config = AgentConfig.gemini(project_root=tmp_path, model='gemini-test', api_key=secret, base_url='https://gemini.example').to_config()
