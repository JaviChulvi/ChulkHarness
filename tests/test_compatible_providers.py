...
from chulk.llm import OpenAICompatibleProvider, OpenRouterProvider, resolve_model_capabilities, supported_llm_providers
...
from chulk.llm.providers.compatible import DEFAULT_OPENROUTER_BASE_URL, HOSTED_OPENAI_COMPATIBLE_CAPABILITIES, OPENROUTER_CAPABILITIES, HostedOpenAICompatibleClient, OpenRouterChatCompletionsClient, openrouter_default_headers
...
def _response(*, content: str | None, tool_calls: list[object] | None=None, usage=None):
...
def _tool_call() -> object:
    return SimpleNamespace(id='call_1', type='function', function=SimpleNamespace(name='calculator', arguments=json.dumps({'expression': '2 + 2'})))
...
def _hosted(completions: FakeChatCompletions) -> HostedOpenAICompatibleClient:
    return HostedOpenAICompatibleClient(model='vendor/model', api_key='test-key', base_url='https://models.example/v1', client=FakeClient(completions))

def _openrouter(completions: FakeChatCompletions) -> OpenRouterChatCompletionsClient:
    return OpenRouterChatCompletionsClient(model='vendor/model', api_key='test-key', client=FakeClient(completions))

@pytest.mark.parametrize('client_factory', [_hosted, _openrouter])
def test_compatible_providers_follow_text_and_output_limit_contract(client_factory) -> None:
    completions = FakeChatCompletions([_response(content='first'), _response(content='second')])
...
@pytest.mark.parametrize('client_factory', [_hosted, _openrouter])
def test_compatible_providers_follow_native_single_tool_contract(client_factory) -> None:
    completions = FakeChatCompletions([_response(content=None, tool_calls=[_tool_call()])])
    client = client_factory(completions)
...
@pytest.mark.parametrize('model', ['moonshotai/kimi-k3', 'deepseek/deepseek-v4-flash', 'qwen/qwen3.5-397b-a17b', 'z-ai/glm-5.2', 'minimax/minimax-m3', 'mistralai/mistral-medium-3-5', 'meta-llama/llama-4-maverick', 'google/gemma-4-26b-a4b-it'])
def test_current_openrouter_open_models_use_the_compatible_action_path(model: str) -> None:
    completions = FakeChatCompletions([_response(content=None, tool_calls=[_tool_call()])])
    client = OpenRouterChatCompletionsClient(model=model, api_key='test-key', client=FakeClient(completions))
    result = client.complete_action(MESSAGES, tools=[_calculator_tool()])
    ...
    assert capabilities.context_window_tokens == 8192

@pytest.mark.parametrize('client_factory', [_hosted, _openrouter])
@pytest.mark.parametrize(('unsupported_kwargs', 'field'), [({'hosted_mcp_servers': [object()]}, 'hosted_mcp_servers'), ({'mcp_approval_callback': lambda _request: True}, 'mcp_approval_callback')])
def test_compatible_providers_reject_unsupported_hosted_mcp_arguments(client_factory, unsupported_kwargs: dict, field: str) -> None:
...
@pytest.mark.asyncio
@pytest.mark.parametrize('client_factory', [_hosted, _openrouter])
async def test_compatible_providers_async_reject_unsupported_hosted_mcp_arguments(client_factory) -> None:
    completions = FakeChatCompletions()
    client = client_factory(completions)
...
@pytest.mark.parametrize(('factory', 'provider'), [(_hosted, 'openai-compatible'), (_openrouter, 'openrouter')])
...
def test_compatible_providers_follow_streaming_contract(client_factory) -> None:
    ...
    completions = FakeChatCompletions([SimpleNamespace(choices=[SimpleNamespace(delta=SimpleNamespace(content='hel'), finish_reason=None)], usage=None), SimpleNamespace(choices=[SimpleNamespace(delta=SimpleNamespace(content='lo'), finish_reason='stop')], usage=usage)])
    client = client_factory(completions)
...
@pytest.mark.parametrize(('factory', 'provider'), [(_hosted, 'openai-compatible'), (_openrouter, 'openrouter')])
def test_compatible_providers_preserve_typed_request_errors(factory, provider: str) -> None:
...
@pytest.mark.parametrize(('kwargs', 'provider', 'field'), [({'model': '', 'api_key': 'key', 'base_url': 'https://models.example/v1'}, 'openai-compatible', 'model'), ({'model': 'vendor/model', 'api_key': None, 'base_url': 'https://models.example/v1'}, 'openai-compatible', 'api_key'), ({'model': 'vendor/model', 'api_key': 'key', 'base_url': ''}, 'openai-compatible', 'base_url')])
def test_hosted_compatible_requires_explicit_connection_fields(kwargs, provider: str, field: str) -> None:
...
def test_openrouter_defaults_and_attribution_headers_are_explicit(monkeypatch) -> None:
    ...
    client = OpenRouterChatCompletionsClient(model='vendor/model', api_key='test-key', site_url=' https://app.example ', app_name=' Chulk Harness ', timeout_seconds=7, max_retries=1)
    assert client.base_url == DEFAULT_OPENROUTER_BASE_URL
    assert client.default_headers == {'HTTP-Referer': 'https://app.example', 'X-OpenRouter-Title': 'Chulk Harness'}
    assert captured == {'api_key': 'test-key', 'base_url': DEFAULT_OPENROUTER_BASE_URL, 'timeout_seconds': 7, 'max_retries': 1, 'default_headers': client.default_headers, 'model': 'vendor/model'}
...
def test_capabilities_are_explicit_and_conservative() -> None:
    for capabilities in (HOSTED_OPENAI_COMPATIBLE_CAPABILITIES, OPENROUTER_CAPABILITIES):
...
def test_compatible_provider_environment_uses_explicit_models_and_connections(tmp_path) -> None:
    generic = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'openai-compatible', 'CHULK_MODEL': 'vendor/generic-model', 'CHULK_OPENAI_COMPATIBLE_API_KEY': 'generic-key', 'CHULK_OPENAI_COMPATIBLE_BASE_URL': 'https://models.example/v1'})
    router = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'openrouter', 'CHULK_MODEL': 'vendor/router-model', 'OPENROUTER_API_KEY': 'router-key'})
    assert provider_connection_from_config('openai-compatible', generic) == LLMProviderConnection(api_key='generic-key', base_url='https://models.example/v1')
    assert provider_connection_from_config('openrouter', router) == LLMProviderConnection(api_key='router-key', base_url=DEFAULT_OPENROUTER_BASE_URL)
...
def test_hosted_compatible_environment_requires_an_explicit_model(tmp_path, provider: str) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_MODEL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': provider})

def test_public_compatible_builders_and_cli_specs_use_the_same_config(tmp_path) -> None:
    generic = AgentConfig.openai_compatible(project_root=tmp_path, model='vendor/generic-model', api_key='generic-key', base_url='https://models.example/v1').to_config()
    router = AgentConfig.openrouter(project_root=tmp_path, model='vendor/router-model', api_key='router-key').to_config()
...
def test_compatible_fallback_entries_require_models(tmp_path) -> None:
    with pytest.raises(ValueError, match='explicit model'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_FALLBACK_PROVIDERS': 'openrouter'})
