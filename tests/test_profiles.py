...
from chulk.profiles import AgentProfile, CredentialRef, ProfileAlreadyExistsError, ProfileNotFoundError, ProfileOwnershipError, ProfileRuntimeFactory, SQLiteProfileStore
...
def test_explicit_profiles_get_unique_owner_controlled_runtime_paths(tmp_path):
    ...
    project.mkdir()
    created = store.create_profile('Work_Agent', project_root=project, allowed_skills=('files', 'files', 'shell'), allowed_mcp_servers=(), credential_refs=(CredentialRef('WORK_API_KEY'),))
    ...
    assert profile.credential_refs[0].to_dict() == {'name': 'WORK_API_KEY', 'source': 'environment'}
...
def test_profile_ids_are_immutable_and_persistent_paths_cannot_overlap(tmp_path):
    ...
    with pytest.raises(ProfileOwnershipError):
        store.create(AgentProfile(id='other', project_root=project, runtime_dir=existing.runtime_dir, store_path=existing.store_path, traces_dir=existing.traces_dir))
    with pytest.raises(ProfileOwnershipError):
        store.create(AgentProfile(id='outside', project_root=project, runtime_dir=tmp_path / 'outside', store_path=tmp_path / 'outside' / 'store.sqlite', traces_dir=tmp_path / 'outside' / 'traces'))
...
def test_profile_runtime_factory_isolates_databases_traces_and_project_skills(tmp_path):
    ...
    store = SQLiteProfileStore(base_config.runtime_dir / 'control.sqlite', base_config=base_config)
    project = tmp_path / 'profile-project'
    project.mkdir()
    store.create_profile('work', project_root=project, permission_profile='read-only', allowed_skills=(), allowed_mcp_servers=(), system_prompt='Profile-owned instructions.')
...
def test_profile_runtime_records_owned_sessions_and_emits_profile_events(tmp_path):
    ...
    project = tmp_path / 'profile-project'
    project.mkdir()
    stored = store.create_profile('work', project_root=project, allowed_skills=(), system_prompt='Profile-owned instructions.')
    factory = ProfileRuntimeFactory(base_config, profile_store=store)
    agent = factory.create_agent('work', llm_client=ProfileLLM(), tool_specs=[], skill_specs=None, allowed_skill_names=('files',))
    try:
        assert agent.run_turn('hello') == 'profile response'
        conversation = agent._components.session_store.get_conversation(agent.state.conversation_id)
        event = project_event(agent, TraceEvent.TURN_STARTED, {'turn': {'user_message': 'hello'}})
    finally:
        agent.close()
...
def test_profile_runtime_rejects_an_execution_backend_mismatch(tmp_path):
    ...
    with pytest.raises(ValueError, match='does not match'):
        factory.create_agent('work', llm_client=ProfileLLM(), tool_specs=[], skill_specs=[], execution_backend=WrongBackend())

def test_create_agent_rejects_conflicting_profile_metadata(tmp_path):
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    with pytest.raises(ValueError, match='profile_id'):
        create_agent(config, llm_client=ProfileLLM(), tool_specs=[], skill_specs=[], profile_id='work', conversation_metadata={'profile_id': 'personal'})
