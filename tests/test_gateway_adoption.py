...
def test_adoption_copies_cursor_delivery_checkpoint_and_terminal_rows_once(tmp_path) -> None:
    ...
    claim = legacy.begin_execution(adapter='telegram', update_id=1, destination_id='9', now=NOW)
    assert claim.execution_token is not None
    assert legacy.record_response(adapter='telegram', update_id=1, execution_token=claim.execution_token, response_parts=('first', 'second'))
    delivery = legacy.claim_delivery(adapter='telegram', update_id=1, now=NOW)
    assert delivery is not None
    assert delivery.delivery_token is not None
    assert legacy.mark_response_part_delivered(adapter='telegram', update_id=1, delivery_token=delivery.delivery_token, expected_part=0)
    legacy.ignore(adapter='telegram', update_id=2, destination_id='9')
    result = adopt_legacy_telegram_state(control_db_path=control_path, profile_db_path=legacy_path, profile_id='work', now=NOW)
    repeated = adopt_legacy_telegram_state(control_db_path=control_path, profile_db_path=legacy_path, profile_id='work', now=NOW)
...
def test_adoption_quarantines_in_flight_execution_and_never_queues_it(tmp_path) -> None:
    ...
    control_path = tmp_path / 'control.sqlite'
    legacy = SQLiteAdapterUpdateLedger(legacy_path)
    legacy.begin_execution(adapter='telegram', update_id=3, destination_id='9', now=NOW)
    adopt_legacy_telegram_state(control_db_path=control_path, profile_db_path=legacy_path, profile_id='work', now=NOW)
    ledger = SQLiteGatewayLedger(control_path)
