...
import pytest
from chulk.discord import DISCORD_MESSAGE_LIMIT, DiscordChannelAdapter, DiscordConfigError, DiscordMessage, load_discord_config, split_discord_envelope, split_discord_text
from chulk.gateway import ChannelScope, DeliveryState, DeliveryTarget, GatewayRuntime, OutboundEnvelope, SQLiteGatewayLedger, SQLiteGatewayRouter, TextPart
...
def _message(content: str, *, message_id: str='1', guild_id: str | None=None, thread_id: str | None=None, author_is_bot: bool=False) -> DiscordMessage:
    return DiscordMessage(message_id=message_id, author_id='user-7', channel_id='channel-9', thread_id=thread_id, guild_id=guild_id, content=content, author_is_bot=author_is_bot)

def test_discord_config_is_environment_only_and_validated(tmp_path: Path) -> None:
    env_file = tmp_path / '.env'
    env_file.write_text('CHULK_DISCORD_BOT_TOKEN=file-token\nCHULK_DISCORD_ACCOUNT_ID=team-bot\n', encoding='utf-8')
    config = load_discord_config({'CHULK_DISCORD_BOT_TOKEN': 'process-token'}, env_file=env_file)
    ...
    with pytest.raises(DiscordConfigError, match='MAX_PENDING'):
        load_discord_config({'CHULK_DISCORD_BOT_TOKEN': 'token', 'CHULK_DISCORD_MAX_PENDING': 'many'})

@pytest.mark.asyncio
async def test_discord_adapter_normalizes_direct_group_and_thread_messages() -> None:
    transport = FakeDiscordTransport((_message('ignore', message_id='1', author_is_bot=True), _message('  hello  ', message_id='2'), _message('/status', message_id='3', guild_id='guild-4', thread_id='thread-5')))
...
async def test_discord_deliveries_are_bounded_and_target_threads() -> None:
    ...
    envelope = OutboundEnvelope(profile_id='default', conversation_id='conversation', target=DeliveryTarget('discord', 'primary', 'channel-9', thread_id='thread-5'), text='answer')
    delivered = await adapter.deliver(envelope)
    oversized = await adapter.deliver(OutboundEnvelope(profile_id='default', conversation_id='conversation', target=envelope.target, text='x' * (DISCORD_MESSAGE_LIMIT + 1)))
...
def test_discord_responses_split_before_the_durable_outbox() -> None:
    text = 'line ' * 500 + '\n' + 'tail ' * 300
    envelope = OutboundEnvelope(profile_id='default', conversation_id='conversation', target=DeliveryTarget('discord', 'primary', 'channel-9'), text=text, envelope_id='response')
    ...
    assert tuple((item.envelope_id for item in records)) == tuple((f'response:{index}' for index in range(len(records))))
    assert records[-1].final
    assert all((not item.final for item in records[:-1]))

@pytest.mark.asyncio
async def test_discord_direct_message_consumes_pairing_without_agent_execution(tmp_path: Path) -> None:
    transport = FakeDiscordTransport()
    adapter = DiscordChannelAdapter(transport)
    ledger = SQLiteGatewayLedger(tmp_path / 'control.sqlite')
    router = SQLiteGatewayRouter(tmp_path / 'control.sqlite')
    challenge = router.create_pairing(adapter='discord', account_id='primary', profile_id='default', ttl_seconds=600)
    executed = False
    ...
    async def execute(_profile_id, _envelope):
        ...
        executed = True
        return ()
    runtime = GatewayRuntime(ledger=ledger, router=router, adapters=(adapter,), executor=execute)
