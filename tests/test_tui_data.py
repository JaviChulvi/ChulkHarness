...
class FakeControlClient:
    ...
    def list_profiles(self):
        return {'profiles': [{'id': 'default', 'model_profile_id': 'balanced'}]}

    def list_conversations(self, profile_id, *, limit):
        return {'conversations': [{'id': 'conv-1', 'title': 'Operator test', 'status': 'active', 'turn_count': 2}]}

    def list_permissions(self, profile_id, conversation_id, *, status, limit):
        return {'permissions': [{'id': 'perm-1', 'tool_name': 'shell', 'status': 'pending'}]}
    ...
    def list_events(self, profile_id, conversation_id, *, after=None):
        ...
        return {'events': [{'id': 'event-1', 'event': {'name': 'model.delta', 'payload': {'text': 'Streaming evidence'}}}], 'next_cursor': 'event-1'}
    ...
    def query_usage(self, profile_id, *, conversation_id=None, group_by=None, limit):
        if conversation_id:
            return {'entries': [{'resource_kind': 'model', 'turn_id': 'turn-1', 'cost': {'amount': '0.12', 'currency': 'USD'}}]}
        return {'groups': [{'key': 'model', 'total_cost': '0.12'}]}

    def list_traces(self, profile_id, *, limit):
        return {'traces': [{'conversation_id': 'conv-1', 'available': True, 'artifact_count': 1}]}
    ...
    def get_command(self, profile_id, conversation_id, command_id):
        return {'command': {'id': command_id, 'status': self.command_status, 'result': {'content': 'Finished cleanly.'} if self.command_status == 'completed' else None}}

    def decide_permission(self, profile_id, conversation_id, request_id, *, decision):
    ...
    def control_goal(self, profile_id, item_id, *, action, revision, instruction=None):
...
def test_control_api_data_source_builds_complete_operator_snapshot() -> None:
    ...
    source = ControlApiDataSource(client, profile_id='default')
...
def test_control_api_data_source_runs_conversation_and_supervision_actions() -> None:
    ...
    source.control_work('goal', {'id': 'goal-1', 'revision': 3}, 'pause')
    source.cancel_conversation()
    source.steer_goal({'id': 'goal-1', 'revision': 3}, 'Focus on the failing lane')
