...
import sqlite3
import pytest
from chulk import Agent, AgentConfig, ChulkError, ConfigurationError, ErrorDetails, MemoryError, PermissionDeniedError, ProviderError, SafetyError, ToolExecutionError, TraceError
...
def _agent(tmp_path) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient(), tools=[], skills=[])

def _raise(exc: Exception):
    raise exc

def test_hierarchy_is_available_from_stable_top_level():
    categories = {ConfigurationError: 'configuration', ProviderError: 'provider', ToolExecutionError: 'tool_execution', PermissionDeniedError: 'permission_denied', SafetyError: 'safety', TraceError: 'trace', MemoryError: 'memory'}
...
def test_configuration_failure_maps_field_and_preserves_cause(tmp_path):
    with pytest.raises(ConfigurationError) as caught:
        Agent(config=AgentConfig(project_root=tmp_path, max_skills_per_turn=0), llm=FakeLLMClient())
...
def test_facade_maps_provider_tool_permission_safety_and_memory_failures(tmp_path):
    cases = [(LLMError('provider unavailable', retryable=True), ProviderError), (ToolValidationError('lookup', [ToolValidationIssue(path='$.query', message='is required', expected='string')], {'type': 'object'}), ToolExecutionError), (TerminalPermissionDenied('write_file', 'policy denied', policy_name='read-only'), SafetyError), (ValueError('Path is outside the project root'), SafetyError), (MemorySecretError('Credential-like data is not allowed in durable memory.'), SafetyError), (sqlite3.OperationalError('database is locked'), MemoryError)]
    for index, (internal, public_type) in enumerate(cases):
        facade = _agent(tmp_path / str(index))
        facade.runtime.run_turn = lambda *args, _error=internal, **kwargs: _raise(_error)
...
@pytest.mark.parametrize(('policy_method', 'facade_method', 'arguments'), [('list_pending', 'list_memory_proposals', ()), ('approve', 'approve_memory_proposal', ('proposal-id',)), ('reject', 'reject_memory_proposal', ('proposal-id',))])
def test_memory_proposal_operations_map_store_failures(tmp_path, monkeypatch, policy_method, facade_method, arguments):
...
def test_provider_and_tool_details_are_structured(tmp_path):
    facade = _agent(tmp_path)
    provider_failure = LLMError('unavailable', code='rate_limit', retryable=True, fallback_eligible=True)
    facade.runtime.run_turn = lambda *args, **kwargs: _raise(provider_failure)
    ...
    validation_failure = ToolValidationError('lookup', [ToolValidationIssue(path='$.token', message='invalid', actual='token=super-secret')], {'type': 'object'})
    facade.runtime.run_turn = lambda *args, **kwargs: _raise(validation_failure)
...
def test_public_error_redacts_messages_details_and_serialization():
    secrets = ('sk-abcdefghijklmnopqrstuvwxyz', 'token=super-secret-token', 'password: hunter2', 'Authorization: Bearer abc.def.ghi')
    error = ProviderError('request failed with token=super-secret-token and sk-abcdefghijklmnopqrstuvwxyz', details=ErrorDetails(provider='openai', extensions={'api_key': 'sk-abcdefghijklmnopqrstuvwxyz', 'nested': {'password': 'hunter2', 'message': 'Authorization: Bearer abc.def.ghi'}}))
...
async def test_async_cancellation_is_not_wrapped(tmp_path):
    ...
    facade.runtime.run_turn_async = cancel
    with pytest.raises(asyncio.CancelledError):
        await facade.run('hello')

def test_recoverable_tool_failure_remains_a_result():
    registry = ToolRegistry()
    registry.register(Tool(name='explode', description='Fail safely', args_schema={'type': 'object', 'properties': {}, 'additionalProperties': False}, callable=lambda arguments: _raise(RuntimeError('token=secret-value'))))
