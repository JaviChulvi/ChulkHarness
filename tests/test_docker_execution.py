...
import subprocess
import pytest
from chulk import CommandExecutionRequest, DockerExecutionBackend, DockerPolicy, DockerUnavailableError, EnvironmentPolicy, ExecutionSessionRequest, NetworkPolicy, ProcessHandle, ProcessStartRequest, SecretPolicy, WorkspacePolicyError
...
def _completed(arguments: list[str], *, returncode: int=0, stdout: str='', stderr: str='') -> subprocess.CompletedProcess[str]:
...
def test_docker_container_uses_locked_down_flags_and_cleans_up(tmp_path: Path, monkeypatch) -> None:
    (tmp_path / 'seed.txt').write_text('seed', encoding='utf-8')
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test:latest', user='1000:1000'))
    captured = []
    ...
    assert ['--user', '1000:1000'] == run_command[run_command.index('--user'):run_command.index('--user') + 2]
    ...
    assert ['--entrypoint', '/bin/sh'] == run_command[run_command.index('--entrypoint'):run_command.index('--entrypoint') + 2]
...
def test_docker_environment_requires_explicit_secret_approval(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv('SAFE_VALUE', 'safe')
    monkeypatch.setenv('SERVICE_TOKEN', 'secret')
    monkeypatch.setattr('chulk.execution.docker.shutil.which', lambda _name: '/docker')
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test', user='1000'), environment_policy=EnvironmentPolicy(allowed_names=('SAFE_VALUE', 'SERVICE_TOKEN')))
    captured = []
    _fake_cli(backend, captured)
    session = backend.open_session(ExecutionSessionRequest(turn_id='environment'))
    environment = next((command[1] for command, _timeout in captured if command[0] == 'env-content'))
    ...
    session.close()
    backend.close()
    approved = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test', user='1000'), environment_policy=EnvironmentPolicy(allowed_names=('SAFE_VALUE', 'SERVICE_TOKEN')), secret_policy=SecretPolicy(allowed_environment_names=('SERVICE_TOKEN',)))
    approved_captured = []
    _fake_cli(approved, approved_captured)
    approved_session = approved.open_session(ExecutionSessionRequest(turn_id='approved'))
    approved_environment = next((command[1] for command, _timeout in approved_captured if command[0] == 'env-content'))
    assert approved_environment == 'SAFE_VALUE=safe\nSERVICE_TOKEN=secret\n'
    approved_session.close()
    approved.close()

def test_docker_unavailability_is_diagnostic_and_never_materializes(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr('chulk.execution.docker.shutil.which', lambda _name: None)
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test'))
...
def test_docker_reports_daemon_and_image_failures(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr('chulk.execution.docker.shutil.which', lambda _name: '/docker')
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test'))
    backend._run_cli = lambda arguments, timeout_seconds: _completed(list(arguments), returncode=1)
    ...
    def image_missing(arguments, *, timeout_seconds):
        command = list(arguments)
        return _completed(command, returncode=1 if command[:2] == ['image', 'inspect'] else 0)
...
def test_docker_removes_container_when_startup_probe_fails(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr('chulk.execution.docker.shutil.which', lambda _name: '/docker')
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test'))
    captured = []
...
def test_docker_rejects_root_container_user(tmp_path: Path, user: str) -> None:
    with pytest.raises(ValueError, match='non-root'):
        DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test', user=user))
...
def test_docker_rejects_implicit_environment_forwarding(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match='explicit allowlist'):
        DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test'), environment_policy=EnvironmentPolicy(inherit_all=True))

def test_docker_command_and_managed_process_transports_are_contained(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr('chulk.execution.docker.shutil.which', lambda _name: '/docker')
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image='local/test', user='1000'))
    captured_cli = []
    _fake_cli(backend, captured_cli)
    session = backend.open_session(ExecutionSessionRequest(turn_id='transport'))
    decision = session.shell_execution_policy.prepare(ShellExecutionRequest(command='printf contained', cwd=session.project_root, timeout_seconds=1, stdout_limit_bytes=100, stderr_limit_bytes=100))
    ...
    def capture_start(**kwargs):
        ...
        return ToolResult(tool_name='process.start', success=True, observation='captured', value=ProcessHandle('process-test', backend.name, session.workspace.workspace_id))
    monkeypatch.setattr(backend.process_registry, 'start', capture_start)
    result = session.start_process(ProcessStartRequest('printf managed', interactive=True))
    ...
    assert prepared.command[:4] == ('docker', 'exec', '--interactive', 'container-id')
    assert prepared.signal_callback is not None
    prepared.signal_callback('TERM')
    assert any((command[:3] == ['exec', 'container-id', '/bin/sh'] for command, _timeout in captured_cli))
    session.close()
    backend.close()
_DOCKER_TEST_IMAGE = os.environ.get('CHULK_DOCKER_TEST_IMAGE')

@pytest.mark.skipif(os.environ.get('CHULK_RUN_DOCKER_TESTS') != '1' or not _DOCKER_TEST_IMAGE, reason='Set CHULK_RUN_DOCKER_TESTS=1 and CHULK_DOCKER_TEST_IMAGE to run')
def test_docker_backend_integration(tmp_path: Path) -> None:
    backend = DockerExecutionBackend(tmp_path, docker_policy=DockerPolicy(image=_DOCKER_TEST_IMAGE or ''))
