...
def _agent(tmp_path, llm: LLMClient | None=None, **kwargs) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=llm or StreamingLLM(), tools=[], skills=[], **kwargs)
...
def test_event_envelope_and_catalog_are_stable(tmp_path):
    ...
    assert all((event.causation_id == previous.event_id for previous, event in zip(events, events[1:])))
    assert all((set(event.to_dict()) == {'event_id', 'name', 'timestamp', 'schema_version', 'conversation_id', 'turn_id', 'profile_id', 'execution_scope', 'run_id', 'step_id', 'correlation_id', 'causation_id', 'source_event_id', 'idempotency_key', 'payload', 'extensions'} for event in events))
...
def test_event_reader_maps_schema_v1_to_the_implicit_default_profile():
    event = AgentEvent.from_dict({'name': 'run.started', 'timestamp': '2026-01-01T00:00:00+00:00', 'schema_version': 1, 'conversation_id': 'conversation', 'turn_id': 'turn', 'payload': {'message': 'hello'}, 'extensions': {}})
...
def test_event_reader_preserves_schema_v2_profile_ownership():
    event = AgentEvent.from_dict({'name': 'run.started', 'timestamp': '2026-01-01T00:00:00+00:00', 'schema_version': 2, 'profile_id': 'work', 'conversation_id': 'conversation', 'turn_id': 'turn', 'payload': {'message': 'hello'}, 'extensions': {}})
...
def test_event_reader_preserves_schema_v3_identity_and_causation():
    event = AgentEvent.from_dict({'event_id': 'event-2', 'name': 'step.started', 'timestamp': '2026-01-01T00:00:00+00:00', 'schema_version': 3, 'profile_id': 'work', 'conversation_id': 'conversation', 'turn_id': 'turn', 'execution_scope': {'tenant_id': 'tenant', 'workspace_id': 'workspace', 'actor_id': 'operator', 'agent_id': 'agent', 'agent_version': '1.0.0', 'run_id': 'run-1', 'conversation_id': 'conversation', 'trigger_id': 'trigger-1', 'channel_id': None, 'parent_run_id': None, 'grants': ['tools:read']}, 'run_id': 'run-1', 'step_id': 'step-1', 'correlation_id': 'turn', 'causation_id': 'event-1', 'source_event_id': 'trigger-1', 'idempotency_key': 'transition-1', 'payload': {'status': 'running'}, 'extensions': {}})
...
def test_learning_proposal_changes_emit_typed_content_free_events(tmp_path):
    ...
    proposal = service.create(Skills.LearningProposalDraft(kind=Skills.LearningProposalKind.MEMORY_CREATE, rationale='Explicit durable preference.', content='User prefers direct summaries.'))
    facade.approve_learning_proposal(proposal.id)
    learning_events = [event for event in events if event.name == EventName.LEARNING_PROPOSAL_CHANGED.value]
    assert [event.payload.action for event in learning_events] == ['created', 'approved']
    assert all((isinstance(event.payload, LearningProposalChangedPayload) for event in learning_events))
...
def test_tool_permission_and_plan_lifecycle_use_curated_names(tmp_path):
    tool_llm = StreamingLLM([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '2 + 2'})}), json.dumps({'type': 'final_answer', 'content': 'Four.'})])
    tool_agent = Agent(config=AgentConfig(project_root=tmp_path / 'tool'), llm=tool_llm, tools=[Tools.calculator], skills=[])
    ...
    plan_payload = {'summary': 'Update the documentation.', 'steps': [{'id': '1', 'title': 'Edit documentation', 'description': 'Apply the requested documentation update.', 'status': 'pending'}]}
    plan_agent = _agent(tmp_path / 'plan', llm=StreamingLLM([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})]))
...
def test_callback_and_generator_receive_public_events_without_terminal_duplication(tmp_path):
    ...
    generated = list(facade.run_events('hello', on_event=run_callbacks.append, on_delta=deltas.append))
...
async def test_async_generator_does_not_reuse_a_previous_failed_turn_after_close(tmp_path):
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=StreamingLLM(), tools=[], skills=[])
...
def test_event_channel_finish_preserves_queued_events_under_backpressure():
    ...
    def event(name: str) -> AgentEvent:
        return AgentEvent(name=name, conversation_id='conversation', payload=RunStartedPayload(message=name))
...
def test_sync_generator_early_close_cleans_callbacks_and_gate(tmp_path):
    llm = StreamingLLM([json.dumps({'type': 'final_answer', 'content': 'first'}), json.dumps({'type': 'final_answer', 'content': 'second'})], delay=0.03)
...
async def test_async_generator_matches_sync_sequence_and_terminal_result(tmp_path):
    ...
    async_facade = AsyncAgent(config=AgentConfig(project_root=tmp_path / 'async'), llm=StreamingLLM(), tools=[], skills=[])
...
async def test_async_generator_close_cancels_delivery_and_releases_gate(tmp_path):
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=StreamingLLM([json.dumps({'type': 'final_answer', 'content': 'first'}), json.dumps({'type': 'final_answer', 'content': 'second'})], delay=0.03), tools=[], skills=[])
...
async def test_async_consumer_cancellation_keeps_standard_semantics(tmp_path):
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=StreamingLLM(delay=0.05), tools=[], skills=[])
