...
from chulk.llm.factory import LLMProviderConnection, create_llm_client, provider_connection_from_config, supported_llm_providers
from chulk.llm.providers.anthropic import ANTHROPIC_CAPABILITIES, DEFAULT_ANTHROPIC_MAX_OUTPUT_TOKENS, AnthropicMessagesClient, normalize_anthropic_usage
...
def _response(*blocks: object, usage: object | None=None) -> object:
    return SimpleNamespace(content=list(blocks), usage=usage or SimpleNamespace(input_tokens=4, output_tokens=2), stop_reason='end_turn')

def _text(value: str) -> object:
    return SimpleNamespace(type='text', text=value)

def _tool_use(call_id: str, *, expression: object='2 + 2') -> object:
    return SimpleNamespace(type='tool_use', id=call_id, name='calculator', input={'expression': expression})
...
def test_anthropic_text_request_splits_system_and_honors_output_limit() -> None:
    ...
    messages = [{'role': 'system', 'content': 'Be concise.'}, {'role': 'developer', 'content': 'Use plain language.'}, {'role': 'user', 'content': 'Hi'}]
    ...
    assert second == 'again'
    assert fake.messages.calls[0] == {'model': 'claude-test', 'messages': [{'role': 'user', 'content': 'Hi'}], 'max_tokens': DEFAULT_ANTHROPIC_MAX_OUTPUT_TOKENS, 'system': 'Be concise.\n\nUse plain language.'}
    assert fake.messages.calls[1]['max_tokens'] == 123

def test_anthropic_usage_normalizes_cache_tokens() -> None:
    usage = normalize_anthropic_usage({'input_tokens': 5, 'output_tokens': 7, 'cache_creation_input_tokens': 11, 'cache_read_input_tokens': 13})
...
def test_anthropic_uses_one_native_tool_call_and_disables_parallel_use() -> None:
    ...
    client = AnthropicMessagesClient(model='claude-test', client=fake)
    result = client.complete_action([{'role': 'user', 'content': 'what is 2+2?'}], tools=[_calculator_tool()], max_output_tokens=321)
...
def test_anthropic_native_text_becomes_final_answer() -> None:
    ...
    client = AnthropicMessagesClient(model='claude-test', client=fake)
    result = client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[_calculator_tool()])
...
def test_anthropic_planning_policy_enables_native_transport_without_regular_tools() -> None:
    ...
    assert result.action == FinalAnswerAction(type='final_answer', content='native final')
    assert [item['name'] for item in fake.messages.calls[0]['tools']] == [PLAN_TOOL_NAME]
    assert fake.messages.calls[0]['tool_choice'] == {'type': 'any', 'disable_parallel_tool_use': True}
    assert result.metadata['action_transport'] == 'provider_native'

def test_anthropic_rejects_multiple_tool_calls_and_uses_json_fallback() -> None:
    fake = FakeAnthropicClient([_response(_tool_use('toolu_1'), _tool_use('toolu_2', expression='3 + 3')), _response(_text(_final_answer_json('safe fallback')))])
    client = AnthropicMessagesClient(model='claude-test', client=fake)
...
def test_anthropic_falls_back_only_for_unsupported_native_transport() -> None:
    fake = FakeAnthropicClient([RuntimeError('native tools unsupported'), _response(_text(_final_answer_json('fallback')))])
    client = AnthropicMessagesClient(model='claude-test', client=fake)
    result = client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[_calculator_tool()])
    assert isinstance(result.action, FinalAnswerAction)
...
def test_anthropic_does_not_fallback_for_authentication_failures() -> None:
    ...
    with pytest.raises(LLMError) as error:
        client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[_calculator_tool()])
...
def test_anthropic_streams_text_and_normalizes_final_usage() -> None:
    events = iter([SimpleNamespace(type='message_start', message=SimpleNamespace(usage=SimpleNamespace(input_tokens=4, output_tokens=0, cache_creation_input_tokens=0, cache_read_input_tokens=0))), SimpleNamespace(type='content_block_delta', delta=SimpleNamespace(type='text_delta', text='hel')), SimpleNamespace(type='content_block_delta', delta=SimpleNamespace(type='text_delta', text='lo')), SimpleNamespace(type='message_delta', delta=SimpleNamespace(stop_reason='end_turn'), usage=SimpleNamespace(output_tokens=2)), SimpleNamespace(type='message_stop')])
...
def test_anthropic_rejects_hosted_mcp_without_making_a_request() -> None:
    ...
    client = AnthropicMessagesClient(model='claude-test', client=fake)
    with pytest.raises(LLMError) as error:
        client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[_calculator_tool()], hosted_mcp_servers=[{'type': 'mcp'}])
...
def test_anthropic_dependency_is_loaded_lazily(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fail_anthropic_import(name: str, globals: Mapping[str, object] | None=None, locals: Mapping[str, object] | None=None, fromlist: Sequence[str]=(), level: int=0) -> Any:
        if name == 'anthropic':
...
def test_anthropic_environment_uses_canonical_key_alias_and_optional_base_url(tmp_path: Path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'anthropic', 'CHULK_MODEL': 'claude-test', 'CHULK_ANTHROPIC_API_KEY': 'canonical-key', 'ANTHROPIC_API_KEY': 'fallback-key', 'CHULK_ANTHROPIC_BASE_URL': 'https://anthropic.example'})
    assert config.anthropic_api_key == 'canonical-key'
    assert config.anthropic_base_url == 'https://anthropic.example'
    assert provider_connection_from_config('anthropic', config) == LLMProviderConnection(api_key='canonical-key', base_url='https://anthropic.example')

def test_anthropic_environment_falls_back_to_standard_key_alias(tmp_path: Path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'anthropic', 'CHULK_MODEL': 'claude-test', 'ANTHROPIC_API_KEY': 'standard-key'})
...
def test_anthropic_environment_requires_explicit_model(tmp_path: Path) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_MODEL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'anthropic', 'CHULK_ANTHROPIC_API_KEY': 'test-key'})

def test_anthropic_factory_builds_native_client_with_bound_capabilities(monkeypatch: pytest.MonkeyPatch) -> None:
    sdk_client = object()
    captured: dict[str, object] = {}
    ...
    def fake_constructor(**kwargs: object) -> object:
        ...
        return sdk_client
    monkeypatch.setitem(sys.modules, 'anthropic', SimpleNamespace(Anthropic=fake_constructor))
    client = create_llm_client(provider='anthropic', model='claude-test', connection=LLMProviderConnection(api_key='test-key', base_url='https://anthropic.example'), timeout_seconds=12.5, max_retries=3)
    ...
    assert client.model_capabilities.context_window_tokens == 200000
    assert captured == {'api_key': 'test-key', 'base_url': 'https://anthropic.example', 'timeout': 12.5, 'max_retries': 3}

def test_anthropic_sdk_builder_cli_spec_and_safe_config_output(tmp_path: Path) -> None:
    config = AgentConfig.anthropic(project_root=tmp_path, model='claude-test', api_key='secret-anthropic-key', base_url='https://anthropic.example').to_config()
...
def test_anthropic_cli_fallback_spec_requires_and_preserves_explicit_model(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match='explicit model'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_FALLBACK_PROVIDERS': 'anthropic'})
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_FALLBACK_PROVIDERS': 'anthropic:claude-fallback'})
    chain = create_cli_llm(config)
