...
from chulk.tools.memory import archive_memory, delete_memory, list_memories, restore_memory, search_memory, summarize_memories, update_memory

def _stores(tmp_path):
    path = tmp_path / 'shared.sqlite'
    return (SQLiteMemoryStore(path, namespace='tenant:alpha'), SQLiteMemoryStore(path, namespace='tenant:beta'))

def test_scoped_retrieval_covers_fts_like_vector_tags_profile_and_dedupe(tmp_path) -> None:
    alpha, beta = _stores(tmp_path)
    alpha_id = alpha.save_memory('Shared keyword with alpha-only context.', tags=['preference', 'shared'], embedding=[1.0, 0.0])
    beta_id = beta.save_memory('Shared keyword with beta-only context.', tags=['preference', 'shared'], embedding=[1.0, 0.0])
    alpha_duplicate = alpha.save_memory('Shared keyword with alpha-only context.', tags=['workflow'])
    ...
    assert {item.id for item in alpha.search_memory_by_embedding([1.0, 0.0], limit=10)} == {alpha_id}
    assert {item.id for item in beta.search_memory_by_embedding([1.0, 0.0], limit=10)} == {beta_id}
...
def test_scoped_mutations_and_maintenance_cannot_cross_namespace(tmp_path) -> None:
    ...
    with sqlite3.connect(alpha.db_path) as conn:
        conn.execute("UPDATE memories SET updated_at = '2000-01-01T00:00:00+00:00' WHERE id = ?", (alpha_id,))
...
def test_scoped_proposal_review_never_crosses_namespace(tmp_path) -> None:
    ...
    assert [item.id for item in alpha.list_memory_proposals(status=None)] == [alpha_proposal]
    assert [item.id for item in beta.list_memory_proposals(status=None)] == [beta_proposal]
...
def test_scoped_concurrent_writers_share_one_database_without_mixing(tmp_path) -> None:
    ...
    def write(store: SQLiteMemoryStore, prefix: str, index: int) -> str:
        return store.save_memory(f'{prefix} concurrent memory {index}', dedupe=False)
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(write, store, prefix, index) for store, prefix in ((alpha, 'alpha'), (beta, 'beta')) for index in range(20)]
...
def test_scoped_tools_and_prompt_selection_use_the_bound_store(tmp_path) -> None:
    ...
    alpha_id = alpha.save_memory('Alpha prompt marker about namespace isolation.', tags=['preference'])
    beta_id = beta.save_memory('Beta prompt marker about namespace isolation.', tags=['preference'])
    assert 'Alpha prompt marker' in search_memory({'query': 'namespace isolation'}, alpha).observation
    ...
    assert update_memory({'memory_id': beta_id, 'content': 'crossed'}, alpha).error == 'not_found'
    ...
    llm = ScriptedLLMClient([{'type': 'final_answer', 'content': 'Scoped answer.'}])
    with Agent(config=AgentConfig(project_root=tmp_path, store_path=path, memory_namespace='tenant:alpha'), llm=llm, tools=[], skills=[], capabilities=Capabilities.read_only().with_memory(MemoryMode.READ_ONLY)) as agent:
