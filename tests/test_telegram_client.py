...
def test_get_updates_extracts_only_text_messages() -> None:
    ...
    def request(url: str, payload: dict[str, object], timeout: float) -> object:
        ...
        return {'ok': True, 'result': [{'update_id': 7, 'message': {'chat': {'id': 11, 'type': 'private'}, 'from': {'id': 13}, 'text': 'hello'}}, {'update_id': 8, 'message': {'chat': {'id': 11}, 'photo': []}}]}
    client = TelegramClient('top-secret', request_json=request)
    updates = client.get_updates(offset=7, timeout_seconds=30)
    assert [(item.update_id, item.chat_id, item.user_id, item.text, item.chat_type) for item in updates] == [(7, 11, 13, 'hello', 'private')]
    assert calls[0][1] == {'offset': 7, 'timeout': 30, 'allowed_updates': ['message']}
    assert calls[0][2] >= 35
...
def test_send_attachment_uses_bounded_typed_upload() -> None:
    ...
    def upload(*args):
        calls.append(args)
        return {'ok': True, 'result': {}}
    client = TelegramClient('secret', request_json=lambda *_args: {'ok': True, 'result': {}}, request_upload=upload)
    client.send_attachment(42, b'pdf', mime_type='application/pdf', file_name='../../report.pdf', caption='Here')
    assert calls[0][0].endswith('/sendDocument')
    assert calls[0][1] == {'chat_id': 42, 'caption': 'Here'}
    assert calls[0][2:6] == ('document', b'pdf', '../../report.pdf', 'application/pdf')
...
def test_get_updates_normalizes_voice_and_photo_attachments() -> None:
    ...
    def request(_url: str, _payload: dict[str, object], _timeout: float) -> object:
        return {'ok': True, 'result': [{'update_id': 1, 'message': {'chat': {'id': 11, 'type': 'private'}, 'from': {'id': 13}, 'voice': {'file_id': 'voice-1', 'mime_type': 'audio/ogg'}}}, {'update_id': 2, 'message': {'chat': {'id': 11, 'type': 'private'}, 'from': {'id': 13}, 'caption': 'What is shown?', 'photo': [{'file_id': 'small'}, {'file_id': 'large'}]}}]}
    updates = TelegramClient('secret', request_json=request).get_updates(offset=None, timeout_seconds=1)
...
def test_get_updates_normalizes_iphone_video() -> None:
    ...
    def request(_url: str, _payload: dict[str, object], _timeout: float) -> object:
        return {'ok': True, 'result': [{'update_id': 3, 'message': {'chat': {'id': 11, 'type': 'private'}, 'from': {'id': 13}, 'caption': 'What happened?', 'video': {'file_id': 'video-1', 'mime_type': 'video/quicktime', 'file_name': 'IMG_0123.MOV'}}}]}
    update = TelegramClient('secret', request_json=request).get_updates(offset=None, timeout_seconds=1)[0]
...
def test_download_file_resolves_path_and_enforces_bound() -> None:
    ...
    assert calls[0][1] == 20
    oversized = TelegramClient('secret', request_json=request, request_binary=lambda *_args: b'x' * 21)
...
def test_chat_action_and_command_registration_use_bot_api_payloads() -> None:
    ...
    assert calls[1] == ('setMyCommands', {'commands': [{'command': command, 'description': description} for command, description in TELEGRAM_COMMANDS]})
