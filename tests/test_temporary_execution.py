...
import subprocess
import pytest
from chulk import Agent, AgentConfig, Capabilities, ChangeDisposition, ChangeSetApproval, CommandExecutionRequest, EnvironmentPolicy, ExecutionSessionRequest, FileReadRequest, FileWriteRequest, GitWorktreeBackend, GitWorktreePolicy, NetworkPolicy, ResourcePolicy, SecretPolicy, ShellExecutionDecision, TemporaryWorkspaceBackend, UnsafePathAction, WorkspaceMaterializationPolicy, WorkspacePersistence, WorkspacePolicyError
from tests.support import RepeatingFakeLLMClient, final_action, tool_action
import chulk.execution.temporary as temporary_module

FakeLLM = RepeatingFakeLLMClient

def _tool_call(name: str, **arguments) -> str:
    return tool_action(name, **arguments)
...
def _git(repository: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(['git', '-C', str(repository), *arguments], text=True, capture_output=True, check=True)
...
def test_temporary_session_shares_workspace_but_does_not_mutate_host(tmp_path):
    ...
    backend = TemporaryWorkspaceBackend(tmp_path, require_shell_containment=False)
    session = backend.open_session(ExecutionSessionRequest(turn_id='shared'))
    write = session.write_file(FileWriteRequest('input.txt', 'workspace\n', overwrite=True))
    command = session.run_command(CommandExecutionRequest('python -c "from pathlib import Path;print(Path(\'input.txt\').read_text().strip());Path(\'from-command.txt\').write_text(\'command\')"'))
    ...
    assert {change.path for change in command.change_set.changes} == {'from-command.txt', 'input.txt'}
...
def test_temporary_shell_fails_closed_without_host_containment_opt_in(tmp_path):
    ...
    session = backend.open_session(ExecutionSessionRequest(turn_id='containment-required'))
    result = session.run_command(CommandExecutionRequest(f'''python -c "from pathlib import Path;Path({str(outside)!r}).write_text('escaped')"'''))
...
def test_persistent_workspace_requires_explicit_cleanup(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, persistence=WorkspacePersistence.PERSISTENT)
...
def test_approved_apply_handles_modify_create_delete_and_binary(tmp_path):
    ...
    session.write_file(FileWriteRequest('modified.txt', 'new\n', overwrite=True))
    session.run_command(CommandExecutionRequest('python -c "from pathlib import Path;Path(\'created.txt\').write_text(\'created\');Path(\'deleted.txt\').unlink();Path(\'binary.bin\').write_bytes(bytes.fromhex(\'006e6577\'))"'))
    change_set = session.latest_change_set
    assert change_set is not None
    session.close()
    applied = backend.apply_change_set(change_set.change_set_id, approval=_approval(change_set.change_set_id))
    ...
    repeated = backend.apply_change_set(change_set.change_set_id, approval=_approval(change_set.change_set_id))
...
def test_apply_rejects_mismatched_approval(tmp_path):
    ...
    with pytest.raises(ValueError, match='does not match'):
        backend.apply_change_set(result.change_set.change_set_id, approval=ChangeSetApproval('different', approved_by='test-host'))
...
def test_closed_backend_rejects_change_application(tmp_path):
    ...
    with pytest.raises(RuntimeError, match='backend is closed'):
        backend.apply_change_set(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
...
def test_conflict_detection_prevents_partial_application(tmp_path):
    ...
    result = session.write_file(FileWriteRequest('b.txt', 'b-new', overwrite=True))
    assert result.change_set is not None
    (tmp_path / 'b.txt').write_text('external', encoding='utf-8')
    applied = backend.apply_change_set(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
...
def test_apply_revalidates_each_file_and_rolls_back_on_race(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(temporary_module, '_assert_expected_host_state', race_before_second_apply)
    applied = backend.apply_change_set(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
    assert applied.success is False
...
def test_apply_rejects_symlinked_host_parent_without_touching_target(tmp_path):
    ...
    result = session.write_file(FileWriteRequest('nested/file.txt', 'changed', overwrite=True))
    assert result.change_set is not None
...
def test_materialization_copies_only_explicitly_allowlisted_paths(tmp_path):
    ...
    backend = TemporaryWorkspaceBackend(tmp_path, materialization_policy=WorkspaceMaterializationPolicy(allowed_paths=('allowed',)))
...
def test_materialization_rejects_path_traversal_symlinks_and_large_files(tmp_path):
    ...
    with pytest.raises(WorkspacePolicyError, match='symlink'):
        TemporaryWorkspaceBackend(tmp_path).open_session(ExecutionSessionRequest(turn_id='symlink'))
    (tmp_path / 'link.txt').unlink()
    traversal_policy = WorkspaceMaterializationPolicy(allowed_paths=('../outside.txt',))
    with pytest.raises(WorkspacePolicyError, match='Unsafe workspace path'):
        TemporaryWorkspaceBackend(tmp_path, materialization_policy=traversal_policy).open_session(ExecutionSessionRequest(turn_id='traversal'))
    (tmp_path / 'large.txt').write_text('large', encoding='utf-8')
    with pytest.raises(WorkspacePolicyError, match='configured limit'):
        TemporaryWorkspaceBackend(tmp_path, resource_policy=ResourcePolicy(max_file_bytes=4)).open_session(ExecutionSessionRequest(turn_id='large'))
...
def test_materialization_rejects_special_files_and_hardlinks(tmp_path):
    ...
    with pytest.raises(WorkspacePolicyError, match='Special workspace file'):
        TemporaryWorkspaceBackend(tmp_path).open_session(ExecutionSessionRequest(turn_id='fifo'))
    fifo.unlink()
    ...
    with pytest.raises(WorkspacePolicyError, match='Hard-linked'):
        TemporaryWorkspaceBackend(tmp_path).open_session(ExecutionSessionRequest(turn_id='hardlink'))
...
def test_shell_created_unsafe_change_fails_closed(tmp_path):
    ...
    result = session.run_command(CommandExecutionRequest('python -c "from pathlib import Path;Path(\'.env\').write_text(\'SECRET=value\')"'))
    assert result.success is False
...
def test_default_environment_does_not_forward_secret_like_values(tmp_path, monkeypatch):
    ...
    result = session.run_command(CommandExecutionRequest('python -c "import os;print(os.environ.get(\'CHULK_TEST_SECRET_TOKEN\', \'missing\'))"'))
    assert result.success is True
    assert result.stdout == 'missing\n'

def test_explicit_secret_forwarding_requires_both_allowlists(tmp_path):
    environment = EnvironmentPolicy(allowed_names=('PATH', 'CHULK_TEST_SECRET_TOKEN'), overrides=(('CHULK_TEST_SECRET_TOKEN', 'forwarded'),))
    without_secret_approval = TemporaryWorkspaceBackend(tmp_path, environment_policy=environment, require_shell_containment=False)
    session = without_secret_approval.open_session(ExecutionSessionRequest(turn_id='secret-denied'))
    denied = session.run_command(CommandExecutionRequest('python -c "import os;print(os.environ.get(\'CHULK_TEST_SECRET_TOKEN\', \'missing\'))"'))
    assert denied.stdout == 'missing\n'
    approved_backend = TemporaryWorkspaceBackend(tmp_path, environment_policy=environment, secret_policy=SecretPolicy(allowed_environment_names=('CHULK_TEST_SECRET_TOKEN',)), require_shell_containment=False)
    approved_session = approved_backend.open_session(ExecutionSessionRequest(turn_id='secret-approved'))
    approved = approved_session.run_command(CommandExecutionRequest('python -c "import os;print(os.environ.get(\'CHULK_TEST_SECRET_TOKEN\', \'missing\'))"'))
...
def test_legacy_shell_policy_rewrite_and_containment_are_preserved(tmp_path):
    ...
    class RewritingPolicy:
        ...
        def prepare(self, request):
            return ShellExecutionDecision.allow('python -c "from pathlib import Path;Path(\'rewritten.txt\').write_text(\'rewritten\')"', policy_name='rewritten-contained', shell=True, containment_applied=True)
    backend = TemporaryWorkspaceBackend(tmp_path, shell_execution_policy=RewritingPolicy(), require_shell_containment=True)
    session = backend.open_session(ExecutionSessionRequest(turn_id='rewrite'))
    result = session.run_command(CommandExecutionRequest('python -c "raise SystemExit(99)"'))
    assert result.success is True
...
def test_network_deny_fails_without_unsafe_host_fallback(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, network_policy=NetworkPolicy.DENY)
    session = backend.open_session(ExecutionSessionRequest(turn_id='network'))
    result = session.run_command(CommandExecutionRequest('python -c "print(\'must not run\')"'))
    assert result.success is False
...
def test_patch_preview_is_bounded_and_marked(tmp_path):
    ...
    backend = TemporaryWorkspaceBackend(tmp_path, resource_policy=ResourcePolicy(max_file_bytes=10000, max_patch_bytes=80))
    session = backend.open_session(ExecutionSessionRequest(turn_id='patch-limit'))
    result = session.write_file(FileWriteRequest('content.txt', 'n€w\n' * 100, overwrite=True))
    assert result.change_set is not None
...
def test_apply_preserves_created_and_modified_file_modes(tmp_path):
    ...
    result = session.run_command(CommandExecutionRequest('python -c "import os;from pathlib import Path;os.chmod(\'existing.sh\', 0o755);Path(\'created.sh\').write_text(\'#!/bin/sh\\\\n\');os.chmod(\'created.sh\', 0o700)"'))
    assert result.change_set is not None
    by_path = {change.path: change for change in result.change_set.changes}
    assert by_path['existing.sh'].mode_before == 420
    assert by_path['existing.sh'].mode_after == 493
    assert by_path['created.sh'].mode_before is None
    assert by_path['created.sh'].mode_after == 448
    session.close()
    applied = backend.apply_change_set(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
    assert applied.success is True
    assert existing.stat().st_mode & 4095 == 493
    assert (tmp_path / 'created.sh').stat().st_mode & 4095 == 448
...
def test_apply_detects_host_mode_conflict(tmp_path):
    ...
    result = session.run_command(CommandExecutionRequest('python -c "import os;os.chmod(\'script.sh\', 0o755)"'))
    assert result.change_set is not None
    target.chmod(384)
    applied = backend.apply_change_set(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
    assert applied.success is False
    ...
    assert target.stat().st_mode & 4095 == 384

def test_snapshot_honors_rejected_ignored_paths(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, materialization_policy=WorkspaceMaterializationPolicy(ignored_action=UnsafePathAction.REJECT), require_shell_containment=False)
    session = backend.open_session(ExecutionSessionRequest(turn_id='ignored-change'))
    result = session.run_command(CommandExecutionRequest('python -c "from pathlib import Path;Path(\'node_modules\').mkdir();Path(\'node_modules/generated.js\').write_text(\'generated\')"'))
    assert result.success is False
    assert result.error == 'ignored_workspace_path'
    assert result.change_set is None

def test_retained_change_sets_are_count_bounded(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, resource_policy=ResourcePolicy(max_retained_change_sets=1))
...
async def test_async_temporary_session_and_apply(tmp_path):
    ...
    result = await session.write_file_async(FileWriteRequest('async.txt', 'async content'))
    assert result.change_set is not None
    await session.aclose()
    applied = await backend.apply_change_set_async(result.change_set.change_set_id, approval=_approval(result.change_set.change_set_id))
    assert applied.success is True
...
def test_runtime_returns_reviewable_changes_without_touching_host(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path)
    facade = Agent(config=AgentConfig(project_root=tmp_path, permission_profile='workspace-write'), capabilities=Capabilities.full(), execution_backend=backend, llm=FakeLLM([_tool_call('write_file', path='runtime.txt', content='review'), _final()]), skills=[], permission_callback=lambda request, record: True)
...
def test_git_worktree_strategy_isolated_cleanup_and_apply(tmp_path):
    ...
    assert workspace_root.exists()
    result = session.write_file(FileWriteRequest('tracked.txt', 'changed\n', overwrite=True))
    assert result.change_set is not None
    session.close()
    assert not workspace_root.exists()
    ...
    assert _git(repository, 'worktree', 'list', '--porcelain').stdout.count('worktree ') == 1
...
def test_git_worktree_dirty_policy_and_branch_cleanup(tmp_path):
    ...
    with pytest.raises(WorkspacePolicyError, match='clean source'):
        GitWorktreeBackend(repository).open_session(ExecutionSessionRequest(turn_id='dirty'))
    (repository / 'dirty.txt').unlink()
    backend = GitWorktreeBackend(repository, git_policy=GitWorktreePolicy(detached=False))
...
def test_git_worktree_rejects_narrow_allowlist_and_conflicting_lifecycle(tmp_path):
    with pytest.raises(WorkspacePolicyError, match='repository root'):
        GitWorktreeBackend(tmp_path, materialization_policy=WorkspaceMaterializationPolicy(allowed_paths=('tracked.txt',)))
    with pytest.raises(ValueError, match='conflicts'):
        GitWorktreeBackend(tmp_path, git_policy=GitWorktreePolicy(remove_on_close=False), persistence=WorkspacePersistence.EPHEMERAL)
...
def test_temporary_backend_rejects_direct_apply_policy(tmp_path):
    with pytest.raises(ValueError, match='cannot apply changes directly'):
        TemporaryWorkspaceBackend(tmp_path, change_disposition=ChangeDisposition.APPLY_DIRECTLY)

def test_temporary_root_cannot_be_nested_inside_source(tmp_path):
    with pytest.raises(ValueError, match='cannot be inside'):
        TemporaryWorkspaceBackend(tmp_path, temporary_root=tmp_path / '.temporary')

def test_discard_policy_does_not_publish_change_sets(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, change_disposition=ChangeDisposition.DISCARD)
...
async def test_async_backend_close_cleans_retained_workspaces(tmp_path):
    backend = TemporaryWorkspaceBackend(tmp_path, persistence=WorkspacePersistence.PERSISTENT)
    session = await backend.open_session_async(ExecutionSessionRequest(turn_id='async-backend-close'))
    workspace_root = session.project_root
    await session.aclose()
