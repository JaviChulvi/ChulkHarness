...
import sqlite3
import pytest
from chulk.evals import AsyncEvalRunner, AsyncEvalStore, AsyncSQLiteEvalStore, CallableGrader, EvalCase, EvalDataset, EvalReference, EvalReport, EvalRunStatus, EvalRunner, EvalStore, EvalSuite, EvalTarget, EvalTurn, ExactAnswerGrader, GradeResult, LatencyGrader, MetricThreshold, SQLiteEvalStore, compare_reports
...
def _case(case_id: str, answer: str='done') -> EvalCase:
    return EvalCase(case_id, (EvalTurn('run', ({'type': 'final_answer', 'content': answer},)),), EvalReference(answer=answer))
...
def test_report_json_round_trip_preserves_public_results_and_events() -> None:
    ...
    class AgentDouble:
        ...
        def run_result(self, _message, *, on_event, **_kwargs):
            ...
            on_event(AgentEvent(name='run.completed', conversation_id='conversation', profile_id='default', payload=SerializedEventPayload(data={'safe': True})))
            return _result()

        def close(self):
            return None
    suite = EvalSuite('round-trip', EvalDataset((_case('one'),)), (EvalTarget('agent', lambda _context: AgentDouble()),), (ExactAnswerGrader(),), required_graders=('answer.exact',), sampling={'temperature': 0, 'seed': 42})
...
def test_interrupted_run_is_checkpointed_and_resumes_without_repeating_trials(tmp_path: Path) -> None:
    ...
    suite = EvalSuite('resume', EvalDataset((_case('one'), _case('two'))), (EvalTarget('agent', factory),), (ExactAnswerGrader(),), required_graders=('answer.exact',), store=store)
...
async def test_async_runner_resumes_an_interrupted_checkpoint(tmp_path: Path) -> None:
    ...
    class AsyncAgentDouble:
        ...
        async def aclose(self):
            return None
    suite = EvalSuite('async-resume', EvalDataset((_case('one'), _case('two'))), (EvalTarget('agent', lambda context: AsyncAgentDouble(context.case_id)),), (ExactAnswerGrader(),), required_graders=('answer.exact',), store=store)
...
def test_v20_store_migrates_provenance_and_idempotent_normalized_records(tmp_path: Path) -> None:
    ...
    report = EvalRunner().run(EvalSuite('migration', EvalDataset((_case('one'),)), (EvalTarget('agent', lambda _context: _Agent()),), (ExactAnswerGrader(),), required_graders=('answer.exact',), sampling={'top_p': 0.8}))
    assert isinstance(report, EvalReport)
    store.save_report(report)
    store.save_report(report)
    with sqlite3.connect(path) as connection:
        connection.row_factory = sqlite3.Row
        run = connection.execute('SELECT status, suite_fingerprint, target_fingerprints_json, grader_versions_json, sampling_json FROM eval_runs WHERE id = ?', (report.id,)).fetchone()
        assert run is not None
        assert run['status'] == 'completed'
        assert len(run['suite_fingerprint']) == 64
        assert json.loads(run['sampling_json']) == {'top_p': 0.8}
        assert connection.execute('SELECT COUNT(*) FROM eval_trials WHERE run_id = ?', (report.id,)).fetchone()[0] == 1
        assert connection.execute('SELECT COUNT(*) FROM eval_grades WHERE run_id = ?', (report.id,)).fetchone()[0] == 1

def test_store_rejects_report_id_collisions_across_execution_scopes(tmp_path: Path) -> None:
    path = tmp_path / 'scoped.sqlite'
    first_scope = ExecutionScope('tenant-a', 'workspace-a', None, 'agent', '1', 'run-a')
    second_scope = ExecutionScope('tenant-b', 'workspace-b', None, 'agent', '1', 'run-b')
    first = SQLiteEvalStore(path, scope=first_scope)
    second = SQLiteEvalStore(path, scope=second_scope)
    report = EvalRunner().run(EvalSuite('tenant-a-suite', EvalDataset((_case('one'),)), (EvalTarget('agent', lambda _context: _Agent()),)))
    assert isinstance(report, EvalReport)
...
def test_store_redacts_report_and_normalized_payloads(tmp_path: Path) -> None:
    ...
    class SecretAgent:
        ...
        def run_result(self, _message, **_kwargs):
            return RunResult(f'OPENAI_API_KEY={secret}', RunStatus.COMPLETED, None, 'conversation', None, extension_metadata={'api_key': secret})

        def close(self):
            return None
    path = tmp_path / 'redacted.sqlite'
    store = SQLiteEvalStore(path)
    report = EvalRunner().run(EvalSuite('redaction', EvalDataset((_case('one'),)), (EvalTarget('agent', lambda _context: SecretAgent()),), store=store))
    assert isinstance(report, EvalReport)
    trial = report.cases[0].trials[0]
    secret_grade = GradeResult('secret.error', 0.0, False, 'provider failed', error=f'provider failed with {secret}')
    trial = replace(trial, grades=(secret_grade,), exception=f'request failed with {secret}')
    report = replace(report, cases=(replace(report.cases[0], trials=(trial,), passed=False),))
    store.save_report(report)
    with sqlite3.connect(path) as connection:
        payloads = [connection.execute('SELECT report_json FROM eval_runs WHERE id = ?', (report.id,)).fetchone()[0]]
        payloads.extend((row[0] for table in ('eval_trials', 'eval_turns') for row in connection.execute(f'SELECT payload_json FROM {table} WHERE run_id = ?', (report.id,))))
        payloads.extend((str(item) for item in connection.execute('SELECT exception FROM eval_trials WHERE run_id = ?', (report.id,)).fetchone() if item is not None))
        payloads.extend((str(item) for item in connection.execute('SELECT error FROM eval_grades WHERE run_id = ?', (report.id,)).fetchone() if item is not None))
    persisted = '\n'.join(payloads)
...
def test_baseline_identity_score_deltas_and_coverage_gate(tmp_path: Path) -> None:
    ...
    baseline_suite = EvalSuite('coverage', EvalDataset((_case('one'), _case('two'))), (target,), (VersionedExactGrader(),), required_graders=('answer.exact',), store=store)
    baseline = EvalRunner().run(baseline_suite)
    assert isinstance(baseline, EvalReport)
    store.set_baseline('coverage', baseline.id)
    current_suite = EvalSuite('coverage', EvalDataset((_case('one'),)), (target,), (VersionedExactGrader(),), required_graders=('answer.exact',), thresholds={'baseline_coverage': MetricThreshold(min=1.0)}, store=store)
...
def test_grader_identity_tracks_configuration_and_callable_behavior() -> None:
    ...
    def contract(grader: object) -> tuple[str, str]:
        report = EvalRunner().run(EvalSuite('identity', dataset, (target,), (grader,)))
        assert isinstance(report, EvalReport)
        grader_metadata = report.metadata['graders'][0]
        return (str(grader_metadata['identity']), str(report.metadata['suite_fingerprint']))
    ...
    def failing(_case, _trial):
        return False
    assert contract(CallableGrader('custom', passing)) != contract(CallableGrader('custom', failing))
