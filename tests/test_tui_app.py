...
async def _wait_for_latest_worker(app: OperatorApp, group: str) -> None:
    workers = [worker for worker in app.workers if worker.group == group and (not worker.is_finished)]
    if workers:
...
class FakeOperatorSource:
    ...
    def refresh(self) -> OperatorSnapshot:
        return OperatorSnapshot(profile_id=self.profile_id, selected_conversation_id=self.selected_conversation_id, profiles=({'id': 'default', 'model_profile_id': 'balanced'}, {'id': 'research', 'model_profile_id': 'deep'}), conversations=({'id': 'conv-1', 'title': 'Release investigation', 'status': 'active', 'turn_count': 3},), permissions=({'id': 'perm-1', 'tool_name': 'shell', 'status': 'pending'},), proposals=({'id': 'proposal-1', 'kind': 'skill', 'status': 'pending'},), goals=({'id': 'goal-1', 'objective': 'Ship operator controls', 'status': 'running', 'revision': 5},), tasks=({'id': 'task-1', 'title': 'Verify controls', 'status': 'failed', 'revision': 2},), jobs=({'id': 'job-1', 'prompt_preview': 'Run release checks', 'status': 'paused', 'revision': 4},), usage=({'key': 'model', 'total_cost': '0.02'},), traces=({'conversation_id': 'conv-1', 'status': 'active', 'available': True, 'artifact_count': 2},), artifacts=({'artifact_id': 'artifact-1', 'name': 'shell-output.txt', 'kind': 'tool-output', 'size_bytes': 128},), timeline=(TimelineEntry('system', 'Connected.'),))
...
async def test_operator_app_renders_all_control_surfaces_and_submits_prompt() -> None:
    ...
    async with app.run_test(size=(160, 48)) as pilot:
        ...
        assert 'ATTENTION' in str(app.query_one('#evidence-pulse', Static).render())
...
async def test_operator_app_remains_usable_narrow_monochrome_and_reconnects() -> None:
    ...
    async with app.run_test(size=(76, 30)) as pilot:
        ...
        app.action_reload()
        await _wait_for_latest_worker(app, 'snapshot')
        assert 'profile recovered' in str(app.query_one('#status-line', Static).render())
