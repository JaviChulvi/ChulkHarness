...
def test_profile_cli_create_list_inspect_and_use(monkeypatch, tmp_path, capsys):
    ...
    project.mkdir()
    assert main(['profile', 'create', 'work', '--project-root', str(project), '--permission-profile', 'read-only', '--skill', 'files', '--credential-env', 'WORK_API_KEY', '--json']) == 0
    created = json.loads(capsys.readouterr().out)
    assert created['profile']['id'] == 'work'
    assert created['profile']['credential_refs'] == [{'name': 'WORK_API_KEY', 'source': 'environment'}]
...
def test_selected_profile_and_one_shot_override_use_isolated_config(monkeypatch, tmp_path, capsys):
    ...
    assert main(['exec', '--profile', 'personal', 'hello', '--json'], llm_client_factory=factory) == 0
    ...
    store = SQLiteProfileStore(tmp_path / '.chulk' / 'control.sqlite', base_config=base_config)
