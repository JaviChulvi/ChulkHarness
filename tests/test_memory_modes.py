...
def _agent(root, mode: str, responses: list[str] | None=None) -> Agent:
    return Agent(config=AgentConfig(project_root=root, permission_profile='workspace-write'), capabilities=Capabilities(files='off', memory=mode, utilities=False), llm=FakeLLM(responses), skills=[])
...
def test_sdk_config_applies_opt_in_retention_at_local_runtime_startup(tmp_path):
    ...
    config = AgentConfig(project_root=tmp_path, memory_retention_policy=MemoryRetentionPolicy(max_active_items=1)).to_config()
    assert config.memory_retention_policy is not None
    facade = Agent(config=config, capabilities=Capabilities(files='off', memory='read-only', utilities=False), llm=FakeLLM(), skills=[])
...
def test_off_mode_disables_tools_retrieval_and_inferred_writes(tmp_path, monkeypatch):
    facade = _agent(tmp_path, 'off')

    def fail_extraction(*_args, **_kwargs):
        raise AssertionError('write-disabled memory must not run candidate extraction')
    monkeypatch.setattr('chulk.memory.context.route_memory_candidates', fail_extraction)
...
def test_read_only_mode_retrieves_without_running_memory_extraction(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_async_write_disabled_modes_skip_candidate_extraction(tmp_path, monkeypatch, mode):
    facade = _agent(tmp_path, mode.value)
    facade.runtime.memory_context.async_policy = AsyncMemoryPolicy(facade.runtime.memory_context.store, mode)

    def fail_extraction(*_args, **_kwargs):
        raise AssertionError('write-disabled memory must not run candidate extraction')
    monkeypatch.setattr('chulk.memory.context.extract_memory_candidates', fail_extraction)
    await facade.runtime.memory_context.extract_async('Remember that project beta uses Postgres.')
...
def test_manual_mode_persists_proposals_across_restart_and_approves(tmp_path):
    ...
    assert [memory.content for memory in restarted.runtime.memory_context.store.list_memories()] == ['project alpha uses SQLite']
...
def test_concurrent_proposal_approval_persists_exactly_one_memory(tmp_path, monkeypatch):
    ...
    def force_original_race_window(content: str, *, threshold: float=0.9):
        race.wait(timeout=2)
        return None
...
def test_automatic_mode_persists_inferred_memory_without_proposal(tmp_path):
    ...
    assert facade.list_memory_proposals() == ()
    assert [memory.content for memory in facade.runtime.memory_context.store.list_memories()] == ['project alpha uses SQLite']

def test_manual_explicit_save_tool_creates_review_proposal(tmp_path):
    facade = _agent(tmp_path, 'manual', [json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'save_memory', 'arguments_json': json.dumps({'content': 'User prefers concise answers'})}), json.dumps({'type': 'final_answer', 'content': 'proposed'})])
