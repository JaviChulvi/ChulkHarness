...
from chulk.gateway import SHARED_CHANNEL_COMMANDS, parse_channel_command, shared_command_help, shared_command_spec

def test_shared_channel_commands_have_stable_unique_names() -> None:
    names = tuple((item.name for item in SHARED_CHANNEL_COMMANDS))
    assert names == ('new', 'status', 'stop', 'model', 'skills', 'memory', 'agents', 'jobs')
