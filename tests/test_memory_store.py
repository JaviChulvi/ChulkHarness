...
import sqlite3
import pytest
from chulk.memory import DEFAULT_MEMORY_NAMESPACE, MemoryRetentionPolicy, SQLiteMemoryStore, normalize_memory_namespace, select_memories_for_prompt, text_to_embedding
...
def test_sqlite_memory_store_saves_searches_lists_and_deletes(tmp_path):
    ...
    memory_id = store.save_memory('Javier prefers direct repo-grounded implementation details.', tags=['preference', 'workflow'], metadata={'source': 'test'}, importance=8)
...
def test_sqlite_memory_store_updates_memory(tmp_path):
    ...
    updated = store.update_memory(memory_id, content='Updated memory about ChulkHarness', tags=['project', 'chulk'], metadata={'repo': 'chulk'}, importance=5)
...
def test_sqlite_memory_store_fts_embedding_and_source_confidence(tmp_path):
    store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    memory_id = store.save_memory('ChulkHarness memory retrieval should use FTS before embeddings.', tags=['project', 'search'], source='test', confidence=0.7, importance=6)
...
def test_sqlite_memory_store_summarizes_and_finds_profile_memories(tmp_path):
    store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    profile_id = store.save_memory('Prefer exact files, exact commands, and direct answers.', tags=['persona', 'preference'], importance=9)
...
def test_sqlite_memory_store_deduplicates_archives_restores_and_compacts(tmp_path):
    ...
    second_id = store.save_memory('Prefer exact commands and tested changes.', tags=['preference'], dedupe=False)
...
def test_sqlite_memory_store_applies_archive_only_retention_by_namespace(tmp_path):
    ...
    proposal_id = store.create_memory_proposal('pending memory', evidence='user asked to remember it', conversation_id='conversation-1', turn_id='turn-1')
    created_at = datetime.fromisoformat(store.get_memory(old_id).updated_at)
    retention_time = created_at + timedelta(days=31)
    archived = store.apply_retention(MemoryRetentionPolicy(max_age_days=30), now=retention_time)
    assert archived == 1
...
def test_combined_retention_counts_only_nonexpired_memories(tmp_path):
    store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    expired_ids = [store.save_memory(f'expired memory record{index:03d}', importance=10) for index in range(20)]
    fresh_ids = [store.save_memory(f'fresh memory record{index:03d}', importance=1) for index in range(90)]
    ...
    with sqlite3.connect(store.db_path) as conn:
        conn.executemany('UPDATE memories SET updated_at = ? WHERE id = ?', [(expired_at, memory_id) for memory_id in expired_ids])
    archived = store.apply_retention(MemoryRetentionPolicy(max_age_days=30, max_active_items=100), now=now)
    assert archived == 20
    assert all((store.get_memory(memory_id, include_archived=True).archived_at == now.astimezone(timezone.utc).isoformat() for memory_id in expired_ids))
...
def test_memory_retention_policy_validates_limits_and_clock(tmp_path):
    ...
    with pytest.raises(ValueError, match='timezone-aware'):
        store.apply_retention(MemoryRetentionPolicy(max_age_days=1), now=datetime(2026, 8, 29))
...
def test_sqlite_memory_store_validates_inputs(tmp_path):
    store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    for kwargs in [{'content': ''}, {'content': 'x', 'importance': 0}, {'content': 'x', 'importance': 11}, {'content': 'x', 'confidence': -0.1}, {'content': 'x', 'confidence': 1.1}]:
        try:
