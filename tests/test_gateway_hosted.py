...
from chulk.gateway import AsyncDurableGatewayRunSubmitter, AsyncPublishedDefinitionGatewayResolver, AuthenticationState, ChannelIdentity, ChannelScope, DurableGatewayRunSubmitter, GatewayRunTarget, InboundEnvelope, PublishedDefinitionGatewayResolver, TextPart, TrustLevel
...
def _record(status: DefinitionStatus=DefinitionStatus.PUBLISHED):
    return SimpleNamespace(status=status, definition=SimpleNamespace(agent_id='support', version='1.0.0', digest='sha256:definition'))
...
def test_definition_resolver_requires_one_published_revision() -> None:
    ...
    class Definitions:
        ...
        def get(self, scope, agent_id, version):
            ...
            return _record()
    resolver = PublishedDefinitionGatewayResolver(Definitions(), lambda route, envelope: _scope())
    ...
    with pytest.raises(ValueError, match='not published'):
        PublishedDefinitionGatewayResolver(RevokedDefinitions(), lambda route, envelope: _scope()).resolve(SimpleNamespace(profile_id='work'), _envelope())
...
async def test_async_gateway_resolver_and_submitter_are_native() -> None:
    ...
    resolver = AsyncPublishedDefinitionGatewayResolver(Definitions(), scope_factory)
    target = await resolver.resolve(SimpleNamespace(profile_id='work'), _envelope())
    store = AsyncInMemoryRunStore()
    submitter = AsyncDurableGatewayRunSubmitter(store)
    run_id = await submitter.submit(target, _envelope(), idempotency_key='gateway:event-1')
