...
import json
from chulk.core.reflection import ReflectionParseError, build_reflection_messages, parse_reflection_response
...
def test_reflection_references_plan_evidence_without_copying_its_content() -> None:
    ...
    plan.approve()
    turn = TurnState(user_message='Please make the change.', active_plan=plan, plan_approved=True)
...
def test_reflection_includes_bounded_step_description_and_acceptance_criteria() -> None:
    ...
    step = PlanStep(id='1', title='Generic work', description=description, acceptance_criteria=[criterion])
    plan = Plan(summary='Complete the work.', steps=[step])
    plan.approve()
    turn = TurnState(user_message='Please complete it.', active_plan=plan, plan_approved=True)
