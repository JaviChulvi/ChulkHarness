...
import json
import pytest
from chulk import Agent, AgentConfig, ContextReport, ContextSection, Cost, Observation, Plan, PlanStatus, PlanStep, PlanStepEvidence, PlanStepStatus, RunResult, RunStatus, Tool, ToolCall, Usage
from chulk._sdk.results import cost_snapshot, usage_snapshot
from chulk.llm import LLMCost, LLMUsage
from tests.support import RepeatingFakeLLMClient, final_action

class FakeLLM(RepeatingFakeLLMClient):
    provider = 'test'
    model = 'typed-results'
...
def test_run_result_uses_typed_immutable_records(tmp_path):
    ...
    llm = FakeLLM([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'lookup', 'arguments_json': json.dumps({'query': 'sdk'})}), _final()])
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[lookup], skills=[])
    ...
    with pytest.raises(TypeError):
        result.extension_metadata['new'] = True
    with pytest.raises(TypeError):
        result.extension_metadata['adapter']['version'] = 2
    with pytest.raises(TypeError):
...
def test_snapshot_is_detached_from_later_runtime_mutation(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLM([_final()]), tools=[], skills=[])
...
def test_plan_and_context_snapshots_use_finite_statuses_and_tuples(tmp_path):
    plan_payload = {'summary': 'Update docs', 'steps': [{'id': '1', 'title': 'Edit docs', 'description': 'Apply the documentation update.', 'status': 'pending', 'depends_on': [], 'acceptance_criteria': ['Docs are updated']}]}
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLM([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})]), tools=[], skills=[])
...
def test_unknown_statuses_do_not_masquerade_as_current_values():
    step = PlanStep(id='1', title='Future', description='Future', status='future')
    plan = Plan(summary='Future', status='future', steps=(step,))
    result = RunResult(content='', status='future', turn_id=None, conversation_id='conversation', trace_path=None, plan=plan)
...
def test_cache_write_fields_survive_public_snapshots_and_old_payloads():
    usage = usage_snapshot(LLMUsage(input_tokens=100, cache_write_input_tokens=20))
    cost = cost_snapshot(LLMCost(amount=Decimal('0.5'), pricing_known=True, cache_write_input_cost=Decimal('0.1')))
...
def test_cache_write_fields_preserve_public_positional_constructors():
    usage = Usage(1, 2, 3, 4, 5, 6, 7, True, True, 'legacy', {'old': True})
    cost = Cost(Decimal('1'), 'EUR', True, True, Decimal('0.2'), Decimal('0.1'), Decimal('0.7'), 'test', 'legacy', 'https://models.example/pricing', '2026-07-13')
...
def test_every_nested_contract_collection_is_immutable():
    ...
    call = ToolCall(tool_name='lookup', arguments={'filters': {'tags': ['sdk']}}, iteration=1, metadata={'timing': {'attempts': [1]}})
    observation = Observation(tool_name='lookup', content='done', output_metadata={'artifacts': [{'path': 'result.txt'}]})
    section = ContextSection(name='history', label='History', char_count=1, estimated_tokens=1, metadata={'roles': {'user': 1}})
    evidence = PlanStepEvidence(content='verified', metadata={'checks': ['tests']})
    step = PlanStep(id='1', title='Verify', description='Verify', depends_on=('0',), acceptance_criteria=('tests pass',), evidence=(evidence,))
    ...
    for mapping, key in ((usage.raw, 'new'), (call.arguments, 'new'), (call.metadata, 'new'), (observation.output_metadata, 'new'), (section.metadata, 'new'), (evidence.metadata, 'new')):
        with pytest.raises(TypeError):
            mapping[key] = True
...
def test_failure_and_terminal_event_serialization_preserve_typed_shape(tmp_path):
    ...
    failed = RunResult(content='failed', status=RunStatus.FAILED, turn_id='turn', conversation_id='conversation', trace_path=None, errors=('failed',))
