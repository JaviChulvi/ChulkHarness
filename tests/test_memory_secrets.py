...
import pytest
from chulk.memory import AsyncMemoryPolicy, MemoryExtractionCandidate, MemoryPolicy, SQLiteMemoryStore
from chulk.memory.security import MemorySecretError
from chulk.tools.memory import async_import_memories_tool, async_save_memory_tool, async_update_memory_tool
...
@pytest.mark.parametrize('content', ['OPENAI_API_KEY=sk-proj-FakeCredentialValue123456789', 'Database password: correct-horse-battery-staple', 'Authorization: Bearer FakeBearerCredential123456789', 'GitHub token github_pat_FakeCredentialValue123456789', 'Database URL is postgresql://user:FakePassword123@localhost/app'])
...
def test_save_memory_rejects_secrets_nested_in_metadata(tmp_path):
    ...
    with pytest.raises(MemorySecretError) as exc_info:
        store.save_memory('User prefers concise answers.', tags=['preference'], metadata={'provider': {'accessToken': secret}})
...
def test_import_preflights_all_memories_before_writing_any_rows(tmp_path):
    ...
    markdown.write_text(f'- [preference] User prefers concise answers.\n- [project] OPENAI_API_KEY={secret}\n', encoding='utf-8')
    with pytest.raises(MemorySecretError) as exc_info:
...
def test_create_proposal_rejects_secret_in_evidence(tmp_path):
    ...
    with pytest.raises(MemorySecretError) as exc_info:
        store.create_memory_proposal('User prefers concise answers.', tags=['preference'], evidence=f'Authorization: Bearer {secret}')
    assert secret not in str(exc_info.value)
...
def test_approval_rechecks_legacy_pending_proposal_before_saving(tmp_path):
    ...
    with sqlite3.connect(store.db_path) as conn:
        conn.execute('UPDATE memory_proposals SET content = ? WHERE id = ?', (f'OPENAI_API_KEY={secret}', proposal_id))
    with pytest.raises(MemorySecretError) as exc_info:
...
@pytest.mark.parametrize('content', ['User prefers environment variables for API keys.', 'User uses a password manager.', 'Keep token budgets under 4,000 tokens.', 'OPENAI_API_KEY should be configured before startup.', 'The password is strong and stored in a password manager.'])
...
def test_inferred_secret_candidates_are_skipped_without_aborting_the_turn(tmp_path, mode):
    ...
    candidate = MemoryExtractionCandidate(content=f'OPENAI_API_KEY={secret}', tags=['explicit', 'user'])
    result = policy.handle_candidates([candidate], conversation_id='conversation-id', turn_id='turn-id', evidence=f'Remember OPENAI_API_KEY={secret}')
...
@pytest.mark.asyncio
@pytest.mark.parametrize(('tool_factory', 'arguments'), [(async_save_memory_tool, {'content': 'OPENAI_API_KEY=sk-proj-FakeHostedSaveCredential123456789'}), (async_update_memory_tool, {'memory_id': 'memory-1', 'metadata': {'provider': {'accessToken': 'FakeHostedUpdateCredential123456789'}}})])
async def test_async_memory_tools_reject_secrets_before_hosted_write(tool_factory, arguments):
...
async def test_async_memory_import_preflights_before_hosted_write(tmp_path):
    ...
    result = await registry.run_async('import_memories', {'path': 'MEMORY.md'})
    assert not result.success
    assert result.error is not None
...
async def test_async_memory_policy_rejects_secrets_before_hosted_write(mode):
    ...
    result = await policy.handle_candidates([MemoryExtractionCandidate(content=f'OPENAI_API_KEY={secret}', tags=['explicit', 'user'])], conversation_id='conversation-id', turn_id='turn-id', evidence=f'Remember OPENAI_API_KEY={secret}')
    assert result.accepted_memory_ids == ()
    assert result.proposal_ids == ()
    assert store.calls == []
...
async def test_async_memory_policy_rechecks_proposal_before_hosted_approval():
    ...
    class ProposalStore:
        ...
        async def list_memory_proposals(self, **_kwargs):
            return [SimpleNamespace(id='proposal-1', content=f'OPENAI_API_KEY={secret}', tags=[], metadata={}, source='manual_review', evidence='legacy proposal', conversation_id='conversation-id', turn_id='turn-id')]
...
async def test_async_memory_policy_rechecks_mapping_proposal_before_approval():
    ...
    class ProposalStore:
        ...
        async def list_memory_proposals(self, **_kwargs):
            return [{'id': 'proposal-1', 'content': f'OPENAI_API_KEY={secret}', 'tags': [], 'metadata': {}, 'source': 'manual_review', 'evidence': 'legacy proposal', 'conversation_id': 'conversation-id', 'turn_id': 'turn-id'}]
