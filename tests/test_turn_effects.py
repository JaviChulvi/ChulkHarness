...
from chulk.core.plan_execution import PlanExecution, PlanStepVerification, PlanStepVerificationRequest
...
@pytest.mark.parametrize(('outcome', 'response', 'pending'), [(TransitionOutcome.STOP, None, None), (TransitionOutcome.AWAIT_RESULT, None, None), (TransitionOutcome.CONTINUE, 'unexpected', None), (TransitionOutcome.PROCEED, None, PendingReflection(proposed_answer='unexpected'))])
def test_transition_application_rejects_mismatched_reducer_outcome(outcome: TransitionOutcome, response: str | None, pending: PendingReflection | None) -> None:
...
def test_transition_application_preserves_reducer_outcome() -> None:
    application = _validate_application(outcome=TransitionOutcome.STOP, response='done', pending=None)
...
def test_finished_tool_adds_bounded_redacted_action_before_observation() -> None:
    ...
    effects = TurnEffects(state=state, memory=memory, get_llm_client=lambda: ScriptedLLMClient([]), plan=PlanExecution(state=state, memory=memory, trace=trace), trace=trace, redact_text=redact, artifact_writer=lambda _name, _content: None, planning_tool_names=lambda: frozenset(), max_tool_calls_per_turn=5, max_reflection_attempts=0, max_observation_chars=1000, max_tool_stdout_chars=1000, max_tool_stderr_chars=1000)
    turn = TurnState(user_message='Use the lookup tool.')
    action = ToolCallAction(type='tool_call', tool_name='lookup', arguments={'payload': 'UNIQUE_ARGUMENT_' + 'x' * 5000, 'token': 'SECRET'})
    pending = effects._start_tool(turn, ExecuteToolEffect(action=action, phase='execution'))
    blocked_message = effects._finish_tool(turn, FinishToolEffect(disposition='none'), pending=pending, result=ToolResult(tool_name='lookup', success=True, observation='lookup complete', exit_code=0))
    ...
    action_payload = json.loads(action_context.removeprefix('<executed_tool_action>\n').removesuffix('\n</executed_tool_action>'))
    ...
    observation_events = [payload for event, payload in events if event == TraceEvent.TOOL_OBSERVATION and payload is not None]
    assert observation_events[-1]['tool_action_context'] == action_context
    assert observation_events[-1]['observation_index'] == 1
    assert 'lookup complete' in observation_events[-1]['turn']['observations'][0]['content']
    completed_payload = next((payload for event, payload in events if event == TraceEvent.TOOL_CALL_COMPLETED and payload is not None))
...
def test_planned_tool_observation_checkpoint_includes_step_evidence() -> None:
    ...
    def trace(event_type: str, payload: dict | None) -> None:
        events.append((event_type, payload))
    effects = TurnEffects(state=state, memory=memory, get_llm_client=lambda: ScriptedLLMClient([]), plan=PlanExecution(state=state, memory=memory, trace=trace), trace=trace, redact_text=lambda _event, text, _metadata: (text, {'redacted': False}), artifact_writer=lambda _name, _content: None, planning_tool_names=lambda: frozenset(), max_tool_calls_per_turn=5, max_reflection_attempts=0, max_observation_chars=1000, max_tool_stdout_chars=1000, max_tool_stderr_chars=1000)
    plan = Plan(summary='Use one tool.', steps=[PlanStep(id='1', title='Lookup', description='Run lookup.')])
    plan.approve()
    plan.steps[0].mark('in_progress')
    turn = TurnState(user_message='Use lookup.', active_plan=plan, plan_approved=True)
    pending = effects._start_tool(turn, ExecuteToolEffect(action=ToolCallAction(type='tool_call', tool_name='lookup', arguments={}), phase='execution'))
    effects._finish_tool(turn, FinishToolEffect(disposition='evidence'), pending=pending, result=ToolResult(tool_name='lookup', success=True, observation='lookup complete'))
    observation_payload = next((payload for event, payload in events if event == TraceEvent.TOOL_OBSERVATION and payload is not None))
...
def test_terminal_tool_outcome_blocks_step_before_observation_checkpoint() -> None:
    ...
    def trace(event_type: str, payload: dict | None) -> None:
        events.append((event_type, payload))
    effects = TurnEffects(state=state, memory=memory, get_llm_client=lambda: ScriptedLLMClient([]), plan=PlanExecution(state=state, memory=memory, trace=trace), trace=trace, redact_text=lambda _event, text, _metadata: (text, {'redacted': False}), artifact_writer=lambda _name, _content: None, planning_tool_names=lambda: frozenset(), max_tool_calls_per_turn=5, max_reflection_attempts=0, max_observation_chars=1000, max_tool_stdout_chars=1000, max_tool_stderr_chars=1000)
    plan = Plan(summary='Use one failing tool.', steps=[PlanStep(id='1', title='Lookup', description='Run lookup.')])
    plan.approve()
    plan.steps[0].mark('in_progress')
    turn = TurnState(user_message='Use lookup.', active_plan=plan, plan_approved=True)
    pending = effects._start_tool(turn, ExecuteToolEffect(action=ToolCallAction(type='tool_call', tool_name='lookup', arguments={}), phase='execution'))
    blocked_message = effects._finish_tool(turn, FinishToolEffect(disposition='block', blocked_reason='Tool retry limit exhausted.'), pending=pending, result=ToolResult(tool_name='lookup', success=False, observation='lookup failed', error='timeout'))
    observation_payload = next((payload for event, payload in events if event == TraceEvent.TOOL_OBSERVATION and payload is not None))
    blocked_payload = next((payload for event, payload in events if event == TraceEvent.PLAN_STEP_BLOCKED and payload is not None))
...
def test_presented_plan_stays_in_canonical_state_not_model_history() -> None:
    ...
    plan = Plan(summary='Change the behavior.', steps=[PlanStep(id='1', title='Implement', description='Update the component.')])
...
def test_plan_completion_verifier_rejection_keeps_step_in_progress() -> None:
    ...
    def verifier(request: PlanStepVerificationRequest) -> PlanStepVerification:
        ...
        return PlanStepVerification(passed=False, evidence='The required test has not passed.')
    execution = PlanExecution(state=state, memory=memory, trace=lambda _event, _payload: None, verifier=verifier)
    turn, step = _active_plan_step(execution)
    result = execution.apply_step_result(turn, PlanStepUpdateAction(type='plan_step_update', step_id=step.id, status='completed', evidence='The model says the work is done.'))
...
async def test_async_plan_completion_verifier_records_authoritative_evidence() -> None:
    ...
    async def verifier(request: PlanStepVerificationRequest) -> PlanStepVerification:
        ...
        return PlanStepVerification(passed=True, evidence='pytest tests/test_feature.py passed.')
    execution = PlanExecution(state=state, memory=memory, trace=lambda _event, _payload: None, async_verifier=verifier)
    turn, step = _active_plan_step(execution)
    result = await execution.apply_step_result_async(turn, PlanStepUpdateAction(type='plan_step_update', step_id=step.id, status='completed', evidence='Focused test passed.'))
    assert result is None
    assert step.status == 'completed'
    assert [record.tool_name for record in step.evidence] == ['plan_step_update', 'plan_step_verifier']
...
def _active_plan_step(execution: PlanExecution) -> tuple[TurnState, PlanStep]:
    step = PlanStep(id='implementation', title='Implement', description='Implement and verify the behavior.', acceptance_criteria=['The focused test passes.'])
