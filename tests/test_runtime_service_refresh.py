...
def test_catalog_updates_model_and_execution_registry_together() -> None:
    ...
    replacement.register(Tool(name='inspect_repo', description='Inspect the repository.', args_schema={'type': 'object'}, callable=lambda _arguments: None))
    ...
    snapshot = agent._turn_effects.snapshot(TurnState(user_message='inspect'), require_plan=True)
...
def test_model_client_override_updates_client_dependent_effects() -> None:
    ...
    events: list[str] = []
    replacement = StreamingScriptedLLMClient([FinalAnswerAction(type='final_answer', content='streamed')])
    agent = Agent(ScriptedLLMClient([]), event_callback=lambda event, _payload: events.append(event))
    with agent._model_transport.override_client(replacement):
...
def test_event_capture_and_permission_replacement_are_owned() -> None:
    ...
    agent = Agent(ScriptedLLMClient([]), event_callback=lambda event, _payload: base_events.append(event))
    replacement_policy = ToolPermissionPolicy(name='evaluation')
    agent._tool_executor.set_permission_policy(replacement_policy)
    with agent.events.capture(lambda event, _payload: captured_events.append(event)):
