...
import time
import pytest
from chulk import CommandExecutionRequest, ExecutionSessionRequest, FileListRequest, FileReadRequest, FileSearchRequest, FileWriteRequest, GitWorktreeBackend, HostExecutionBackend, ProcessLogsRequest, ProcessPollRequest, ProcessStartRequest, ProcessState, TemporaryWorkspaceBackend

def _git(repository: Path, *arguments: str) -> None:
    subprocess.run(['git', '-C', str(repository), *arguments], text=True, capture_output=True, check=True)
...
def _backend(kind: str, root: Path):
    ...
    if kind == 'temporary':
        return TemporaryWorkspaceBackend(root, require_shell_containment=False)
    if kind == 'git':
        _git(root, 'init')
        ...
        return GitWorktreeBackend(root, require_shell_containment=False)
    raise AssertionError(f'Unknown backend kind: {kind}')

@pytest.mark.parametrize('kind', ['host', 'temporary', pytest.param('git', marks=pytest.mark.skipif(shutil.which('git') is None, reason='Git is required'))])
def test_execution_backend_contract(kind, tmp_path):
    ...
    command = session.run_command(CommandExecutionRequest('python -c "from pathlib import Path;print(Path(\'created.txt\').read_text())"'))
    ...
    assert write.metadata['execution_workspace_id'] == read.metadata['execution_workspace_id']
    session.close()
...
async def test_async_execution_backend_contract(kind, tmp_path):
    ...
    (root / 'seed.txt').write_text('seed data\n', encoding='utf-8')
    backend = _backend(kind, root)
    session = await backend.open_session_async(ExecutionSessionRequest(turn_id=f'{kind}-async'))
    write = await session.write_file_async(FileWriteRequest('created.txt', 'created data'))
    read = await session.read_file_async(FileReadRequest('created.txt'))
    searched = await session.search_files_async(FileSearchRequest('created data'))
    assert write.success is True
...
def test_managed_process_contract(kind: str, tmp_path: Path) -> None:
    ...
    (root / 'seed.txt').write_text('seed data\n', encoding='utf-8')
    backend = _backend(kind, root)
    session = backend.open_session(ExecutionSessionRequest(conversation_id=f'{kind}-owner', turn_id=f'{kind}-process'))
    started = session.start_process(ProcessStartRequest(_python_command("print('contract-process')")))
...
async def test_async_managed_process_contract(tmp_path: Path) -> None:
    ...
    session = await backend.open_session_async(ExecutionSessionRequest(conversation_id='async-owner', turn_id='async-process'))
    started = await session.start_process_async(ProcessStartRequest(_python_command("print('async-process')")))
    deadline = time.monotonic() + 5
    while True:
