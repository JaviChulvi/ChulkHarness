...
from chulk.storage import SQLITE_BUSY_TIMEOUT_MS, SQLITE_SCHEMA_VERSION, SQLITE_WAL_AUTOCHECKPOINT_PAGES, SQLiteMigration, SQLiteMigrationError, UnsupportedSQLiteSchemaVersionError, create_sqlite_backup, initialize_sqlite_database, sqlite_connection
...
def test_legacy_database_is_backed_up_migrated_and_deterministically_renumbered(tmp_path):
    ...
    with sqlite3.connect(path) as conn:
        ...
        messages = conn.execute('SELECT id, ordinal FROM conversation_messages ORDER BY ordinal').fetchall()
    ...
    with sqlite3.connect(report.backup_path) as backup:
        backup_ordinals = backup.execute('SELECT ordinal FROM conversation_messages ORDER BY id').fetchall()
...
def test_memory_namespace_migration_preserves_v6_rows_and_rebuilds_fts(tmp_path):
    ...
    with sqlite3.connect(path) as conn:
        ...
        proposal_columns = {row['name'] for row in conn.execute('PRAGMA table_info(memory_proposals)')}
        fts_columns = {row['name'] for row in conn.execute('PRAGMA table_info(memories_fts)')}
        indexes = {row['name'] for row in conn.execute("SELECT name FROM sqlite_schema WHERE type = 'index'")}
        fts_rows = conn.execute('SELECT memory_id, namespace FROM memories_fts').fetchall()
        assert conn.execute('PRAGMA user_version').fetchone()[0] == SQLITE_SCHEMA_VERSION
    backups = list(tmp_path.glob('legacy-memory-v6.sqlite.backup-v6-*.sqlite'))
    assert len(backups) == 1
    with sqlite3.connect(backups[0]) as backup:
        backup_columns = {row[1] for row in backup.execute('PRAGMA table_info(memories)')}
        backup_row = backup.execute("SELECT content, tags, metadata, importance FROM memories WHERE id = 'memory-1'").fetchone()
    ...
    assert {'idx_memories_namespace_active', 'idx_memory_proposals_namespace_status'} <= indexes
    assert [(row['memory_id'], row['namespace']) for row in fts_rows] == [('memory-1', 'default')]
    assert 'namespace' not in backup_columns
    assert backup_row == ('legacy scoped memory', '["project"]', '{"origin":"v6"}', 7)
...
def test_failed_migration_rolls_back_and_retains_validated_private_backup(tmp_path):
    ...
    with pytest.raises(SQLiteMigrationError) as exc_info:
        initialize_sqlite_database(path, migrations=(SQLiteMigration(1, 'deliberate-failure', fail_after_writes),))
    ...
    with sqlite3.connect(path) as conn:
        ...
        assert conn.execute("SELECT 1 FROM sqlite_schema WHERE name = 'partial_change'").fetchone() is None
    ...
    if os.name == 'posix':
        assert stat.S_IMODE(error.backup_path.stat().st_mode) == 384
...
def test_online_backup_includes_committed_wal_content(tmp_path):
    ...
    with sqlite_connection(path) as anchor:
        store.save_message('conversation-1', role='user', content='committed in WAL', message_key='message-1')
        wal_path = Path(f'{path}-wal')
        ...
        if os.name == 'posix':
            for runtime_path in (path, wal_path, Path(f'{path}-shm'), backup_path):
                assert stat.S_IMODE(runtime_path.stat().st_mode) == 384
...
def test_wal_reader_does_not_block_a_message_writer(tmp_path):
    ...
    try:
        ...
        store.save_message('conversation-1', role='user', content='writer completes', message_key='message-1')
        assert reader.execute('SELECT count(*) FROM conversation_messages').fetchone()[0] == 0
        reader.commit()
...
def test_contending_writer_waits_and_succeeds_after_lock_release(tmp_path):
    ...
    def write() -> None:
        started.set()
        store.save_message('conversation-1', role='user', content='waited for lock', message_key='message-1')
    try:
...
def test_exhausted_busy_timeout_surfaces_locked_error_and_allows_retry(monkeypatch, tmp_path):
    ...
    try:
        with pytest.raises(sqlite3.OperationalError, match='locked') as exc_info:
            store.save_message('conversation-1', role='user', content='retryable write', message_key='message-1')
...
def test_concurrent_message_writers_receive_unique_contiguous_ordinals(tmp_path):
    ...
    def write_message(index: int) -> None:
        race.wait(timeout=5)
        store.save_message('conversation-1', role='user', content=f'message {index}', message_key=f'message-{index}')
...
def test_concurrent_initializers_and_cross_store_writers_share_one_database(tmp_path):
    ...
    def write_message(index: int) -> None:
        write_race.wait(timeout=5)
        session_store.save_message('conversation-1', role='user', content=f'message {index}', message_key=f'message-{index}')
    ...
    assert [message.ordinal for message in session_store.list_messages('conversation-1')] == list(range(1, writer_count + 1))
    with sqlite3.connect(path) as conn:
        assert conn.execute('PRAGMA user_version').fetchone()[0] == SQLITE_SCHEMA_VERSION
...
def test_optional_fts_is_not_part_of_the_required_schema_migration(monkeypatch, tmp_path):
    ...
    with sqlite3.connect(store.db_path) as conn:
        assert conn.execute('PRAGMA user_version').fetchone()[0] == SQLITE_SCHEMA_VERSION
        assert conn.execute("SELECT 1 FROM sqlite_schema WHERE name = 'memories_fts'").fetchone() is None
