...
@pytest.mark.parametrize(('script', 'expected_type'), [({'type': 'final_answer', 'content': 'done'}, FinalAnswerAction), (ToolCallAction(type='tool_call', tool_name='lookup', arguments={'id': 'A-1'}), ToolCallAction)])
...
def test_testing_module_exports_only_documented_test_utilities() -> None:
    ...
    assert testing.__all__ == ['HostedContractError', 'HostedContractReport', 'ScriptedLLMClient', 'assert_async_durable_execution_contract', 'assert_async_gateway_store_contract', 'assert_async_hosted_services_contract', 'assert_async_parent_child_run_contract', 'assert_durable_execution_contract', 'assert_gateway_store_contract', 'assert_hosted_services_contract', 'assert_parent_child_run_contract']
