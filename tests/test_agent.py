...
from chulk.core.model_transport import MAX_SUMMARY_CHARS, _fallback_context_checkpoint, _format_messages_for_summary, _normalize_checkpoint, _parse_checkpoint, _render_checkpoint
from chulk.errors import ConfigurationError
from chulk.llm import FallbackChain, LLMActionError, LLMActionResult, LLMCapabilities, LLMClient, LLMCost, LLMError, LLMResponse, LLMUsage
...
from chulk.tools import Tool, ToolExecutionContext, ToolFailureKind, ToolRegistry, apply_patch_tool, calculator_tool, list_files_tool, read_file_tool, shell_tool, write_file_tool
from chulk.tools.permissions import PermissionDecision, ToolPermissionLevel, ToolPermissionPolicy, permission_policy_for_profile
...
class NativeActionRecordingLLMClient(LLMClient):
    ...
    def complete_action(self, messages: list[dict[str, str]], *, max_repair_attempts: int=2, max_output_tokens: int | None=None, tools: list[object] | None=None, **kwargs) -> LLMActionResult:
        self.requests.append(messages)
        self.tool_batches.append(tools)
        return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='native ok'), raw_response='{"type":"final_answer","content":"native ok"}', metadata={'action_transport': 'provider_native'})
...
class ChargedFailureActionLLMClient(LLMClient):
    ...
    def _complete_action_response_once(self, messages: list[dict[str, str]], **kwargs) -> LLMResponse:
        ...
        raise LLMActionError('provider charged then failed', usage=usage, cost=cost, code='server_error', retryable=True, fallback_eligible=True)
...
class ChargedSuccessActionLLMClient(LLMClient):
    ...
    def _complete_action_response_once(self, messages: list[dict[str, str]], **kwargs) -> LLMResponse:
        ...
        return LLMResponse(content=json.dumps({'type': 'final_answer', 'content': 'fallback ok'}), usage=usage, cost=cost, provider=self.provider, model=self.model)
...
class RecordingMCPClient:
    ...
    def list_tools(self):
        return [{'name': 'search_docs', 'description': 'Search docs.', 'inputSchema': {'type': 'object', 'properties': {'query': {'type': 'string'}}, 'required': ['query'], 'additionalProperties': False}}]
...
def create_test_skill_registry(tmp_path):
    ...
    for name, description in {'shell': 'Use this skill when the user request requires terminal inspection or command execution.', 'memory': 'Use this skill when the user request involves saving or retrieving durable information.', 'files': 'Use this skill when the user request requires reading, editing, or creating files.'}.items():
...
def test_turn_state_records_serialize():
    tool_call = ToolCallRecord(tool_name='calculator', arguments={'expression': '1 + 1'}, iteration=1)
    tool_call.success = True
    observation = ObservationRecord(tool_name='calculator', content='Tool calculator finished with success.', output_metadata={'success': True})
    turn = TurnState(user_message='what is 1 + 1?', available_tool_names=['calculator'], tool_calls=[tool_call], observations=[observation])
...
def test_agent_sends_user_message_and_stores_response():
    ...
    assert agent.memory.messages == [{'role': 'user', 'content': 'hello'}, {'role': 'assistant', 'content': 'Hi Javier'}]
...
def test_agent_streams_validated_final_answer_when_provider_supports_streaming():
    ...
    assert [event[0] for event in stream_events] == [TraceEvent.MODEL_STREAM_STARTED, TraceEvent.MODEL_STREAM_DELTA, TraceEvent.MODEL_STREAM_COMPLETED]
...
def test_agent_includes_recent_conversation_history():
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'first answer'}), json.dumps({'type': 'final_answer', 'content': 'second answer'})])
    agent = Agent(llm)
...
def test_conversation_memory_trims_to_limit():
    ...
    assert memory.messages == [{'role': 'assistant', 'content': 'two'}, {'role': 'user', 'content': 'three'}]

def test_low_history_limit_keeps_current_user_and_complete_tool_result():
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '2 + 2'})}), json.dumps({'type': 'final_answer', 'content': 'The result is 4.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, memory=ConversationMemory(max_messages=1), tool_registry=registry)
    assert agent.run_turn('Calculate 2 + 2.') == 'The result is 4.'
    follow_up_history = llm.requests[1][1:]
    assert [message['role'] for message in follow_up_history] == ['user', 'assistant', 'observation']
...
def test_agent_calls_calculator_tool_then_returns_final_answer():
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '(2 + 3) * 4'})}), json.dumps({'type': 'final_answer', 'content': 'The result is 20.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry)
    response = agent.run_turn('what is (2 + 3) * 4?')
    assert response == 'The result is 20.'
    assert agent.state.tool_calls == [{'tool_name': 'calculator', 'arguments': {'expression': '(2 + 3) * 4'}, 'phase': 'execution', 'success': True}]
...
def test_agent_reflection_approves_final_answer():
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'Hi Javier'}), json.dumps({'approved': True, 'reason': 'The answer addresses the greeting.', 'feedback': None})])
...
def test_agent_reflection_feedback_triggers_one_more_action_loop(tmp_path):
    ...
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'Done.'}), json.dumps({'approved': False, 'reason': 'The answer claims completion without mentioning the missing tool evidence.', 'feedback': 'Explain that no tool evidence was gathered before answering.'}), json.dumps({'type': 'final_answer', 'content': 'I do not have tool evidence for completion.'})])
    ...
    assert any((event['type'] == 'tool_observation' and event['payload']['tool_name'] == 'reflection_feedback' for event in events))
...
def test_agent_reflection_attempt_limit_prevents_revision_loop():
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'First answer.'}), json.dumps({'approved': False, 'reason': 'Needs one more pass.', 'feedback': 'Return a revised final answer.'}), json.dumps({'type': 'final_answer', 'content': 'Second answer.'})])
    agent = Agent(llm, max_reflection_attempts=1)
...
def test_agent_calls_apply_patch_tool_then_returns_final_answer(tmp_path):
    ...
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'apply_patch', 'arguments_json': json.dumps({'patch': '\n'.join(['--- a/notes.txt', '+++ b/notes.txt', '@@ -1 +1 @@', '-hello', '+hello chulk'])})}), json.dumps({'type': 'final_answer', 'content': 'Updated notes.txt.'})])
    registry = ToolRegistry()
    ...
    assert agent.state.tool_calls == [{'tool_name': 'apply_patch', 'arguments': {'patch': '--- a/notes.txt\n+++ b/notes.txt\n@@ -1 +1 @@\n-hello\n+hello chulk'}, 'phase': 'execution', 'success': True}]
...
def test_runtime_status_footer_reports_budget_and_last_rejected_call() -> None:
    client = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'missing_tool', 'arguments': {}}), json.dumps({'type': 'final_answer', 'content': 'Recovered.'})])
    ...
    assert system_content.index('</action_protocol>') < system_content.index('<runtime_status>')
    ...
    assert any((section['name'] == 'runtime_status' for section in agent.state.turns[-1].context_reports[-1]['sections']))
...
def test_agent_uses_native_action_prompt_and_passes_tool_specs_when_supported(tmp_path):
    ...
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
    request_payload = next((event['payload'] for event in events if event['type'] == TraceEvent.MODEL_REQUEST_STARTED))
...
def test_hosted_mcp_is_visible_in_native_context_without_tracing_authorization(tmp_path):
    ...
    class HostedNativeClient(NativeActionRecordingLLMClient):
        capabilities = LLMCapabilities(supports_native_tool_calling=True, supports_hosted_mcp_tools=True)
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com', authorization='secret-token')
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'hosted-mcp-context')
    llm = HostedNativeClient()
    agent = Agent(llm, mcp_servers=(server,), trace_logger=trace_logger)
    assert agent.run_turn('search docs') == 'native ok'
    report = agent.state.last_context_report
    native_section = next((section for section in report['sections'] if section['name'] == 'native_tools'))
...
def test_agent_uses_one_json_contract_for_a_mixed_fallback_chain():
    ...
    class NativeFailingClient(LLMClient):
        ...
        def _complete_action_response_once(self, messages, *, tools=None, planning_tools=None, hosted_mcp_servers=None, mcp_approval_callback=None, **kwargs):
            self.requests.append(messages)
            self.native_options.append({'tools': tools, 'planning_tools': planning_tools, 'hosted_mcp_servers': hosted_mcp_servers, 'mcp_approval_callback': mcp_approval_callback})
            raise LLMError('primary unavailable', code='server_error', retryable=True, fallback_eligible=True)
    primary = NativeFailingClient()
    plan_payload = {'summary': 'Use the JSON transport.', 'steps': [{'id': '1', 'title': 'Complete the change', 'description': 'Complete the requested change.'}]}
    secondary = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload), 'step_update_json': '{}'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')
    bridge_tool = create_mcp_bridge_tools([server], client_factory=lambda _server: RecordingMCPClient())[0]
    registry.register(bridge_tool)
    agent = Agent(FallbackChain([primary, secondary]), tool_registry=registry, mcp_servers=(server,), mcp_bridge_tool_names=[bridge_tool.name])
    response = agent.run_planned_turn('hello')
    assert 'Use /approve' in response
    assert primary.native_options == [{'tools': None, 'planning_tools': None, 'hosted_mcp_servers': None, 'mcp_approval_callback': None}]
...
def test_agent_async_keeps_native_options_off_for_a_mixed_fallback_chain():
    ...
    class AsyncNativeFailingClient(LLMClient):
        ...
        async def _acomplete_action_response_once(self, messages, *, tools=None, planning_tools=None, hosted_mcp_servers=None, mcp_approval_callback=None, **kwargs):
            self.requests.append(messages)
            self.native_options.append({'tools': tools, 'planning_tools': planning_tools, 'hosted_mcp_servers': hosted_mcp_servers, 'mcp_approval_callback': mcp_approval_callback})
            raise LLMError('primary unavailable', code='server_error', retryable=True, fallback_eligible=True)
    primary = AsyncNativeFailingClient()
    plan_payload = {'summary': 'Use the async JSON transport.', 'steps': [{'id': '1', 'title': 'Complete the change', 'description': 'Complete the requested change.'}]}
    secondary = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload), 'step_update_json': '{}'})])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')
    registry = ToolRegistry()
    bridge_tool = create_mcp_bridge_tools([server], client_factory=lambda _server: RecordingMCPClient())[0]
...
def test_native_planning_tools_follow_the_plan_lifecycle():
    ...
    class PlanningPolicyRecordingClient(LLMClient):
        ...
        def __init__(self) -> None:
            self.actions = [PlanAction(type='plan', plan=Plan(summary='Make one change.', steps=[PlanStep(id='1', title='Make the change', description='Complete the requested change.')])), PlanStepUpdateAction(type='plan_step_update', step_id='1', status='completed', evidence='The change is complete.'), FinalAnswerAction(type='final_answer', content='done')]
            self.planning_policies = []
            self.action_schemas = []

        def complete_action(self, messages, *, planning_tools=None, action_schema=None, **kwargs):
            self.planning_policies.append(planning_tools)
            self.action_schemas.append(action_schema)
            return LLMActionResult(action=self.actions.pop(0), raw_response='{"type":"recorded"}', metadata={'action_transport': 'provider_native'})
    ...
    assert [(policy.propose_plan, policy.update_plan_step) for policy in llm.planning_policies] == [(True, False), (False, True), (False, False)]
    assert [schema['properties']['type']['enum'] for schema in llm.action_schemas] == [['plan'], ['plan_step_update'], ['final_answer']]
...
def test_agent_rejects_irreducibly_oversized_prompt_before_provider_request(tmp_path):
    ...
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'unused'})])
    agent = Agent(llm, trace_logger=trace_logger, system_prompt='mandatory ' + 'x' * 5000, context_budget=ContextBudget(max_prompt_tokens=100, response_reserve_tokens=0))
...
def test_agent_rejects_required_context_before_compaction_provider_request():
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'first turn complete'})])
    agent = Agent(llm, system_prompt='mandatory ' + 'x' * 5000, context_budget=ContextBudget(max_prompt_tokens=5000, response_reserve_tokens=0))
    agent.run_turn('old context ' + 'x' * 5000)
    agent._model_transport.context_budget = ContextBudget(max_prompt_tokens=100, response_reserve_tokens=0)
...
def test_checkpoint_fallback_preserves_newest_compacted_context_first():
    checkpoint = _fallback_context_checkpoint({'objective': 'old objective', 'evidence_hints': ['old evidence']}, None, [{'role': 'user', 'content': 'older context'}, {'role': 'assistant', 'content': 'newest context'}])
    assert checkpoint['evidence_hints'][:2] == ['assistant: newest context', 'user: older context']

def test_checkpoint_source_limit_keeps_newest_messages():
    source = _format_messages_for_summary([{'role': 'user', 'content': 'older ' + 'x' * 12000}, {'role': 'assistant', 'content': 'newest context survives'}])
...
def test_checkpoint_parser_rejects_malformed_json_objects():
    ...
    assert _parse_checkpoint(json.dumps({'objective': 'Continue', 'decisions': 'not a list'})) is None

def test_checkpoint_metadata_and_rendered_content_stay_within_global_limit():
    checkpoint = _normalize_checkpoint({'objective': 'o' * 600, **{key: ['x' * 300 for _ in range(6)] for key in ('constraints', 'decisions', 'completed', 'blocked', 'next_actions', 'evidence_hints')}})
...
def test_agent_accepts_external_context_and_prompt_metadata(tmp_path):
    ...
    response = agent.run_turn('answer from source', context_sections=[TurnContextSection(id='src-42', title='Quarterly planning source', content='Q3 launch is blocked by data migration.')], prompt_profile='polp-search', locale='es-ES', extension_metadata={'confidence': 0.81})
    turn = agent.state.turns[0]
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
...
def test_agent_summarizes_omitted_history_before_action_request():
    llm = RecordingLLMClient([json.dumps({'objective': 'Continue the current context-compaction task.', 'decisions': ['Keep the context compaction decision.'], 'next_actions': ['Update context tests.']}), json.dumps({'type': 'final_answer', 'content': 'continued with summary'})])
    memory = ConversationMemory(max_messages=10)
    memory.add_user_message('old decision ' + 'a' * 2500)
    memory.add_assistant_message('old answer ' + 'b' * 2500)
    agent = Agent(llm, memory=memory, context_budget=ContextBudget(max_prompt_tokens=1200, response_reserve_tokens=0))
...
def test_agent_injects_profile_and_relevant_long_term_memories(tmp_path):
    ...
    profile_id = memory_store.save_memory('Javier prefers exact file paths and direct implementation steps.', tags=['persona', 'preference'], importance=9)
    relevant_id = memory_store.save_memory('ChulkHarness long-term memory is backed by SQLite.', tags=['project', 'memory'], importance=5)
...
def test_agent_injects_relevant_skill_without_loading_unrelated_skills(tmp_path):
    ...
    assert system_prompt.count('Use this skill when the user request requires terminal inspection') == 1
...
def test_agent_traces_explainable_skill_version_digest_and_omissions(tmp_path):
    skills_dir = tmp_path / 'skills'
    for name, description in (('review', 'Review Python code.'), ('other', 'Unrelated workflow.')):
        skill_dir = skills_dir / name
        skill_dir.mkdir(parents=True)
    ...
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'reviewed'})])
    agent = Agent(llm, skill_registry=skill_registry, trace_logger=trace_logger)
    agent.run_turn('/review inspect this')
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
    payload = next((event['payload'] for event in events if event['type'] == 'skill_selection_completed'))
    selected = payload['skills'][0]
    decisions = {decision['skill_name']: decision for decision in payload['decisions']}
...
def test_pending_plan_does_not_restore_skill_after_capability_is_removed(tmp_path):
    ...
    plan = Plan(summary='Use the previous context.', steps=[PlanStep(id='1', title='Inspect', description='Inspect safely.')])
    turn = TurnState(user_message='plan this', turn_id='turn-pending', loaded_skill_names=['network'])
    turn.wait_for_plan_approval(plan)
    state = AgentState(turns=[turn], active_plan=plan, pending_plan_turn_id=turn.turn_id)
    agent = Agent(RecordingLLMClient([]), state=state, skill_registry=skill_registry)
...
def test_agent_traces_full_model_request_with_redaction(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'ok'})])
    agent = Agent(llm, trace_logger=trace_logger, system_prompt='System prompt includes OPENAI_API_KEY=sk-testsecret123456 and normal instructions.', trace_max_prompt_chars=10000)
    response = agent.run_turn('hello')
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
    ...
    assert payload['messages'][-1] == {'role': 'user', 'content': 'hello', 'content_char_count': 5, 'returned_content_char_count': 5, 'truncated': False}

def test_agent_truncates_large_model_request_trace(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'ok'})])
    agent = Agent(llm, trace_logger=trace_logger, system_prompt='x' * 200, trace_max_prompt_chars=40)
    agent.run_turn('hello')
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
...
def test_agent_selects_memory_and_file_skills_for_matching_requests(tmp_path):
    skill_registry = create_test_skill_registry(tmp_path)
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'Memory skill selected.'}), json.dumps({'type': 'final_answer', 'content': 'Files skill selected.'})])
...
def test_agent_can_run_safe_shell_tool(tmp_path):
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'run_cmd', 'arguments': {'command': 'printf hello'}}), json.dumps({'type': 'final_answer', 'content': 'The command printed hello.'})])
    registry = ToolRegistry()
...
def test_agent_blocks_confirmation_tool_without_permission_callback(tmp_path):
    calls = []
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'dangerous', 'arguments': {'value': 'run'}}), json.dumps({'type': 'final_answer', 'content': 'I could not run the tool without approval.'})])
    registry = ToolRegistry()
    registry.register(Tool(name='dangerous', description='Dangerous test tool.', args_schema={'type': 'object', 'properties': {'value': {'type': 'string'}}, 'required': ['value'], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('dangerous', True, 'ran'), requires_confirmation=True))
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_denied_mcp_bridge_permission_does_not_call_server(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'mcp_docs_search_docs', 'arguments': {'query': 'MCP'}}), json.dumps({'type': 'final_answer', 'content': 'I did not call the MCP server.'})])
    mcp_client = RecordingMCPClient()
    registry = ToolRegistry()
    bridge_tool = create_mcp_bridge_tools([MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')], client_factory=lambda _server: mcp_client)[0]
    registry.register(bridge_tool)
    approvals = []
...
def test_external_mcp_result_forces_fresh_approval_for_followup_write(tmp_path):
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'mcp_docs_search_docs', 'arguments': {'query': 'MCP'}}), json.dumps({'type': 'tool_call', 'tool_name': 'write_file', 'arguments': {'path': 'injected.txt', 'content': 'changed'}}), json.dumps({'type': 'final_answer', 'content': 'The follow-up write required approval.'})])
    registry = ToolRegistry()
    bridge_tool = create_mcp_bridge_tools([MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')], client_factory=lambda _server: RecordingMCPClient())[0]
    registry.register(bridge_tool)
    registry.register(write_file_tool(tmp_path))
    agent = Agent(llm, tool_registry=registry, permission_policy=permission_policy_for_profile('full-access'))
    response = agent.run_turn('search docs')
...
def test_external_mcp_result_requires_approval_in_a_later_turn(tmp_path):
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'mcp_docs_search_docs', 'arguments': {'query': 'MCP'}}), json.dumps({'type': 'final_answer', 'content': 'Found remote instructions.'}), json.dumps({'type': 'tool_call', 'tool_name': 'write_file', 'arguments': {'path': 'injected.txt', 'content': 'changed'}}), json.dumps({'type': 'final_answer', 'content': 'The later write required approval.'})])
    registry = ToolRegistry()
    registry.register(create_mcp_bridge_tools([MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')], client_factory=lambda _server: RecordingMCPClient())[0])
    registry.register(write_file_tool(tmp_path))
    agent = Agent(llm, tool_registry=registry, permission_policy=permission_policy_for_profile('full-access'))
...
def test_hosted_mcp_result_forces_fresh_approval_for_followup_write(tmp_path):
    ...
    class HostedMCPResultClient(LLMClient):
        ...
        def complete_action(self, messages, **kwargs):
            self.calls += 1
            if self.calls == 1:
                return LLMActionResult(action=ToolCallAction(type='tool_call', tool_name='write_file', arguments={'path': 'injected.txt', 'content': 'changed'}), raw_response='hosted MCP output and follow-up write', metadata={'action_transport': 'provider_native', 'provider_mcp_output': [{'server_label': 'docs'}]})
            return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='The hosted follow-up write required approval.'), raw_response='hosted follow-up denied', metadata={'action_transport': 'provider_native'})
    registry = ToolRegistry()
    registry.register(write_file_tool(tmp_path))
    agent = Agent(HostedMCPResultClient(), tool_registry=registry, permission_policy=permission_policy_for_profile('full-access'))
    response = agent.run_turn('search docs')
...
def test_agent_runs_confirmation_tool_when_permission_callback_allows(tmp_path):
    calls = []
    approvals = []
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'dangerous', 'arguments': {'value': 'run'}}), json.dumps({'type': 'final_answer', 'content': 'The approved tool ran.'})])
    registry = ToolRegistry()
    registry.register(Tool(name='dangerous', description='Dangerous test tool.', args_schema={'type': 'object', 'properties': {'value': {'type': 'string'}}, 'required': ['value'], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('dangerous', True, 'ran'), requires_confirmation=True))
...
def test_agent_async_turn_awaits_tool_and_passes_context():
    ...
    registry = ToolRegistry()
    registry.register(Tool(name='lookup', description='Async lookup.', args_schema={'type': 'object', 'properties': {'query': {'type': 'string'}}, 'required': ['query'], 'additionalProperties': False}, callable=lookup, accepts_context=True))
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'lookup', 'arguments': {'query': 'policy'}}), json.dumps({'type': 'final_answer', 'content': 'policy found'})])
    agent = Agent(llm, tool_registry=registry)
    response = asyncio.run(agent.run_turn_async('lookup policy', tool_context=ToolExecutionContext(metadata={'org_id': 'org-1'})))
    turn = agent.state.turns[0]
...
def test_agent_sync_turn_classifies_async_tool_misuse():
    ...
    registry = ToolRegistry()
    registry.register(Tool(name='lookup', description='Async lookup.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lookup))
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'lookup', 'arguments': {}}), json.dumps({'type': 'final_answer', 'content': 'async not run'})])
    agent = Agent(llm, tool_registry=registry)
...
def test_agent_traces_tool_call_lifecycle(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': '1 + 1'}}), json.dumps({'type': 'final_answer', 'content': 'The result is 2.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
    response = agent.run_turn('what is 1 + 1?')
...
def test_agent_planned_turn_creates_pending_plan_without_running_tools(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    plan_payload = {'summary': 'Calculate the answer safely.', 'steps': [{'id': '1', 'title': 'Run calculator', 'description': 'Use the calculator tool for the arithmetic.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_planned_turn_allows_read_only_reconnaissance_before_plan(tmp_path):
    ...
    (tmp_path / 'src' / 'chulk' / 'core.py').write_text('class Agent:\n    pass\n', encoding='utf-8')
    plan_payload = {'summary': 'Add subagent support based on the inspected runtime.', 'steps': [{'id': '1', 'title': 'Extend agent runtime', 'description': 'Use the inspected src/chulk/core.py shape to add subagent orchestration.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': json.dumps({'path': 'src/chulk/core.py'})}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    registry = ToolRegistry()
...
def test_agent_planned_turn_uses_registered_permission_metadata_for_reconnaissance():
    calls = []
    plan_payload = {'summary': 'Implement the feature using the inspected project metadata.', 'steps': [{'id': '1', 'title': 'Implement project metadata support', 'description': 'Update the runtime to consume the inspected project metadata.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'inspect_project_metadata', 'arguments_json': '{}'}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    registry = ToolRegistry()
    registry.register(Tool(name='inspect_project_metadata', description='Inspect project metadata without side effects.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('inspect_project_metadata', True, 'Project type is Python.'), permission_level=ToolPermissionLevel.READ))
    agent = Agent(llm, tool_registry=registry)
    ...
    assert '<read_only_reconnaissance_tools>inspect_project_metadata</read_only_reconnaissance_tools>' in llm.requests[0][0]['content']

def test_agent_planned_turn_blocks_read_named_tool_declared_as_write():
    calls = []
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': '{}'})])
    registry = ToolRegistry()
    registry.register(Tool(name='read_file', description='A misleadingly named mutating tool.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('read_file', True, 'mutated'), permission_level=ToolPermissionLevel.WRITE))
    agent = Agent(llm, tool_registry=registry)
...
def test_agent_planned_turn_revises_reconnaissance_only_plan(tmp_path):
    (tmp_path / 'src' / 'chulk').mkdir(parents=True)
    (tmp_path / 'src' / 'chulk' / 'main.py').write_text('def main():\n    pass\n', encoding='utf-8')
    (tmp_path / 'src' / 'chulk' / 'config.py').write_text('class Config:\n    pass\n', encoding='utf-8')
    weak_plan_payload = {'summary': 'Explore the current codebase before designing subagents.', 'steps': [{'id': '1', 'title': 'Read main.py', 'description': 'Understand the agent loop.', 'status': 'pending'}, {'id': '2', 'title': 'Read config.py', 'description': 'Understand configuration patterns.', 'status': 'pending'}]}
    strong_plan_payload = {'summary': 'Implement subagent support in the inspected runtime.', 'steps': [{'id': '1', 'title': 'Add subagent state models', 'description': 'Extend src/chulk/core/state.py with records for child task requests and results.', 'status': 'pending'}, {'id': '2', 'title': 'Implement subagent orchestration', 'description': 'Update src/chulk/core/agent.py to spawn isolated child agents and collect results.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'list_files', 'arguments_json': json.dumps({'path': 'src/chulk', 'pattern': '*.py'})}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(weak_plan_payload)}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': json.dumps({'path': 'src/chulk/main.py'})}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(strong_plan_payload)})])
    registry = ToolRegistry()
    ...
    assert any((event['type'] == 'tool_observation' and event['payload']['tool_name'] == 'planning_feedback' for event in events))
...
def test_agent_planned_turn_revises_direct_answer_into_plan():
    plan_payload = {'summary': 'Add subagent delegation support.', 'steps': [{'id': '1', 'title': 'Add subagent action type', 'description': 'Extend src/chulk/core/actions.py with a delegation action for child-agent work.', 'status': 'pending'}, {'id': '2', 'title': 'Implement delegation runtime', 'description': 'Update src/chulk/core/agent.py to create child agents and return their observations.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'final_answer', 'content': 'Here is how I would add subagents conceptually.'}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    agent = Agent(llm)
    response = agent.run_planned_turn('How would you add subagents?')
    turn = agent.state.turns[0]
...
def test_agent_planned_turn_requests_plan_when_reconnaissance_budget_is_exhausted(tmp_path):
    ...
    plan_payload = {'summary': 'Implement the feature with the gathered context.', 'steps': [{'id': '1', 'title': 'Update agent runtime', 'description': 'Modify src/chulk/core/agent.py using the files already inspected.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': json.dumps({'path': 'a.py'})}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': json.dumps({'path': 'b.py'})}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'read_file', 'arguments_json': json.dumps({'path': 'c.py'})}), json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    registry = ToolRegistry()
    registry.register(read_file_tool(tmp_path))
...
def test_agent_planned_turn_blocks_mutating_tools_before_plan(tmp_path):
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'write_file', 'arguments_json': json.dumps({'path': 'created.txt', 'content': 'nope'})})])
    registry = ToolRegistry()
    registry.register(write_file_tool(tmp_path))
    agent = Agent(llm, tool_registry=registry)
...
def test_agent_run_planned_turn_forces_plan_for_one_turn():
    plan_payload = {'summary': 'Plan a design change.', 'steps': [{'id': '1', 'title': 'Add subagent task model', 'description': 'Create a state record for delegated child-agent work.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The design step is complete.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'Plan approved and completed.'})])
    agent = Agent(llm)
    response = agent.run_planned_turn('How would you add subagents?')
...
def test_agent_approve_plan_resumes_turn_and_tracks_plan_steps(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    plan_payload = {'summary': 'Calculate the answer safely.', 'steps': [{'id': '1', 'title': 'Run calculator', 'description': 'Use the calculator tool for the arithmetic.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '2 + 2'})}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The calculator returned 4.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'The result is 4.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_executes_multiple_plan_steps_with_dependencies():
    plan_payload = {'summary': 'Calculate two values.', 'steps': [{'id': '1', 'title': 'Calculate four', 'description': 'Use the calculator for 2 + 2.', 'status': 'pending', 'acceptance_criteria': ['Calculator result for 2 + 2 is known.'], 'retry_limit': 0}, {'id': '2', 'title': 'Calculate six', 'description': 'Use the calculator for 3 + 3.', 'status': 'pending', 'depends_on': ['1'], 'acceptance_criteria': ['Calculator result for 3 + 3 is known.'], 'retry_limit': 0}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '2 + 2'})}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': '2 + 2 returned 4.', 'reason': None})}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '3 + 3'})}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '2', 'status': 'completed', 'evidence': '3 + 3 returned 6.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'The results are 4 and 6.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry)
...
def test_agent_rejects_premature_final_answer_until_plan_steps_complete():
    plan_payload = {'summary': 'Complete one tracked step.', 'steps': [{'id': '1', 'title': 'Confirm work', 'description': 'Record explicit step completion.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'final_answer', 'content': 'Done too early.'}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The tracked step is complete.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'Done after completion.'})])
    agent = Agent(llm)
...
def test_agent_blocks_plan_immediately_when_step_tool_fails(tmp_path):
    ...
    def failing_tool(_arguments):
        return ToolResult(tool_name='fail_tool', success=False, observation='The tool failed deliberately.', error='deliberate_failure')
    registry = ToolRegistry()
    registry.register(Tool(name='fail_tool', description='Always fail.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=failing_tool))
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    plan_payload = {'summary': 'Try a failing tool.', 'steps': [{'id': '1', 'title': 'Run failing tool', 'description': 'Call a tool that fails.', 'status': 'pending'}, {'id': '2', 'title': 'Never run', 'description': 'This step depends on the failed step.', 'status': 'pending', 'depends_on': ['1']}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'fail_tool', 'arguments_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'Should not be used.'})])
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_retries_failed_plan_step_within_budget_then_completes():
    ...
    def flaky_tool(_arguments):
        ...
        call_count += 1
        if call_count == 1:
            return ToolResult(tool_name='flaky_tool', success=False, observation='The first attempt failed.', error='transient_failure', failure_kind=ToolFailureKind.ENVIRONMENT)
        return ToolResult(tool_name='flaky_tool', success=True, observation='The recovery attempt succeeded.')
    registry = ToolRegistry()
    registry.register(Tool(name='flaky_tool', description='Fail once, then succeed.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=flaky_tool))
    plan_payload = {'summary': 'Recover one flaky operation.', 'steps': [{'id': '1', 'title': 'Run flaky operation', 'description': 'Run the operation and recover from one transient failure.', 'status': 'pending', 'acceptance_criteria': ['The operation succeeds.'], 'retry_limit': 1}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'flaky_tool', 'arguments_json': '{}'}), json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'flaky_tool', 'arguments_json': '{}'}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The recovery attempt succeeded.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'The flaky operation recovered and completed.'})])
    agent = Agent(llm, tool_registry=registry)
...
def test_agent_blocks_failed_plan_step_only_after_retry_budget_is_exhausted(tmp_path):
    ...
    def failing_tool(_arguments):
        return ToolResult(tool_name='fail_tool', success=False, observation='The tool failed deliberately.', error='deliberate_failure', failure_kind=ToolFailureKind.ENVIRONMENT)
    registry = ToolRegistry()
    registry.register(Tool(name='fail_tool', description='Always fail.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=failing_tool))
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'retry-session')
    plan_payload = {'summary': 'Try a failing tool with one recovery attempt.', 'steps': [{'id': '1', 'title': 'Run failing tool', 'description': 'Call a tool and use one recovery attempt if needed.', 'status': 'pending', 'retry_limit': 1}]}
    tool_call = json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'fail_tool', 'arguments_json': '{}'})
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)}), tool_call, tool_call, json.dumps({'type': 'final_answer', 'content': 'Should not be used.'})])
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
    agent.run_planned_turn('run the failing plan')
    response = agent.approve_plan()
    turn = agent.state.turns[0]
    ...
    events = [json.loads(line) for line in trace_logger.path.read_text(encoding='utf-8').splitlines()]
    assert response == 'Plan step blocked: Run failing tool. Tool fail_tool failed with deliberate_failure. Step retry limit exhausted after 1 retry.'
    assert turn.status == 'blocked'
...
def test_agent_reject_plan_finishes_without_tools(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    plan_payload = {'summary': 'Add project inspection support.', 'steps': [{'id': '1', 'title': 'Add file inspection workflow', 'description': 'Implement a project inspection path using the existing file tools.', 'status': 'pending'}]}
    llm = RecordingLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload)})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_truncates_tool_output_but_preserves_full_artifact(tmp_path):
    ...
    def big_output_tool(_arguments):
        return ToolResult(tool_name='big_output', success=True, observation='Produced a long output.', stdout=full_stdout)
    registry = ToolRegistry()
    registry.register(Tool(name='big_output', description='Return long output for testing.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=big_output_tool))
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'big_output', 'arguments': {}}), json.dumps({'type': 'final_answer', 'content': 'Reviewed.'})])
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger, max_tool_stdout_chars=120, max_observation_chars=1000)
    ...
    artifact_text = trace_logger.read_artifact(stdout_artifact['artifact_id'], mode='head', max_bytes=20000).content
    trace_text = trace_logger.path.read_text(encoding='utf-8')
...
def test_agent_preserves_artifact_when_full_observation_is_truncated(tmp_path):
    ...
    def verbose_tool(_arguments):
        return ToolResult(tool_name='verbose', success=True, observation=long_observation, stdout=full_stdout)
    registry = ToolRegistry()
    registry.register(Tool(name='verbose', description='Return verbose output for testing.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=verbose_tool))
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'verbose', 'arguments': {}}), json.dumps({'type': 'final_answer', 'content': 'Reviewed.'})])
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger, max_tool_stdout_chars=120, max_observation_chars=500)
    agent.run_turn('produce verbose output')
    observation = agent.state.observations[0]['observation']
    output_metadata = agent.state.observations[0]['output_metadata']
    observation_artifact = next((artifact for artifact in output_metadata['artifacts'] if artifact['field'] == 'observation'))
    artifact_text = trace_logger.read_artifact(observation_artifact['artifact_id'], mode='head', max_bytes=20000).content
...
def test_agent_repairs_invalid_model_json():
    llm = RecordingLLMClient(['Claro, puedo ayudarte.', json.dumps({'type': 'final_answer', 'content': 'Claro, puedo ayudarte.'})])
    agent = Agent(llm)
    response = agent.run_turn('hello')
...
def test_agent_feeds_unknown_tool_observation_back_to_model():
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'missing_tool', 'arguments': {}}), json.dumps({'type': 'final_answer', 'content': 'I could not use that tool.'})])
    agent = Agent(llm)
...
def test_agent_feeds_invalid_tool_arguments_back_to_model(tmp_path):
    trace_logger = JSONLTraceLogger(tmp_path / 'traces', 'test-session')
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': 123}}), json.dumps({'type': 'final_answer', 'content': 'I corrected the tool arguments issue.'})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
    agent = Agent(llm, tool_registry=registry, trace_logger=trace_logger)
...
def test_agent_enforces_tool_call_limit():
    llm = RecordingLLMClient([json.dumps({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': '1 + 1'}}), json.dumps({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': '2 + 2'}})])
    registry = ToolRegistry()
    registry.register(calculator_tool())
...
def test_model_evidence_surrounds_accounting(asynchronous, accounting_fails):
    ...
    class Client(NativeActionRecordingLLMClient):
        ...
        def complete_action(self, messages, **kwargs):
            return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='ok'), raw_response='{"type":"final_answer","content":"ok"}', repair_attempts=1, errors=['repaired input'], metadata={'provider_mcp_output': True})
