...
from chulk.evals import CaseResult, EvalReport, EvalRunStatus, EvalTurnResult, GradeResult, SQLiteEvalStore, TrialResult
...
def _report(report_id: str, *, started_at: str='2026-08-10T00:00:00+00:00', provider: str='openai', model: str='gpt-eval', tags: tuple[str, ...]=('smoke',), trace_path: Path | None=None, passed: bool=True, status: EvalRunStatus=EvalRunStatus.COMPLETED, reason: str='matched reference') -> EvalReport:
    result = RunResult('safe output', RunStatus.COMPLETED, 'turn-1', 'conversation-1', trace_path, usage=Usage(input_tokens=3, output_tokens=2, total_tokens=5), cost=Cost(amount=Decimal('0.001'), pricing_known=True))
    grade = GradeResult('answer.exact', 1.0 if passed else 0.0, passed, reason, {'required': True, 'grader_version': '2', 'grader_identity': 'answer-exact-v2', 'evidence': {'expected': 'safe output'}})
    trial = TrialResult('case-1', 'sdk', 1, (EvalTurnResult(0, result, (), 0.25),), 0.25, (grade,))
    case = CaseResult('case-1', 'sdk', (trial,), passed)
    metrics = {'case_count': 1.0, 'pass_rate': 1.0 if passed else 0.0, 'p95_latency_seconds': 0.25, 'total_tokens': 5.0, 'total_cost': 0.001, 'target.sdk.pass_rate': 1.0 if passed else 0.0, f'provider.{provider}.pass_rate': 1.0 if passed else 0.0, f'model.{model}.pass_rate': 1.0 if passed else 0.0, **{f'tag.{tag}.pass_rate': 1.0 if passed else 0.0 for tag in tags}}
    return EvalReport(report_id, 'starter', 'digest', started_at, '2026-08-10T00:00:01+00:00', (case,), metrics, () if passed else ('pass_rate below 1.0',), (), {'mode': 'scripted', 'targets': [{'name': 'sdk', 'provider': provider, 'model': model, 'fingerprint': 'sdk-v1'}], 'graders': [{'name': 'answer.exact', 'version': '2', 'identity': 'answer-exact-v2'}]}, status)
...
def _client(tmp_path: Path, *reports: EvalReport, baseline: str | None=None):
    ...
    app = create_control_app(config, token_store=tokens, enable_eval_dashboard=True)
...
def test_eval_run_api_filters_after_payload_matching_and_paginates(tmp_path: Path) -> None:
    _, app, auth = _client(tmp_path, _report('run-old', started_at='2026-08-10T00:00:00+00:00'), _report('run-middle', started_at='2026-08-10T01:00:00+00:00', provider='anthropic', model='claude-eval', tags=('safety',)), _report('run-new', started_at='2026-08-10T02:00:00+00:00'))
    with TestClient(app) as client:
        first = client.get('/v1/evals/runs?provider=openai&tag=smoke&limit=1', headers=auth)
        second = client.get('/v1/evals/runs?provider=openai&tag=smoke&limit=1&offset=1', headers=auth)
        indexed = client.get('/v1/evals/runs?status=completed&mode=scripted&started_after=2026-08-10T00:30:00%2B00:00', headers=auth)
        ...
        assert [item['id'] for item in indexed.json()['runs']] == ['run-new', 'run-middle']
...
def test_eval_detail_includes_baseline_dimensions_evidence_and_missing_trace(tmp_path: Path) -> None:
    malicious = '<img src=x onerror="alert(1)">'
    baseline = _report('baseline', passed=False)
    missing = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)}).traces_dir / 'deleted.jsonl'
    ...
    with TestClient(app) as client:
        ...
        assert value['dimensions']['targets'] == [{'name': 'sdk', 'provider': 'openai', 'model': 'gpt-eval', 'pass_rate': 1.0}]
...
def test_eval_trace_is_bounded_to_trace_store_redacted_and_handles_missing(tmp_path: Path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    logger = JSONLTraceLogger(config.traces_dir, 'conversation-1')
    logger.log('turn_started', {'turn': {'turn_id': 'turn-1'}, 'api_key': 'sk-dashboard-secret-123456'})
