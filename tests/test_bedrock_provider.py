...
from chulk.llm.capabilities import BEDROCK_DEFAULT_CONTEXT_WINDOW_TOKENS, BEDROCK_DEFAULT_RESPONSE_RESERVE_TOKENS, resolve_model_capabilities
...
def _tool_call(call_id: str='call_1') -> object:
    return SimpleNamespace(id=call_id, type='function', function=SimpleNamespace(name='calculator', arguments=json.dumps({'expression': '2 + 2'})))
...
def _client(completions: FakeChatCompletions) -> BedrockOpenAICompatibleClient:
    return BedrockOpenAICompatibleClient(model=MODEL, api_key='test-key', base_url=BASE_URL, client=FakeClient(completions))

@pytest.mark.parametrize(('model', 'api_key', 'base_url', 'missing_field'), [(' ', 'key', BASE_URL, 'model'), (MODEL, None, BASE_URL, 'api_key'), (MODEL, 'key', '', 'base_url')])
def test_bedrock_requires_explicit_connection_fields_even_with_fake_client(model: str | None, api_key: str | None, base_url: str | None, missing_field: str) -> None:
    with pytest.raises(LLMConfigurationError, match=missing_field) as raised:
        BedrockOpenAICompatibleClient(model=model, api_key=api_key, base_url=base_url, client=FakeClient(FakeChatCompletions()))
...
def test_bedrock_preserves_caller_selected_connection() -> None:
    client = BedrockOpenAICompatibleClient(model=f' {MODEL} ', api_key=' test-key ', base_url=f' {BASE_URL} ', client=FakeClient(FakeChatCompletions()))
...
def test_bedrock_normalizes_chat_completions_usage() -> None:
    usage = SimpleNamespace(prompt_tokens=10, completion_tokens=4, total_tokens=14, prompt_cache_hit_tokens=3, prompt_cache_miss_tokens=7)
...
def test_bedrock_normalizes_one_native_tool_call() -> None:
    ...
    result = _client(completions).complete_action(MESSAGES, tools=[_calculator_tool()])
...
def test_bedrock_rejects_parallel_native_actions_and_uses_json_fallback() -> None:
    ...
    completions = FakeChatCompletions([_response(content=None, tool_calls=[_tool_call('call_1'), _tool_call('call_2')]), _response(content=fallback)])
    result = _client(completions).complete_action(MESSAGES, tools=[_calculator_tool()])
...
def test_bedrock_environment_prefers_canonical_connection_values(tmp_path: Path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'bedrock', 'CHULK_MODEL': MODEL, 'CHULK_BEDROCK_API_KEY': 'canonical-key', 'BEDROCK_API_KEY': 'bedrock-key', 'AWS_BEARER_TOKEN_BEDROCK': 'aws-key', 'CHULK_BEDROCK_BASE_URL': BASE_URL, 'CHULK_BASE_URL': 'https://legacy.example/openai/v1'})
    assert config.bedrock_api_key == 'canonical-key'
    assert config.bedrock_base_url == BASE_URL
    assert provider_connection_from_config('bedrock', config) == LLMProviderConnection(api_key='canonical-key', base_url=BASE_URL)

@pytest.mark.parametrize(('environment', 'expected_key'), [({'BEDROCK_API_KEY': 'bedrock-key', 'AWS_BEARER_TOKEN_BEDROCK': 'aws-key'}, 'bedrock-key'), ({'AWS_BEARER_TOKEN_BEDROCK': 'aws-key'}, 'aws-key')])
def test_bedrock_environment_supports_api_key_aliases_in_order(tmp_path: Path, environment: dict[str, str], expected_key: str) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'bedrock', 'CHULK_MODEL': MODEL, 'CHULK_BEDROCK_BASE_URL': BASE_URL, **environment})
...
def test_bedrock_environment_supports_legacy_base_url_alias(tmp_path: Path) -> None:
    ...
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'bedrock', 'CHULK_MODEL': MODEL, 'BEDROCK_API_KEY': 'test-key', 'CHULK_BASE_URL': legacy_base_url})
...
def test_bedrock_environment_requires_explicit_model(tmp_path: Path) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_MODEL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'bedrock', 'CHULK_BEDROCK_API_KEY': 'test-key', 'CHULK_BEDROCK_BASE_URL': BASE_URL})
...
def test_bedrock_environment_requires_explicit_base_url(tmp_path: Path) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_BEDROCK_BASE_URL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'bedrock', 'CHULK_MODEL': MODEL, 'CHULK_BEDROCK_API_KEY': 'test-key'})

def test_bedrock_fallback_requires_explicit_base_url(tmp_path: Path) -> None:
    with pytest.raises(ConfigValueError, match='CHULK_BEDROCK_BASE_URL'):
        load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'openai', 'CHULK_MODEL': 'gpt-4.1-mini', 'CHULK_LLM_FALLBACK_PROVIDERS': f'bedrock:{MODEL}', 'CHULK_BEDROCK_API_KEY': 'test-key'})

def test_bedrock_factory_builds_shared_transport_with_bound_capabilities(monkeypatch: pytest.MonkeyPatch) -> None:
    completions = FakeChatCompletions()
    ...
    client = create_llm_client(provider='bedrock', model=MODEL, connection=LLMProviderConnection(api_key='test-key', base_url=BASE_URL), timeout_seconds=12.5, max_retries=3)
    ...
    assert captured == {'api_key': 'test-key', 'base_url': BASE_URL, 'timeout': 12.5, 'max_retries': 3}
...
def test_bedrock_profile_and_model_capabilities_are_registered() -> None:
    ...
    assert capabilities.default_response_reserve_tokens == BEDROCK_DEFAULT_RESPONSE_RESERVE_TOKENS

def test_bedrock_agent_config_and_cli_use_public_provider_spec(tmp_path: Path) -> None:
    config = AgentConfig.bedrock(project_root=tmp_path, model=MODEL, api_key='test-key', base_url=BASE_URL).to_config()
...
def test_bedrock_show_config_redacts_credentials(tmp_path: Path) -> None:
    ...
    config = AgentConfig.bedrock(project_root=tmp_path, model=MODEL, api_key=secret, base_url=BASE_URL).to_config()
