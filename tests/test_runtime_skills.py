...
class _AsyncRegistry(_SyncRegistry):
    ...
    async def register_path(self, path: Path | str) -> _Skill:
        return super().register_path(path)

    async def get_skill(self, name: str) -> _Skill | None:
...
def _specs() -> list[object]:
    return [SkillAllowlistRef(('review', 'missing')), SkillPinRef(('pinned',)), SkillRef(skill_path=Path('path-skill')), 'raw']
...
async def test_sync_and_async_skill_specs_share_normalized_resolution():
    ...
    sync = resolve_skill_specs(sync_registry, _specs())
