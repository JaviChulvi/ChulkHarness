...
from chulk.scheduling import AsyncHostedScheduleRunSubmitter, HostedScheduledOccurrence, HostedScheduleRunSubmitter

def _occurrence(run_id: str) -> HostedScheduledOccurrence:
    scope = ExecutionScope(tenant_id='tenant-a', workspace_id='workspace-a', actor_id='scheduler', agent_id='digest-agent', agent_version='2.0.0', run_id=run_id, trigger_id='daily-summary')
    return HostedScheduledOccurrence(schedule_id='daily-summary', target=GatewayRunTarget(scope=scope, definition_id='digest-agent', definition_version='2.0.0', definition_digest='sha256:definition'), scheduled_for=datetime(2026, 7, 27, 8, tzinfo=timezone.utc), idempotency_key='schedule:daily-summary:2026-07-27')
