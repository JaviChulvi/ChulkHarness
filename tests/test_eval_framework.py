...
from chulk.evals import AsyncEvalRunner, CallableGrader, EvalCase, EvalDataset, EvalReference, EvalReport, EvalRunner, EvalSafetyPolicy, EvalSuite, EvalTarget, EvalTurn, EvalTurnResult, ExactAnswerGrader, GradeResult, LLMJudgeGrader, MetricThreshold, SQLiteEvalStore, StatusGrader, ToolCallGrader, TrialResult, compare_reports, export_report
...
def _agent(context):
    return Agent(config=AgentConfig(project_root=context.workspace), llm=context.llm, tools=[], skills=[])

def _suite(dataset: EvalDataset, **kwargs) -> EvalSuite:
    return EvalSuite(name='support', dataset=dataset, targets=(EvalTarget('sdk', _agent),), graders=(ExactAnswerGrader(), StatusGrader()), required_graders=('answer.exact', 'run.status'), thresholds={'pass_rate': MetricThreshold(min=1.0)}, **kwargs)

def test_dataset_jsonl_is_strict_versioned_and_digest_stable(tmp_path: Path) -> None:
    path = tmp_path / 'cases.jsonl'
    payload = {'schema_version': 1, 'id': 'ready', 'turns': [{'input': 'Ready?', 'scripted_responses': [{'type': 'final_answer', 'content': 'Ready.'}]}], 'reference': {'answer': 'Ready.'}, 'tags': ['smoke']}
...
def test_eval_models_deep_freeze_nested_public_data() -> None:
    ...
    case = EvalCase('immutable', (EvalTurn('hello'),), reference, metadata=metadata)
...
def test_runner_executes_multiturn_public_agent_and_quality_gate() -> None:
    case = EvalCase('conversation', (EvalTurn('Remember alpha', ({'type': 'final_answer', 'content': 'Remembered.'},)), EvalTurn('What did I say?', ({'type': 'final_answer', 'content': 'alpha'},))), EvalReference(answer='alpha'))
...
async def test_async_runner_matches_sync_result_shape() -> None:
    ...
    async def factory(context):
        return AsyncAgent(config=AgentConfig(project_root=context.workspace), llm=context.llm, tools=[], skills=[])
    case = EvalCase('ready', (EvalTurn('Ready?', ({'type': 'final_answer', 'content': 'Ready.'},)),), EvalReference(answer='Ready.'))
    suite = EvalSuite('async', EvalDataset((case,)), (EvalTarget('async-sdk', factory),), (ExactAnswerGrader(),), required_graders=('answer.exact',))
...
def test_side_effecting_tools_are_denied_unless_named() -> None:
    dangerous = Tool(name='write_record', description='Write a record.', args_schema={'type': 'object', 'properties': {}, 'additionalProperties': False}, callable=lambda _arguments: 'written', permission_level=ToolPermissionLevel.WRITE)

    def factory(context):
        return Agent(config=AgentConfig(project_root=context.workspace), llm=context.llm, tools=[dangerous], skills=[])
    case = EvalCase('safe', (EvalTurn('Do nothing', ({'type': 'final_answer', 'content': 'done'},)),))
    base = dict(name='safe', dataset=EvalDataset((case,)), targets=(EvalTarget('sdk', factory),))
    denied = EvalRunner().run(EvalSuite(**base))
    allowed = EvalRunner().run(EvalSuite(**base, safety=EvalSafetyPolicy(allowed_tool_names=('write_record',))))
...
def test_callable_and_llm_judge_graders_are_normalized() -> None:
    case = EvalCase('quality', (EvalTurn('Answer', ({'type': 'final_answer', 'content': 'clear answer'},)),))
    judge = ScriptedLLMClient(['{"score": 0.9, "passed": true, "reason": "Clear"}'])
    suite = EvalSuite('quality', EvalDataset((case,)), (EvalTarget('sdk', _agent),), (CallableGrader('custom', lambda _case, _trial: 0.75), LLMJudgeGrader(judge, 'The answer must be clear.')), required_graders=('custom', 'quality.judge'), max_total_cost=1.0, safety=EvalSafetyPolicy(allow_unknown_cost=True))
...
def test_tool_grader_matches_arguments_results_and_failures() -> None:
    reference = EvalReference(tool_sequence=('lookup',), tool_arguments={'lookup': {'id': 7}}, tool_results={'lookup': {'found': True}}, tool_failures=())
    case = EvalCase('tools', (EvalTurn('look up', reference=reference),))
    result = RunResult('done', RunStatus.COMPLETED, None, 'test', None, tool_calls=(ToolCall('lookup', {'id': 7}, 1, success=True),), observations=(Observation('lookup', '{"found": true}'),))
    trial = TrialResult('tools', 'sdk', 1, (EvalTurnResult(0, result, (), 0.01),), 0.01)
...
def test_sqlite_store_baseline_comparison_and_all_exports(tmp_path: Path) -> None:
    ...
    assert comparison.matched_graders == ('sdk:ready:answer.exact', 'sdk:ready:run.status')
...
def test_all_export_formats_redact_live_report_payloads(tmp_path: Path) -> None:
    report = {'id': 'live-secret-run', 'suite_name': 'live', 'status': 'completed', 'passed': False, 'metrics': {'pass_rate': 0.0}, 'threshold_failures': [], 'operational_errors': ['provider failed with Bearer live-secret-token'], 'metadata': {'api_key': 'metadata-secret'}, 'cases': [{'target_name': 'api_key=target-secret', 'case_id': 'secret-case', 'passed': False, 'trials': [{'duration_seconds': 0.1, 'exception': 'password=trial-secret', 'turns': [{'result': {'content': 'sk-liveanswer123456', 'tool_calls': [{'tool_name': 'lookup', 'arguments': {'authorization': 'tool-secret'}}], 'observations': [{'content': 'token=observation-secret'}], 'errors': ['cookie=error-secret']}, 'events': [{'payload': {'credential': 'event-secret'}}]}], 'grades': []}]}]}
    secrets = ('live-secret-token', 'metadata-secret', 'target-secret', 'trial-secret', 'sk-liveanswer123456', 'tool-secret', 'observation-secret', 'error-secret', 'event-secret')
    for suffix, format_name in (('json', 'json'), ('jsonl', 'jsonl'), ('xml', 'junit'), ('html', 'html')):
        output = export_report(report, tmp_path / f'live-report.{suffix}', format=format_name)
        rendered = output.read_text(encoding='utf-8')
        assert '[redacted]' in rendered
        assert all((secret not in rendered for secret in secrets))

def test_html_export_prioritizes_verdict_cases_and_graders(tmp_path: Path) -> None:
    report = {'schema_version': 1, 'id': 'run-123', 'suite_name': 'support<script>', 'status': 'completed', 'passed': True, 'started_at': '2026-08-10T21:20:46+00:00', 'threshold_failures': [], 'operational_errors': [], 'metrics': {'pass_rate': 1.0, 'case_count': 1, 'p95_latency_seconds': 0.186, 'total_tokens': 26285, 'total_cost': 0, 'grader.answer.exact.pass_rate': 1.0}, 'cases': [{'target_name': 'sdk<script>', 'case_id': 'ready<script>', 'passed': True, 'trials': [{'duration_seconds': 0.186, 'grades': [{'grader': 'answer.exact<script>', 'score': 1.0, 'passed': True, 'error': None, 'details': {'required': True}}, {'grader': 'style.informational', 'score': 0.5, 'passed': False, 'error': None, 'details': {'required': False}}]}]}]}
...
def test_html_export_surfaces_failed_quality_and_operational_details(tmp_path: Path) -> None:
    report = {'id': 'failed-run', 'suite_name': 'failed', 'status': 'completed', 'passed': False, 'metrics': {'pass_rate': 0.0}, 'cases': [], 'threshold_failures': ['pass_rate < 1'], 'operational_errors': ['provider <offline>']}
...
def test_thresholds_are_explicit_and_informational_graders_do_not_fail() -> None:
    case = EvalCase('info', (EvalTurn('Answer', ({'type': 'final_answer', 'content': 'actual'},)),), EvalReference(answer='expected'))
    suite = EvalSuite('info', EvalDataset((case,)), (EvalTarget('sdk', _agent),), (ExactAnswerGrader(),))
    report = EvalRunner().run(suite)
...
def test_suite_rejects_unknown_required_graders_fixtures_and_duplicate_targets() -> None:
    ...
    with pytest.raises(ValueError, match='unknown required graders'):
        EvalSuite('invalid', EvalDataset((EvalCase('case', (EvalTurn('hello'),)),)), (EvalTarget('sdk', _agent),), required_graders=('missing',))
    ...
    with pytest.raises(ValueError, match='target names must be unique'):
        EvalSuite('invalid', EvalDataset((EvalCase('case', (EvalTurn('hello'),)),)), (EvalTarget('sdk', _agent), EvalTarget('sdk', _agent)))

def test_metrics_are_grouped_by_target_provider_model_and_tag() -> None:
    case = EvalCase('ready', (EvalTurn('Ready?', ({'type': 'final_answer', 'content': 'Ready.'},)),), EvalReference(answer='Ready.'), tags=('smoke',))
    suite = EvalSuite('grouped', EvalDataset((case,)), (EvalTarget('sdk', _agent, provider='fake', model='model-1'),), (ExactAnswerGrader(),), required_graders=('answer.exact',))
...
def test_metrics_report_cache_and_success_adjusted_efficiency() -> None:
    ...
    class MeteredAgent:
        ...
        def run_result(self, _message, **_kwargs):
            return RunResult('expected' if self.case_id == 'pass' else 'wrong', RunStatus.COMPLETED, None, 'conversation', None, usage=Usage(input_tokens=100, output_tokens=20, total_tokens=120, cache_hit_input_tokens=60, cache_miss_input_tokens=30, cache_write_input_tokens=10), cost=Cost(Decimal('1.00'), pricing_known=True))

        def close(self):
            return None

    def suite(*case_ids: str) -> EvalSuite:
        cases = tuple((EvalCase(case_id, (EvalTurn('Answer'),), EvalReference(answer='expected')) for case_id in case_ids))
        return EvalSuite('efficiency', EvalDataset(cases), (EvalTarget('sdk', lambda context: MeteredAgent(context.case_id)),), (ExactAnswerGrader(),), required_graders=('answer.exact',))
...
def test_compare_reports_includes_paired_case_outcomes() -> None:
    ...
    def report(report_id: str, outcomes: dict[str, bool]) -> dict[str, object]:
        return {'id': report_id, 'suite_name': 'paired', 'metrics': {'pass_rate': sum(outcomes.values()) / len(outcomes)}, 'metadata': {'targets': [{'name': 'agent', 'fingerprint': 'target-v1'}]}, 'cases': [{'target_name': 'agent', 'case_id': case_id, 'passed': passed, 'trials': []} for case_id, passed in outcomes.items()]}
    comparison = compare_reports(report('current', {'one': True, 'two': True, 'three': True, 'four': False}), report('baseline', {'one': False, 'two': False, 'three': False, 'four': False}))
...
def test_sync_runner_honors_explicit_bounded_concurrency() -> None:
    ...
    suite = EvalSuite('parallel', dataset, (EvalTarget('slow', lambda _context: SlowAgent()),), concurrency=2)
    report = EvalRunner().run(suite)
    assert not report.operational_errors
    assert peak == 2

def test_required_grader_errors_are_operational_and_report_has_provenance() -> None:
    case = EvalCase('broken-grader', (EvalTurn('Answer', ({'type': 'final_answer', 'content': 'answer'},)),))

    def fail(_case, _trial):
        raise RuntimeError('grader unavailable')
    suite = EvalSuite('broken-grader', EvalDataset((case,)), (EvalTarget('sdk', _agent),), (CallableGrader('required', fail),), required_graders=('required',))
    report = EvalRunner().run(suite)
...
async def test_runners_reject_grades_for_a_different_grader_identity() -> None:
    ...
    class MismatchedGrader:
        ...
        async def grade_async(self, _case, _trial):
            return GradeResult('typo', 1.0, True, 'incorrect identity')
    case = EvalCase('mismatched-grader', (EvalTurn('Answer', ({'type': 'final_answer', 'content': 'answer'},)),))
    suite = EvalSuite('mismatched-grader', EvalDataset((case,)), (EvalTarget('sdk', _agent),), (MismatchedGrader(),), required_graders=('required',))
