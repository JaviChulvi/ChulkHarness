...
import sys
import pytest
from chulk import Agent, AgentConfig, AsyncAgent, ConfigurationError, LocalPluginRegistry, PluginCategory, Plugins, inspect_plugin_directory, plugins
...
def build_agent(tmp_path, **kwargs) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient(), tools=[], skills=[], **kwargs)
...
def test_sync_facade_registers_audits_and_loads_reviewed_plugins(tmp_path):
    ...
    registered = facade.register_local_plugin(package, approved_by='operator', acknowledge_host_authority=True, granted_capabilities=('files:read',))
    listed = facade.list_plugins()
    report = facade.audit_plugins()
    loaded = facade.load_plugin_entry_point('sample-plugin', PluginCategory.TOOL, 'sample', available_capabilities=('files:read',))
...
def test_sync_facade_exposes_managed_install_uninstall_and_rollback(tmp_path):
    package = write_plugin(tmp_path / 'source')
    facade = build_agent(tmp_path)
    installed = facade.install_plugin(package, approved_by='operator', acknowledge_host_authority=True)
    uninstalled = facade.uninstall_plugin('sample-plugin', approved_by='operator')
    restored = facade.rollback_plugin('sample-plugin', approved_by='operator')
...
async def test_async_facade_has_plugin_operation_parity(tmp_path):
    ...
    registered = await facade.register_local_plugin(package, approved_by='operator', acknowledge_host_authority=True, granted_capabilities=('files:read',))
    report, listed = await asyncio.gather(facade.audit_plugins(), facade.list_plugins())
    assert inspected.compatible is True
    assert listed == (registered,)
    assert report.verified_plugins == ('sample-plugin',)
    await facade.close()
...
async def test_async_facade_has_managed_lifecycle_parity(tmp_path):
    package = write_plugin(tmp_path / 'source')
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient(), tools=[], skills=[])
    installed = await facade.install_plugin(package, approved_by='operator', acknowledge_host_authority=True)
    disabled = await facade.uninstall_plugin('sample-plugin', approved_by='operator')
    restored = await facade.rollback_plugin('sample-plugin', approved_by='operator')
    assert installed.action.value == 'install'
...
def test_agent_startup_fails_closed_when_reviewed_plugin_changes(tmp_path):
    package = write_plugin(tmp_path)
    facade = build_agent(tmp_path)
    facade.register_local_plugin(package, approved_by='operator', acknowledge_host_authority=True, granted_capabilities=('files:read',))
    facade.close()
...
def test_agent_rejects_registry_owned_by_another_profile(tmp_path):
    registry = LocalPluginRegistry(tmp_path / '.chulk', profile_id='another')
