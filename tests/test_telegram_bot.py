...
def _config(tmp_path: Path):
    return load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'gemini', 'CHULK_MODEL': 'gemini-test', 'CHULK_GEMINI_API_KEY': 'fake'})

def _update(text: str, *, update_id: int=1, user_id: int=7, chat_type: str='private') -> TelegramUpdate:
    return TelegramUpdate(update_id=update_id, chat_id=9, user_id=user_id, text=text, chat_type=chat_type)
...
def _bot(tmp_path: Path, client: FakeClient, agents: list[FakeAgent]) -> TelegramAgentBot:
    ...
    def factory(chat_id: int, conversation_id: str | None) -> FakeAgent:
        ...
        agents.append(agent)
        return agent
    return TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7}), poll_timeout_seconds=1, scheduling_enabled=True), client=client, agent_factory=factory)
...
async def test_bot_routes_messages_and_commands_to_one_chat_agent(tmp_path: Path) -> None:
    ...
    assert kwargs['extension_metadata'] == {'source': 'telegram', 'telegram_chat_id': 9}
...
async def test_bot_processes_media_before_normal_agent_turn(tmp_path: Path) -> None:
    ...
    update = TelegramUpdate(update_id=4, chat_id=9, user_id=7, text='Summarize', chat_type='private', attachment=TelegramAttachment('voice-1', 'voice', 'audio/ogg'))
...
async def test_bot_closes_owned_media_processor_once(tmp_path: Path) -> None:
    ...
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7})), client=FakeClient(), media_processor=processor, owns_media_processor=True)
    await bot.close()
    await bot.close()
...
async def test_media_deadline_is_sanitized_and_releases_chat_lock(tmp_path: Path) -> None:
    ...
    class BlockingAgent(FakeAgent):
        ...
        async def run_input(self, user_input: UserInput, **kwargs: object) -> str:
            ...
            return 'too late'
    client = FakeClient()
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path), 'CHULK_LLM_PROVIDER': 'gemini', 'CHULK_MODEL': 'gemini-test', 'CHULK_GEMINI_API_KEY': 'fake', 'CHULK_LLM_TIMEOUT_SECONDS': '0.01'})
    bot = TelegramAgentBot(config=config, telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7})), client=client, agent_factory=lambda chat_id, conversation_id: BlockingAgent(conversation_id or f'new-{chat_id}'))
    update = TelegramUpdate(update_id=5, chat_id=9, user_id=7, text='Summarize', chat_type='private', attachment=TelegramAttachment('voice-1', 'voice', 'audio/ogg'))
    await bot.handle_update(update)
    assert client.sent[-1] == (9, 'The configured media provider could not interpret that attachment.')
...
async def test_bot_rejects_iwork_package_before_download(tmp_path: Path) -> None:
    ...
    bot._media_processor = FakeMediaProcessor()
    update = TelegramUpdate(update_id=5, chat_id=9, user_id=7, text='Summarize', chat_type='private', attachment=TelegramAttachment('pages-1', 'document', 'application/octet-stream', 'proposal.pages'))
    await bot.handle_update(update)
...
async def test_bot_resumes_conversation_mapped_in_sqlite(tmp_path: Path) -> None:
    ...
    agents: list[FakeAgent] = []
    bot = _bot(tmp_path, client, agents)
    bot.session_store.create_conversation('existing-conversation', provider='gemini', model='gemini-test', metadata={TELEGRAM_CHAT_METADATA_KEY: 9})
...
@pytest.mark.asyncio
async def test_poll_retry_after_send_failure_does_not_repeat_agent_execution(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_restart_delivers_recorded_response_without_reexecuting_update(tmp_path: Path) -> None:
...
async def test_multipart_retry_resumes_after_last_delivery_checkpoint(tmp_path: Path) -> None:
    ...
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7}), poll_timeout_seconds=1), client=client, agent_factory=factory)
...
@pytest.mark.asyncio
async def test_concurrent_bot_does_not_execute_or_advance_an_active_update(tmp_path: Path) -> None:
    ...
    first = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7}), poll_timeout_seconds=1), client=FakeClient((update,)), agent_factory=blocking_factory)
    second_agents: list[FakeAgent] = []
...
async def test_bot_executes_and_delivers_due_scheduled_job(tmp_path: Path) -> None:
    ...
    agents: list[FakeAgent] = []
    bot = _bot(tmp_path, client, agents)
    job = bot.schedule_store.create(adapter='telegram', destination_id='9', prompt='scheduled research', next_run_at=datetime.now(timezone.utc))
...
@pytest.mark.asyncio
async def test_scheduler_loop_survives_a_recoverable_iteration_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_bot_lists_and_cancels_only_chat_scheduled_jobs(tmp_path: Path) -> None:
    client = FakeClient()
    bot = _bot(tmp_path, client, [])
    job = bot.schedule_store.create(adapter='telegram', destination_id='9', prompt='remind me', next_run_at=datetime(2030, 1, 1, tzinfo=timezone.utc))
...
@pytest.mark.asyncio
async def test_default_agent_adds_only_bounded_web_network_tool(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = FakeClient()
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7}), tavily_api_key='search-secret', web_search_max_results=3, scheduling_enabled=True), client=client)
    agent = bot._default_agent_factory(9, None)
    try:
        names = {tool.name for tool in captured['tools']}
        assert names == {'calculator', 'read_file', 'list_files', 'search_files', 'search_memory', 'list_memories', 'summarize_memories', 'schedule_task', 'current_time', 'list_scheduled_tasks', 'cancel_scheduled_task', 'web_search'}
...
@pytest.mark.asyncio
async def test_multi_user_safe_mode_excludes_memory_tools_and_prompt_selection(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}
    ...
    class CapturingAgent:
        ...
        async def close(self) -> None:
            pass
    monkeypatch.setattr(telegram_bot_module, 'AsyncAgent', CapturingAgent)
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7, 8}), long_term_memory_enabled=False), client=FakeClient())
    agent = bot._default_agent_factory(9, None)
    try:
        names = {tool.name for tool in captured['tools']}
        assert {'search_memory', 'list_memories', 'summarize_memories'}.isdisjoint(names)
        assert captured['capabilities'].memory is MemoryMode.OFF
    finally:
        await agent.close()

@pytest.mark.asyncio
async def test_multi_user_agents_receive_distinct_nondefault_memory_namespaces(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class CapturingAgent:
        ...
        async def close(self) -> None:
            pass
    monkeypatch.setattr(telegram_bot_module, 'AsyncAgent', CapturingAgent)
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7, 8})), client=FakeClient())
    first = bot._default_agent_factory(9, None)
    second = bot._default_agent_factory(10, None)
    try:
        assert [item['memory_namespace'] for item in captured] == ['telegram:chat:9', 'telegram:chat:10']
        for item in captured:
            assert item['capabilities'].memory is MemoryMode.READ_ONLY
            assert {'search_memory', 'list_memories', 'summarize_memories'} <= {tool.name for tool in item['tools']}
    finally:
        await first.close()
        await second.close()

@pytest.mark.asyncio
async def test_owner_route_selects_an_isolated_profile_without_startup_overwrite(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}
    ...
    profile_factory.profile_store.create_profile('work', project_root=tmp_path)
    ...
    router.add_route(adapter='telegram', account_id='primary', principal_id='7', profile_id='work')
    bot = TelegramAgentBot(config=config, telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7})), client=FakeClient(), gateway_ledger=ledger, gateway_router=router, profile_runtime_factory=profile_factory)
...
def test_session_metadata_persists_telegram_chat_mapping(tmp_path: Path) -> None:
    ...
    SessionRecorder(store, 'telegram-conversation', provider='gemini', model='gemini-test', metadata={TELEGRAM_CHAT_METADATA_KEY: 9})
...
def test_scheduling_is_not_added_when_adapter_does_not_opt_in(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}
    ...
    class CapturingAgent:
        ...
        def __init__(self, **kwargs: object) -> None:
            captured.update(kwargs)
    monkeypatch.setattr(telegram_bot_module, 'AsyncAgent', CapturingAgent)
    bot = TelegramAgentBot(config=_config(tmp_path), telegram_config=TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7})), client=FakeClient())
