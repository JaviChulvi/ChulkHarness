...
import pytest
from chulk.skills import LEGACY_SKILL_VERSION, SkillManifestError, SkillRegistry, load_skill_package, skill_package_digest
...
def test_explicit_manifest_loads_all_supported_fields(tmp_path):
    ...
    root = path.parent
    resources = ('instructions.md', 'references/guide.md', 'scripts/check.py', 'templates/report.md', 'examples/example.md', 'tests/test_example.py')
...
@pytest.mark.parametrize(('front_matter', 'message'), [('name: example\nname: duplicate', 'duplicate key'), ('name: &value example\ndescription: *value', 'anchors are not allowed'), ('name: !custom example', 'custom YAML tag'), ('- name\n- example', 'must be a mapping'), ('schema_version: 2\nname: example', 'unsupported skill schema_version'), ('schema_version: 1\nname: example\nowner: core', 'unsupported skill manifest fields'), ('schema-version: 1\nschema_version: 1\nname: example', 'duplicate normalized skill manifest key')])
def test_manifest_rejects_unsafe_or_unsupported_yaml(tmp_path, front_matter, message):
    path = write_package(tmp_path, f'---\n{front_matter}\n---\n# Example\n\nInstructions.\n')
...
@pytest.mark.parametrize(('field', 'value', 'message'), [('version', 'latest', 'semantic version'), ('platforms', '[plan9]', 'unsupported skill platforms'), ('required_tools', '[read-file]', 'invalid skill required_tools'), ('required_capabilities', '[Vision]', 'invalid skill required_capabilities'), ('secret_refs', '[lowercase]', 'invalid skill secret_refs'), ('configuration', '[]', 'configuration must be a string-keyed mapping'), ('references', '[../outside.md]', 'unsafe skill references path')])
def test_manifest_validates_typed_fields(tmp_path, field, value, message):
    version = '' if field == 'version' else 'version: 1.0.0\n'
    path = write_package(tmp_path, f'---\nschema_version: 1\nname: example\n{version}description: Example workflow.\n{field}: {value}\n---\n# Example\n')
    with pytest.raises(SkillManifestError, match=message):
        load_skill_package(path)

def test_manifest_rejects_missing_and_escaped_resources(tmp_path):
    missing = write_package(tmp_path, '---\nschema_version: 1\nname: example\nversion: 1.0.0\ndescription: Example workflow.\nreferences: [missing.md]\n---\n# Example\n')
    ...
    missing.write_text(missing.read_text(encoding='utf-8').replace('missing.md', 'linked.md'), encoding='utf-8')
    with pytest.raises(SkillManifestError, match='escapes or is missing'):
        load_skill_package(missing)
