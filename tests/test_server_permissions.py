...
from chulk.approvals import ApprovalStatus, ApprovalSubmission, DurableApprovalService, SQLiteApprovalStore
from chulk.runs import RunSubmission, SQLiteRunStore, StepDefinition
from chulk.server import DurablePermissionBroker, PermissionBroker, PermissionDecisionConflictError, PublicEventJournal, list_profile_permissions
...
def _request(arguments=None) -> PermissionRequest:
    return PermissionRequest(tool_name='shell', permission_level=ToolPermissionLevel.SHELL, arguments=arguments or {'command': 'printf hello', 'api_key': 'secret'}, requires_confirmation=True, policy_name='workspace-write', reason='shell tool requires approval')

def _record() -> PermissionDecisionRecord:
    return PermissionDecisionRecord(tool_name='shell', permission_level=ToolPermissionLevel.SHELL, decision=PermissionDecision.ASK, reason='tool call requires user approval', policy_name='workspace-write', requires_confirmation=True)

def _broker(tmp_path, **kwargs) -> PermissionBroker:
    path = tmp_path / 'store.sqlite'
    journal = PublicEventJournal(path, profile_id='work')
    return PermissionBroker(path, profile_id='work', conversation_id='conversation-1', turn_id=lambda: 'turn-1', journal=journal, **kwargs)

def test_permission_broker_waits_for_one_idempotent_api_decision(tmp_path) -> None:
    broker = _broker(tmp_path)
    answer: list[PermissionDecision] = []
    worker = threading.Thread(target=lambda: answer.append(broker.callback(_request(), _record())))
    worker.start()
    for _ in range(100):
    ...
    resolved = broker.decide(pending[0].id, 'allow', idempotency_key='decision-1', reason='owner approved')
    duplicate = broker.decide(pending[0].id, 'allow', idempotency_key='decision-1', reason='ignored duplicate')
    ...
    assert [item.event.name for item in events] == ['permission.requested', 'permission.resolved']

def test_permission_broker_publishes_request_before_concurrent_decision(tmp_path, monkeypatch) -> None:
    broker = _broker(tmp_path)
    ...
    def decide_request(request_id: str) -> None:
        try:
            broker.decide(request_id, 'allow', idempotency_key='decision-concurrent')
        except BaseException as exc:
            errors.append(exc)
        finally:
    ...
    assert errors == []
    assert [item.event.name for item in journal.list('conversation-1')] == ['permission.requested', 'permission.resolved']
...
def test_profile_permission_inbox_spans_owned_conversations(tmp_path) -> None:
    first = _broker(tmp_path)
    second = PermissionBroker(tmp_path / 'store.sqlite', profile_id='work', conversation_id='conversation-2', turn_id=lambda: 'turn-2')
    first.create(_request({'command': 'first'}))
    second.create(_request({'command': 'second'}))
    inbox = list_profile_permissions(tmp_path / 'store.sqlite', profile_id='work', status='pending', limit=10)
    assert [item.conversation_id for item in inbox] == ['conversation-1', 'conversation-2']

def test_control_broker_decides_the_durable_hosted_approval_inbox(tmp_path) -> None:
    path = tmp_path / 'hosted.sqlite'
    scope = ExecutionScope(tenant_id='tenant-a', workspace_id='support', actor_id='operator-a', agent_id='support-agent', agent_version='2.0.0', run_id='run-1', conversation_id='conversation-1')
    runs = SQLiteRunStore(path)
    approvals = SQLiteApprovalStore(path)
    runs.submit(scope, RunSubmission(idempotency_key='message-1', input_digest='sha256:input', definition_digest='sha256:definition', steps=(StepDefinition(id='write', name='Write ticket'),)))
    claim = runs.claim(scope, worker_id='worker-a')
    assert claim is not None
    runs.start_step(scope, claim, 'write')
    paused = DurableApprovalService(approvals, runs).request(scope, claim, ApprovalSubmission(step_id='write', tool_name='update_ticket', tool_version='2.1.0', schema_version='3', arguments_digest='sha256:arguments', policy_version='policy-4', preview={'ticket_id': '42'}, expires_at=datetime.now(timezone.utc) + timedelta(minutes=5)))
    ...
    decided = broker.decide(paused.approval.id, 'allow', idempotency_key='control-decision-1', reason='operator verified the write')
    duplicate = broker.decide(paused.approval.id, 'allow', idempotency_key='control-decision-1', reason='operator verified the write')
