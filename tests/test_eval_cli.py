...
def _report(report_id: str='run-1', *, suite_name: str='starter', status: EvalRunStatus=EvalRunStatus.COMPLETED, mode: str='scripted', provider: str='fake', model: str='v1', tags: tuple[str, ...]=(), started_at: str | None=None) -> EvalReport:
    ...
    return EvalReport(report_id, suite_name, 'digest', now, now, (), metrics, metadata={'mode': mode, 'targets': [{'name': 'agent', 'provider': provider, 'model': model, 'fingerprint': f'{provider}:{model}'}], 'graders': []}, status=status)
...
def test_eval_parser_exposes_resume_filters_and_coverage_gate() -> None:
    run = build_parser().parse_args(['eval', 'run', 'suite.py:suite', '--resume', 'run-id'])
    listing = build_parser().parse_args(['eval', 'list', '--status', 'completed', '--provider', 'fake', '--tag', 'smoke', '--offset', '2'])
    comparison = build_parser().parse_args(['eval', 'compare', 'run-id', '--min-coverage', '0.9'])
...
def test_eval_list_filters_status_mode_target_provider_model_tag_and_page(tmp_path: Path) -> None:
    store = SQLiteEvalStore(tmp_path / 'filters.sqlite')
    store.save_report(_report('match', provider='openai', model='gpt-test', tags=('smoke',), started_at='2026-08-10T10:00:00+00:00'))
    store.save_report(_report('other', status=EvalRunStatus.INTERRUPTED, mode='live', provider='other', model='other-model', started_at='2026-08-09T10:00:00+00:00'))
    output: list[str] = []
    code = run_eval_command('list', store=store, status='completed', mode='scripted', target_name='agent', provider='openai', model='gpt-test', tags=('smoke',), started_after='2026-08-10T00:00:00+00:00', started_before='2026-08-11T00:00:00+00:00', limit=1, offset=0, json_output=True, output_func=output.append)
    assert code == 0
...
def test_eval_run_overrides_and_all_exit_codes(tmp_path: Path) -> None:
    ...
    output: list[str] = []
    errors: list[str] = []
    passed = run_eval_command('run', store=store, suite_ref=suite_ref, tags=('smoke',), trials=2, concurrency=2, timeout_seconds=2, provider='fake', model='fake-model', json_output=True, output_func=output.append, error_func=errors.append)
    ...
    assert run_eval_command('run', store=store, suite_ref=f'{quality_path}:suite', output_func=output.append, error_func=errors.append) == 1
    assert run_eval_command('run', store=store, suite_ref=suite_ref, mode='live', output_func=output.append, error_func=errors.append) == 2
    operational_path = tmp_path / 'operational.py'
    operational_path.write_text(_operational_suite_source(), encoding='utf-8')
    assert run_eval_command('run', store=store, suite_ref=f'{operational_path}:suite', output_func=output.append, error_func=errors.append) == 3
    cost_path = tmp_path / 'cost.py'
    cost_path.write_text(_cost_suite_source(), encoding='utf-8')
    assert run_eval_command('run', store=store, suite_ref=f'{cost_path}:suite', mode='live', max_total_cost=1.0, output_func=output.append, error_func=errors.append) == 3
...
def test_eval_resume_export_formats_and_baseline_coverage_exit(tmp_path: Path) -> None:
    ...
    marker.touch()
    suite_path.write_text(_resumable_suite_source(marker=marker, calls=calls), encoding='utf-8')
    ...
    marker.unlink()
    output: list[str] = []
    assert run_eval_command('run', store=store, suite_ref=f'{suite_path}:suite', resume_from=interrupted_id, json_output=True, output_func=output.append) == 0
    ...
    assert run_eval_command('compare', store=store, report_id=interrupted_id, min_baseline_coverage=1.0, output_func=output.append) == 0
    for suffix, format_name in (('json', 'json'), ('jsonl', 'jsonl'), ('xml', 'junit'), ('html', 'html')):
        destination = tmp_path / f'report.{suffix}'
        assert run_eval_command('export', store=store, report_id=interrupted_id, output_path=destination, export_format=format_name, output_func=output.append) == 0
    ...
    assert run_eval_command('export', store=store, report_id='unfinished', output_path=unfinished_xml, export_format='junit', output_func=output.append) == 0
...
def test_yaml_paths_are_relative_and_real_main_eval_flow_is_credential_free(tmp_path: Path, monkeypatch) -> None:
    ...
    assert main(['eval', 'init', str(project), '--json'], output_func=output.append, error_func=errors.append) == 0
    exit_code = main(['eval', 'run', f"{project / 'evals' / 'suite.py'}:suite", '--tag', 'smoke', '--json'], output_func=output.append, error_func=errors.append)
...
def test_eval_run_output_redacts_report_before_store_roundtrip(tmp_path: Path) -> None:
    ...
    suite_path.write_text(_suite_source(answer=secret, expected=secret), encoding='utf-8')
    output_path = tmp_path / 'live-report.json'
    exit_code = run_eval_command('run', store=SQLiteEvalStore(tmp_path / 'runs.sqlite'), suite_ref=f'{suite_path}:suite', output_path=output_path, export_format='json', output_func=lambda _message: None)
