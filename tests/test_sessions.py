...
class RecordingPromptHistory:
    ...
    def replace(self, messages) -> None:
        self.items = [message.content if hasattr(message, 'content') else message['content'] for message in messages if (message.role if hasattr(message, 'role') else message['role']) == 'user' and (not ((message.metadata if hasattr(message, 'metadata') else message.get('metadata')) or {}).get('internal'))]
...
def test_session_store_saves_messages_and_turn_snapshots(tmp_path):
    ...
    store.save_message('conversation-1', role='assistant', content='hi back', turn_id='turn-1', message_key='turn-1:assistant')
...
def test_session_recorder_persists_tool_action_before_observation(tmp_path):
    store = SQLiteSessionStore(tmp_path / 'store.sqlite')
    recorder = SessionRecorder(store, 'conversation-1', provider='test', model='mock')
    turn = TurnState(user_message='Use lookup.', turn_id='turn-1')
    recorder.callback(TraceEvent.TURN_STARTED, {'turn': turn.to_dict()})
    recorder.callback(TraceEvent.TOOL_OBSERVATION, {'turn_id': turn.turn_id, 'tool_name': 'lookup', 'tool_action_context': '<executed_tool_action>\n{"tool_name":"lookup","arguments_json":"{}"}\n</executed_tool_action>', 'observation': 'Lookup completed.', 'output_metadata': {}})
    assert store.load_recent_messages('conversation-1', limit=10) == [{'role': 'assistant', 'content': '<executed_tool_action>\n{"tool_name":"lookup","arguments_json":"{}"}\n</executed_tool_action>'}, {'role': 'observation', 'content': 'Lookup completed.'}]
    rendered_history = TerminalUI(color_enabled=False).history(store.list_messages('conversation-1', limit=10))
...
def test_async_session_recorder_persists_checkpoint_metadata(tmp_path):
    store = SQLiteSessionStore(tmp_path / 'store.sqlite')
    recorder = AsyncSessionRecorder(store, 'conversation-async-checkpoint', provider='test', model='mock')

    async def persist() -> None:
        recorder.callback(TraceEvent.CONTEXT_SUMMARY_CREATED, {'summary': 'Objective: Continue work.', 'source_message_count': 2, 'checkpoint': {'version': 1, 'objective': 'Continue work.', 'decisions': ['Keep the durable checkpoint.']}})
...
def test_session_recorders_atomically_dedupe_replayed_tool_observations(monkeypatch, tmp_path):
    path = tmp_path / 'store.sqlite'
    first_store = SQLiteSessionStore(path)
    first_recorder = SessionRecorder(first_store, 'conversation-1', provider='test', model='mock')
    turn = TurnState(user_message='Use lookup.', turn_id='turn-1')
    first_recorder.callback(TraceEvent.TURN_STARTED, {'turn': turn.to_dict()})
    turn.observations.append(ObservationRecord(tool_name='lookup', content='First lookup completed.'))
    first_payload = {'turn_id': turn.turn_id, 'observation_index': 1, 'tool_name': 'lookup', 'tool_action_context': 'Executed lookup with query one.', 'observation': 'First lookup completed.', 'output_metadata': {}, 'turn': turn.to_dict()}
    ...
    with sqlite3.connect(path) as conn:
        ...
        assert conn.execute("SELECT count(*) FROM conversation_messages WHERE role != 'user'").fetchone()[0] == 0
    monkeypatch.setattr(session_store_module, '_insert_message', original_insert_message)
    first_recorder.callback(TraceEvent.TOOL_OBSERVATION, first_payload)
    first_recorder.callback(TraceEvent.TOOL_OBSERVATION, first_payload)
    second_store = SQLiteSessionStore(path)
    second_recorder = SessionRecorder(second_store, 'conversation-1', provider='test', model='mock')
    turn.observations.append(ObservationRecord(tool_name='lookup', content='Second lookup completed.'))
    second_recorder.callback(TraceEvent.TOOL_OBSERVATION, {'turn_id': turn.turn_id, 'tool_name': 'lookup', 'tool_action_context': 'Executed lookup with query two.', 'observation': 'Second lookup completed.', 'output_metadata': {}, 'turn': turn.to_dict()})
    second_recorder.callback(TraceEvent.TOOL_OBSERVATION, first_payload)
    with sqlite3.connect(path) as conn:
        observation_keys = [row[0] for row in conn.execute('SELECT observation_key FROM conversation_observations ORDER BY observation_key')]
        message_keys = [row[0] for row in conn.execute("\n                SELECT message_key\n                FROM conversation_messages\n                WHERE role != 'user'\n                ORDER BY ordinal\n                ")]
    assert observation_keys == ['turn-1:observation:1', 'turn-1:observation:2']
    assert message_keys == ['turn-1:tool_action:1', 'turn-1:observation:1', 'turn-1:tool_action:2', 'turn-1:observation:2']
    restored_turn = SQLiteSessionStore(path).load_turns('conversation-1')[0]
    assert [record.content for record in restored_turn.observations] == ['First lookup completed.', 'Second lookup completed.']

def test_session_recorder_checkpoints_approved_and_plan_step_progress(tmp_path):
    path = tmp_path / 'store.sqlite'
    first_store = SQLiteSessionStore(path)
    first_recorder = SessionRecorder(first_store, 'conversation-1', provider='test', model='mock')
    plan = Plan(summary='Execute two durable steps.', steps=[PlanStep(id='1', title='Complete work', description='Finish the first step.'), PlanStep(id='2', title='Blocked work', description='Record a blocked second step.', depends_on=['1'])])
    turn = TurnState(user_message='Run the durable plan.', turn_id='turn-1')
    turn.wait_for_plan_approval(plan)
    first_recorder.callback(TraceEvent.PLAN_CREATED, {'turn_id': turn.turn_id, 'plan': plan.to_dict(), 'turn': turn.to_dict()})
    turn.approve_plan()
    first_recorder.callback(TraceEvent.PLAN_APPROVED, {'turn_id': turn.turn_id, 'plan': plan.to_dict(), 'turn': turn.to_dict()})
    ...
    second_recorder.callback(TraceEvent.PLAN_STEP_STARTED, {'turn_id': turn.turn_id, 'step': first_step.to_dict(), 'plan': plan.to_dict(), 'turn': turn.to_dict()})
    ...
    completed_payload = {'turn_id': turn.turn_id, 'step': first_step.to_dict(), 'plan': plan.to_dict(), 'turn': turn.to_dict()}
    ...
    second_recorder.callback(TraceEvent.PLAN_STEP_STARTED, {'turn_id': turn.turn_id, 'step': second_step.to_dict(), 'plan': plan.to_dict(), 'turn': turn.to_dict()})
    second_step.block('A durable blocker was recorded.')
    second_recorder.callback(TraceEvent.PLAN_STEP_BLOCKED, {'turn_id': turn.turn_id, 'step': second_step.to_dict(), 'plan': plan.to_dict(), 'turn': turn.to_dict()})
...
def test_session_recorder_atomically_persists_terminal_message_and_turn(tmp_path, recorder_type):
    ...
    rejected_plan = Plan(summary='Reject this plan.', steps=[PlanStep(id='1', title='Work', description='Do work.')])
    ...
    cases = [(TraceEvent.FINAL_ANSWER, final_turn, {'content': 'finished'}, 'completed', 'final'), (TraceEvent.PLAN_REJECTED, rejected_turn, {}, 'plan_rejected', 'plan_rejected'), (TraceEvent.TURN_FAILED, failed_turn, {'message': 'failed safely', 'status': 'failed'}, 'failed', 'failed'), (TraceEvent.TURN_FAILED, blocked_turn, {'message': 'blocked safely', 'status': 'blocked'}, 'blocked', 'failed'), (TraceEvent.PLAN_STEP_BLOCKED, blocked_turn, {}, 'blocked', 'failed')]
    for index, (event_type, turn, payload, status, message_kind) in enumerate(cases):
        ...
        recorder = recorder_type(store, f'conversation-{index}', provider='test', model='mock')
        recorder.callback(event_type, {'turn_id': turn.turn_id, 'turn': turn.to_dict(), **payload})
        ...
        terminal_message = store.load_terminal_turn_message(f'conversation-{index}', turn.turn_id)
...
def test_final_answer_checkpoint_survives_crash_before_turn_finished(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    agent = create_agent(config, lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'durable final'})]))
    ...
    resumed_agent = create_agent(config, lambda _config: resumed_llm, conversation_id=agent.state.conversation_id)
...
def test_session_store_round_trips_turn_context_metadata(tmp_path):
    ...
    turn = TurnState(user_message='hello', turn_id='turn-context', context_sections=[TurnContextSection(id='src-1', title='Source', source='drive://src-1', content='Source text.', metadata={'score': 0.92})], prompt_profile='polp-search', locale='es-ES', extension_metadata={'confidence': 0.8}, tool_context_metadata={'org_id': 'org-1'})
    store.save_turn_snapshot('conversation-1', turn.to_dict())
...
def test_session_store_saves_summary_and_loads_unsummarized_messages(tmp_path):
    ...
    store.save_message('conversation-1', role='assistant', content='Plan display for the user only.', message_key='plan-display', metadata={'prompt_excluded': True})
    ...
    store.save_conversation_summary('conversation-1', content='Old question and answer were about context compaction.', source_message_count=2)
    summary = store.load_latest_summary('conversation-1')
    assert summary is not None
    messages = store.load_recent_messages('conversation-1', limit=10, after_ordinal=summary.metadata['source_message_ordinal'])
    ...
    assert [message.content for message in store.list_messages('conversation-1')] == ['old question', 'Plan display for the user only.', 'old answer', 'latest question']

def test_session_store_restores_pending_plan_turn(tmp_path):
    store = SQLiteSessionStore(tmp_path / 'store.sqlite')
    store.create_conversation('conversation-1', provider='test', model='mock')
    plan = Plan(summary='Change the code.', steps=[PlanStep(id='1', title='Edit runtime', description='Update the runtime code.', acceptance_criteria=['Runtime is updated.'])])
...
def test_pending_plan_restores_skills_memories_and_current_dependencies(tmp_path):
    ...
    profile_id = memory_store.save_memory('Prefer exact verification.', tags=['preference'])
    relevant_id = memory_store.save_memory('The pending change targets the runtime.', tags=['project'])
    plan = Plan(summary='Use the restored execution context.', steps=[PlanStep(id='1', title='Run the context tool', description='Use the current host dependency.')])
    turn = TurnState(user_message='plan this', turn_id='turn-pending', loaded_skill_names=['review'], loaded_memory_ids=[profile_id, relevant_id], tool_context_metadata={'org_id': 'persisted-org'})
    turn.wait_for_plan_approval(plan)
    ...
    registry.register(Tool(name='use_dependency', description='Use a host dependency.', args_schema={'type': 'object', 'properties': {}}, callable=lambda _arguments, context: captured_contexts.append(context) or ToolResult('use_dependency', True, 'dependency used'), accepts_context=True))
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'use_dependency', 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': '{}'}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The dependency was used.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'done'})])
    agent = CoreAgent(llm, state=state, memory=memory, memory_store=memory_store, skill_registry=skill_registry, tool_registry=registry, default_tool_context=ToolExecutionContext(metadata={'runtime': 'current'}, deps={'token': 'live'}))
...
def test_resumable_plan_does_not_restore_memories_when_retrieval_is_off(tmp_path):
    memory_store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    memory_id = memory_store.save_memory('ARCHIVED_MEMORY_MARKER must stay out of the prompt.', tags=['project'])
    memory_store.archive_memory(memory_id)
    plan = Plan(summary='Finish restored work.', steps=[PlanStep(id='1', title='Finish', description='Return the final answer.')])
    turn = TurnState(user_message='finish this', turn_id='turn-memory-off', loaded_memory_ids=[memory_id])
    turn.wait_for_plan_approval(plan)
    turn.approve_plan()
    plan.steps[0].mark('completed')
    state = AgentState(turns=[turn], active_plan=plan, loaded_memory_ids=[memory_id])
    memory = ConversationMemory()
    memory.add_user_message(turn.user_message)
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'done without memory'})])
    agent = CoreAgent(llm, state=state, memory=memory, memory_store=memory_store, memory_policy=MemoryPolicy(memory_store, 'off'))
...
def test_session_store_preserves_blocked_plan_status(tmp_path):
    store = SQLiteSessionStore(tmp_path / 'store.sqlite')
    store.create_conversation('conversation-1', provider='test', model='mock')
    plan = Plan(summary='Run risky work.', steps=[PlanStep(id='1', title='Run tool', description='Run a tool that can fail.')])
    plan.approve()
...
def test_create_agent_continues_latest_approved_plan_after_restart(monkeypatch, tmp_path):
    ...
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    plan = Plan(summary='Continue durable approved work.', steps=[PlanStep(id='1', title='Finish durable work', description='Complete the work after restart.')])
    turn = TurnState(user_message='run the durable work', turn_id='turn-approved')
    turn.wait_for_plan_approval(plan)
    turn.approve_plan()
    plan.steps[0].mark('in_progress')
    store.save_message(conversation_id, turn_id=turn.turn_id, role='user', content=turn.user_message, message_key=f'{turn.turn_id}:user')
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    llm = FakeLLMClient([json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'Durable work completed after restart.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'resumed work complete'})])
    agent = create_agent(config, lambda _config: llm, conversation_id=conversation_id)
    ...
    assert agent.run_turn('start unrelated work') == 'An approved plan is waiting to continue. Use /approve to resume it or /reject to cancel it before starting a new turn.'
    assert llm.requests == []
...
def test_restored_approved_plan_can_be_cancelled_without_resuming(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    conversation_id = 'conversation-cancel-resumed-plan'
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    plan = Plan(summary='Continue approved work.', steps=[PlanStep(id='1', title='Completed mutation', description='Already done.'), PlanStep(id='2', title='Remaining mutation', description='Continue once.', depends_on=['1'])])
    ...
    assert message == 'Approved plan cancelled. No further steps will run; any work already completed was not rolled back.'
    assert llm.requests == []
...
def test_create_agent_does_not_resume_older_approved_plan(monkeypatch, tmp_path):
    ...
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    old_plan = Plan(summary='Old approved work.', steps=[PlanStep(id='1', title='Old work', description='Do old work.')])
    old_turn = TurnState(user_message='old request', turn_id='turn-old', started_at='2026-01-01T00:00:00+00:00')
    old_turn.wait_for_plan_approval(old_plan)
    old_turn.approve_plan()
    store.save_turn_snapshot(conversation_id, old_turn.to_dict())
    latest_turn = TurnState(user_message='new request', turn_id='turn-new', started_at='2026-01-02T00:00:00+00:00')
    latest_turn.complete('new request complete')
    store.save_turn_snapshot(conversation_id, latest_turn.to_dict())
    agent = create_agent(config, lambda _config: FakeLLMClient(), conversation_id=conversation_id)
    assert agent.has_resumable_plan() is False
    assert agent.state.active_plan is None

@pytest.mark.parametrize('response_recorded', [False, True])
def test_restart_blocks_uncheckpointed_hosted_mcp_request(monkeypatch, tmp_path, response_recorded):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    conversation_id = f'conversation-hosted-mcp-{response_recorded}'
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    plan = Plan(summary='Perform a remote mutation.', steps=[PlanStep(id='1', title='Mutate remote state', description='Run the hosted operation once.')])
    ...
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    store.save_model_request(conversation_id, {'turn_id': turn.turn_id, 'request_index': 2, 'hosted_mcp_enabled': True, 'hosted_mcp_server_labels': ['remote']})
    if response_recorded:
        store.save_model_response(conversation_id, {'turn_id': turn.turn_id, 'request_index': 2, 'content': 'provider response arrived before checkpoint'})
    assert len(store.load_uncheckpointed_hosted_mcp_requests(conversation_id, turn.turn_id, checkpointed_request_count=1)) == 1
    assert store.load_uncheckpointed_hosted_mcp_requests(conversation_id, turn.turn_id, checkpointed_request_count=2) == []
    llm = FakeLLMClient()
    agent = create_agent(config, lambda _config: llm, conversation_id=conversation_id)
...
def test_restart_terminalizes_in_progress_turn_with_blocked_plan(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    conversation_id = 'conversation-blocked-plan-checkpoint'
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    plan = Plan(summary='Run one fallible step.', steps=[PlanStep(id='1', title='Run mutation', description='Run once.')])
    ...
    restored_turn = agent.state.turns[-1]
    assert restored_turn.status == 'blocked'
    assert restored_turn.final_answer == 'Plan step blocked: Run mutation. Retry limit exhausted.'
    assert agent.has_resumable_plan() is False
    assert any((message.content == restored_turn.final_answer for message in store.list_messages(conversation_id)))

def test_create_agent_reconciles_legacy_terminal_messages_without_replay(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    cases = [('final', 'completed', 'legacy final answer', 'active'), ('plan_rejected', 'plan_rejected', 'Plan rejected. No tools were run.', 'plan_rejected'), ('failed', 'failed', 'legacy turn failed', 'failed')]
    for index, (message_kind, expected_status, content, conversation_status) in enumerate(cases):
        ...
        store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
        plan = Plan(summary=f'Legacy {message_kind} plan.', steps=[PlanStep(id='1', title='Legacy work', description='Complete legacy work once.')])
        turn = TurnState(user_message=f'legacy request {index}', turn_id=turn_id)
        turn.wait_for_plan_approval(plan)
        if message_kind != 'plan_rejected':
            turn.approve_plan()
            plan.steps[0].mark('completed' if message_kind == 'final' else 'in_progress')
        store.save_message(conversation_id, turn_id=turn_id, role='user', content=turn.user_message, message_key=f'{turn_id}:user')
        store.save_turn_snapshot(conversation_id, turn.to_dict())
        store.save_message(conversation_id, turn_id=turn_id, role='assistant', content=content, message_key=f'{turn_id}:assistant:{message_kind}')
        if conversation_status != 'active':
            store.update_conversation_status(conversation_id, conversation_status)
        llm = FakeLLMClient()
        agent = create_agent(config, lambda _config, client=llm: client, conversation_id=conversation_id)
        restored_turn = agent.state.turns[-1]
...
def test_tool_intent_is_durable_before_callable_runs(monkeypatch, tmp_path):
    ...
    def inspect_intent(_arguments):
        with sqlite3.connect(config.store_path) as conn:
            ...
            tool_call = conn.execute('SELECT success, ended_at FROM conversation_tool_calls').fetchone()
            turn_row = conn.execute('SELECT turn_json FROM conversation_turns').fetchone()
        turn_payload = json.loads(turn_row['turn_json'])
        observed_intents.append((tool_call['success'], tool_call['ended_at'], turn_payload['tool_calls'][-1]['success'], turn_payload['tool_calls'][-1]['ended_at']))
        return ToolResult('inspect_intent', True, 'intent was durable')
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'inspect_intent', 'arguments_json': '{}'}), json.dumps({'type': 'final_answer', 'content': 'done'})])
    agent = create_agent(config, lambda _config: llm, tool_specs=[Tool(name='inspect_intent', description='Inspect the durable tool intent.', args_schema={'type': 'object', 'properties': {}}, callable=inspect_intent)])
    assert agent.run_turn('inspect the intent checkpoint') == 'done'
    assert observed_intents == [(None, None, None, None)]
    assert SQLiteSessionStore(config.store_path).load_tool_calls_without_observations(agent.state.conversation_id, agent.state.turns[-1].turn_id) == []
...
def test_restart_blocks_unresolved_non_plan_tool_intent(monkeypatch, tmp_path):
    ...
    turn.tool_calls.append(ToolCallRecord(tool_name='external_mutation', arguments={'value': 'once'}, iteration=1))
    store.save_message(conversation_id, turn_id=turn.turn_id, role='user', content=turn.user_message, message_key=f'{turn.turn_id}:user')
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    agent = create_agent(config, lambda _config: FakeLLMClient(), conversation_id=conversation_id)
    restored_turn = agent.state.turns[-1]
...
def test_unresolved_tool_recovery_persists_status_and_message_atomically(monkeypatch, tmp_path):
    ...
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    monkeypatch.setattr(store, 'save_turn_snapshot', lambda *_args, **_kwargs: pytest.fail('recovery must not save the snapshot separately'))
    monkeypatch.setattr(store, 'save_message', lambda *_args, **_kwargs: pytest.fail('recovery must not save the message separately'))
    block_unresolved_tool_intent(store, conversation_id, turn, [{'tool_name': 'external_mutation', 'iteration': 1}])
    restored_turn = store.load_turns(conversation_id)[0]
    assert restored_turn.status == 'blocked'
    assert any((message.content == restored_turn.final_answer for message in store.list_messages(conversation_id)))

def test_restart_blocks_completed_call_without_observation_missing_from_legacy_snapshot(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    ...
    completed_call = ToolCallRecord(tool_name='first_mutation', arguments={}, iteration=1)
    completed_call.finish(ToolResult('first_mutation', True, 'done'))
    turn.tool_call_count = 2
    turn.tool_calls.append(completed_call)
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    second_call = ToolCallRecord(tool_name='second_mutation', arguments={}, iteration=2)
    second_call.finish(ToolResult('second_mutation', True, 'mutation completed'))
    store.save_tool_call(conversation_id, {**second_call.to_dict(), 'turn_id': turn.turn_id})
    with sqlite3.connect(config.store_path) as conn:
        assert conn.execute('SELECT success FROM conversation_tool_calls WHERE iteration = 2').fetchone()[0] == 1
    agent = create_agent(config, lambda _config: FakeLLMClient(), conversation_id=conversation_id)
    restored_turn = agent.state.turns[-1]
    assert restored_turn.status == 'blocked'
    assert 'second_mutation (iteration 2)' in (restored_turn.final_answer or '')

def test_restart_blocks_unresolved_tool_intent_even_if_call_row_completed(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    conversation_id = 'conversation-unresolved-intent'
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    plan = Plan(summary='Perform one external mutation.', steps=[PlanStep(id='1', title='Mutate external state', description='Run the mutation exactly once.')])
    turn = TurnState(user_message='mutate once', turn_id='turn-unresolved')
    turn.wait_for_plan_approval(plan)
    turn.approve_plan()
    plan.steps[0].mark('in_progress')
    record = ToolCallRecord(tool_name='external_mutation', arguments={'value': 'once'}, iteration=1, plan_step_id='1')
    turn.tool_call_count = 1
    turn.tool_calls.append(record)
    store.save_message(conversation_id, turn_id=turn.turn_id, role='user', content=turn.user_message, message_key=f'{turn.turn_id}:user')
    store.save_tool_call(conversation_id, {**record.to_dict(), 'turn_id': turn.turn_id, 'turn': turn.to_dict()})
    completed_call = record.to_dict()
    completed_call.update({'turn_id': turn.turn_id, 'ended_at': '2026-01-01T00:00:01+00:00', 'success': True})
    store.save_tool_call(conversation_id, completed_call)
    with sqlite3.connect(config.store_path) as conn:
        assert conn.execute('SELECT success FROM conversation_tool_calls').fetchone()[0] == 1
    mutations = []
    agent = create_agent(config, lambda _config: FakeLLMClient(), conversation_id=conversation_id, tool_specs=[Tool(name='external_mutation', description='Mutate external state.', args_schema={'type': 'object', 'properties': {'value': {'type': 'string'}}}, callable=lambda arguments: mutations.append(arguments) or ToolResult('external_mutation', True, 'mutated'))])
    restored_turn = agent.state.turns[-1]
    assert restored_turn.status == 'blocked'
    assert restored_turn.active_plan is not None
    ...
    assert mutations == []
    assert SQLiteSessionStore(config.store_path).get_conversation(conversation_id).status == 'blocked'
    assert any(('will not replay it automatically' in message.content for message in SQLiteSessionStore(config.store_path).list_messages(conversation_id)))
...
def test_create_agent_resumes_conversation_summary_without_covered_raw_messages(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'first answer'}), json.dumps({'objective': 'Continue the compaction approach.', 'decisions': ['First turn established the compaction approach.']}), json.dumps({'type': 'final_answer', 'content': 'second answer'})])
    first_agent = create_agent(config, lambda _config: llm)
    first_agent.run_turn('old context ' + 'x' * 12000)
    first_agent._model_transport.context_budget = ContextBudget(max_prompt_tokens=8000, response_reserve_tokens=0)
    ...
    summary = SQLiteSessionStore(config.store_path).load_latest_summary(first_agent.state.conversation_id)
    assert summary is not None
    assert summary.metadata['checkpoint_v1']['version'] == 1
    assert summary.metadata['checkpoint_v1']['decisions'] == ['First turn established the compaction approach.']

def test_create_agent_resumes_summary_across_prompt_excluded_display_message(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    store = SQLiteSessionStore(config.store_path)
    conversation_id = 'conversation-summary-plan-display'
    store.create_conversation(conversation_id, provider=config.llm_provider, model=config.model)
    store.save_message(conversation_id, role='user', content='OLD_COVERED_QUESTION', message_key='covered-user')
    store.save_message(conversation_id, role='assistant', content='Plan display that must remain user-visible only.', message_key='plan-display', metadata={'prompt_excluded': True})
    store.save_message(conversation_id, role='assistant', content='covered answer', message_key='covered-assistant')
    store.save_message(conversation_id, role='user', content='uncovered question', message_key='uncovered-user')
    store.save_conversation_summary(conversation_id, content='The covered exchange established durable context.', source_message_count=2)
    llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'used summary'})])
    agent = create_agent(config, lambda _config: llm, conversation_id=conversation_id)
    assert agent.memory.recent() == [{'role': 'user', 'content': 'uncovered question'}]
...
def test_create_agent_registers_mcp_bridge_tools_for_local_provider(monkeypatch, tmp_path):
    ...
    (tmp_path / '.chulk' / 'mcp.json').write_text(json.dumps({'servers': [{'label': 'docs', 'server_url': 'https://mcp.example.com', 'allowed_tools': ['search_docs']}]}), encoding='utf-8')
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    ...
    def fake_create_bridge_tools(servers):
        ...
        return [Tool(name='mcp_docs_search_docs', description='Bridge docs search.', args_schema={'type': 'object', 'properties': {}, 'additionalProperties': False}, callable=lambda _arguments: ToolResult('mcp_docs_search_docs', True, 'ok'), requires_confirmation=True)]
    ...
    assert agent.catalog.active_registry.get('mcp_docs_search_docs').requires_confirmation is True
    assert [event['type'] for event in events[:4]] == ['session_started', 'mcp_config_loaded', 'mcp_tool_discovery_completed', 'turn_started']
...
def test_agent_persists_model_tool_observation_and_final_answer(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    llm = FakeLLMClient([json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': json.dumps({'expression': '2 + 2'})}), json.dumps({'type': 'final_answer', 'content': 'The result is 4.'})])
    llm.provider = 'openai'
    ...
    with sqlite3.connect(config.store_path) as conn:
        ...
        model_request = conn.execute('SELECT request_json, usage_json, cost_json FROM conversation_model_requests ORDER BY request_index LIMIT 1').fetchone()
...
def test_create_agent_resumes_pending_plan_and_approves(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    plan_response = json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps({'summary': 'Make a small change.', 'steps': [{'id': '1', 'title': 'Update code', 'description': 'Implement the requested change.', 'status': 'pending'}]})})
    ...
    stored_plan_messages = session_store.load_recent_messages(first_agent.state.conversation_id, limit=10)
    assert stored_plan_messages == [{'role': 'user', 'content': 'plan a change'}]
    visible_history = TerminalUI(color_enabled=False).history(session_store.list_messages(first_agent.state.conversation_id, limit=10))
    assert 'Make a small change.' in visible_history
    assert any(('Use /approve to execute this plan' in message.content for message in session_store.list_messages(first_agent.state.conversation_id, limit=10)))
    resumed_llm = FakeLLMClient([json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The small change was completed.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'approved work complete'})])
    resumed_agent = create_agent(config, lambda _config: resumed_llm, conversation_id=first_agent.state.conversation_id)
...
def test_plan_display_is_excluded_from_live_execution_prompt(monkeypatch, tmp_path):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    config = load_config()
    llm = FakeLLMClient([json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps({'summary': 'Keep display text out of model history.', 'steps': [{'id': '1', 'title': 'Finish work', 'description': 'Complete the planned work.', 'status': 'pending'}]})}), json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The work completed.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'done'})])
    agent = create_agent(config, lambda _config: llm)
    ...
    assert not any(('Use /approve to execute this plan' in message['content'] for request in execution_requests for message in request))
...
def test_resumed_low_history_limit_keeps_turn_anchor_and_tool_result(monkeypatch, tmp_path):
    ...
    turn.wait_for_plan_approval(Plan(summary='Finish the inspected work.', steps=[PlanStep(id='1', title='Finish work', description='Complete the work using the inspection result.')]))
    store.save_turn_snapshot(conversation_id, turn.to_dict())
    action_context = '<executed_tool_action>\n{"tool_name":"lookup","arguments_json":"{}"}\n</executed_tool_action>'
    store.save_message(conversation_id, role='user', content=turn.user_message, turn_id=turn_id, message_key=f'{turn_id}:user')
    store.save_message(conversation_id, role='assistant', content=action_context, turn_id=turn_id, message_key=f'{turn_id}:tool_action:1')
    store.save_message(conversation_id, role='observation', content='Lookup completed.', turn_id=turn_id, message_key=f'{turn_id}:observation:1')
    resumed_llm = FakeLLMClient([json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The inspected work was completed.', 'reason': None})}), json.dumps({'type': 'final_answer', 'content': 'Work complete.'})])
    resumed_agent = create_agent(config, lambda _config: resumed_llm, conversation_id=conversation_id)
    assert resumed_agent.approve_plan() == 'Work complete.'
    resumed_history = resumed_llm.requests[0][1:]
    assert resumed_history == [{'role': 'user', 'content': turn.user_message}, {'role': 'assistant', 'content': action_context}, {'role': 'observation', 'content': 'Lookup completed.'}]

def test_cli_lists_resumes_and_shows_history(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    first_exit = main(['--once', 'first persisted message'], llm_client_factory=lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'first persisted answer'})]))
    ...
    second_exit = main([], input_func=lambda _prompt: next(inputs), llm_client_factory=lambda _config: interactive_llm)
...
def test_cli_resume_flag_starts_in_existing_session(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    first_exit = main(['--once', 'remember startup resume'], llm_client_factory=lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'remembered'})]))
    store = SQLiteSessionStore(tmp_path / 'chulk' / 'store.sqlite')
    session_id = store.list_conversations()[0].id
    resumed_llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'resumed directly'})])
    inputs = iter(['continue directly', '/q'])
    second_exit = main(['--resume', session_id[:8]], input_func=lambda _prompt: next(inputs), llm_client_factory=lambda _config: resumed_llm)
    output = capsys.readouterr().out
    assert first_exit == 0
    assert second_exit == 0
...
def test_cli_continue_resumes_latest_nonempty_session(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    main(['--once', 'latest session marker'], llm_client_factory=lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'stored'})]))
    continued_llm = FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'continued latest'})])
    inputs = iter(['continue', '/q'])
    exit_code = main(['--continue'], input_func=lambda _prompt: next(inputs), llm_client_factory=lambda _config: continued_llm)
    output = capsys.readouterr().out
    assert exit_code == 0
...
def test_cli_resume_reloads_arrow_key_prompt_history(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv('CHULK_PROJECT_ROOT', str(tmp_path))
    first_exit = main(['--once', 'persisted prompt for arrows'], llm_client_factory=lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'persisted answer'})]))
    store = SQLiteSessionStore(tmp_path / 'chulk' / 'store.sqlite')
    session_id = store.list_conversations()[0].id
    ...
    second_exit = main([], input_func=lambda _prompt: next(inputs), llm_client_factory=lambda _config: FakeLLMClient())
    capsys.readouterr()
    assert first_exit == 0
    assert second_exit == 0
...
def test_cli_context_command_shows_latest_prompt_report(monkeypatch, tmp_path, capsys):
    ...
    exit_code = main([], input_func=lambda _prompt: next(inputs), llm_client_factory=lambda _config: FakeLLMClient([json.dumps({'type': 'final_answer', 'content': 'context ready'})]))
    output = capsys.readouterr().out
    assert exit_code == 0
...
def test_session_search_index_backfills_and_tracks_only_eligible_messages(tmp_path: Path) -> None:
    path = tmp_path / 'store.sqlite'
    store = SQLiteSessionStore(path)
    store.create_conversation('conversation-1', provider='test', model='mock')
    store.save_message('conversation-1', role='user', content='eligible user evidence', message_key='eligible-user')
    store.save_message('conversation-1', role='assistant', content='eligible assistant evidence', message_key='eligible-assistant')
    store.save_message('conversation-1', role='assistant', content='hidden internal prompt', message_key='internal', metadata={'internal': True})
    store.save_message('conversation-1', role='user', content='sensitive customer token', message_key='sensitive', metadata={'sensitive': True})
    store.save_message('conversation-1', role='observation', content='raw tool output', message_key='observation')
    if not store.fts_enabled:
        pytest.skip('SQLite build does not provide FTS5')
    with store._connect() as conn:
        indexed = conn.execute('SELECT content FROM session_messages_fts ORDER BY rowid').fetchall()
    assert [row['content'] for row in indexed] == ['eligible user evidence', 'eligible assistant evidence']
    with store._connect() as conn:
        conn.execute('DELETE FROM session_messages_fts')
    assert store.rebuild_search_index() == 2
    with store._connect() as conn:
        rebuilt = conn.execute('SELECT content FROM session_messages_fts ORDER BY rowid').fetchall()
    assert [row['content'] for row in rebuilt] == ['eligible user evidence', 'eligible assistant evidence']

def test_session_search_index_backfills_messages_written_before_store_open(tmp_path: Path) -> None:
    path = tmp_path / 'store.sqlite'
...
def test_async_recorder_retains_copied_events_in_order_after_failed_flush(failure):
    ...
    payload = {'summary': 'first', 'source_message_count': '2', 'summarized_message_count': '1', 'checkpoint': {'decisions': ['original']}}
...
def test_session_recorders_preserve_malformed_payload_coercion(tmp_path, recorder_type):
    ...
    recorder.callback(TraceEvent.TOOL_OBSERVATION, {'turn_id': 123, 'observation_index': True, 'tool_name': None, 'observation': 42, 'output_metadata': [], 'tool_action_context': 9, 'turn': []})
    recorder.callback(TraceEvent.CONTEXT_SUMMARY_CREATED, {'summary': 42, 'source_message_count': '2', 'summarized_message_count': None, 'checkpoint': [], 'fallback': 'yes'})
    if isinstance(recorder, AsyncSessionRecorder):
        asyncio.run(recorder.flush())
    assert store.max_observation_index('conversation', 'turn') == 1
    assert store.load_recent_messages('conversation', limit=10)[-1] == {'role': 'observation', 'content': '42'}
    summary = store.load_latest_summary('conversation')
    assert summary.content == '42'
    assert summary.metadata == {'event': TraceEvent.CONTEXT_SUMMARY_CREATED, 'turn_id': 'turn', 'summarized_message_count': 0, 'fallback': True, 'checkpoint_v1': {}, 'source_message_ordinal': 2}
