...
from chulk.tracing import ArtifactAccessError, JSONLTraceLogger, Trace, TraceArtifactStore
...
def test_artifact_reads_are_bounded_and_support_slice_head_tail(tmp_path) -> None:
    ...
    reference = logger.write_artifact('tool-output', 'HEAD-' + 'middle-' * 40 + 'TAIL')
    ...
    sliced = logger.read_artifact(artifact_id, mode='slice', offset=5, max_bytes=14)
    head_tail = logger.read_artifact(artifact_id, mode='head_tail', max_bytes=20)
...
def test_artifact_reader_rejects_oversized_and_hash_mismatch(tmp_path) -> None:
    ...
    with pytest.raises(ArtifactAccessError, match='integrity-read bound'):
        logger.artifact_store.read(oversized['artifact_id'], max_artifact_bytes=64)
...
def test_trace_manifest_reports_artifact_integrity_without_embedding_content(tmp_path) -> None:
    traces_dir = tmp_path / 'traces'
    ...
    assert summary['artifact_total_bytes'] == len('valid sensitive content') + len('same-size secret')
...
def test_artifact_inventory_reports_missing_and_unrecorded_files(tmp_path) -> None:
    ...
    (logger.artifacts_dir / f'{unrecorded_id}.txt').write_text('orphan', encoding='utf-8')
...
def test_artifact_reader_is_opt_in_as_tool_and_available_as_host_api(tmp_path) -> None:
    client = ScriptedLLMClient([{'type': 'final_answer', 'content': 'done'}])
    with Agent(config=AgentConfig(project_root=tmp_path), llm=client, tools=[Tools.read_trace_artifact], skills=[]) as agent:
        reference = agent.runtime.trace_logger.write_artifact('host-evidence', 'bounded evidence')
        artifact_id = reference['artifact_id']
        host_view = agent.read_artifact(artifact_id, mode='slice', max_bytes=7)
        tool_result = agent.tool_registry.run('read_trace_artifact', {'artifact_id': artifact_id, 'mode': 'tail', 'max_bytes': 8})
    assert host_view['content'] == 'bounded'
    assert tool_result.success is True
    assert tool_result.value['content'] == 'evidence'
    default_agent = Agent(config=AgentConfig(project_root=tmp_path / 'default'), llm=ScriptedLLMClient([{'type': 'final_answer', 'content': 'done'}]), tools=[], skills=[])
    try:
        assert 'read_trace_artifact' not in {tool.name for tool in default_agent.tool_registry.list_tools()}
    finally:
        default_agent.close()
