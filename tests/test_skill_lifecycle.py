...
import stat
import pytest
from chulk.skills import LearningProposalKind, LearningProposalStatus, SQLiteSkillLifecycleStore, SkillApprovalError, SkillConflictError, SkillLifecycleManager, SkillLifecycleStatus, SkillLockFile, load_skill_package, proposal_diff

def skill_content(*, name: str='review', version: str='1.0.0', body: str='# Review\n\nReview carefully.\n', references: tuple[str, ...]=()) -> str:
    reference_line = f"references: [{', '.join(references)}]\n" if references else ''
    return f'---\nschema_version: 1\nname: {name}\nversion: {version}\ndescription: Review workflow.\nrequired_capabilities: []\n{reference_line}source: project\ntrust: reviewed\n---\n{body}'

def write_skill(skills_dir: Path, content: str, *, name: str='review') -> Path:
...
def manager(tmp_path):
    store = SQLiteSkillLifecycleStore(tmp_path / 'store.sqlite', profile_id='default')
    project_skills = tmp_path / 'project' / '.chulk' / 'skills'
    profile_skills = tmp_path / 'profile-runtime' / 'skills'
    lifecycle = SkillLifecycleManager(store, project_skills_dir=project_skills, profile_skills_dir=profile_skills)
...
def test_register_existing_writes_distinct_credential_free_locks(tmp_path):
    ...
    write_skill(profile_skills, skill_content(name='personal', body='# Personal\n'), name='personal')
    ...
    assert 'credential' not in lifecycle.project_lock.path.read_text(encoding='utf-8')
    if os.name == 'posix':
        assert stat.S_IMODE(lifecycle.project_lock.path.stat().st_mode) == 420
        assert stat.S_IMODE(lifecycle.profile_lock.path.stat().st_mode) == 384
    assert {record.name for record in store.list_skills()} == {'personal', 'review'}
...
def test_host_approval_creates_validated_skill_and_updates_lock(tmp_path):
    ...
    proposal = store.create_proposal(kind=LearningProposalKind.SKILL_CREATE, target_name='review', rationale='The workflow succeeded repeatedly.', content=content, required_capabilities=(), verification_steps=('validate the manifest',), metadata={'scope': 'project'})
...
def test_patch_requires_reviewed_base_and_preserves_package_resources(tmp_path):
    ...
    changed = skill_content(version='1.1.0', body='# Review\n\nReview with tests.\n', references=('guide.md',))
    proposal = store.create_proposal(kind='skill_patch', target_name='review', rationale='Verified improvement.', content=changed, diff=proposal_diff(before=original, after=changed, name='review'), metadata={'scope': 'project', 'base_digest': base.digest})
    approved = lifecycle.approve(proposal.id, approved_by='operator')
    assert approved.status is LearningProposalStatus.APPROVED
...
def test_patch_conflict_leaves_package_and_proposal_unchanged(tmp_path):
    ...
    proposal = store.create_proposal(kind='skill_patch', target_name='review', rationale='Reviewed against the first version.', content=changed, diff=proposal_diff(before=original, after=changed, name='review'), metadata={'scope': 'project', 'base_digest': base.digest})
    ...
    assert store.get_proposal(proposal.id).status is LearningProposalStatus.PENDING

def test_failed_database_approval_restores_package_and_lock(tmp_path, monkeypatch):
    store, lifecycle, project_skills, _profile_skills = manager(tmp_path)
    original = skill_content()
    path = write_skill(project_skills, original)
    lifecycle.register_existing(scope='project')
    ...
    changed = skill_content(version='1.1.0')
    proposal = store.create_proposal(kind='skill_patch', target_name='review', rationale='Reviewed update.', content=changed, diff=proposal_diff(before=original, after=changed, name='review'), metadata={'scope': 'project', 'base_digest': base.digest})
...
def test_archive_and_rollback_restore_exact_package_snapshot(tmp_path):
    ...
    proposal = store.create_proposal(kind='skill_archive', target_name='review', rationale='Workflow is obsolete.', diff=proposal_diff(before=content, after=None, name='review'), metadata={'scope': 'project', 'base_digest': current.digest})
    approved = lifecycle.approve(proposal.id, approved_by='operator')
    assert approved.status is LearningProposalStatus.APPROVED
    assert approved.applied_revision_id == current.active_revision_id
    assert not path.parent.exists()
    assert lifecycle.project_lock.get('review').status is SkillLifecycleStatus.ARCHIVED
    assert lifecycle.verify_locks() == ()
    restored = lifecycle.rollback(current.active_revision_id, scope='project', approved_by='operator')
...
def test_rollback_rejects_revision_from_another_scope(tmp_path):
    store, lifecycle, project_skills, _profile_skills = manager(tmp_path)
    write_skill(project_skills, skill_content())
    revision_id = lifecycle.register_existing(scope='project')[0].active_revision_id
    with pytest.raises(SkillConflictError, match='different lifecycle scope'):
        lifecycle.rollback(revision_id, scope='profile', approved_by='operator')

def test_proposal_with_embedded_secret_is_never_applied(tmp_path):
    store, lifecycle, project_skills, _profile_skills = manager(tmp_path)
    content = skill_content(body='# Review\n\nOPENAI_API_KEY=sk-abcdefghijklmnop\n')
    proposal = store.create_proposal(kind='skill_create', target_name='review', rationale='Proposed workflow.', content=content, metadata={'scope': 'project'})
...
def test_registration_rejects_symlinked_skill_package(tmp_path):
    ...
    try:
        (project_skills / 'review').symlink_to(outside / 'review', target_is_directory=True)
    except OSError:
        pytest.skip('symlinks are unavailable')
...
def test_registration_rejects_partial_pending_create(tmp_path):
    store, lifecycle, project_skills, _profile_skills = manager(tmp_path)
    content = skill_content()
    store.create_proposal(kind='skill_create', target_name='review', rationale='Pending host review.', content=content, metadata={'scope': 'project'})
...
def test_registration_rejects_reappearing_archived_package(tmp_path):
    ...
    lifecycle.register_existing(scope='project')
    current = store.get_skill('review')
    proposal = store.create_proposal(kind='skill_archive', target_name='review', rationale='No longer needed.', diff=proposal_diff(before=content, after=None, name='review'), metadata={'scope': 'project', 'base_digest': current.digest})
    lifecycle.approve(proposal.id, approved_by='operator')
    write_skill(project_skills, content)
...
def test_verify_locks_reports_database_mismatch(tmp_path):
    ...
    assert lifecycle.verify_locks() == ('project:review:database_digest_mismatch',)
...
def test_registration_rejects_tampered_lock_metadata(tmp_path):
    ...
    with pytest.raises(SkillConflictError, match='governed state'):
        lifecycle.register_existing(scope='project')
    assert lifecycle.verify_locks() == ('project:review:lock_metadata_mismatch',)

def test_lock_rejects_path_traversal_entry(tmp_path):
    path = tmp_path / 'skills.lock'
    path.write_text(json.dumps({'schema_version': 1, 'scope': 'project', 'skills': {'../escape': {'name': '../escape', 'version': '1.0.0', 'digest': f"sha256:{'a' * 64}", 'source': 'project', 'trust': 'reviewed', 'status': 'active', 'revision_id': 'revision', 'required_tools': [], 'required_capabilities': [], 'installed_at': '2026-01-01T00:00:00+00:00', 'review_state': 'approved'}}}), encoding='utf-8')
