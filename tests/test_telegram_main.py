...
def test_media_configuration_failure_returns_sanitized_startup_exit(tmp_path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setattr(telegram_main, 'load_telegram_config', lambda **_kwargs: TelegramConfig(bot_token='fake', allowed_user_ids=frozenset({7})))
