...
from chulk.server import ConversationMessageRequest, PermissionDecisionRequest, PublicEventCursorExpiredError, PublicEventJournal

def _event(conversation_id: str, text: str, *, profile_id: str='work') -> AgentEvent:
    return AgentEvent(name='model.delta', profile_id=profile_id, conversation_id=conversation_id, turn_id='turn-1', payload=ModelDeltaPayload(text))
...
def test_public_event_journal_enforces_profile_ownership_and_retention(tmp_path) -> None:
    journal = PublicEventJournal(tmp_path / 'store.sqlite', profile_id='work', retention=2)
    first = journal.append(_event('conversation-1', 'one'))
...
def test_public_event_journal_rejects_oversized_events(tmp_path) -> None:
    journal = PublicEventJournal(tmp_path / 'store.sqlite', profile_id='work', max_event_bytes=250)
    with pytest.raises(ValueError, match='payload limit'):
        journal.append(_event('conversation-1', 'x' * 1000))

def test_server_request_models_reject_invalid_or_unbounded_inputs() -> None:
    request = ConversationMessageRequest.from_dict({'message': ' hello ', 'mode': 'plan', 'idempotency_key': 'key-1'})
    decision = PermissionDecisionRequest.from_dict({'decision': 'allow', 'idempotency_key': 'decision-1'})
