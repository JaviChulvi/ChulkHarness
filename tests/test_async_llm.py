...
def test_public_llm_exports_every_provider_wrapper() -> None:
    assert {'BindableLLM', 'OpenAIProvider', 'DeepSeekProvider', 'LocalProvider', 'OpenAICompatibleProvider', 'OpenRouterProvider', 'AnthropicProvider', 'BedrockProvider', 'GeminiProvider'} <= set(public_llm.__all__)
...
class _AsyncActionScript(LLMClient):
    ...
    async def _acomplete_action_response_once(self, messages: list[dict[str, str]], **kwargs: object) -> LLMResponse:
        self.async_messages.append(messages)
        content = self.responses.pop(0)
        usage = LLMUsage(input_tokens=3, output_tokens=2, total_tokens=5, estimated=False, source='provider')
        return LLMResponse(content=content, usage=usage, provider=self.provider, model=self.model)
...
class _SyncActionOnly(LLMClient):
    ...
    def complete_action(self, messages: list[dict[str, str]], **kwargs: object) -> LLMActionResult:
        self.calls += 1
        return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='sync compatibility'), raw_response=_final_answer('sync compatibility'))

def _final_answer(content: str) -> str:
    return json.dumps({'type': 'final_answer', 'content': content})

@pytest.mark.parametrize(('exception_name', 'expected_code'), [('ReadTimeout', 'timeout'), ('ConnectTimeout', 'timeout'), ('ConnectError', 'connection_error'), ('RemoteProtocolError', 'connection_error')])
def test_http_client_transient_errors_are_classified_without_sdk_imports(exception_name: str, expected_code: str) -> None:
...
async def test_openai_malformed_hosted_mcp_approval_advances_async_fallback() -> None:
    ...
    async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id=None, server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))])])
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=async_endpoint))
    ...
    result = await fallback.acomplete_action(MESSAGES, tools=[], hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
...
async def test_openai_mcp_approval_continuation_keeps_async_planning_contract_and_fails_closed_on_text() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))]), SimpleNamespace(id='resp_2', output_text='phase-illegal final text', usage=None, output=[])])
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=async_endpoint))
    secondary = _AsyncActionScript([_final_answer('must not run')])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')
    with pytest.raises(LLMError) as raised:
        await fallback.acomplete_action([{'role': 'system', 'content': 'Finish with a plan step update.'}, {'role': 'user', 'content': 'Search the docs and update the step.'}], planning_tools=PlanningToolAvailability(update_plan_step=True), hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
...
async def test_openai_hosted_mcp_call_without_final_action_does_not_replay_async_fallback() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_call', id='call_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}), output='created', status='completed')])])
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=async_endpoint))
    secondary = _AsyncActionScript([_final_answer('must not run')])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='write_api', transport='streamable_http', server_url='https://mcp.example.com')
    with pytest.raises(LLMError) as raised:
        await fallback.acomplete_action(MESSAGES, tools=[], hosted_mcp_servers=[server])
    assert raised.value.code == 'action_shape_error'
    assert raised.value.retryable is False
    assert raised.value.fallback_eligible is False
    assert len(fallback.last_attempts) == 1
...
async def test_openai_timeout_after_approved_mcp_call_does_not_replay_async_fallback() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}))]), TimeoutError('response timed out after approval')])
    async_client = _AsyncClientWithOptions(async_endpoint)
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=async_client)
    secondary = _AsyncActionScript([_final_answer('must not run')])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='write_api', transport='streamable_http', server_url='https://mcp.example.com')
    with pytest.raises(LLMError) as raised:
        await fallback.acomplete_action(MESSAGES, tools=[], hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
...
async def test_openai_pre_execution_mcp_rejection_can_advance_async_fallback() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([LLMError('hosted MCP tools are unsupported', provider='openai', model='openai-test', code='unsupported_feature', fallback_eligible=True)])
    async_client = _AsyncClientWithOptions(async_endpoint)
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=async_client)
    secondary = _AsyncActionScript([_final_answer('fallback ok')])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com', approval='never')
    result = await FallbackChain([primary, secondary]).acomplete_action(MESSAGES, tools=[], hosted_mcp_servers=[server])
    assert result.action == FinalAnswerAction(type='final_answer', content='fallback ok')
    assert async_client.retry_options == [{'max_retries': 0}]
    assert len(secondary.async_messages) == 1
    assert sync.calls == 0
...
async def test_openai_mcp_execution_blocks_async_action_repair_and_fallback() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_call', id='call_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}), output='created', status='completed'), SimpleNamespace(type='function_call', name=PLAN_TOOL_NAME, arguments='{}')])])
    async_client = _AsyncClientWithOptions(async_endpoint)
    primary = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=async_client)
    secondary = _AsyncActionScript([_final_answer('must not run')])
    fallback = FallbackChain([primary, secondary])
...
async def test_openai_uses_injected_native_async_responses_client() -> None:
    ...
    client = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=async_endpoint))
...
async def test_shared_chat_transport_uses_injected_native_async_client() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(choices=[SimpleNamespace(message=SimpleNamespace(content='chat async'))], usage=None)])
    client = DeepSeekChatCompletionsClient(model='deepseek-test', api_key='test', client=SimpleNamespace(chat=SimpleNamespace(completions=sync)), async_client=SimpleNamespace(chat=SimpleNamespace(completions=async_endpoint)))
...
async def test_anthropic_uses_injected_native_async_messages_client() -> None:
    sync = _FailingSyncEndpoint()
    async_endpoint = _AsyncEndpoint([SimpleNamespace(content=[SimpleNamespace(type='text', text='anthropic async')], usage=None)])
    client = AnthropicMessagesClient(model='claude-test', client=SimpleNamespace(messages=sync), async_client=SimpleNamespace(messages=async_endpoint))
...
async def test_gemini_uses_injected_native_async_generate_content_client() -> None:
    ...
    client = GeminiGenerateContentClient(model='gemini-test', client=SimpleNamespace(models=sync), async_client=SimpleNamespace(models=async_models))
...
async def test_provider_native_async_action_path_never_calls_sync_transport(provider: str) -> None:
    sync = _FailingSyncEndpoint()
    if provider == 'openai':
        async_endpoint = _AsyncEndpoint([SimpleNamespace(id='resp_1', output_text='openai action', output=[], usage=None)])
        client: LLMClient = OpenAIResponsesClient(model='openai-test', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=async_endpoint))
    elif provider == 'chat':
        async_endpoint = _AsyncEndpoint([SimpleNamespace(choices=[SimpleNamespace(message=SimpleNamespace(content='chat action', tool_calls=[]))], usage=None)])
        client = DeepSeekChatCompletionsClient(model='deepseek-test', client=SimpleNamespace(chat=SimpleNamespace(completions=sync)), async_client=SimpleNamespace(chat=SimpleNamespace(completions=async_endpoint)))
    elif provider == 'anthropic':
        async_endpoint = _AsyncEndpoint([SimpleNamespace(content=[SimpleNamespace(type='text', text='anthropic action')], usage=None)])
        client = AnthropicMessagesClient(model='claude-test', client=SimpleNamespace(messages=sync), async_client=SimpleNamespace(messages=async_endpoint))
    else:
...
async def test_async_agent_and_planned_turn_never_call_sync_llm(tmp_path) -> None:
    ...
    plan_payload = {'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps({'summary': 'Implement native async planning', 'steps': [{'id': '1', 'title': 'Implement', 'description': 'Update the agent action loop to await the native async provider path.', 'status': 'pending', 'acceptance_criteria': ['Async planned turns do not call sync LLM methods.'], 'retry_limit': 0}]}), 'step_update_json': '{}'}
...
async def test_cancelling_native_async_provider_does_not_fallback_or_call_sync() -> None:
    ...
    primary = OpenAIResponsesClient(model='primary', client=SimpleNamespace(responses=sync), async_client=SimpleNamespace(responses=HangingResponses()))
