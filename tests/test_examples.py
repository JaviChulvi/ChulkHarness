...
def _run_example(path: str, *, cwd: Path=ROOT, runtime_dir: Path | None=None, arguments: tuple[str, ...]=()) -> subprocess.CompletedProcess[str]:
    env = dict(os.environ)
    for name in ('OPENAI_API_KEY', 'CHULK_LLM_PROVIDER', 'CHULK_MODEL', 'CHULK_EXAMPLE_MODE'):
        env.pop(name, None)
    ...
    return subprocess.run([sys.executable, str(ROOT / path), *arguments], cwd=cwd, env=env, capture_output=True, text=True, timeout=30, check=False)
...
def test_review_bot_generates_a_normalized_trace_from_its_real_run(tmp_path: Path) -> None:
    ...
    completed = _run_example('examples/repo_review_bot/app.py', cwd=tmp_path, arguments=('--write-sample', str(sample)))
    assert completed.returncode == 0, completed.stderr
...
def test_evaluation_suite_is_credential_free_and_passes_quality_gate(tmp_path: Path) -> None:
...
def test_software_engineer_eval_repairs_and_validates_disposable_project(tmp_path: Path) -> None:
