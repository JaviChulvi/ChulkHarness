...
from chulk.llm.tools import PLAN_STEP_UPDATE_TOOL_NAME, PLAN_TOOL_NAME, PlanningToolAvailability, provider_action_tools
...
def test_build_agent_prompt_reports_named_sections():
    ...
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3)
...
def test_native_agent_prompt_omits_tool_catalog_and_accounts_for_declarations():
    ...
    native_declarations = provider_action_tools([calculator], planning_tools=PlanningToolAvailability())
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=True, native_tool_declarations=native_declarations)
    system_prompt = prompt.messages[0]['content']
    report = prompt.context_report.to_dict()
    tools_section = next((section for section in report['sections'] if section['name'] == 'tools'))
    native_tools_section = next((section for section in report['sections'] if section['name'] == 'native_tools'))
    ...
    assert tools_section['metadata'] == {'tool_names': ['calculator'], 'delivery': 'provider_native', 'schemas_embedded': False}
    assert native_tools_section['item_count'] == 1
    assert native_tools_section['metadata'] == {'tool_names': ['calculator'], 'delivery': 'provider_native', 'schemas_embedded': False}
    ...
    assert report['message_estimated_tokens'] + report['request_overhead_estimated_tokens'] == report['estimated_tokens']
...
def test_json_agent_prompt_embeds_full_tool_catalog_without_native_overhead():
    ...
    calculator = calculator_tool()
    registry.register(calculator)
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=False)
    system_prompt = prompt.messages[0]['content']
    report = prompt.context_report.to_dict()
    tools_section = next((section for section in report['sections'] if section['name'] == 'tools'))
    ...
    assert tools_section['metadata'] == {'tool_names': ['calculator'], 'delivery': 'prompt', 'schemas_embedded': True}
...
def test_native_planning_only_declaration_is_available_without_prompt_schema():
    ...
    native_declarations = provider_action_tools([], planning_tools=PlanningToolAvailability(propose_plan=True))
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3, planning_enabled=True, require_plan=True, native_action_protocol=True, native_tool_declarations=native_declarations)
    system_prompt = prompt.messages[0]['content']
    report = prompt.context_report.to_dict()
    tools_section = next((section for section in report['sections'] if section['name'] == 'tools'))
    native_tools_section = next((section for section in report['sections'] if section['name'] == 'native_tools'))
...
def test_plan_prompt_exposes_only_registered_read_only_tools():
    ...
    memory.add_user_message('plan this')
    registry = ToolRegistry()
    read_tool = Tool(name='inspect_state', description='Inspect state.', args_schema={'type': 'object', 'properties': {}}, callable=lambda _arguments: ToolResult('inspect_state', True, 'ok'), permission_level=ToolPermissionLevel.READ)
    write_tool = Tool(name='change_state', description='Change state.', args_schema={'type': 'object', 'properties': {}}, callable=lambda _arguments: ToolResult('change_state', True, 'ok'), permission_level=ToolPermissionLevel.WRITE)
    registry.register(read_tool)
    registry.register(write_tool)
    json_prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, planning_enabled=True, require_plan=True)
    native_prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, planning_enabled=True, require_plan=True, native_action_protocol=True, native_tool_declarations=provider_action_tools([read_tool, write_tool], planning_tools=PlanningToolAvailability(propose_plan=True)))
    ...
    assert [declaration['name'] for declaration in native_prompt.native_tool_declarations] == ['inspect_state', PLAN_TOOL_NAME]
...
def test_build_agent_prompt_omits_unselected_skill_sections():
    ...
    assert not any((section['name'] == 'available_skills' for section in report['sections']))
...
def test_minimal_prompt_omits_unavailable_sections_and_actions():
    ...
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3)
    system_prompt = prompt.messages[0]['content']
    section_names = [section['name'] for section in prompt.context_report.to_dict()['sections']]
...
def test_plan_prompt_without_tools_is_domain_neutral_and_plan_only():
    ...
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=1, planning_enabled=True, require_plan=True)
    system_prompt = prompt.messages[0]['content']
...
def test_build_agent_prompt_injects_external_context_and_prompt_metadata():
    ...
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3, context_sections=[TurnContextSection(id='src-1', title='Handbook', source='drive://handbook', content='The handbook says onboarding takes three days.')], prompt_profile='polp-search', locale='es-ES')
    system_prompt = prompt.messages[0]['content']
    report = prompt.context_report.to_dict()
...
def test_build_agent_prompt_injects_conversation_summary_section():
    memory = ConversationMemory()
    memory.replace([{'role': 'user', 'content': 'latest question'}], conversation_summary='Earlier work chose prompt compaction and ruled out long-term memory.', summary_message_count=4)
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3)
    system_prompt = prompt.messages[0]['content']
    report = prompt.context_report.to_dict()
...
def test_context_budget_trims_old_observations_and_keeps_latest_user():
    ...
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3, context_budget=ContextBudget(max_prompt_tokens=950, response_reserve_tokens=0))
...
def test_context_budget_trims_complete_old_history_blocks():
    ...
    memory.add_user_message('latest question')
    prompt = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=ToolRegistry(), max_skill_content_chars=1000, max_tool_calls_per_turn=3, context_budget=ContextBudget(max_prompt_tokens=1200, response_reserve_tokens=0))
    payload = json.dumps(prompt.messages)
    report = prompt.context_report.to_dict()
    assert 'latest question' in payload
...
def test_large_native_declaration_and_fallback_reserve_trim_more_history():
    ...
    small_unbounded = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=empty_registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=True, native_tool_declarations=[])
    large_tool = Tool(name='large_context_tool', description='Large native declaration ' + 'd' * 4000, args_schema={'type': 'object', 'properties': {'payload': {'type': 'string', 'description': 'x' * 8000}}, 'required': ['payload'], 'additionalProperties': False}, callable=lambda _arguments: None)
    large_registry = ToolRegistry()
    large_registry.register(large_tool)
    large_declarations = provider_action_tools([large_tool], planning_tools=PlanningToolAvailability())
    large_unbounded = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=large_registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=True, native_tool_declarations=large_declarations)
    ...
    tight_budget = ContextBudget(max_prompt_tokens=small_report['budget_estimated_tokens'], response_reserve_tokens=0)
    small_bounded = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=empty_registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=True, native_tool_declarations=[], context_budget=tight_budget)
    large_bounded = build_agent_prompt(system_prompt='Base prompt.', memory=memory, profile_memories=[], relevant_memories=[], selected_skills=[], tool_registry=large_registry, max_skill_content_chars=1000, max_tool_calls_per_turn=3, native_action_protocol=True, native_tool_declarations=large_declarations, context_budget=tight_budget)
    assert small_bounded.context_report.omitted_message_count == 0
    assert large_bounded.context_report.omitted_message_count > small_bounded.context_report.omitted_message_count
