...
def test_skill_registry_supports_simple_front_matter(tmp_path):
    skills_dir = tmp_path / 'skills'
    write_skill(skills_dir, 'python', '\n'.join(['---', 'name: python-coding', 'description: Python implementation workflow.', 'keywords: [python, pytest, refactor]', 'owner: core', '---', '# Python Coding']))
    registry = SkillRegistry(skills_dir)
    registry.load_metadata()
...
def test_skill_registry_loads_multiple_directories_with_project_override(tmp_path):
    ...
    project_files_path = write_skill(project_skills_dir, 'files', '# Files Skill\n\nProject-specific file workflow.\n')
    registry = SkillRegistry(project_skills_dir, skills_dirs=(bundled_skills_dir(), project_skills_dir))
    registry.load_metadata()
...
def test_bundled_skills_keep_a_distinct_origin():
    registry = SkillRegistry(bundled_skills_dir(), skills_dirs=(bundled_skills_dir(),))
    registry.load_metadata()
