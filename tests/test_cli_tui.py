...
class FakeApp:
    ...
    def __init__(self, source, *, refresh_seconds: float, no_color: bool) -> None:
        self.source = source
...
def test_tui_parser_exposes_safe_connection_options() -> None:
    args = build_parser().parse_args(['tui', '--url', 'http://localhost:9000', '--token-file', 'owner.token', '--refresh-seconds', '4', '--profile', 'operations', '--no-color'])
...
def test_tui_launcher_reads_token_file_without_exposing_raw_token(tmp_path) -> None:
    ...
    errors: list[str] = []
    result = run_tui_command(config=config, profile_id='operations', url='http://127.0.0.1:8765', token_file=token_file, refresh_seconds=3, no_color=True, error_func=errors.append, app_factory=FakeApp)
...
def test_tui_launcher_reports_missing_token_without_creating_it(tmp_path) -> None:
    config = load_config({'CHULK_PROJECT_ROOT': str(tmp_path)})
    errors: list[str] = []
    result = run_tui_command(config=config, profile_id='default', url='http://127.0.0.1:8765', token_file=None, refresh_seconds=2, no_color=False, error_func=errors.append, app_factory=FakeApp)
    assert result == 2
