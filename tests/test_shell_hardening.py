...
from chulk.tools.shell import ShellExecutionDecision, ShellExecutionRequest, run_shell_command

def test_stdout_is_bounded_while_child_is_running(tmp_path: Path) -> None:
    command = _python_command("import os, time; os.write(1, b'HEAD-' + (b'x' * 2000) + b'-TAIL'); time.sleep(30)")
    started_at = time.monotonic()
    result = run_shell_command({'command': command}, tmp_path, stdout_limit_bytes=128, stderr_limit_bytes=64)
...
def test_stderr_has_an_independent_live_byte_limit(tmp_path: Path) -> None:
    ...
    result = run_shell_command({'command': command}, tmp_path, stdout_limit_bytes=512, stderr_limit_bytes=73)
    assert result.error == 'output_limit_exceeded'
...
def test_timeout_kills_group_and_preserves_bounded_partial_output(tmp_path: Path) -> None:
    ...
    result = run_shell_command({'command': command, 'timeout_seconds': 1}, tmp_path, default_timeout_seconds=1, stdout_limit_bytes=64, stderr_limit_bytes=64)
    assert not result.success
...
def test_output_overflow_kills_descendant_processes(tmp_path: Path) -> None:
    ...
    command = f'sleep 30 & echo $! > child.pid; {overflow_command}; wait'
    result = run_shell_command({'command': command}, tmp_path, stdout_limit_bytes=64, stderr_limit_bytes=64)
    child_pid = int((tmp_path / 'child.pid').read_text(encoding='utf-8').strip())
    child_alive = _wait_for_process_exit(child_pid)
    if child_alive:
        os.kill(child_pid, signal.SIGKILL)
...
def test_required_containment_fails_closed_before_starting_child(tmp_path: Path) -> None:
    ...
    result = run_shell_command({'command': f'printf unsafe > {shlex.quote(str(marker))}'}, tmp_path, require_containment=True)
    assert not result.success
    ...
    assert result.metadata['execution_policy'] == {'name': 'direct-local', 'containment_required': True, 'containment_applied': False, 'uses_shell': True}
...
def test_host_policy_can_supply_a_contained_non_shell_transport(tmp_path: Path) -> None:
    ...
    class RecordingPolicy:
        ...
        def prepare(self, request: ShellExecutionRequest) -> ShellExecutionDecision:
            ...
            return ShellExecutionDecision.allow((sys.executable, '-c', "print('contained')"), policy_name='test-sandbox', shell=False, environment=os.environ, containment_applied=True)
    policy = RecordingPolicy()
    result = run_shell_command({'command': 'ignored by host wrapper'}, tmp_path, stdout_limit_bytes=91, stderr_limit_bytes=47, execution_policy=policy, require_containment=True)
    ...
    assert result.metadata['execution_policy'] == {'name': 'test-sandbox', 'containment_required': True, 'containment_applied': True, 'uses_shell': False}

def test_host_policy_termination_callback_runs_before_transport_cleanup(tmp_path: Path) -> None:
    signals: list[str] = []

    class CallbackPolicy:

        def prepare(self, request: ShellExecutionRequest) -> ShellExecutionDecision:
            return ShellExecutionDecision.allow((sys.executable, '-c', 'import time; time.sleep(5)'), policy_name='callback-contained', shell=False, containment_applied=True, termination_callback=signals.append)
    result = run_shell_command({'command': 'wrapped', 'timeout_seconds': 1}, tmp_path, default_timeout_seconds=1, execution_policy=CallbackPolicy(), require_containment=True)
    assert result.error == 'timeout'
    assert signals == ['KILL']
...
def test_host_policy_can_deny_without_starting_child(tmp_path: Path) -> None:
    ...
    result = run_shell_command({'command': 'printf denied'}, tmp_path, execution_policy=DenyingPolicy())
    assert not result.success
...
def test_host_policy_can_mark_a_denial_as_fatal(tmp_path: Path) -> None:
    ...
    class FatalPolicy:
        ...
        def prepare(self, request: ShellExecutionRequest) -> ShellExecutionDecision:
            return ShellExecutionDecision.deny('host safety invariant failed', policy_name='host-safety', fatal=True)
    result = run_shell_command({'command': 'printf denied'}, tmp_path, execution_policy=FatalPolicy())
    assert not result.success
    assert result.error == 'execution_policy_denied'
    assert result.failure_kind == ToolFailureKind.FATAL_SAFETY
...
def test_default_registry_wires_host_configured_byte_limits(tmp_path: Path) -> None:
    capabilities = Capabilities(files=FileAccess.OFF, shell=True, memory=MemoryMode.OFF, utilities=False)
    registry = create_default_tool_registry(tmp_path, capabilities=capabilities, max_tool_stdout_bytes=83, max_tool_stderr_bytes=41)
...
@pytest.mark.parametrize('command', ['true;rm -rf target', "sh -c 'rm -rf target'", 'env FLAG=1 bash -lc "rm --recursive --force target"', 'find . -exec rm -rf {} +', "eval 'rm -rf target'", 'cmd=rm; flags=-rf; $cmd $flags target', 'cmd=rm; env $cmd -rf target', '$(printf rm) -rf target', 'eval "$UNRESOLVED_COMMAND"', "printf 'rm -rf target' | sh", "command eval 'rm -rf target'", '${UNSET_COMMAND:-rm} -rf target'])
...
def test_static_shell_scripts_without_destructive_content_remain_allowed(tmp_path: Path) -> None:
