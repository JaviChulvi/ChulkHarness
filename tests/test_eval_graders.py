...
import json
import pytest
from chulk.evals import CallableGrader, ContainsGrader, CostBudgetGrader, EvalCase, EvalDataset, EvalReference, EvalRunner, EvalSuite, EvalTarget, EvalTurn, EvalTurnResult, EventSequenceGrader, ExactAnswerGrader, JSONSchemaGrader, LLMJudgeGrader, LatencyGrader, MemoryRetrievalGrader, NoErrorGrader, PlanGrader, RegexGrader, RubricDimension, RubricJudgeGrader, SkillSelectionGrader, StatusGrader, TokenBudgetGrader, ToolCallGrader, TrialResult
from chulk.events import AgentEvent, SerializedEventPayload
from chulk.llm import LLMClient
from chulk.llm.usage import LLMCost, LLMResponse, LLMUsage
from chulk.results import Cost, Observation, Plan, PlanStatus, RunResult, RunStatus, ToolCall, Usage

def _case(reference: EvalReference | None=None) -> EvalCase:
    return EvalCase('case', (EvalTurn('question'),), reference)

def _trial(*, content: str='answer', status: RunStatus=RunStatus.COMPLETED, errors: tuple[str, ...]=(), tool_calls: tuple[ToolCall, ...]=(), observations: tuple[Observation, ...]=(), skills: tuple[str, ...]=(), memories: tuple[str, ...]=(), events: tuple[AgentEvent, ...]=(), usage: Usage | None=Usage(total_tokens=7), cost: Cost | None=Cost(amount=Decimal('0.02'), pricing_known=True), plan: Plan | None=None, duration: float=0.1) -> TrialResult:
    result = RunResult(content, status, None, 'conversation', None, usage=usage, cost=cost, tool_calls=tool_calls, observations=observations, loaded_skill_names=skills, loaded_memory_ids=memories, errors=errors, plan=plan)
    return TrialResult('case', 'target', 1, (EvalTurnResult(0, result, events, duration),), duration)

def test_answer_status_and_error_graders_cover_success_and_failure() -> None:
    reference = EvalReference(answer='{"ok": true}', contains=('ok', 'true'), regex='"ok"\\s*:\\s*true', json_schema={'type': 'object', 'properties': {'ok': {'type': 'boolean'}}, 'required': ['ok'], 'additionalProperties': False}, status='completed')
...
def test_tool_event_skill_memory_and_plan_graders_match_public_evidence() -> None:
    reference = EvalReference(tool_sequence=('lookup',), tool_arguments={'lookup': {'id': 7}}, tool_results={'lookup': {'found': True}}, tool_failures=(), event_sequence=('run.started', 'run.completed'), skill_names=('research',), memory_ids=('memory-1',))
    events = tuple((AgentEvent(name, 'conversation', SerializedEventPayload()) for name in ('run.started', 'model.response.completed', 'run.completed')))
    trial = _trial(tool_calls=(ToolCall('lookup', {'id': 7}, 1, success=True),), observations=(Observation('lookup', '{"found": true}'),), skills=('research',), memories=('memory-1',), events=events, plan=Plan('done', PlanStatus.COMPLETED))
    case = _case(reference)
...
class _JudgeClient(LLMClient):
    ...
    def complete_response(self, messages, *, max_output_tokens=None):
        ...
        return LLMResponse(self.content, usage=LLMUsage(input_tokens=8, output_tokens=3, total_tokens=11, cache_hit_input_tokens=4, cache_miss_input_tokens=4), cost=LLMCost(Decimal('0.03'), pricing_known=True), provider='judge-provider', model='judge-model')
...
class _SequencedJudgeClient(_JudgeClient):
    ...
    def complete_response(self, messages, *, max_output_tokens=None):
        ...
        return super().complete_response(messages, max_output_tokens=max_output_tokens)

def test_llm_judge_is_strict_records_identity_and_supports_pairwise() -> None:
    case = _case(EvalReference(answer='reference'))
    trial = _trial()
    grader = LLMJudgeGrader(_JudgeClient('{"score": 0.9, "passed": true, "reason": "clear"}'), 'Be clear.')
    ...
    malformed = LLMJudgeGrader(_JudgeClient('{"score": 0.9, "passed": true, "reason": "clear", "extra": 1}'), 'Be clear.').grade(case, trial)
...
def test_rubric_judge_weights_dimensions_and_enforces_vetoes() -> None:
    dimensions = (RubricDimension('Accuracy', {'excellent': 'All facts are correct.', 'fail': 'Core facts are wrong.'}, weight=2), RubricDimension('Completeness', {'excellent': 'No required facts are missing.', 'fail': 'Key facts are missing.'}), RubricDimension('Hallucination', {'pass': 'Every claim is supported.', 'fail': 'Any unsupported claim is present.'}, veto=True))
    response = json.dumps({'dimensions': [{'name': 'Accuracy', 'score': 0.9, 'reason': 'Accurate.'}, {'name': 'Completeness', 'score': 0.6, 'reason': 'One omission.'}, {'name': 'Hallucination', 'score': 1, 'reason': 'Supported.'}], 'reason': 'Strong overall.'})
    grade = RubricJudgeGrader(_JudgeClient(response), dimensions).grade(_case(), _trial())
    assert grade.passed
    ...
    vetoed_response['dimensions'][2] = {'name': 'Hallucination', 'score': 0, 'reason': 'Unsupported claim found.'}
    vetoed = RubricJudgeGrader(_JudgeClient(json.dumps(vetoed_response)), dimensions).grade(_case(), _trial())
...
async def test_rubric_pairwise_judge_swaps_order_and_flags_position_bias() -> None:
    dimension = RubricDimension('Quality', {'excellent': 'Better answer.', 'fail': 'Worse answer.'})
    position_biased = _JudgeClient('{"winner": "A", "reason": "A is better."}')
    biased_grade = RubricJudgeGrader(position_biased, (dimension,)).grade_pairwise(_case(), _trial(), _trial(content='baseline'))
    ...
    consistent = _SequencedJudgeClient('{"winner": "A", "reason": "A is better."}', '{"winner": "B", "reason": "B is better."}')
    consistent_grade = await RubricJudgeGrader(consistent, (dimension,)).grade_pairwise_async(_case(), _trial(), _trial(content='baseline'))
...
def test_judge_provider_failure_is_an_operational_error_and_is_accounted() -> None:
    ...
    class Agent:
        ...
        def close(self):
            return None
    case = _case()
    successful = EvalSuite('judge-accounting', EvalDataset((case,)), (EvalTarget('target', lambda _context: Agent()),), (LLMJudgeGrader(_JudgeClient('{"score": 1, "passed": true, "reason": "ok"}'), 'Good'),), required_graders=('quality.judge',), max_total_cost=1.0)
    ...
    failing = EvalSuite('judge-failure', EvalDataset((case,)), (EvalTarget('target', lambda _context: Agent()),), (LLMJudgeGrader(_FailingJudgeClient(), 'Good'),), required_graders=('quality.judge',), max_total_cost=1.0)
...
def test_model_judges_require_and_incrementally_enforce_a_cost_cap() -> None:
    ...
    class CountingJudgeClient(_JudgeClient):
        ...
        def complete_response(self, messages, *, max_output_tokens=None):
            self.calls += 1
            response = super().complete_response(messages, max_output_tokens=max_output_tokens)
            return LLMResponse(response.content, usage=response.usage, cost=LLMCost(Decimal('0.60'), pricing_known=True), provider=response.provider, model=response.model)
    judge = CountingJudgeClient('{"score": 1, "passed": true, "reason": "ok"}')
    cases = tuple((EvalCase(f'case-{index}', (EvalTurn('question'),)) for index in range(3)))
    ...
    class Agent:
        ...
        def close(self):
            return None
    base = {'name': 'judge-budget', 'dataset': EvalDataset(cases), 'targets': (EvalTarget('target', lambda _context: Agent()),), 'graders': (LLMJudgeGrader(judge, 'Good'),)}
