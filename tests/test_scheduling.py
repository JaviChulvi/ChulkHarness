...
def test_schedule_store_claims_completes_and_repeats_jobs(tmp_path) -> None:
    ...
    once = store.create(adapter='telegram', destination_id='9', prompt='once', next_run_at=now)
    recurring = store.create(adapter='telegram', destination_id='9', prompt='repeat', next_run_at=now, interval_seconds=3600)
...
def test_running_cancellation_cannot_be_revived_by_stale_completion(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    now = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    job = store.create(adapter='telegram', destination_id='9', prompt='cancel me', next_run_at=now)
...
def test_two_workers_claim_a_due_job_only_once(tmp_path) -> None:
    ...
    now = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    job = first_store.create(adapter='telegram', destination_id='9', prompt='one worker', next_run_at=now)
    barrier = Barrier(2)
...
def test_expired_claim_is_replaced_and_stale_worker_cannot_transition(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    now = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    job = store.create(adapter='telegram', destination_id='9', prompt='lease', next_run_at=now)
    stale = store.claim_due(adapter='telegram', now=now, lease_seconds=10)[0]
    current = store.claim_due(adapter='telegram', now=now + timedelta(seconds=11), lease_seconds=10)[0]
    assert stale.claim_token is not None
    assert current.claim_token is not None
    assert current.claim_token != stale.claim_token
    assert not store.complete(job.id, stale.claim_token, finished_at=now + timedelta(seconds=12))
    assert not store.fail(job.id, stale.claim_token, 'stale', failed_at=now + timedelta(seconds=12))
    assert store.complete(job.id, current.claim_token, finished_at=now + timedelta(seconds=12))

def test_active_worker_can_renew_lease_but_stale_worker_cannot(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    now = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    job = store.create(adapter='telegram', destination_id='9', prompt='renew', next_run_at=now)
    claimed = store.claim_due(adapter='telegram', now=now, lease_seconds=10)[0]
    assert claimed.claim_token is not None
    assert store.renew_lease(job.id, claimed.claim_token, now=now + timedelta(seconds=5), lease_seconds=10)
    assert not store.renew_lease(job.id, 'not-the-owner', now=now + timedelta(seconds=6), lease_seconds=10)
    assert store.claim_due(adapter='telegram', now=now + timedelta(seconds=11), lease_seconds=10) == ()
    assert not store.renew_lease(job.id, claimed.claim_token, now=now + timedelta(seconds=16), lease_seconds=10)

def test_recurring_retry_preserves_original_cadence(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    scheduled = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    job = store.create(adapter='telegram', destination_id='9', prompt='repeat', next_run_at=scheduled, interval_seconds=3600)
    first = store.claim_due(adapter='telegram', now=scheduled)[0]
    assert first.claim_token is not None
    assert store.fail(job.id, first.claim_token, 'temporary', retry_seconds=600, failed_at=scheduled + timedelta(minutes=1))
    ...
    assert store.complete(job.id, retry.claim_token, finished_at=retry_at + timedelta(minutes=1))
...
def test_schedule_store_scopes_list_and_cancel_to_destination(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    job = store.create(adapter='telegram', destination_id='9', prompt='private', next_run_at=datetime.now(timezone.utc) + timedelta(hours=1))
...
def test_scheduling_tools_parse_local_time_and_manage_jobs(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    tools = {value.name: value for value in scheduled_job_tools(store, adapter='telegram', destination_id='9', timezone_name='Europe/Madrid')}
...
def test_schedule_tool_rejects_past_times(tmp_path) -> None:
    store = SQLiteScheduleStore(tmp_path / 'store.sqlite')
    tool = scheduled_job_tools(store, adapter='telegram', destination_id='9', timezone_name='UTC')[0]
