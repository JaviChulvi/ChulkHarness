...
from chulk.gateway import AuthenticationState, ChannelIdentity, ChannelScope, DeliveryReceipt, DeliveryState, DeliveryTarget, GatewayRunTarget, InboundEnvelope, OutboundEnvelope, SQLiteGatewayLedger, TextPart, TrustLevel
...
def _inbound(event_id: str, *, destination_id: str='chat-9', principal_id: str='user-7') -> InboundEnvelope:
    return InboundEnvelope(event_id=event_id, idempotency_key=f'telegram:primary:{event_id}', identity=ChannelIdentity('telegram', 'primary', principal_id), destination_id=destination_id, parts=(TextPart(f'message {event_id}'),), scope=ChannelScope.DIRECT, authentication=AuthenticationState.AUTHENTICATED, trust=TrustLevel.TRUSTED)

def _outbound(inbox_id: str, *, profile_id: str='work') -> OutboundEnvelope:
    return OutboundEnvelope(envelope_id=f'reply-{inbox_id}', profile_id=profile_id, conversation_id=f'conversation-{inbox_id}', target=DeliveryTarget('telegram', 'primary', 'chat-9'), text='done')
...
def test_adapter_lease_has_one_live_owner_and_owned_cursor(tmp_path) -> None:
    ...
    with pytest.raises(RuntimeError, match='already running'):
        ledger.start_adapter('telegram', 'primary', now=NOW + timedelta(seconds=5), lease_seconds=10)
    assert first.instance_token is not None
    assert not ledger.save_cursor('telegram', 'primary', '20', instance_token='not-the-owner')
    assert ledger.save_cursor('telegram', 'primary', '20', instance_token=first.instance_token)
...
def test_cooperative_stop_prevents_further_lease_renewal(tmp_path) -> None:
    ledger = SQLiteGatewayLedger(tmp_path / 'control.sqlite')
    status = ledger.start_adapter('telegram', 'primary', now=NOW, lease_seconds=10)
    assert status.instance_token is not None
    assert ledger.request_adapter_stop('telegram', 'primary')
    assert not ledger.renew_adapter('telegram', 'primary', status.instance_token, now=NOW + timedelta(seconds=1), lease_seconds=10)
...
def test_ingest_binds_one_scope_and_published_definition(tmp_path) -> None:
    ...
    scope = ExecutionScope(tenant_id='tenant-a', workspace_id='workspace-a', actor_id='actor-a', agent_id='support', agent_version='1.0.0', run_id='run-hosted')
    ...
    with pytest.raises(ValueError, match='different hosted run'):
        ledger.ingest(envelope, profile_id='work', run_target=GatewayRunTarget(scope=scope, definition_id='support', definition_version='1.0.0', definition_digest='sha256:changed'))
...
def test_hosted_gateway_migration_preserves_v4_inbox_rows(tmp_path) -> None:
    ...
    encoded = json.dumps({'event_id': envelope.event_id, 'idempotency_key': envelope.idempotency_key, 'identity': {'adapter': envelope.identity.adapter, 'account_id': envelope.identity.account_id, 'principal_id': envelope.identity.principal_id}, 'destination_id': envelope.destination_id, 'parts': [{'kind': 'text', 'text': part.text, 'external_content': True}], 'scope': envelope.scope.value, 'authentication': envelope.authentication.value, 'trust': envelope.trust.value, 'thread_id': envelope.thread_id, 'received_at': envelope.received_at, 'external_content': envelope.external_content, 'extensions': {}}, separators=(',', ':'), sort_keys=True)
...
def test_execution_claims_preserve_fifo_and_enforce_concurrency_limits(tmp_path) -> None:
    ...
    other = ledger.ingest(_inbound('3', destination_id='chat-10'), profile_id='personal').record
    ...
    assert ledger.complete_execution(first.id, claim.execution_token, (_outbound(first.id),))
...
def test_execution_claim_respects_queue_wave_cutoff(tmp_path) -> None:
    ledger = SQLiteGatewayLedger(tmp_path / 'control.sqlite')
    first = ledger.ingest(_inbound('1'), profile_id='work').record
    second = ledger.ingest(_inbound('2', destination_id='chat-10'), profile_id='personal').record
    claim = ledger.claim_execution(global_limit=2, profile_limit=1, queued_before=first.created_at)
    assert claim is not None
    assert claim.record.id == first.id
    assert ledger.complete_execution(first.id, claim.execution_token, (_outbound(first.id),))
    assert ledger.claim_execution(global_limit=2, profile_limit=1, queued_before=first.created_at) is None
...
def test_expired_execution_is_quarantined_and_never_reclaimed(tmp_path) -> None:
    ...
    claim = ledger.claim_execution(global_limit=1, profile_limit=1, now=NOW, lease_seconds=10)
    assert claim is not None
    recovered = ledger.recover_expired_executions(now=NOW + timedelta(seconds=11))
    ...
    assert ledger.claim_execution(global_limit=1, profile_limit=1, now=NOW + timedelta(seconds=12)) is None
...
def test_outbox_delivery_is_retryable_and_records_attempt_checkpoints(tmp_path) -> None:
    ...
    assert ledger.complete_execution(inbox.id, execution.execution_token, (response,))
    first = ledger.claim_delivery(now=NOW)
    assert first is not None
    assert first.delivery_token is not None
    assert first.attempt_count == 1
    assert ledger.record_delivery(first.id, first.delivery_token, DeliveryReceipt(envelope_id=first.id, state=DeliveryState.RETRYABLE, attempt=1, checkpoint='part-0', retry_after_seconds=5, error_message='offline'), now=NOW)
    ...
    assert ledger.record_delivery(retried.id, retried.delivery_token, DeliveryReceipt(envelope_id=retried.id, state=DeliveryState.DELIVERED, attempt=2, checkpoint='complete'), now=NOW + timedelta(seconds=6))
...
def test_transport_acceptance_is_not_a_final_delivery_checkpoint(tmp_path) -> None:
    ...
    assert ledger.record_delivery(delivery.id, delivery.delivery_token, DeliveryReceipt(envelope_id=delivery.id, state=DeliveryState.ACCEPTED, attempt=1), now=NOW)
    ...
    assert ledger.reconcile_delivery(delivery.id, DeliveryReceipt(envelope_id=delivery.id, state=DeliveryState.DELIVERED, attempt=1, adapter_message_id='provider-42'), now=NOW + timedelta(seconds=1))
    assert ledger.inbox_complete(inbox.id)

def test_backpressure_counts_executed_work_with_pending_delivery(tmp_path) -> None:
    ledger = SQLiteGatewayLedger(tmp_path / 'control.sqlite')
    inbox = ledger.ingest(_inbound('1'), profile_id='work', max_pending=1).record
    execution = ledger.claim_execution(global_limit=1, profile_limit=1)
    assert execution is not None
    assert ledger.complete_execution(inbox.id, execution.execution_token, (_outbound(inbox.id),))
    with pytest.raises(RuntimeError, match='pending limit'):
        ledger.ingest(_inbound('2', destination_id='chat-10'), profile_id='work', max_pending=1)
...
def test_cancellation_is_durable_for_queued_and_active_work(tmp_path) -> None:
    ...
    active = ledger.ingest(_inbound('2', destination_id='chat-10'), profile_id='work').record
