...
def test_typed_dependencies_are_hidden_from_schema_and_injected(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), capabilities=Capabilities.none(), deps=Dependencies('tenant-a'), llm=FakeLLM([_tool_call('tenant_lookup', query='invoices'), _final()]), tools=[tenant_lookup], skills=[])
...
def test_run_dependencies_override_agent_dependencies(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), deps=Dependencies('default'), llm=FakeLLM([_tool_call('tenant_lookup'), _final()]), tools=[tenant_lookup], skills=[])
...
def test_request_dependencies_are_released_when_a_turn_raises(tmp_path):
    ...
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FailingLLM(), tools=[], skills=[])
...
def test_missing_dependencies_fail_before_side_effects(tmp_path):
    ...
    def guarded_write(value: str, context: ToolContext[Dependencies]) -> str:
        ...
        return 'written'
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FakeLLM([_tool_call('guarded_write', value='x'), _final()]), tools=[guarded_write], skills=[])
...
def test_model_cannot_supply_injected_context_argument():
    ...
    result = registry.run('guarded_lookup', {'value': 'x', 'context': {'deps': {'tenant': 'attacker'}}}, context=ToolContext(deps=Dependencies('trusted')))
...
async def test_async_tools_receive_typed_dependencies(tmp_path):
    ...
    async def tenant_lookup(query: str, context: ToolContext[Dependencies]) -> str:
        ...
        received.append(context.deps.tenant)
        return query
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), deps=Dependencies('async-tenant'), llm=FakeLLM([_tool_call('tenant_lookup', query='docs'), _final()]), tools=[tenant_lookup], skills=[])
