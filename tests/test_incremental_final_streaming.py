...
import json
import pytest
from chulk import Agent, AgentConfig, AsyncAgent, AsyncHostedRuntime, EventName, FinalAnswerDeliveryStatus, FinalAnswerPolicyDecision, FinalAnswerStreamingMode, ExecutionScope, OutputPolicyFailureMode
...
class IncrementalLLM(LLMClient):
    ...
    def stream_final_answer(self, messages, *, max_output_tokens=None) -> Iterator[LLMStreamChunk]:
        self.sync_stream_called = True
        if self.fail_after == 0 and (not self.chunks):
            raise LLMError('stream failed', code='server_error', retryable=True, fallback_eligible=True)
    ...
    async def astream_final_answer(self, messages, *, max_output_tokens=None) -> AsyncIterator[LLMStreamChunk]:
...
def _agent(tmp_path, llm: LLMClient, **kwargs) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[], final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL, **kwargs)
...
def test_policy_runs_before_publication_and_blocked_text_never_leaks(tmp_path) -> None:
    ...
    assert [event.payload.text for event in transformed_events if event.name == EventName.MODEL_DELTA.value] == ['SECRET']
...
def test_fail_closed_policy_exception_never_leaks_rejected_chunk(tmp_path) -> None:
    ...
    class BrokenPolicy:
        ...
        def reset(self, *, turn_id: str):
            return None
    facade = _agent(tmp_path, IncrementalLLM(('rejected',)), output_policy=BrokenPolicy(), output_policy_failure_mode=OutputPolicyFailureMode.CLOSED)
    events = list(facade.run_events('hello'))
...
def test_fallback_only_switches_before_stream_output(tmp_path) -> None:
    ...
    facade = _agent(tmp_path / 'buffered', FallbackChain([buffered_failure, buffered_fallback]), output_policy=buffered_policy)
...
async def test_async_stream_uses_native_iterator_and_async_policy(tmp_path) -> None:
    ...
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[], final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL, async_output_policy=AsyncUppercasePolicy())
...
async def test_async_stream_idle_timeout_closes_provider_and_records_stall(tmp_path) -> None:
    ...
    class StalledLLM(IncrementalLLM):
        ...
        async def astream_final_answer(self, messages, *, max_output_tokens=None):
            ...
            try:
                ...
                self.closed = True
    llm = StalledLLM()
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path, llm_timeout_seconds=0.05), llm=llm, tools=[], skills=[], final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL)
    ...
    assert turn.extension_metadata['final_answer_delivery']['failure_kind'] == 'stream_idle_timeout'
    await facade.close()

@pytest.mark.asyncio
async def test_async_hosted_create_awaits_policy_and_honors_failure_mode(tmp_path) -> None:
    ...
    runtime = await AsyncHostedRuntime.create(config=AgentConfig(project_root=tmp_path / 'policy'), llm=IncrementalLLM(('native ', 'hosted')), tools=[], skills=[], services=InMemoryServiceHub().async_services(), execution_scope=scope, final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL, async_output_policy=policy)
    events = [event async for event in runtime.run_events_async('hello')]
    assert [event.payload.text for event in events if event.name == EventName.MODEL_DELTA.value] == ['NATIVE ', 'HOSTED', '!']
    assert events[-1].payload.result.content == 'NATIVE HOSTED!'
    assert policy.calls == [('process', 'native '), ('process', 'hosted'), ('complete', 2)]
    await runtime.close()
    ...
    class BrokenPolicy(RecordingPolicy):
        ...
        async def process(self, chunk):
            raise RuntimeError('policy unavailable')
    fail_open = await AsyncHostedRuntime.create(config=AgentConfig(project_root=tmp_path / 'fail-open'), llm=IncrementalLLM(('permitted',)), tools=[], skills=[], services=InMemoryServiceHub().async_services(), execution_scope=scope, final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL, async_output_policy=BrokenPolicy(), output_policy_failure_mode=OutputPolicyFailureMode.OPEN)
    ...
    class WaitingLLM(IncrementalLLM):
        ...
        async def astream_final_answer(self, messages, *, max_output_tokens=None):
            ...
            try:
                ...
                self.closed = True
    llm = WaitingLLM()
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[], final_answer_streaming=FinalAnswerStreamingMode.INCREMENTAL)
