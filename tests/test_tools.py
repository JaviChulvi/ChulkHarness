...
from chulk.tools.permissions import PermissionDecision, ToolPermissionLevel, ToolPermissionPolicy, normalize_permission_level, permission_policy_for_profile
from chulk.tools.registry import PLAN_STEP_UPDATE_TOOL_NAME, PLAN_TOOL_NAME, ToolExecutionContext, ToolFailureKind, ToolResult
...
def test_registry_rejects_internal_planning_action_names():
    for name in (PLAN_TOOL_NAME, PLAN_STEP_UPDATE_TOOL_NAME):
        registry = ToolRegistry()
        try:
            registry.register(Tool(name=name, description='Must not shadow an internal action.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda _arguments: ToolResult(name, True, 'unused')))
...
def test_tool_permission_policy_requires_confirmation_by_default():
    tool = Tool(name='dangerous', description='Dangerous test tool.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda _arguments: ToolResult('dangerous', True, 'ran'), requires_confirmation=True, permission_level=ToolPermissionLevel.SHELL)
...
def test_registry_rejects_unknown_tool_permission_level():
    registry = ToolRegistry()
    try:
        registry.register(Tool(name='bad_permission', description='Bad permission level.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda _arguments: ToolResult('bad_permission', True, 'ran'), permission_level='not-a-level'))
    except ValueError as exc:
        assert 'Unknown tool permission level' in str(exc)
    else:
...
def test_registry_logs_tool_calls():
    ...
    assert registry.call_log == [{'tool_name': 'calculator', 'arguments': {'expression': '1 + 1'}, 'success': True, 'error': None, 'observation': '1 + 1 = 2'}]
...
def test_registry_runs_async_tool_with_context():
    ...
    result = asyncio.run(registry.run_async('lookup', {'query': 'handbook'}, context=ToolExecutionContext(metadata={'org_id': 'org-1'})))
...
def test_registry_run_async_offloads_executor_tool():
    ...
    registry.register(Tool(name='blocking', description='Blocking sync tool.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=blocking, run_in_executor=True))
...
def test_default_run_cmd_yields_event_loop_when_run_async(tmp_path):
    ...
    async def run_probe():
        command_task = asyncio.create_task(registry.run_async('run_cmd', {'command': command, 'timeout_seconds': 1}))
...
def test_registry_run_async_propagates_cancellation():
    ...
    registry.register(Tool(name='cancelled', description='Cancelled async tool.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=cancelled))
...
def _permission_test_tool(name: str, permission_level: ToolPermissionLevel, *, requires_confirmation: bool=False) -> Tool:
    return Tool(name=name, description='Permission profile test tool.', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=lambda _arguments: ToolResult(name, True, 'ran'), requires_confirmation=requires_confirmation, permission_level=permission_level)
...
def test_registry_validates_required_arguments():
    ...
    assert result.metadata['validation_errors'] == [{'path': 'expression', 'message': 'Missing required argument', 'expected': None, 'actual': None}]

def test_registry_reports_multiple_argument_validation_errors():
    calls = []
    registry = ToolRegistry()
    registry.register(Tool(name='profile', description='Validate profile arguments.', args_schema={'type': 'object', 'properties': {'age': {'type': 'integer', 'minimum': 1, 'maximum': 120}, 'tags': {'type': 'array', 'items': {'type': 'string', 'minLength': 1}, 'maxItems': 3}}, 'required': ['age'], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('profile', True, 'ok')))
    ...
    assert result.metadata['validation_errors'] == [{'path': 'extra', 'message': 'Unknown argument', 'expected': None, 'actual': None}, {'path': 'age', 'message': 'number is too small', 'expected': '>= 1', 'actual': '0'}, {'path': 'tags[1]', 'message': 'string is too short', 'expected': 'at least 1 characters', 'actual': '0 characters'}]

def test_registry_validates_additional_properties_schema():
    calls = []
    registry = ToolRegistry()
    registry.register(Tool(name='scores', description='Validate dynamic score map.', args_schema={'type': 'object', 'properties': {'values': {'type': 'object', 'additionalProperties': {'type': 'integer'}}, 'states': {'type': 'array', 'items': {'type': 'object', 'additionalProperties': {'type': 'string', 'enum': ['active', 'paused']}}}}, 'required': ['values', 'states'], 'additionalProperties': False}, callable=lambda arguments: calls.append(arguments) or ToolResult('scores', True, 'ok')))
    ...
    assert result.metadata['validation_errors'] == [{'path': 'values.b', 'message': 'value has the wrong type', 'expected': 'integer', 'actual': 'string'}, {'path': 'states[0].north', 'message': 'value is not one of the allowed options', 'expected': 'active, paused', 'actual': "'unknown'"}]

def test_registry_rejects_invalid_tool_schema_at_registration():
    registry = ToolRegistry()
    try:
        registry.register(Tool(name='broken_schema', description='Broken schema.', args_schema={'type': 'object', 'properties': {}, 'required': ['missing'], 'additionalProperties': 'nope'}, callable=lambda _arguments: ToolResult('broken_schema', True, 'unused')))
    except ValueError as exc:
        message = str(exc)
    else:
...
def test_registry_catches_tool_exceptions():
    ...
    registry = ToolRegistry()
    registry.register(Tool(name='broken', description='Broken test tool', args_schema={'type': 'object', 'properties': {}, 'required': [], 'additionalProperties': False}, callable=broken_tool))
...
def test_shell_blocks_recursive_force_rm_variants(tmp_path):
    blocked_commands = ['rm -fr /', 'rm -f -r target', 'rm -Rf target', 'rm --force --recursive target', 'rm --recursive --force target']
...
def test_shell_allows_benign_rm_like_commands(tmp_path):
    ...
    commands = ['rm file.txt', "printf 'needle' | grep -rf pattern .", "printf 'perf record'"]
...
def test_file_read_policy_blocks_sensitive_paths_but_keeps_source_and_templates_readable(tmp_path):
    files = {'.env': 'OPENAI_API_KEY=secret-env', '.env.production': 'OPENAI_API_KEY=secret-production', 'config/credentials.json': 'secret-credentials', 'keys/private.pem': 'secret-private-key', 'keys/id_rsa.backup': 'secret-backed-up-key', '.git/config': 'secret-git-state', '.chulk/mcp.json': 'secret-runtime-config', '.chulk/store.sqlite': 'secret-memory-state', '.chulk/store.sqlite-wal': 'secret-memory-wal', '.chulk/store.sqlite-shm': 'secret-memory-shm', '.chulk/store.sqlite-journal': 'secret-memory-journal', '.chulk/store.sqlite.backup-v1-20260716.sqlite': 'secret-memory-backup', '.chulk/traces/session.jsonl': 'secret-sdk-trace', 'traces/session.jsonl': 'secret-cli-trace', 'data/cache.db': 'secret-database-state', 'data/cache.sqlite.bak': 'secret-backed-up-database-state', '.env.example': 'OPENAI_API_KEY=', '.chulk/skills/reviewer/SKILL.md': 'Review project code.', 'src/example.py': "VALUE = 'normal source'\n"}
    ...
    registry = ToolRegistry()
    registry.register(read_file_tool(tmp_path))
    for relative_path in ['.env', '.env.production', 'config/credentials.json', 'keys/private.pem', 'keys/id_rsa.backup', '.git/config', '.chulk/mcp.json', '.chulk/store.sqlite', '.chulk/store.sqlite-wal', '.chulk/store.sqlite-shm', '.chulk/store.sqlite-journal', '.chulk/store.sqlite.backup-v1-20260716.sqlite', '.chulk/traces/session.jsonl', 'traces/session.jsonl', 'data/cache.db', 'data/cache.sqlite.bak']:
...
def test_list_and_search_files_do_not_expose_sensitive_paths_or_contents(tmp_path):
    files = {'.env': 'policy-needle secret-env-value', 'config/credentials.json': 'policy-needle secret-credentials-value', '.git/config': 'policy-needle secret-git-value', '.chulk/mcp.json': 'policy-needle secret-runtime-value', '.chulk/store.sqlite': 'policy-needle secret-memory-value', '.chulk/store.sqlite-wal': 'policy-needle secret-memory-wal-value', '.chulk/store.sqlite-shm': 'policy-needle secret-memory-shm-value', '.chulk/store.sqlite-journal': 'policy-needle secret-memory-journal-value', '.chulk/store.sqlite.backup-v1-20260716.sqlite': 'policy-needle secret-memory-backup-value', '.chulk/traces/session.jsonl': 'policy-needle secret-sdk-trace-value', 'traces/session.jsonl': 'policy-needle secret-cli-trace-value', 'data/cache.sqlite3': 'policy-needle secret-database-value', '.env.example': 'policy-needle template-value', '.chulk/skills/reviewer/SKILL.md': 'policy-needle project-skill-value', 'src/example.py': 'policy-needle normal-source-value'}
    for relative_path, content in files.items():
        path = tmp_path / relative_path
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(content, encoding='utf-8')
...
def test_list_and_python_search_patterns_cannot_escape_project_root(monkeypatch, tmp_path):
    ...
    searched = registry.run('search_files', {'query': 'outside-needle', 'path': '.', 'pattern': '../outside/*.txt'})
    assert listed.success
    assert searched.success
...
def test_search_files_ignores_runtime_trace_artifacts(tmp_path):
    ...
    (tmp_path / 'src' / 'chulk' / 'core' / 'agent.py').write_text('agentic loop lives here\n', encoding='utf-8')
...
def test_apply_patch_tool_modifies_existing_file(tmp_path):
    ...
    result = registry.run('apply_patch', {'patch': '\n'.join(['--- a/notes.txt', '+++ b/notes.txt', '@@ -1,3 +1,3 @@', ' one', '-two', '+TWO', ' three'])})
    assert result.success
...
def test_apply_patch_tool_creates_new_file(tmp_path):
    registry = ToolRegistry()
    registry.register(apply_patch_tool(tmp_path))
    result = registry.run('apply_patch', {'patch': '\n'.join(['--- /dev/null', '+++ b/notes/new.txt', '@@ -0,0 +1,2 @@', '+alpha', '+beta'])})
    assert result.success
...
def test_apply_patch_tool_blocks_unsafe_paths(tmp_path):
    registry = ToolRegistry()
    registry.register(apply_patch_tool(tmp_path))
    for path in ['.env', 'chulk/store.sqlite', 'data/cache.sqlite-wal', 'data/cache.sqlite-shm', 'data/cache.sqlite-journal', 'data/cache.sqlite.backup-v1-20260716.sqlite', '.git/config', 'secrets.txt', 'traces/output.txt']:
        result = registry.run('apply_patch', {'patch': '\n'.join(['--- /dev/null', f'+++ b/{path}', '@@ -0,0 +1 @@', '+blocked'])})
        assert not result.success
...
def test_apply_patch_tool_is_atomic_on_multi_file_failure(tmp_path):
    ...
    registry = ToolRegistry()
    registry.register(apply_patch_tool(tmp_path))
    result = registry.run('apply_patch', {'patch': '\n'.join(['--- a/a.txt', '+++ b/a.txt', '@@ -1 +1 @@', '-a', '+A', '--- a/b.txt', '+++ b/b.txt', '@@ -1 +1 @@', '-not-b', '+B'])})
    assert not result.success
    assert result.error == 'patch_context_mismatch'
    assert (tmp_path / 'a.txt').read_text(encoding='utf-8') == 'a\n'
    assert (tmp_path / 'b.txt').read_text(encoding='utf-8') == 'b\n'

@pytest.mark.parametrize(('patch_hunks', 'mismatch_type', 'hunk_index', 'target_line', 'reread_lines'), [('@@ -4,2 +4,2 @@\n stale-line-4\n-line-5\n+LINE-5', 'context', 1, 4, (1, 7)), ('@@ -1 +1 @@\n-line-1\n+LINE-1\n@@ -4 +4 @@\n-stale-line-4\n+LINE-4', 'removal', 2, 4, (1, 7)), ('@@ -20 +20 @@\n-line-20\n+LINE-20', 'location', 1, 20, (7, 10))])
def test_apply_patch_localizes_context_failures(tmp_path, patch_hunks, mismatch_type, hunk_index, target_line, reread_lines):
    lines = [f'line-{line_number}' for line_number in range(1, 11)]
    (tmp_path / 'notes.txt').write_text('\n'.join(lines) + '\n', encoding='utf-8')
    result = apply_patch_tool(tmp_path).callable({'patch': f'--- a/notes.txt\n+++ b/notes.txt\n{patch_hunks}'})
    assert not result.success
    assert result.error == 'patch_context_mismatch'
    ...
    assert result.metadata['reread_hint'] == {'path': 'notes.txt', 'start_line': reread_lines[0], 'end_line': reread_lines[1]}
    assert f'Patch {mismatch_type} mismatch in notes.txt at hunk {hunk_index}, target line {target_line}.' in result.observation
    assert f'Reread notes.txt lines {reread_lines[0]}-{reread_lines[1]}' in result.observation
...
def test_apply_patch_rolls_back_after_second_commit_write_fails(monkeypatch, tmp_path):
    ...
    result = apply_patch_tool(tmp_path).callable({'patch': '\n'.join(['--- a/a.txt', '+++ b/a.txt', '@@ -1 +1 @@', '-a', '+A', '--- a/b.txt', '+++ b/b.txt', '@@ -1 +1 @@', '-b', '+B'])})
    assert not result.success
...
def test_apply_patch_rolls_back_created_files_and_directories(monkeypatch, tmp_path):
    ...
    def fail_second_write(path, text):
        ...
        if calls == 2:
            raise OSError('disk full')
    monkeypatch.setattr(files_module, '_write_patch_text', fail_second_write)
    result = apply_patch_tool(tmp_path).callable({'patch': '\n'.join(['--- /dev/null', '+++ b/new/first.txt', '@@ -0,0 +1 @@', '+first', '--- /dev/null', '+++ b/new/nested/second.txt', '@@ -0,0 +1 @@', '+second'])})
    assert not result.success
    assert result.error == 'patch_commit_failed'
...
def test_apply_patch_rolls_back_base_exceptions(monkeypatch, tmp_path, interruption):
    ...
    with pytest.raises(interruption):
        apply_patch_tool(tmp_path).callable({'patch': '\n'.join(['--- a/a.txt', '+++ b/a.txt', '@@ -1 +1 @@', '-a', '+A'])})
...
def test_apply_patch_reports_rollback_failure(monkeypatch, tmp_path):
    ...
    assert result.metadata['cause'] == 'OSError'
    assert result.metadata['rollback_errors'] == [{'path': 'a.txt', 'error': 'OSError'}]
...
def test_default_tool_registry_contains_memory_tools_when_store_is_provided(tmp_path):
    ...
    names = {tool.name for tool in registry.list_tools()}
    assert {'save_memory', 'search_memory', 'list_memories', 'delete_memory', 'update_memory', 'summarize_memories', 'archive_memory', 'restore_memory', 'compact_memories', 'import_memories', 'export_memories'} <= names
...
def test_default_io_tools_run_in_executor(tmp_path):
    ...
    for name in {'run_cmd', 'read_file', 'apply_patch', 'write_file', 'list_files', 'search_files', 'save_memory', 'search_memory', 'list_memories', 'delete_memory', 'update_memory', 'summarize_memories', 'archive_memory', 'restore_memory', 'compact_memories', 'import_memories', 'export_memories'}:
        assert tools[name].run_in_executor is True

def test_memory_tools_save_search_update_and_delete(tmp_path):
    memory_store = SQLiteMemoryStore(tmp_path / 'memory.sqlite')
    registry = create_default_tool_registry(tmp_path, memory_store=memory_store)
    save_result = registry.run('save_memory', {'content': 'ChulkHarness should keep memory and skills separate.', 'tags': ['project', 'memory'], 'importance': 7})
