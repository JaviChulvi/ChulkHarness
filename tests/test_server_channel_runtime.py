...
from chulk.gateway import AuthenticationState, ChannelIdentity, ChannelScope, InboundEnvelope, TextPart, TrustLevel
...
def _executor(tmp_path):
    ...
    def build(profile_id, conversation_id, metadata):
        ...
        return create_agent(resolved.config, llm_client=ChannelLLM(), tool_specs=[], skill_specs=[], profile_id=profile_id, conversation_id=conversation_id, conversation_metadata=dict(metadata or {}))
    dispatcher = ConversationDispatcher(factory, agent_builder=build)
    return (ChannelConversationExecutor(dispatcher), dispatcher)

@pytest.mark.asyncio
async def test_shared_new_status_and_normal_message_reuse_channel_conversation(tmp_path) -> None:
...
@pytest.mark.asyncio
async def test_shared_commands_do_not_enter_the_model_and_unknowns_fail_closed(tmp_path) -> None:
    execute, dispatcher = _executor(tmp_path)
...
async def test_requested_missing_conversation_returns_typed_failure(tmp_path) -> None:
    execute, dispatcher = _executor(tmp_path)
    result = (await execute('default', _envelope('hello', event_id='1', conversation_id='missing')))[0]
    assert result.extensions['error'] == 'conversation_not_found'
    assert result.conversation_id == 'missing'
    await dispatcher.close()

@pytest.mark.asyncio
async def test_websocket_client_identity_resumes_across_new_socket_accounts(tmp_path) -> None:
    execute, dispatcher = _executor(tmp_path)
    created = (await execute('default', _envelope('/new', event_id='1', adapter='websocket', account_id='socket-one', destination_id='stable-client')))[0]
    resumed = (await execute('default', _envelope('/status', event_id='2', adapter='websocket', account_id='socket-two', destination_id='stable-client')))[0]
    assert resumed.conversation_id == created.conversation_id
    await dispatcher.close()

@pytest.mark.asyncio
async def test_read_only_command_does_not_create_an_empty_conversation(tmp_path) -> None:
    execute, dispatcher = _executor(tmp_path)
