...
def _accounting(service: object, events: list[tuple[str, dict]]) -> ModelAccounting:
    return ModelAccounting(state=AgentState(), trace=lambda event, payload: events.append((event, payload)), usage_accounting=None, async_usage_accounting=service)
...
async def test_sync_and_async_recording_share_report_and_trace_shape() -> None:
    ...
    sync = ModelAccounting(state=AgentState(), trace=lambda event, payload: sync_events.append((event, payload)), usage_accounting=_UsageService())
    ...
    sync_result = sync.record(sync_turn, request_index=0, usage=_usage(), cost=_cost())
    async_result = await async_accounting.record_async(async_turn, request_index=0, usage=_usage(), cost=_cost())
