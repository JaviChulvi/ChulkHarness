...
def test_load_telegram_config_parses_environment_values() -> None:
    config = load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': ' secret-token ', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '123, 456,123', 'CHULK_TELEGRAM_POLL_TIMEOUT_SECONDS': '20', 'CHULK_TELEGRAM_RETRY_DELAY_SECONDS': '0.5', 'CHULK_TAVILY_API_KEY': ' tavily-secret ', 'CHULK_WEB_SEARCH_MAX_RESULTS': '3', 'CHULK_TELEGRAM_TIMEZONE': 'Europe/Madrid', 'CHULK_TELEGRAM_SCHEDULER_POLL_SECONDS': '2.5', 'CHULK_TELEGRAM_SCHEDULING_ENABLED': 'true', 'CHULK_TELEGRAM_MEMORY_ENABLED': 'false'})
...
def test_load_telegram_config_reads_env_file_with_environment_precedence(tmp_path: Path) -> None:
    ...
    env_file.write_text('CHULK_TELEGRAM_BOT_TOKEN=file-token\nCHULK_TELEGRAM_ALLOWED_USER_IDS=123\n', encoding='utf-8')
    config = load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'process-token'}, env_file=env_file)
...
def test_load_telegram_config_rejects_invalid_user_ids(value: str) -> None:
    with pytest.raises(TelegramConfigError, match='numeric ids'):
        load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': value})
...
def test_load_telegram_config_rejects_excessive_search_results() -> None:
    with pytest.raises(TelegramConfigError, match='WEB_SEARCH_MAX_RESULTS'):
        load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1', 'CHULK_WEB_SEARCH_MAX_RESULTS': '11'})

def test_load_telegram_config_rejects_unknown_timezone() -> None:
    with pytest.raises(TelegramConfigError, match='TIMEZONE'):
        load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1', 'CHULK_TELEGRAM_TIMEZONE': 'Mars/Olympus'})

def test_scheduling_is_disabled_by_default_and_rejects_invalid_boolean() -> None:
    config = load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1'})
    assert config.scheduling_enabled is False
    with pytest.raises(TelegramConfigError, match='SCHEDULING_ENABLED'):
        load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1', 'CHULK_TELEGRAM_SCHEDULING_ENABLED': 'sometimes'})

def test_multi_user_memory_is_enabled_with_durable_namespaces() -> None:
    values = {'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1,2'}
    enabled = load_telegram_config(values)
    disabled = load_telegram_config({**values, 'CHULK_TELEGRAM_MEMORY_ENABLED': 'false'})
...
def test_load_telegram_config_validates_attachment_bound() -> None:
    with pytest.raises(TelegramConfigError, match='CHULK_TELEGRAM_MAX_ATTACHMENT_BYTES'):
        load_telegram_config({'CHULK_TELEGRAM_BOT_TOKEN': 'secret', 'CHULK_TELEGRAM_ALLOWED_USER_IDS': '1', 'CHULK_TELEGRAM_MAX_ATTACHMENT_BYTES': str(21 * 1024 * 1024)})
