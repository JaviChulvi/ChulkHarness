...
from chulk.research import GitPolicy, GitPolicyError, GitService, git_tools
from chulk.research.git import _split_test_command

def _git(root: Path, *arguments: str, input_text: str | None=None) -> str:
    result = subprocess.run(['git', '-C', str(root), *arguments], input=input_text, text=True, capture_output=True, check=True)
...
def test_git_service_returns_structured_status_diff_log_branches_and_worktrees(tmp_path: Path) -> None:
    ...
    assert {entry['path'] for entry in status['entries']} == {'tracked.txt', 'new.txt'}
...
def test_git_service_applies_patch_creates_branch_and_commits_explicit_paths(tmp_path: Path) -> None:
    root = _repository(tmp_path)
    service = GitService(root)
    patch = 'diff --git "a/\\164racked.txt" "b/\\164racked.txt"\n--- "a/\\164racked.txt"\n+++ "b/\\164racked.txt"\n@@ -1 +1,2 @@\n one\n+two\n'
...
def test_git_service_denies_sensitive_submodule_binary_push_and_repository_escape(tmp_path: Path) -> None:
    root = _repository(tmp_path)
    service = GitService(root)
    ...
    with pytest.raises(GitPolicyError, match='Binary'):
        service.apply_patch('diff --git a/image.png b/image.png\nGIT binary patch\nliteral 0\n')
    with pytest.raises(GitPolicyError, match='disabled'):
        service.push(remote='origin', branch='main')
    quoted_sensitive_patch = 'diff --git "a/\\056env" "b/\\056env"\n--- "a/\\056env"\n+++ "b/\\056env"\n@@ -0,0 +1 @@\n+API_KEY=attacker\n'
...
def test_git_service_runs_only_allowlisted_tests_without_a_shell(tmp_path: Path) -> None:
    root = _repository(tmp_path)
    service = GitService(root, policy=GitPolicy(allowed_test_commands=((sys.executable, '-c'),), test_timeout_seconds=5))
    passed = service.run_tests(f'''{shlex_quote(sys.executable)} -c "print('bounded-test')"''')
    ...
    with pytest.raises(ValueError, match='timeout'):
        service.run_tests(f'''{shlex_quote(sys.executable)} -c "print('x')"''', timeout_seconds=6)
    with pytest.raises(GitPolicyError, match='outside'):
        service.run_tests(f'{shlex_quote(sys.executable)} -c pass /tmp/outside')

def test_windows_test_command_splitting_removes_only_balanced_outer_quotes() -> None:
    assert _split_test_command('"C:\\Program Files\\Python\\python.exe" -c "print(\'bounded\')"', windows=True) == ('C:\\Program Files\\Python\\python.exe', '-c', "print('bounded')")
    with pytest.raises(ValueError, match='unbalanced'):
        _split_test_command('"python.exe -m pytest', windows=True)

def test_git_tools_require_confirmation_for_tests_and_every_material_action(tmp_path: Path) -> None:
    ...
    denied = write_tool.callable({'action': 'push', 'remote': 'origin', 'branch': 'main'})
...
def test_git_commit_does_not_absorb_unrelated_staged_paths(tmp_path: Path) -> None:
    ...
    assert _git(root, 'show', '--format=', '--name-only', 'HEAD').splitlines() == ['tracked.txt']
