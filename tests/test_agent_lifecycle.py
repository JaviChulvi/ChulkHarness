...
import pytest
from chulk import Agent, AgentConfig, AgentHandle, AsyncAgent, AsyncAgentHandle, ConfigurationError, ProviderError
...
class FailingLLMClient(LLMClient):
    ...
    def complete(self, messages, *, max_output_tokens=None) -> str:
        raise LLMError('provider unavailable', provider='failing', model='broken', code='server_error', retryable=True, fallback_eligible=True)
...
def _agent(tmp_path, *, llm: FakeLLMClient | None=None) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=llm or FakeLLMClient(), tools=[], skills=[])
...
def test_public_agent_close_is_idempotent_and_rejects_new_work(tmp_path):
    ...
    for operation in (lambda: facade.run('hello'), lambda: facade.run_result('hello'), lambda: facade.plan('hello'), lambda: facade.plan_result('hello'), facade.approve, facade.approve_result, facade.reject, facade.reject_result):
...
def test_sync_context_manager_closes_on_normal_and_exceptional_exit(tmp_path):
    ...
    trace_events = [json.loads(line) for line in normal.trace_path.read_text(encoding='utf-8').splitlines()]
...
async def test_async_context_manager_closes_once_and_rejects_work(tmp_path):
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=FakeLLMClient('async done'), tools=[], skills=[])
...
async def test_async_compatibility_handle_awaits_owned_resource_once():
    ...
    handle = AsyncAgentHandle(AgentHandle(CoreAgent(resource, owned_resources=[resource])))
    await handle.close()
    await handle.close()
    assert resource.close_count == 1
...
def test_runtime_finishes_trace_after_owned_resource_cleanup(tmp_path):
    ...
    handle = AgentHandle(CoreAgent(resource, trace_logger=trace_logger, owned_resources=[resource]))
    handle.close()
...
def test_provider_failure_terminalizes_and_persists_turn_before_reraising(tmp_path):
    facade = Agent(config=AgentConfig(project_root=tmp_path), llm=FailingLLMClient(), tools=[], skills=[])
    ...
    trace_types = [json.loads(line)['type'] for line in facade.trace_path.read_text(encoding='utf-8').splitlines()]
...
def test_exception_terminalization_applies_configured_redaction_before_retention():
    ...
    agent = CoreAgent(FakeLLMClient(), event_callback=fail_callback, redaction_callback=redact)
    with pytest.raises(RuntimeError, match=secret):
        agent.run_turn('trigger redacted failure')
    retained = json.dumps({'turn': agent.state.turns[-1].to_dict(), 'state_errors': agent.state.errors, 'state_final_answer': agent.state.final_answer, 'messages': agent.memory.recent()})
    assert secret not in retained
    assert '[tenant-redacted]' in retained
    assert any((event_type == TraceEvent.TURN_FAILED and metadata.get('path') == 'exception.message' for event_type, metadata in redaction_calls))

def test_event_callbacks_receive_mandatory_baseline_redaction():
    events: list[dict] = []
    agent = CoreAgent(FakeLLMClient(), event_callback=lambda _event_type, payload: events.append(payload))
    agent._trace(TraceEvent.TOOL_CALL_STARTED, {'arguments': {'api_key': 'sk-callback-secret-123456', 'accesskey': 'compact-access-secret', 'privatekey': 'compact-private-secret', 'SSHKey': 'compact-ssh-secret'}, 'observation': 'access_key=AKIAIOSFODNN7EXAMPLE'})
...
def test_custom_event_redactor_cannot_reintroduce_a_secret():
    ...
    agent = CoreAgent(FakeLLMClient(), event_callback=lambda _event_type, payload: events.append(payload), redaction_callback=redact)
...
def test_exception_terminalization_honors_fail_closed_redaction():
    ...
    def fail_callback(event_type: str, payload: dict) -> None:
        if event_type == TraceEvent.MODEL_REQUEST_STARTED:
            raise RuntimeError(f'observer exposed {secret}')
    agent = CoreAgent(FakeLLMClient(), event_callback=fail_callback, redaction_callback=redact, redaction_fail_closed=True)
    with pytest.raises(RuntimeError, match=secret):
...
async def test_async_cancellation_terminalizes_and_persists_turn(tmp_path):
    started = asyncio.Event()
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=HangingAsyncLLMClient(started), tools=[], skills=[])
...
class CountingGate:
    ...
    async def __aexit__(self, *args):
        pass
_GATED_OPERATIONS = [('run', 'run_turn', ('hello',)), ('run_result', 'run_turn', ('hello',)), ('run_input', 'run_input', (UserInput.text('hello'),)), ('run_input_result', 'run_input', (UserInput.text('hello'),)), ('plan', 'run_planned_turn', ('hello',)), ('plan_result', 'run_planned_turn', ('hello',)), ('approve', 'approve_plan', ()), ('approve_result', 'approve_plan', ()), ('reject', 'reject_plan', ()), ('reject_result', 'reject_plan', ()), ('close', 'close', ())]

@pytest.mark.parametrize('method,runtime_method,args', _GATED_OPERATIONS)
@pytest.mark.parametrize('fails', [False, True])
def test_sync_facade_enters_gate_once_and_preserves_error_operation(tmp_path, monkeypatch, method, runtime_method, args, fails):
    facade = _agent(tmp_path)
    gate = CountingGate()
...
@pytest.mark.parametrize('method,runtime_method,args', _GATED_OPERATIONS)
@pytest.mark.parametrize('fails', [False, True])
async def test_async_facade_enters_gate_once_and_preserves_error_operation(tmp_path, monkeypatch, method, runtime_method, args, fails):
