...
import json
import pytest
from chulk.core.actions import STRICT_AGENT_ACTION_JSON_SCHEMA, ActionParseError, FinalAnswerAction, PlanAction, PlanStepUpdateAction, ToolCallAction, parse_model_response
...
def test_parse_final_answer_rejects_embedded_tool_call_fields():
    raw_response = json.dumps({'type': 'final_answer', 'content': 'I will search the files.', 'tool_name': 'search_files', 'arguments_json': '{"query":"agent loop"}'})
    try:
...
def test_parse_final_answer_rejects_embedded_plan_fields():
    raw_response = json.dumps({'type': 'final_answer', 'content': 'Done.', 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps({'summary': 'Ignored plan', 'steps': []}), 'step_update_json': '{}'})
    try:
        parse_model_response(raw_response)
    except ActionParseError as exc:
...
@pytest.mark.parametrize('invalid_placeholder', ['[]', 'null', 'false', '0', '""'])
@pytest.mark.parametrize('field_name', ['arguments_json', 'plan_json', 'step_update_json'])
def test_parse_action_rejects_non_object_json_transports(field_name, invalid_placeholder):
    payload = {'type': 'final_answer', 'content': 'Done.', 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': '{}', field_name: invalid_placeholder}
...
def test_parse_final_answer_rejects_invalid_falsey_tool_name(invalid_tool_name):
    with pytest.raises(ActionParseError, match='tool_name must be a string or null'):
        parse_model_response({'type': 'final_answer', 'content': 'Done.', 'tool_name': invalid_tool_name})
...
def test_parse_action_rejects_unknown_and_conflicting_transport_fields():
    with pytest.raises(ActionParseError, match='unsupported fields: extra'):
        parse_model_response({'type': 'final_answer', 'content': 'Done.', 'extra': True})
    with pytest.raises(ActionParseError, match='must not combine arguments'):
        parse_model_response({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': '1 + 1'}, 'arguments_json': '{"expression":"2 + 2"}'})

def test_parse_tool_call():
    action = parse_model_response(json.dumps({'type': 'tool_call', 'tool_name': 'calculator', 'arguments': {'expression': '1 + 1'}}))
...
def test_parse_tool_call_with_arguments_json_transport():
    action = parse_model_response(json.dumps({'type': 'tool_call', 'content': None, 'tool_name': 'calculator', 'arguments_json': '{"expression":"1 + 1"}'}))
    assert action == ToolCallAction(type='tool_call', tool_name='calculator', arguments={'expression': '1 + 1'})

def test_parse_tool_call_rejects_content_and_plan_fields():
    for extra in ({'content': 'ignored'}, {'content': None, 'plan_json': json.dumps({'summary': 'ignored'})}):
        payload = {'type': 'tool_call', 'tool_name': 'calculator', 'arguments_json': '{}', **extra}
        try:
            parse_model_response(payload)
...
def test_parse_plan_action_with_plan_json_transport():
    plan_json = json.dumps({'summary': 'Inspect the project and report back.', 'steps': [{'id': '1', 'title': 'List files', 'description': 'Use the file listing tool to inspect the project shape.', 'status': 'pending'}]})
    action = parse_model_response(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json}))
...
def test_parse_rich_plan_step_fields():
    plan_json = json.dumps({'summary': 'Implement a feature in two steps.', 'steps': [{'id': '1', 'title': 'Add state', 'description': 'Extend the plan state.', 'status': 'pending', 'depends_on': [], 'acceptance_criteria': ['State serializes dependencies.', 'State serializes evidence.'], 'retry_limit': 2}, {'id': '2', 'title': 'Use state', 'description': 'Wire the agent loop.', 'status': 'pending', 'depends_on': ['1'], 'acceptance_criteria': ['Agent advances after step 1.'], 'retry_limit': 0}]})
    action = parse_model_response(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json}))
    assert isinstance(action, PlanAction)
...
def test_parse_plan_action_rejects_later_dependency():
    plan_json = json.dumps({'summary': 'Invalid dependency.', 'steps': [{'id': '1', 'title': 'First', 'description': 'Cannot depend on the future.', 'depends_on': ['2']}, {'id': '2', 'title': 'Second', 'description': 'Later step.'}]})
    try:
        parse_model_response(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json}))
    except ActionParseError as exc:
...
def test_parse_plan_step_update_action():
    action = parse_model_response(json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'completed', 'evidence': 'The calculator returned 4.', 'reason': None})}))
    assert action == PlanStepUpdateAction(type='plan_step_update', step_id='1', status='completed', evidence='The calculator returned 4.', reason=None)

def test_parse_plan_step_update_rejects_blocked_without_reason():
    try:
        parse_model_response(json.dumps({'type': 'plan_step_update', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': json.dumps({'step_id': '1', 'status': 'blocked', 'evidence': 'The tool cannot run.', 'reason': None})}))
    except ActionParseError as exc:
...
def test_parse_plan_action_rejects_invalid_step_status():
    plan_json = json.dumps({'summary': 'Invalid plan.', 'steps': [{'id': '1', 'title': 'Bad step', 'description': 'This step has an invalid status.', 'status': 'done'}]})
    try:
        parse_model_response(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json}))
    except ActionParseError as exc:
...
def test_parse_plan_action_rejects_pre_advanced_step_status(status):
    plan_json = json.dumps({'summary': 'Invalid pre-advanced plan.', 'steps': [{'id': '1', 'title': 'Skipped work', 'description': 'This work has not run yet.', 'status': status}]})
    with pytest.raises(ActionParseError, match='status must be pending'):
        parse_model_response({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json, 'step_update_json': '{}'})
