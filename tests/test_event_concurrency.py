...
def _sync_agent(tmp_path, llm: LLMClient) -> Agent:
    return Agent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[])
...
def test_one_hundred_sync_runs_have_no_event_cross_talk(tmp_path):
    ...
    threads = [threading.Thread(target=facade.run, args=(f'sync-{index}',), kwargs={'on_event': event_groups[index].append}) for index in range(100)]
...
async def test_one_hundred_async_runs_serialize_and_keep_event_ownership(tmp_path):
    llm = SerializedProbeLLM()
    facade = AsyncAgent(config=AgentConfig(project_root=tmp_path), llm=llm, tools=[], skills=[])
    event_groups: list[list[AgentEvent]] = [[] for _ in range(100)]
    await asyncio.gather(*(facade.run(f'async-{index}', on_event=event_groups[index].append) for index in range(100)))
...
async def test_separate_agents_can_run_independently(tmp_path):
    ...
    first = AsyncAgent(config=AgentConfig(project_root=tmp_path / 'first'), llm=first_llm, tools=[], skills=[])
    second = AsyncAgent(config=AgentConfig(project_root=tmp_path / 'second'), llm=second_llm, tools=[], skills=[])
    first_events, second_events = ([], [])
    await asyncio.gather(first.run('first', on_event=first_events.append), second.run('second', on_event=second_events.append))
