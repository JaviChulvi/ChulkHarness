...
def test_spinner_animates_for_tty_stream():
    stream = TTYBuffer()
    spinner = Spinner(TerminalUI(color_enabled=False), stream=stream, enabled=True, interval_seconds=0.001)
...
def test_turn_summary_formats_token_usage_and_cost():
    ...
    output = ui.turn_summary({'turn': {'model_request_count': 1, 'tool_calls': [], 'loaded_skill_names': [], 'loaded_memory_ids': [], 'model_usage_totals': {'request_count': 1, 'usage': {'input_tokens': 12, 'output_tokens': 3, 'total_tokens': 15, 'estimated': True}, 'cost': {'amount': '0.000004', 'currency': 'USD', 'pricing_known': True, 'estimated': True}}}})
...
def test_help_alignment_is_calculated_before_ansi_styling():
    ...
    output = re.sub('\\x1b\\[[0-9;]*m', '', ui.help_text(CLI_COMMANDS))
...
def test_banner_reflows_without_exceeding_narrow_terminal_width(tmp_path):
    ui = TerminalUI(color_enabled=False, width=36)
    config = SimpleNamespace(llm_provider='openai', model='gpt-4.1-mini', llm_fallback_providers=(), permission_profile='workspace-write', project_root=tmp_path, mcp_servers=())
    agent = SimpleNamespace(state=SimpleNamespace(conversation_id='conversation-123'), tool_registry=SimpleNamespace(list_tools=lambda: [object(), object()]))
