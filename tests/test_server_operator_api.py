...
def test_goal_and_child_routes_are_revision_safe_and_profile_owned(tmp_path) -> None:
    ...
    goal = goal_service.create(title='Ship operator API', acceptance_criteria=('Operator can inspect it.',), steps=(GoalStep(id='build', title='Build', description='Build the operator surface.', acceptance_criterion_ids=('criterion-1',)),), budget=RunBudget(scope=BudgetScope.GOAL))
    child_store = ChildTaskStore(config.store_path)
    task = child_store.create(ChildTask(id='child-one', profile_id='default', spec=ChildTaskSpec(instruction='Inspect the operator surface.'), lineage=ChildTaskLineage()))
    with TestClient(app) as client:
        goals = client.get('/v1/profiles/default/goals', headers=_auth(tokens))
        inspected_goal = client.get(f'/v1/profiles/default/goals/{goal.id}', headers=_auth(tokens))
        foreign_goal = client.get(f'/v1/profiles/other/goals/{goal.id}', headers=_auth(tokens))
        approved = client.post(f'/v1/profiles/default/goals/{goal.id}/actions', headers=_auth(tokens), json={'action': 'approve', 'revision': goal.revision})
        stale = client.post(f'/v1/profiles/default/goals/{goal.id}/actions', headers=_auth(tokens), json={'action': 'run', 'revision': goal.revision})
        tasks = client.get('/v1/profiles/default/tasks', headers=_auth(tokens))
        inspected_task = client.get(f'/v1/profiles/default/tasks/{task.id}', headers=_auth(tokens))
        foreign_task = client.get(f'/v1/profiles/other/tasks/{task.id}', headers=_auth(tokens))
        cancelled = client.post(f'/v1/profiles/default/tasks/{task.id}/actions', headers=_auth(tokens), json={'action': 'cancel', 'revision': task.revision, 'reason': 'No longer needed.'})
...
def test_operator_routes_are_discoverable_and_jobs_no_longer_require_adapter(tmp_path) -> None:
    app, tokens = _app(tmp_path)
    with TestClient(app) as client:
        jobs = client.get('/v1/profiles/default/jobs?include_terminal=true', headers=_auth(tokens))
...
def test_operator_pages_use_resource_bound_cursors(tmp_path) -> None:
    ...
    for index in range(2):
        service.create(title=f'Goal {index}', acceptance_criteria=('Done.',), steps=(GoalStep(id=f'step-{index}', title='Step', description='Complete it.', acceptance_criterion_ids=('criterion-1',)),), budget=RunBudget(scope=BudgetScope.GOAL))
    with TestClient(app) as client:
        first = client.get('/v1/profiles/default/goals?limit=1', headers=_auth(tokens))
        cursor = first.json()['next_cursor']
        second = client.get(f'/v1/profiles/default/goals?limit=1&cursor={cursor}', headers=_auth(tokens))
        mismatched = client.get(f'/v1/profiles/default/jobs?cursor={cursor}', headers=_auth(tokens))
    assert cursor
...
def test_control_api_client_shapes_authenticated_operator_requests() -> None:
    ...
    client = ControlApiClient('http://127.0.0.1:8765/', 'secret-token', timeout_seconds=4.5, opener=opener)
    client.list_jobs('default', status='paused', include_terminal=True, limit=25)
    client.control_goal('default', 'goal/one', action='pause', revision=7)
    first, timeout = requests[0]
    assert timeout == 4.5
    assert first.full_url.endswith('/v1/profiles/default/jobs?status=paused&include_terminal=true&limit=25')
    ...
    assert json.loads(second.data or b'{}') == {'action': 'pause', 'revision': 7}
...
def test_control_api_client_shapes_conversation_and_attention_requests() -> None:
    ...
    def opener(request: Request, *, timeout: float):
        ...
        return _Response({'schema_version': 1, 'ok': True})
    client = ControlApiClient('http://127.0.0.1:8765', 'secret-token', opener=opener)
    client.send_message('operations', 'conversation/one', 'Check deployment', mode='plan', idempotency_key='message-key')
    client.decide_permission('operations', 'conversation/one', 'permission/one', decision='allow', idempotency_key='decision-key')
    message, permission = requests
    assert message.full_url.endswith('/v1/profiles/operations/conversations/conversation%2Fone/messages')
    assert json.loads(message.data or b'{}') == {'message': 'Check deployment', 'mode': 'plan', 'idempotency_key': 'message-key'}
    assert permission.full_url.endswith('/v1/profiles/operations/conversations/conversation%2Fone/permissions/permission%2Fone')
    assert json.loads(permission.data or b'{}') == {'decision': 'allow', 'idempotency_key': 'decision-key'}
...
def test_control_api_client_resumes_and_decodes_sse_events() -> None:
    ...
    def opener(request: Request, *, timeout: float):
        requests.append(request)
        return _StreamResponse([b': heartbeat\n', b'\n', b'id: event-2\n', b'event: model.delta\n', b'data: {"id":"event-2","event":{"name":"model.delta"}}\n', b'\n'])
    client = ControlApiClient('http://127.0.0.1:8765', 'secret-token', opener=opener)
    events = list(client.iter_events('default', 'conversation-1', after='event-1'))
    assert requests[0].get_header('Last-event-id') == 'event-1'
    assert requests[0].get_header('Accept') == 'text/event-stream'
    assert events == [{'id': 'event-2', 'event': 'model.delta', 'data': {'id': 'event-2', 'event': {'name': 'model.delta'}}}]
...
def test_control_api_client_shapes_profile_permission_inbox_request() -> None:
    ...
    assert requests[0].full_url.endswith('/v1/profiles/operations%2Fteam/permissions?status=pending&limit=25')

def test_control_api_client_decodes_stable_api_errors() -> None:

    def opener(_request: Request, *, timeout: float):
        assert timeout == 30.0
        raise HTTPError('http://127.0.0.1/v1/profiles/default/goals/missing', 404, 'not found', {}, BytesIO(json.dumps({'schema_version': 1, 'error': {'code': 'goal_not_found', 'message': 'goal is missing'}}).encode()))
    client = ControlApiClient('http://127.0.0.1', 'secret-token', opener=opener)
    try:
