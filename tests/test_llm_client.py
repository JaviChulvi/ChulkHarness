...
from chulk.core.actions import FinalAnswerAction, PlanAction, ToolCallAction, action_json_schema_for
from chulk.core.state import Plan, PlanStep
from chulk.llm import DeepSeekChatCompletionsClient, FallbackChain, LLMActionError, LLMActionResult, LLM_PROVIDER_REGISTRY, LLMCapabilities, LLMClient, LLMConfigurationError, LLMCost, LLMError, LLMResponse, LLMStreamChunk, LLMUsage, LocalOpenAICompatibleClient, MoonshotChatCompletionsClient, OpenAIResponsesClient, PlanningToolAvailability, create_llm_client, conservative_model_capabilities, resolve_model_capabilities
...
class FakeResponsesResource:
    ...
    def __init__(self, output_text: str='model answer', stream_events: list | None=None, usage=None, output: list | None=None, responses: list | None=None, fail_with_tools: bool=False) -> None:
...
class FakeOpenAIClient:
    ...
    def __init__(self, output_text: str='model answer', stream_events: list | None=None, usage=None, output: list | None=None, responses: list | None=None, fail_with_tools: bool=False) -> None:
        self.responses = FakeResponsesResource(output_text, stream_events=stream_events, usage=usage, output=output, responses=responses, fail_with_tools=fail_with_tools)
...
class FakeChatCompletionsResource:
    ...
    def __init__(self, content: str | None='deepseek answer', usage=None, tool_calls: list | None=None, fail_with_tools: bool=False) -> None:
    ...
    def create(self, **kwargs):
        ...
        if kwargs.get('tools') and self.fail_with_tools:
            raise RuntimeError('tools unsupported')
        return SimpleNamespace(choices=[SimpleNamespace(message=SimpleNamespace(content=self.content, tool_calls=self.tool_calls))], usage=self.usage)

class FakeDeepSeekClient:

    def __init__(self, content: str | None='deepseek answer', usage=None, tool_calls: list | None=None, fail_with_tools: bool=False) -> None:
        self.chat = SimpleNamespace(completions=FakeChatCompletionsResource(content, usage=usage, tool_calls=tool_calls, fail_with_tools=fail_with_tools))

class FakeLocalClient:

    def __init__(self, content: str | None='local answer', tool_calls: list | None=None, fail_with_tools: bool=False) -> None:
        self.chat = SimpleNamespace(completions=FakeChatCompletionsResource(content, tool_calls=tool_calls, fail_with_tools=fail_with_tools))
...
class FailingLLMClient(LLMClient):
    ...
    def complete(self, messages: list[dict[str, str]]) -> str:
        raise LLMError('provider unavailable', code='server_error', retryable=True, fallback_eligible=True)
...
class UsageFailingLLMClient(LLMClient):
    ...
    def complete_response(self, messages: list[dict[str, str]], *, max_output_tokens: int | None=None) -> LLMResponse:
        ...
        cost = LLMCost(amount=Decimal('0.000012'), pricing_known=True, estimated=False, provider=self.provider, model=self.model)
...
class UsageSuccessfulLLMClient(LLMClient):
    ...
    def complete_response(self, messages: list[dict[str, str]], *, max_output_tokens: int | None=None) -> LLMResponse:
        ...
        cost = LLMCost(amount=Decimal('0.000024'), pricing_known=True, estimated=False, provider=self.provider, model=self.model)
...
def fake_calculator_tool():
    return SimpleNamespace(name='calculator', description='Evaluate arithmetic.', args_schema={'type': 'object', 'properties': {'expression': {'type': 'string'}}, 'required': ['expression'], 'additionalProperties': False})

def fake_mcp_bridge_tool():
    return SimpleNamespace(name='mcp_docs_search_docs', description='Bridge docs search.', args_schema={'type': 'object', 'properties': {'query': {'type': 'string'}}, 'required': ['query'], 'additionalProperties': False}, metadata={'mcp_bridge': True})
...
def test_openai_responses_client_sends_instructions_and_input():
    ...
    response = client.complete([{'role': 'system', 'content': 'Be concise.'}, {'role': 'user', 'content': 'Hello'}, {'role': 'assistant', 'content': 'Hi'}, {'role': 'user', 'content': 'Continue'}])
    assert response == 'model answer'
    assert fake_client.responses.kwargs == {'model': 'test-model', 'instructions': 'Be concise.', 'input': [{'role': 'user', 'content': 'Hello'}, {'role': 'assistant', 'content': 'Hi'}, {'role': 'user', 'content': 'Continue'}]}
...
def test_openai_responses_client_streams_text_deltas():
    fake_client = FakeOpenAIClient(stream_events=[SimpleNamespace(type='response.created'), SimpleNamespace(type='response.output_text.delta', delta='hello '), SimpleNamespace(type='response.output_text.delta', delta='world'), SimpleNamespace(type='response.completed')])
    client = OpenAIResponsesClient(model='test-model', client=fake_client)
    chunks = list(client.stream_complete([{'role': 'user', 'content': 'Hello'}]))
    assert [chunk.text for chunk in chunks if chunk.type == 'text_delta'] == ['hello ', 'world']
    assert chunks[-1].type == 'completed'
    assert fake_client.responses.kwargs == {'model': 'test-model', 'instructions': None, 'input': [{'role': 'user', 'content': 'Hello'}], 'stream': True}

def test_openai_responses_client_extracts_usage_and_cost():
    usage = SimpleNamespace(input_tokens=1000, output_tokens=200, total_tokens=1200, input_tokens_details=SimpleNamespace(cached_tokens=100))
...
def test_openai_responses_client_extracts_cache_write_usage_and_cost():
    usage = SimpleNamespace(input_tokens=1000, output_tokens=200, total_tokens=1200, input_tokens_details=SimpleNamespace(cached_tokens=100, cache_write_tokens=200))
    fake_client = FakeOpenAIClient(usage=usage)
...
def test_openai_responses_client_uses_native_tool_calls_when_tools_are_provided():
    fake_client = FakeOpenAIClient(output_text='', output=[SimpleNamespace(type='function_call', name='calculator', arguments=json.dumps({'expression': '1 + 1'}), call_id='call_1')])
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
...
def test_openai_rejects_multiple_function_calls_and_uses_json_fallback():
    fallback_payload = json.dumps({'type': 'final_answer', 'content': 'safe fallback', 'tool_name': None, 'arguments_json': '{}', 'plan_json': '{}', 'step_update_json': '{}'})
    fake_client = FakeOpenAIClient(responses=[SimpleNamespace(id='resp_parallel', output_text='', usage=None, output=[SimpleNamespace(type='function_call', name='calculator', arguments=json.dumps({'expression': '1 + 1'}), call_id='call_1'), SimpleNamespace(type='function_call', name='calculator', arguments=json.dumps({'expression': '2 + 2'}), call_id='call_2')]), SimpleNamespace(id='resp_fallback', output_text=fallback_payload, usage=None, output=[])])
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'calculate both'}], tools=[fake_calculator_tool()])
...
def test_openai_native_text_with_no_effective_tools_omits_tool_fields():
    ...
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[], planning_tools=PlanningToolAvailability())
...
def test_openai_responses_client_sends_hosted_mcp_tools_and_filters_bridge_tools():
    fake_client = FakeOpenAIClient(output_text='native final')
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com', server_description='Docs MCP', allowed_tools=('search_docs',), authorization='secret-token')
    result = client.complete_action([{'role': 'user', 'content': 'search docs'}], tools=[fake_calculator_tool(), fake_mcp_bridge_tool()], hosted_mcp_servers=[server])
    ...
    assert {'type': 'mcp', 'server_label': 'docs', 'server_url': 'https://mcp.example.com', 'require_approval': 'always', 'server_description': 'Docs MCP', 'allowed_tools': ['search_docs'], 'authorization': 'secret-token'} in request_tools

def test_openai_responses_client_continues_after_mcp_approval_request():
    fake_client = FakeOpenAIClient(responses=[SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))]), SimpleNamespace(id='resp_2', output_text='approved answer', usage=None, output=[])])
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    approvals = []
    result = client.complete_action([{'role': 'user', 'content': 'search docs'}], hosted_mcp_servers=[MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')], mcp_approval_callback=lambda approval: approvals.append(approval) or True)
    ...
    assert continuation['input'] == [{'type': 'mcp_approval_response', 'approval_request_id': 'approval_1', 'approve': True}]
    assert result.metadata['provider_mcp_approval'] == [{'approval_request_id': 'approval_1', 'server_label': 'docs', 'name': 'search_docs', 'approved': True}]

def test_openai_mcp_approval_continuation_keeps_planning_contract_and_fails_closed_on_text():
    fake_client = FakeOpenAIClient(responses=[SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))]), SimpleNamespace(id='resp_2', output_text='phase-illegal final text', usage=None, output=[])])
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'must not run'})])
    ...
    try:
        fallback.complete_action([{'role': 'system', 'content': 'Finish with a plan step update.'}, {'role': 'user', 'content': 'Search the docs and update the step.'}], planning_tools=PlanningToolAvailability(update_plan_step=True), hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
...
def test_openai_hosted_mcp_approval_requires_callback():
    fake_client = FakeOpenAIClient(responses=[SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))])])
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    try:
        client.complete_action([{'role': 'user', 'content': 'search docs'}], tools=[], hosted_mcp_servers=[MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')])
    except LLMError as exc:
...
def test_openai_hosted_mcp_shape_error_advances_fallback_chain():
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=FakeOpenAIClient(output_text='', output=[]))
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'fallback ok'})])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')
    result = FallbackChain([primary, secondary]).complete_action([{'role': 'user', 'content': 'search docs'}], tools=[], planning_tools=PlanningToolAvailability(), hosted_mcp_servers=[server])
...
def test_openai_hosted_mcp_call_without_final_action_does_not_replay_on_fallback():
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=FakeOpenAIClient(output_text='', output=[SimpleNamespace(type='mcp_call', id='call_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}), output='created', status='completed')]))
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'must not run'})])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='write_api', transport='streamable_http', server_url='https://mcp.example.com')
    try:
        fallback.complete_action([{'role': 'user', 'content': 'create a record'}], tools=[], hosted_mcp_servers=[server])
    except LLMError as exc:
        assert exc.code == 'action_shape_error'
        assert exc.retryable is False
        assert exc.fallback_eligible is False
...
def test_openai_timeout_after_approved_mcp_call_does_not_replay_on_fallback():
    fake_client = RetryOptionsRecordingOpenAIClient(responses=[SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id='approval_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}))]), TimeoutError('response timed out after approval')])
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'must not run'})])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='write_api', transport='streamable_http', server_url='https://mcp.example.com')
    try:
        fallback.complete_action([{'role': 'user', 'content': 'create a record'}], tools=[], hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
    except LLMError as exc:
...
def test_openai_pre_execution_mcp_rejection_can_advance_fallback():
    fake_client = RetryOptionsRecordingOpenAIClient(responses=[LLMError('hosted MCP tools are unsupported', provider='openai', model='gpt-4.1-mini', code='unsupported_feature', fallback_eligible=True)])
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'fallback ok'})])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com', approval='never')
    result = FallbackChain([primary, secondary]).complete_action([{'role': 'user', 'content': 'search docs'}], tools=[], hosted_mcp_servers=[server])
    assert result.action == FinalAnswerAction(type='final_answer', content='fallback ok')
    assert fake_client.retry_options == [{'max_retries': 0}]
    assert len(secondary.requests) == 1

def test_openai_mcp_execution_blocks_action_repair_and_fallback():
    fake_client = RetryOptionsRecordingOpenAIClient(output_text='', output=[SimpleNamespace(type='mcp_call', id='call_1', server_label='write_api', name='create_record', arguments=json.dumps({'value': 'created'}), output='created', status='completed'), SimpleNamespace(type='function_call', name=PLAN_TOOL_NAME, arguments='{}')])
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'must not run'})])
    fallback = FallbackChain([primary, secondary])
    server = MCPServerConfig(label='write_api', transport='streamable_http', server_url='https://mcp.example.com', approval='never')
    try:
        fallback.complete_action([{'role': 'user', 'content': 'create a record'}], tools=[], hosted_mcp_servers=[server])
...
def test_openai_malformed_hosted_mcp_approval_advances_fallback_chain():
    primary = OpenAIResponsesClient(model='gpt-4.1-mini', client=FakeOpenAIClient(responses=[SimpleNamespace(id='resp_1', output_text='', usage=None, output=[SimpleNamespace(type='mcp_approval_request', id=None, server_label='docs', name='search_docs', arguments=json.dumps({'query': 'MCP'}))])]))
    secondary = ScriptedLLMClient([json.dumps({'type': 'final_answer', 'content': 'fallback ok'})])
    server = MCPServerConfig(label='docs', transport='streamable_http', server_url='https://mcp.example.com')
    fallback = FallbackChain([primary, secondary])
    result = fallback.complete_action([{'role': 'user', 'content': 'search docs'}], tools=[], hosted_mcp_servers=[server], mcp_approval_callback=lambda _approval: True)
    assert result.action == FinalAnswerAction(type='final_answer', content='fallback ok')
...
def test_openai_responses_native_plan_tool_becomes_plan_action():
    fake_client = FakeOpenAIClient(output_text='', output=[SimpleNamespace(type='function_call', name=PLAN_TOOL_NAME, arguments=json.dumps({'summary': 'Make a change.', 'steps': [{'id': '1', 'title': 'Edit file', 'description': 'Update the file.', 'status': 'pending', 'depends_on': [], 'acceptance_criteria': ['File is updated.'], 'retry_limit': 0}]}))])
    client = OpenAIResponsesClient(model='gpt-4.1-mini', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'plan this'}], planning_tools=PlanningToolAvailability(propose_plan=True))
    ...
    assert [tool['name'] for tool in fake_client.responses.kwargs['tools']] == [PLAN_TOOL_NAME]
...
def test_fallback_chain_streams_from_first_successful_provider():
    fallback = FallbackChain([FailingLLMClient(), ScriptedStreamingLLMClient([LLMStreamChunk(type='text_delta', text='fallback '), LLMStreamChunk(type='text_delta', text='worked'), LLMStreamChunk(type='completed')])])
...
def test_fallback_chain_keeps_action_attempts_across_json_repair():
    provider = ScriptedLLMClient(['plain prose', json.dumps({'type': 'final_answer', 'content': 'repaired'})])
...
def test_fallback_chain_honors_a_custom_public_complete_action():
    ...
    class PublicActionClient(LLMClient):
        ...
        def complete_action(self, messages, *, planning_tools=None, **kwargs):
            ...
            return LLMActionResult(action=FinalAnswerAction(type='final_answer', content='custom action'), raw_response='{"type":"custom"}')
    provider = PublicActionClient()
    policy = PlanningToolAvailability(propose_plan=True)
    result = FallbackChain([provider]).complete_action([{'role': 'user', 'content': 'hello'}], planning_tools=policy)
...
def test_fallback_chain_serializes_custom_plan_action_as_a_strict_proposal():
    plan = Plan(summary='Inspect and update the file.', steps=[PlanStep(id='inspect', title='Inspect', description='Read the current implementation.', acceptance_criteria=['The implementation is understood.']), PlanStep(id='update', title='Update', description='Apply the requested change.', depends_on=['inspect'], acceptance_criteria=['The requested behavior is implemented.'], retry_limit=1)])

    class PublicPlanActionClient(LLMClient):

        def complete_action(self, messages, **kwargs):
            return LLMActionResult(action=PlanAction(type='plan', plan=plan), raw_response='{"type":"custom-plan"}')
    result = FallbackChain([PublicPlanActionClient()]).complete_action([{'role': 'user', 'content': 'Plan this'}], action_schema=action_json_schema_for(['plan']))
...
def test_custom_public_action_provider_does_not_collapse_other_provider_repairs():
    ...
    scripted = ScriptedLLMClient(['plain prose', json.dumps({'type': 'final_answer', 'content': 'repaired'})])
...
def test_openai_responses_client_uses_strict_action_schema():
    fake_client = FakeOpenAIClient(json.dumps({'type': 'final_answer', 'content': 'structured answer', 'tool_name': None, 'arguments_json': '{}'}))
    client = OpenAIResponsesClient(model='test-model', client=fake_client)
    result = client.complete_action([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Hello'}])
...
def test_openai_responses_client_uses_phase_narrowed_action_schema():
    plan_payload = {'summary': 'Make the change.', 'steps': [{'id': '1', 'title': 'Make the change', 'description': 'Complete the requested work.', 'status': 'pending'}]}
    fake_client = FakeOpenAIClient(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps(plan_payload), 'step_update_json': '{}'}))
    client = OpenAIResponsesClient(model='test-model', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'Plan this'}], action_schema=action_json_schema_for(['plan']))
...
def test_openai_responses_client_applies_output_limits():
    ...
    client.complete_action([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Hello'}], max_output_tokens=250)
...
def test_deepseek_client_sends_chat_completion_messages():
    ...
    assert fake_client.chat.completions.kwargs == {'model': 'deepseek-v4-flash', 'messages': [{'role': 'system', 'content': 'Be concise.'}, {'role': 'user', 'content': 'Hello'}, {'role': 'assistant', 'content': 'Hi'}, {'role': 'user', 'content': 'Continue'}], 'stream': False}

def test_deepseek_client_uses_native_tool_calls_when_tools_are_provided():
    fake_client = FakeDeepSeekClient(content=None, tool_calls=[SimpleNamespace(id='call_1', type='function', function=SimpleNamespace(name='calculator', arguments=json.dumps({'expression': '2 + 2'})))])
    client = DeepSeekChatCompletionsClient(model='deepseek-v4-flash', client=fake_client)
...
def test_deepseek_client_receives_mcp_bridge_tools_as_function_tools():
    fake_client = FakeDeepSeekClient(content=None, tool_calls=[SimpleNamespace(id='call_mcp', type='function', function=SimpleNamespace(name='mcp_docs_search_docs', arguments=json.dumps({'query': 'MCP'})))])
    client = DeepSeekChatCompletionsClient(model='deepseek-v4-flash', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'search docs'}], tools=[fake_mcp_bridge_tool()])
    assert result.action == ToolCallAction(type='tool_call', tool_name='mcp_docs_search_docs', arguments={'query': 'MCP'})
    tool_names = {tool['function']['name'] for tool in fake_client.chat.completions.kwargs['tools']}
...
def test_deepseek_client_extracts_cache_usage_and_cost():
    usage = SimpleNamespace(prompt_tokens=3000, completion_tokens=500, total_tokens=3500, prompt_cache_hit_tokens=1000, prompt_cache_miss_tokens=2000)
...
def test_deepseek_client_uses_json_object_mode_for_actions():
    fake_client = FakeDeepSeekClient(json.dumps({'type': 'final_answer', 'content': 'structured answer', 'tool_name': None, 'arguments_json': '{}'}))
    client = DeepSeekChatCompletionsClient(model='deepseek-v4-flash', client=fake_client)
    result = client.complete_action([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Hello'}])
    assert result.action.content == 'structured answer'
...
def test_deepseek_client_can_parse_plan_action_from_json_mode():
    plan_json = json.dumps({'summary': 'Inspect before editing.', 'steps': [{'id': '1', 'title': 'Inspect files', 'description': 'List relevant files before editing.', 'status': 'pending'}]})
    fake_client = FakeDeepSeekClient(json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': plan_json}))
    client = DeepSeekChatCompletionsClient(model='deepseek-v4-flash', client=fake_client)
    result = client.complete_action([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Inspect the files'}])
...
def test_local_client_sends_local_template_friendly_chat_completion_messages():
    ...
    assert fake_client.chat.completions.kwargs == {'model': 'google/gemma-4-12b-qat', 'messages': [{'role': 'user', 'content': 'Hello'}, {'role': 'assistant', 'content': 'Hi'}, {'role': 'user', 'content': 'Instructions:\nBe concise.\n\nUser message:\n\nContinue'}], 'stream': False}
...
def test_local_client_guarantees_user_message_for_system_only_requests():
    ...
    assert fake_client.chat.completions.kwargs['messages'] == [{'role': 'user', 'content': 'Instructions:\nReturn action JSON.'}]

def test_local_client_converts_observations_to_user_messages():
    fake_client = FakeLocalClient()
    client = LocalOpenAICompatibleClient(model='google/gemma-4-12b-qat', client=fake_client)
    client.complete([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Find the file.'}, {'role': 'observation', 'content': 'Tool read_file finished with success.'}])
    assert fake_client.chat.completions.kwargs['messages'] == [{'role': 'user', 'content': 'Instructions:\nReturn action JSON.\n\nUser message:\n\nFind the file.\n\nobservation: Tool read_file finished with success.'}]

def test_local_client_actions_use_plain_chat_completion_contract():
    fake_client = FakeLocalClient(json.dumps({'type': 'final_answer', 'content': 'structured answer', 'tool_name': None, 'arguments_json': '{}'}))
    client = LocalOpenAICompatibleClient(model='google/gemma-4-12b-qat', client=fake_client)
    result = client.complete_action([{'role': 'system', 'content': 'Return action JSON.'}, {'role': 'user', 'content': 'Hello'}], max_output_tokens=250)
    assert result.action.content == 'structured answer'
    assert fake_client.chat.completions.kwargs['max_tokens'] == 250
    assert 'response_format' not in fake_client.chat.completions.kwargs
    assert fake_client.chat.completions.kwargs['messages'] == [{'role': 'user', 'content': 'Instructions:\nReturn action JSON.\n\nUser message:\n\nHello'}]
...
def test_local_client_uses_native_tool_calls_when_tools_are_provided():
    fake_client = FakeLocalClient(content=None, tool_calls=[SimpleNamespace(id='call_local', type='function', function=SimpleNamespace(name='calculator', arguments=json.dumps({'expression': '3 + 4'})))])
    client = LocalOpenAICompatibleClient(model='google/gemma-4-12b-qat', client=fake_client)
...
def test_local_client_receives_mcp_bridge_tools_as_function_tools():
    fake_client = FakeLocalClient(content=None, tool_calls=[SimpleNamespace(id='call_local_mcp', type='function', function=SimpleNamespace(name='mcp_docs_search_docs', arguments=json.dumps({'query': 'local MCP'})))])
    client = LocalOpenAICompatibleClient(model='google/gemma-4-12b-qat', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'search docs'}], tools=[fake_mcp_bridge_tool()])
    assert result.action == ToolCallAction(type='tool_call', tool_name='mcp_docs_search_docs', arguments={'query': 'local MCP'})
    tool_names = {tool['function']['name'] for tool in fake_client.chat.completions.kwargs['tools']}
    assert 'mcp_docs_search_docs' in tool_names

def test_local_client_falls_back_to_json_when_native_tools_are_rejected():
    fake_client = FakeLocalClient(json.dumps({'type': 'final_answer', 'content': 'fallback answer', 'tool_name': None, 'arguments_json': '{}'}), fail_with_tools=True)
    client = LocalOpenAICompatibleClient(model='google/gemma-4-12b-qat', client=fake_client)
    result = client.complete_action([{'role': 'user', 'content': 'hello'}], tools=[fake_calculator_tool()])
...
def test_llm_client_repairs_invalid_action_json():
    client = ScriptedLLMClient(['plain prose', json.dumps({'type': 'final_answer', 'content': 'repaired'})])
...
def test_llm_client_repairs_pre_advanced_plan_steps():
    ...
    def plan_response(status: str) -> str:
        return json.dumps({'type': 'plan', 'content': None, 'tool_name': None, 'arguments_json': '{}', 'plan_json': json.dumps({'summary': 'Make the change.', 'steps': [{'id': '1', 'title': 'Make the change', 'description': 'Complete the requested work.', 'status': status}]}), 'step_update_json': '{}'})
    client = ScriptedLLMClient([plan_response('completed'), plan_response('pending')])
    result = client.complete_action([{'role': 'user', 'content': 'Plan this'}], max_repair_attempts=1)
    assert isinstance(result.action, PlanAction)
    assert result.action.plan.steps[0].status == 'pending'
    assert result.repair_attempts == 1
...
def test_llm_client_replaces_prior_repair_instead_of_accumulating_it():
    client = ScriptedLLMClient(['FIRST_INVALID_RESPONSE', 'SECOND_INVALID_RESPONSE', json.dumps({'type': 'final_answer', 'content': 'repaired'})])
    messages = [{'role': 'system', 'content': '<response_protocol>Existing action contract.</response_protocol>'}, {'role': 'user', 'content': 'Hello'}]
...
def test_create_llm_client_selects_deepseek_provider():
    client = create_llm_client(provider='deepseek', model='deepseek-v4-flash', openai_api_key=None, deepseek_api_key='deepseek-key', deepseek_base_url='https://api.deepseek.com', timeout_seconds=60, max_retries=2)
    assert isinstance(client, DeepSeekChatCompletionsClient)

def test_create_llm_client_selects_moonshot_provider():
    client = create_llm_client(provider='moonshot', model='kimi-k3', moonshot_api_key='moonshot-key', moonshot_base_url='https://api.moonshot.ai/v1', timeout_seconds=60, max_retries=2)
    assert isinstance(client, MoonshotChatCompletionsClient)
    assert client.model_capabilities.context_window_tokens == 1048576

def test_create_llm_client_selects_local_provider():
    client = create_llm_client(provider='local', model='google/gemma-4-12b-qat', openai_api_key=None, deepseek_api_key=None, deepseek_base_url='https://api.deepseek.com', local_api_key=None, local_base_url='http://localhost:1234/v1', timeout_seconds=60, max_retries=2)
    assert isinstance(client, LocalOpenAICompatibleClient)
    assert client.base_url == 'http://localhost:1234/v1'
    assert client.model_capabilities.context_window_tokens == 131072
...
def test_resolve_model_capabilities_returns_context_window_and_reserve():
    ...
    assert openai_caps.context_window_tokens == 1047576
    assert openai_caps.default_response_reserve_tokens == 8192
    assert openai_caps.max_input_tokens is None
    assert openai_caps.max_output_tokens == 32768
    assert openai_caps.input_budget_tokens == 1039384
    assert deepseek_caps.context_window_tokens == 1048576
    assert deepseek_caps.default_response_reserve_tokens == 16384
    assert deepseek_caps.max_input_tokens is None
    assert deepseek_caps.max_output_tokens == 393216
    assert deepseek_caps.input_budget_tokens == 1032192
    assert moonshot_caps.context_window_tokens == 1048576
    assert moonshot_caps.default_response_reserve_tokens == 131072
    assert moonshot_caps.max_output_tokens == 1048576
    assert moonshot_caps.input_budget_tokens == 917504
    assert local_caps.context_window_tokens == 262144
    assert local_caps.default_response_reserve_tokens == 4096
    assert local_qwen_caps.context_window_tokens == 262144
    assert local_qwen_caps.default_response_reserve_tokens == 4096
...
def test_conservative_model_capabilities_use_smallest_context_and_largest_reserve():
    ...
    assert combined.context_window_tokens == 262144
    assert combined.default_response_reserve_tokens == 8192
    assert combined.input_budget_tokens == 253952
...
def test_fallback_chain_exposes_conservative_bound_model_capabilities():
    ...
    assert capabilities.context_window_tokens == 262144
    assert capabilities.default_response_reserve_tokens == 8192

def test_factory_attaches_model_capabilities_to_bound_client():
    client = create_llm_client(provider='local', model='custom-model', openai_api_key=None, deepseek_api_key=None, deepseek_base_url='https://api.deepseek.com', local_api_key='local', local_base_url='http://localhost:1234/v1', timeout_seconds=1, max_retries=0)
    assert client.model_capabilities.context_window_tokens == 131072

def test_factory_applies_local_context_before_fallback_aggregation():
    local = create_llm_client(provider='local', model='google/gemma-4-12b-qat', local_api_key='local', local_base_url='http://localhost:1234/v1', local_context_window_tokens=65536, timeout_seconds=1, max_retries=0)
    ...
    assert local.model_capabilities.context_window_tokens == 65536
    assert capabilities is not None
    assert capabilities.context_window_tokens == 65536

def test_runtime_local_context_uses_deployment_limit_for_unknown_models():
    capabilities = resolve_runtime_model_capabilities('local', 'custom-model', local_context_window_tokens=262144)
    assert capabilities.context_window_tokens == 262144

def test_runtime_local_context_never_exceeds_known_architecture_limit():
    capabilities = resolve_runtime_model_capabilities('local', 'gemma3:12b', local_context_window_tokens=262144)
    assert capabilities.context_window_tokens == 131072

def test_runtime_local_context_rejects_zero_input_budget():
    try:
        resolve_runtime_model_capabilities('local', 'custom-model', local_context_window_tokens=4096)
...
def test_resolve_model_capabilities_supports_known_family_aliases():
    ...
    assert caps.context_window_tokens == 1047576
...
def test_resolve_model_capabilities_uses_conservative_defaults_for_local_models():
    ...
    assert caps.context_window_tokens == 131072
