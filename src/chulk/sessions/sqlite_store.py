...
class SQLiteSessionStore:
    ...
    def get_conversation(self, conversation_id_or_prefix: str) -> ConversationRecord:
        ...
        with self._connect() as conn:
            exact = conn.execute(_conversation_select_sql('WHERE conversations.id = ?'), (clean_id,)).fetchone()
            if exact is not None:
                return _row_to_conversation(exact)
            rows = conn.execute(_conversation_select_sql('WHERE conversations.id LIKE ? ORDER BY conversations.updated_at DESC'), (f'{clean_id}%',)).fetchall()
        if not rows:
    ...
    def list_conversations(self, limit: int=20) -> list[ConversationRecord]:
        ...
        with self._connect() as conn:
            rows = conn.execute(_conversation_select_sql('ORDER BY conversations.updated_at DESC LIMIT ?'), (clean_limit,)).fetchall()
        return [_row_to_conversation(row) for row in rows]

    def find_conversation_by_metadata(self, key: str, value: str | int) -> ConversationRecord | None:
        """Return the latest conversation whose metadata has an exact scalar value."""
        clean_key = key.strip()
        if not clean_key:
            raise ValueError('Metadata key cannot be empty')
        with self._connect() as conn:
            rows = conn.execute(_conversation_select_sql('ORDER BY conversations.updated_at DESC')).fetchall()
        for row in rows:
    ...
    def get_adapter_cursor(self, adapter: str) -> int | None:
        ...
        with self._connect() as conn:
            row = conn.execute('SELECT cursor FROM adapter_cursors WHERE adapter = ?', (clean_adapter,)).fetchone()
    ...
    def latest_conversation(self, *, require_turn: bool=True) -> ConversationRecord | None:
        ...
        where = 'WHERE EXISTS (SELECT 1 FROM conversation_turns WHERE conversation_turns.conversation_id = conversations.id)' if require_turn else ''
        with self._connect() as conn:
            row = conn.execute(_conversation_select_sql(f'{where} ORDER BY conversations.updated_at DESC LIMIT 1')).fetchone()
    ...
    def save_message(self, conversation_id: str, *, role: str, content: str, turn_id: str | None=None, message_key: str | None=None, metadata: dict[str, Any] | None=None, created_at: str | None=None) -> None:
        ...
        with self._connect() as conn:
            conn.execute('BEGIN IMMEDIATE')
            _insert_message(conn, conversation_id, turn_id=turn_id, role=role, content=clean_content, message_key=key, metadata=metadata, created_at=now, search_enabled=self.fts_enabled)
    ...
    def list_messages(self, conversation_id: str, *, limit: int=50, after_ordinal: int=0) -> list[MessageRecord]:
        ...
        with self._connect() as conn:
            rows = conn.execute('\n                SELECT *\n                FROM conversation_messages\n                WHERE conversation_id = ?\n                  AND ordinal > ?\n                ORDER BY ordinal DESC\n                LIMIT ?\n                ', (conversation_id, clean_after_ordinal, clean_limit)).fetchall()
        return [_row_to_message(row) for row in reversed(rows)]

    def update_message(self, conversation_id: str, message_id: str, *, content: str | None=None, metadata: dict[str, Any] | None=None) -> bool:
        ...
        with self._connect() as conn:
            ...
            if row is None:
                return False
            next_content = clean_content if clean_content is not None else str(row['content'])
            next_metadata = dict(metadata) if metadata is not None else _safe_json_dict(row['metadata'])
            conn.execute('\n                UPDATE conversation_messages\n                SET content = ?, metadata = ?\n                WHERE conversation_id = ? AND id = ?\n                ', (next_content, json.dumps(next_metadata, sort_keys=True), conversation_id, message_id))
            _replace_session_fts(conn, enabled=self.fts_enabled, message_id=message_id, conversation_id=conversation_id, role=str(row['role']), content=next_content, metadata=next_metadata)
            _touch_conversation(conn, conversation_id, now)
        return True
    ...
    def delete_message(self, conversation_id: str, message_id: str) -> bool:
        ...
        with self._connect() as conn:
            ...
            if row is None:
                return False
            if self.fts_enabled:
                conn.execute('DELETE FROM session_messages_fts WHERE message_id = ?', (message_id,))
    ...
    def save_terminal_turn_bundle(self, conversation_id: str, *, turn_id: str, content: str, message_key: str, turn: dict[str, Any], metadata: dict[str, Any] | None=None) -> bool:
        ...
        clean_content = content.strip()
        if not clean_content or not message_key.strip() or (not _valid_turn_snapshot(turn, expected_turn_id=turn_id)):
            return False
        now = _utc_now()
        with self._connect() as conn:
            conn.execute('BEGIN IMMEDIATE')
            _insert_message(conn, conversation_id, turn_id=turn_id, role='assistant', content=clean_content, message_key=message_key, metadata=metadata, created_at=now, search_enabled=self.fts_enabled)
    ...
    def load_terminal_turn_message(self, conversation_id: str, turn_id: str) -> dict[str, str] | None:
        ...
        message_keys = {f'{turn_id}:assistant:final': 'final', f'{turn_id}:assistant:failed': 'failed', f'{turn_id}:assistant:plan_rejected': 'plan_rejected'}
        with self._connect() as conn:
            row = conn.execute('\n                SELECT content, message_key\n                FROM conversation_messages\n                WHERE conversation_id = ?\n                  AND turn_id = ?\n                  AND message_key IN (?, ?, ?)\n                ORDER BY ordinal DESC\n                LIMIT 1\n                ', (conversation_id, turn_id, *message_keys)).fetchone()
        if row is None:
            return None
    ...
    def load_recent_messages(self, conversation_id: str, limit: int, *, after_ordinal: int=0) -> list[dict[str, str]]:
        ...
        clean_after_ordinal = max(0, after_ordinal)
        with self._connect() as conn:
            rows = conn.execute('\n                SELECT role, content, metadata\n                FROM conversation_messages\n                WHERE conversation_id = ?\n                  AND ordinal > ?\n                ORDER BY ordinal\n                ', (conversation_id, clean_after_ordinal)).fetchall()
        messages = [{'role': str(row['role']), 'content': str(row['content'])} for row in rows if not _message_is_prompt_excluded(row['metadata'])]
        return select_recent_conversation_messages(messages, max_messages=clean_limit)

    def save_conversation_summary(self, conversation_id: str, *, content: str, source_message_count: int, metadata: dict[str, Any] | None=None) -> None:
    ...
    def load_uncheckpointed_hosted_mcp_requests(self, conversation_id: str, turn_id: str, *, checkpointed_request_count: int) -> list[dict[str, object]]:
        ...
        with self._connect() as conn:
            rows = conn.execute('\n                SELECT request_index, request_json, created_at, response_created_at\n                FROM conversation_model_requests\n                WHERE conversation_id = ?\n                  AND turn_id = ?\n                  AND request_index > ?\n                ORDER BY request_index\n                ', (conversation_id, turn_id, max(0, checkpointed_request_count))).fetchall()
        requests: list[dict[str, object]] = []
        for row in rows:
            payload = _safe_json_dict(row['request_json'])
            if payload.get('hosted_mcp_enabled') is not True:
                continue
            requests.append({'request_index': int(row['request_index']), 'server_labels': payload.get('hosted_mcp_server_labels') or [], 'created_at': str(row['created_at']), 'response_recorded': row['response_created_at'] is not None})
        return requests
    ...
    def save_tool_call(self, conversation_id: str, payload: dict[str, Any]) -> None:
        ...
        with self._connect() as conn:
            conn.execute('BEGIN IMMEDIATE')
            conn.execute('\n                INSERT INTO conversation_tool_calls (\n                    id, conversation_id, turn_id, tool_name, resolved_tool_name, arguments, iteration,\n                    phase, started_at, ended_at, success, error, metadata, tool_call_json\n                )\n                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n                ON CONFLICT(conversation_id, turn_id, phase, iteration) DO UPDATE SET\n                    tool_name = excluded.tool_name,\n                    resolved_tool_name = excluded.resolved_tool_name,\n                    arguments = excluded.arguments,\n                    started_at = excluded.started_at,\n                    ended_at = excluded.ended_at,\n                    success = excluded.success,\n                    error = excluded.error,\n                    metadata = excluded.metadata,\n                    tool_call_json = excluded.tool_call_json\n                ', (str(uuid4()), conversation_id, turn_id, tool_name, payload.get('resolved_tool_name'), json.dumps(payload.get('arguments') or {}, sort_keys=True), iteration, phase, str(payload.get('started_at') or now), payload.get('ended_at'), _optional_bool_to_int(payload.get('success')), payload.get('error'), json.dumps(payload.get('metadata') or {}, sort_keys=True), json.dumps(payload, sort_keys=True)))
            if isinstance(turn, dict) and _valid_turn_snapshot(turn, expected_turn_id=turn_id):
                _save_turn_snapshot(conn, conversation_id, turn, now)

    def load_tool_calls_without_observations(self, conversation_id: str, turn_id: str) -> list[dict[str, object]]:
        """Return persisted tool calls without a matching durable observation."""
        with self._connect() as conn:
            call_rows = conn.execute('\n                SELECT tool_name, arguments, iteration, phase, started_at,\n                       ended_at, success\n                FROM conversation_tool_calls\n                WHERE conversation_id = ?\n                  AND turn_id = ?\n                ORDER BY iteration\n                ', (conversation_id, turn_id)).fetchall()
            observation_rows = conn.execute('\n                SELECT tool_name, output_metadata\n                FROM conversation_observations\n                WHERE conversation_id = ?\n                  AND turn_id = ?\n                ORDER BY observation_key\n                ', (conversation_id, turn_id)).fetchall()
        calls = [{'tool_name': str(row['tool_name']), 'arguments': _safe_json_dict(row['arguments']), 'iteration': int(row['iteration']), 'phase': str(row['phase']), 'started_at': str(row['started_at']), 'ended_at': row['ended_at'], 'success': None if row['success'] is None else bool(row['success'])} for row in call_rows]
        ...
        for row in observation_rows:
            ...
            if isinstance(phase, str) and phase and isinstance(iteration, int) and (not isinstance(iteration, bool)) and (iteration > 0):
    ...
    def save_observation(self, conversation_id: str, *, turn_id: str, tool_name: str, content: str, output_metadata: dict[str, Any] | None=None, observation_key: str | None=None) -> None:
        ...
        now = _utc_now()
        with self._connect() as conn:
            conn.execute('\n                INSERT OR IGNORE INTO conversation_observations (\n                    id, conversation_id, turn_id, tool_name, content, output_metadata, observation_key, created_at\n                )\n                VALUES (?, ?, ?, ?, ?, ?, ?, ?)\n                ', (str(uuid4()), conversation_id, turn_id, tool_name, clean_content, json.dumps(output_metadata or {}, sort_keys=True), key, now))

    def save_tool_observation_bundle(self, conversation_id: str, *, turn_id: str, observation_index: int, tool_name: str, content: str, output_metadata: dict[str, Any] | None=None, action_context: str | None=None, turn: dict[str, Any] | None=None) -> None:
        """Atomically persist one tool action, observation, and turn checkpoint."""
        if isinstance(observation_index, bool) or not isinstance(observation_index, int) or observation_index < 1:
        ...
        with self._connect() as conn:
            ...
            if isinstance(action_context, str) and action_context.strip():
                _insert_message(conn, conversation_id, turn_id=turn_id, role='assistant', content=action_context.strip(), message_key=f'{turn_id}:tool_action:{observation_index}', metadata={'tool_name': tool_name, 'internal': True, 'event': 'tool_observation', 'observation_index': observation_index}, created_at=now, search_enabled=self.fts_enabled)
            observation_insert = conn.execute('\n                INSERT OR IGNORE INTO conversation_observations (\n                    id, conversation_id, turn_id, tool_name, content, output_metadata,\n                    observation_key, created_at\n                )\n                VALUES (?, ?, ?, ?, ?, ?, ?, ?)\n                ', (str(uuid4()), conversation_id, turn_id, tool_name, clean_content, json.dumps(output_metadata or {}, sort_keys=True), observation_key, now))
            _insert_message(conn, conversation_id, turn_id=turn_id, role='observation', content=clean_content, message_key=observation_key, metadata={'tool_name': tool_name, 'observation_index': observation_index}, created_at=now, search_enabled=self.fts_enabled)
            if observation_insert.rowcount == 1 and isinstance(turn, dict) and _valid_turn_snapshot(turn, expected_turn_id=turn_id):
                _save_turn_snapshot(conn, conversation_id, turn, now)
            else:
                _touch_conversation(conn, conversation_id, now)
...
def _row_to_conversation(row: sqlite3.Row) -> ConversationRecord:
    return ConversationRecord(id=row['id'], title=row['title'], provider=row['provider'], model=row['model'], trace_path=row['trace_path'], status=row['status'], created_at=row['created_at'], updated_at=row['updated_at'], metadata=_safe_json_dict(row['metadata']), turn_count=int(row['turn_count'] or 0))

def _row_to_message(row: sqlite3.Row) -> MessageRecord:
    return MessageRecord(id=row['id'], conversation_id=row['conversation_id'], turn_id=row['turn_id'], role=row['role'], content=row['content'], ordinal=int(row['ordinal']), created_at=row['created_at'], metadata=_safe_json_dict(row['metadata']))

def _row_to_summary(row: sqlite3.Row) -> ConversationSummaryRecord:
    return ConversationSummaryRecord(id=row['id'], conversation_id=row['conversation_id'], content=row['content'], source_message_count=int(row['source_message_count'] or 0), created_at=row['created_at'], updated_at=row['updated_at'], metadata=_safe_json_dict(row['metadata']))

def _turn_from_dict(payload: dict[str, Any]) -> TurnState:
    from chulk.resources import ApplicationEventIntent, HostResource
    turn = TurnState(user_message=str(payload.get('user_message') or ''), turn_id=str(payload.get('turn_id') or uuid4()), started_at=str(payload.get('started_at') or _utc_now()), ended_at=payload.get('ended_at'), status=str(payload.get('status') or 'completed'), model_request_count=int(payload.get('model_request_count') or 0), tool_call_count=int(payload.get('tool_call_count') or 0), available_tool_names=_safe_string_list(payload.get('available_tool_names')), context_sections=[_context_section_from_dict(item) for item in _safe_dict_list(payload.get('context_sections'))], resources=[HostResource.from_dict(item) for item in _safe_dict_list(payload.get('resources'))], application_events=[ApplicationEventIntent.from_dict(item) for item in _safe_dict_list(payload.get('application_events'))], prompt_profile=payload.get('prompt_profile'), locale=payload.get('locale'), input_parts=_safe_dict_list(payload.get('input_parts')), extension_metadata=_safe_json_object(payload.get('extension_metadata')), tool_context_metadata=_safe_json_object(payload.get('tool_context_metadata')), loaded_memory_ids=_safe_string_list(payload.get('loaded_memory_ids')), extracted_memory_ids=_safe_string_list(payload.get('extracted_memory_ids')), loaded_skill_names=_safe_string_list(payload.get('loaded_skill_names')), errors=_safe_string_list(payload.get('errors')), final_answer=payload.get('final_answer'), active_plan=_plan_from_dict(payload.get('active_plan')), plan_approved=bool(payload.get('plan_approved')), planning_feedback_count=int(payload.get('planning_feedback_count') or 0), planning_tool_limit_feedback_sent=bool(payload.get('planning_tool_limit_feedback_sent')), reflection_count=int(payload.get('reflection_count') or 0), reflections=_safe_dict_list(payload.get('reflections')), context_reports=_safe_dict_list(payload.get('context_reports')), model_usage_reports=_safe_dict_list(payload.get('model_usage_reports')), model_usage_totals=_safe_json_dict(payload.get('model_usage_totals')), plan_execution_feedback_count=int(payload.get('plan_execution_feedback_count') or 0))
...
def _plan_from_dict(payload: Any) -> Plan | None:
    ...
    for index, item in enumerate(_safe_dict_list(payload.get('steps')), start=1):
        ...
        if not isinstance(title, str) or not isinstance(description, str):
            continue
        steps.append(PlanStep(id=str(item.get('id') or index), title=title, description=description, status=str(item.get('status') or 'pending'), depends_on=_safe_string_list(item.get('depends_on')), acceptance_criteria=_safe_string_list(item.get('acceptance_criteria')), retry_limit=int(item.get('retry_limit') or 0), evidence=[_plan_step_evidence_from_dict(record) for record in _safe_dict_list(item.get('evidence'))], started_at=item.get('started_at'), completed_at=item.get('completed_at'), blocked_at=item.get('blocked_at'), blocked_reason=item.get('blocked_reason')))
    if not steps:
        return None
    plan = Plan(summary=summary, steps=steps, created_at=str(payload.get('created_at') or _utc_now()), approved_at=payload.get('approved_at'), rejected_at=payload.get('rejected_at'))
    return plan
...
def _context_section_from_dict(payload: dict[str, Any]) -> TurnContextSection:
    ...
    return TurnContextSection(id=str(payload.get('id') or ''), title=payload.get('title'), source=payload.get('source'), content=str(payload.get('content') or ''), metadata=_safe_json_object(payload.get('metadata')), persist_content=bool(payload.get('persist_content', True)), resource=HostResource.from_dict(resource_value) if isinstance(resource_value, dict) else None)

def _tool_call_from_dict(payload: dict[str, Any]) -> ToolCallRecord:
    return ToolCallRecord(tool_name=str(payload.get('tool_name') or ''), arguments=_safe_json_object(payload.get('arguments')), iteration=int(payload.get('iteration') or 0), phase=str(payload.get('phase') or 'execution'), plan_step_id=payload.get('plan_step_id'), started_at=str(payload.get('started_at') or _utc_now()), ended_at=payload.get('ended_at'), resolved_tool_name=payload.get('resolved_tool_name'), success=payload.get('success'), error=payload.get('error'), failure_kind=payload.get('failure_kind'), metadata=_safe_json_object(payload.get('metadata')))

def _plan_step_evidence_from_dict(payload: dict[str, Any]) -> PlanStepEvidence:
    return PlanStepEvidence(content=str(payload.get('content') or ''), tool_name=payload.get('tool_name'), tool_call_iteration=payload.get('tool_call_iteration'), created_at=str(payload.get('created_at') or _utc_now()), metadata=_safe_json_object(payload.get('metadata')))

def _observation_from_dict(payload: dict[str, Any]) -> ObservationRecord:
    return ObservationRecord(tool_name=str(payload.get('tool_name') or ''), content=str(payload.get('content') or ''), output_metadata=_safe_json_object(payload.get('output_metadata')), created_at=str(payload.get('created_at') or _utc_now()))

def _insert_message(conn: sqlite3.Connection, conversation_id: str, *, turn_id: str | None, role: str, content: str, message_key: str, metadata: dict[str, Any] | None, created_at: str, search_enabled: bool) -> None:
    """Insert one idempotent message using the caller's transaction."""
    next_ordinal = _next_message_ordinal(conn, conversation_id)
    message_id = str(uuid4())
    cursor = conn.execute('\n        INSERT INTO conversation_messages (\n            id, conversation_id, turn_id, role, content, ordinal, message_key, created_at, metadata\n        )\n        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ON CONFLICT(message_key) DO NOTHING\n        ', (message_id, conversation_id, turn_id, role, content, next_ordinal, message_key, created_at, json.dumps(metadata or {}, sort_keys=True)))
    if cursor.rowcount == 1:
        _replace_session_fts(conn, enabled=search_enabled, message_id=message_id, conversation_id=conversation_id, role=role, content=content, metadata=metadata or {})

def _valid_turn_snapshot(turn: dict[str, Any], *, expected_turn_id: str) -> bool:
    return str(turn.get('turn_id', '')).strip() == expected_turn_id and bool(str(turn.get('user_message', '')).strip())

def _save_turn_snapshot(conn: sqlite3.Connection, conversation_id: str, turn: dict[str, Any], updated_at: str) -> None:
    ...
    conn.execute('\n        INSERT INTO conversation_turns (\n            turn_id, conversation_id, user_message, status, started_at, ended_at, final_answer,\n            model_request_count, tool_call_count, loaded_memory_ids, loaded_skill_names,\n            errors, active_plan, turn_json, updated_at\n        )\n        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n        ON CONFLICT(turn_id) DO UPDATE SET\n            user_message = excluded.user_message,\n            status = excluded.status,\n            ended_at = excluded.ended_at,\n            final_answer = excluded.final_answer,\n            model_request_count = excluded.model_request_count,\n            tool_call_count = excluded.tool_call_count,\n            loaded_memory_ids = excluded.loaded_memory_ids,\n            loaded_skill_names = excluded.loaded_skill_names,\n            errors = excluded.errors,\n            active_plan = excluded.active_plan,\n            turn_json = excluded.turn_json,\n            updated_at = excluded.updated_at\n        ', (turn_id, conversation_id, user_message, str(turn.get('status', 'unknown')), str(turn.get('started_at') or updated_at), turn.get('ended_at'), turn.get('final_answer'), int(turn.get('model_request_count') or 0), int(turn.get('tool_call_count') or 0), json.dumps(turn.get('loaded_memory_ids') or [], sort_keys=True), json.dumps(turn.get('loaded_skill_names') or [], sort_keys=True), json.dumps(turn.get('errors') or [], sort_keys=True), json.dumps(active_plan, sort_keys=True) if active_plan else None, json.dumps(turn, sort_keys=True), updated_at))
    _set_conversation_status(conn, conversation_id, _conversation_status_from_turn(turn), updated_at)

def _next_message_ordinal(conn: sqlite3.Connection, conversation_id: str) -> int:
    row = conn.execute('SELECT COALESCE(MAX(ordinal), 0) + 1 AS next_ordinal FROM conversation_messages WHERE conversation_id = ?', (conversation_id,)).fetchone()
    return int(row['next_ordinal'])

def _prompt_source_ordinal(conn: sqlite3.Connection, conversation_id: str, source_message_count: int) -> int:
    """Map a logical prompt-history count to its durable message ordinal."""
    if source_message_count <= 0:
        return 0
    rows = conn.execute('\n        SELECT ordinal, metadata\n        FROM conversation_messages\n        WHERE conversation_id = ?\n        ORDER BY ordinal\n        ', (conversation_id,)).fetchall()
    prompt_ordinals = [int(row['ordinal']) for row in rows if not _message_is_prompt_excluded(row['metadata'])]
...
def _message_is_search_eligible(role: str, metadata: Any) -> bool:
    ...
    if any((values.get(key) is True for key in ('internal', 'prompt_excluded', 'sensitive', 'contains_secrets', 'external_content'))):
        return False
...
def _replace_session_fts(conn: sqlite3.Connection, *, enabled: bool, message_id: str, conversation_id: str, role: str, content: str, metadata: dict[str, Any]) -> None:
    if not enabled:
        return
    conn.execute('DELETE FROM session_messages_fts WHERE message_id = ?', (message_id,))
...
def _touch_conversation(conn: sqlite3.Connection, conversation_id: str, updated_at: str) -> None:
    ...
    if row is not None and (not row['title']):
        ...
        if first_message is not None:
            conn.execute('UPDATE conversations SET title = ? WHERE id = ?', (_title_from_message(first_message['content']), conversation_id))

def _set_conversation_status(conn: sqlite3.Connection, conversation_id: str, status: str, updated_at: str) -> None:
    conn.execute('UPDATE conversations SET status = ?, updated_at = ? WHERE id = ?', (status, updated_at, conversation_id))
