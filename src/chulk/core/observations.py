...
async def format_tool_observation_async(
    *,
    requested_tool_name: str,
    result: ToolResult,
    max_observation_chars: int,
    ...
    """Format an observation while awaiting durable artifact writes."""
    ...
    if result.stdout:
        ...
        metadata["stdout"] = stdout_preview.to_metadata()
        parts.append("stdout:\n" + stdout_preview.text)
        await _append_artifact_note_async(
            parts,
            metadata,
            artifact_writer,
            result.tool_name,
    ...
    metadata["observation"] = observation_preview.to_metadata()
    if observation_preview.truncated:
        artifact = await artifact_writer(
            f"{result.tool_name}-observation",
            full_observation,
        )
        if artifact is not None:
            metadata["artifacts"].append({"field": "observation", **artifact})
            final_observation = _with_required_suffix(
                full_observation,
                suffix=_artifact_note("observation", artifact),
                max_chars=max_observation_chars,
            )
        else:
            final_observation = observation_preview.text
...
async def _append_artifact_note_async(
    parts: list[str],
    metadata: dict[str, Any],
    artifact_writer: AsyncArtifactWriter,
    tool_name: str,
    ...
    if not preview.truncated:
        return
    artifact = await artifact_writer(f"{tool_name}-{field}", content)
    if artifact is None:
        parts.append(
            f"[full {field} omitted from model context; "
            "no artifact writer configured]"
        )
        return
    metadata["artifacts"].append({"field": field, **artifact})
    parts.append(_artifact_note(field, artifact))
