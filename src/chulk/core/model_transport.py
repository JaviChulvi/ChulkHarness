...
from chulk.core.reflection import ReflectionParseError, ReflectionResult, build_reflection_messages, parse_reflection_response
from chulk.core.state import AgentState, TurnState
from chulk.core.trace_format import format_action_trace, format_model_request_trace, format_model_response_trace
from chulk.errors import ConfigurationError, ErrorDetails
from chulk.llm import LLMActionError, LLMActionResult, LLMClient, LLMError, LLMResponse, LLMStreamChunk
from chulk.llm.base import call_async_with_supported_kwargs, call_with_supported_kwargs
from chulk.llm.capabilities import client_requires_mcp_bridge, client_supports_hosted_mcp_tools, client_supports_native_tool_calling
...
AsyncAccountingCallback = Callable[..., Awaitable[tuple[dict | None, dict | None]]]
...
class ModelTransport:
    ...
    def _validate_client(self, client: LLMClient) -> None:
        ...
        if bridge_names and bridge_names.issubset(registered):
            return
        raise RuntimeError('The current LLM client requires MCP bridge tools, but this agent was not assembled with them. Rebuild the agent for the replacement client.')
    ...
    def stream_final_answer(self, draft: str, turn: TurnState) -> FinalAnswerStreamResult:
        ...
        try:
            ...
            self.trace(TraceEvent.MODEL_STREAM_FAILED, {'turn_id': turn.turn_id, 'request_index': request_index, 'source': 'incremental_final_answer', 'error': str(exc), 'partial': bool(parts)})
            raise
    ...
    async def stream_final_answer_async(self, draft: str, turn: TurnState) -> FinalAnswerStreamResult:
        ...
        try:
            ...
            await await_cleanup_after_error(self._release_accounting_async(turn, request_index=request_index, reason='final_answer_stream_idle_timeout' if failure_kind == 'stream_idle_timeout' else 'final_answer_stream_failed'), exc)
            self.trace(TraceEvent.MODEL_STREAM_FAILED, {'turn_id': turn.turn_id, 'request_index': request_index, 'source': 'incremental_final_answer', 'error': str(exc), 'failure_kind': failure_kind, 'partial': bool(parts)})
            await self._flush_async()
            if parts:
    ...
    def _redact_stream_text(self, text: str, sequence: int, turn: TurnState) -> str:
        if self.redact_text is None:
            return text
        permitted, metadata = self.redact_text(TraceEvent.MODEL_STREAM_DELTA, text, {'turn_id': turn.turn_id, 'field': 'text', 'sequence': sequence})
        if metadata.get('redacted') or metadata.get('redaction_error'):
            turn.extension_metadata.setdefault('final_answer_stream_redactions', []).append({'sequence': sequence, **metadata})
        return permitted

    def _apply_output_policy(self, policy: IncrementalOutputPolicy, text: str, sequence: int, turn: TurnState) -> FinalAnswerPolicyDecision:
        chunk = FinalAnswerChunk(text=self._redact_stream_text(text, sequence, turn), sequence=sequence, turn_id=turn.turn_id)
        try:
    ...
    async def _apply_output_policy_async(self, policy: AsyncIncrementalOutputPolicy, text: str, sequence: int, turn: TurnState) -> FinalAnswerPolicyDecision:
        chunk = FinalAnswerChunk(text=self._redact_stream_text(text, sequence, turn), sequence=sequence, turn_id=turn.turn_id)
        try:
    ...
    def _complete_output_policy(self, policy: IncrementalOutputPolicy, sequence: int, turn: TurnState) -> FinalAnswerPolicyDecision:
        try:
    ...
    @staticmethod
    def _reset_output_policy(policy: IncrementalOutputPolicy, turn: TurnState) -> None:
        reset = getattr(policy, 'reset', None)
        if reset is None:
            raise RuntimeError('a buffering output policy must implement reset() for safe fallback')
        reset(turn_id=turn.turn_id)

    async def _complete_output_policy_async(self, policy: AsyncIncrementalOutputPolicy, sequence: int, turn: TurnState) -> FinalAnswerPolicyDecision:
        try:
    ...
    @staticmethod
    async def _reset_output_policy_async(policy: AsyncIncrementalOutputPolicy, turn: TurnState) -> None:
        reset = getattr(policy, 'reset', None)
        if reset is None:
            raise RuntimeError('a buffering async output policy must implement reset() for safe fallback')
        await reset(turn_id=turn.turn_id)

    def _publish_policy_text(self, text: str, turn: TurnState, request_index: int, sequence: int, parts: list[str]) -> int:
        if not text:
            return sequence
        parts.append(text)
        self.trace(TraceEvent.MODEL_STREAM_DELTA, {'turn_id': turn.turn_id, 'request_index': request_index, 'source': 'incremental_final_answer', 'sequence': sequence, 'text': text})
        return sequence + 1

    def _finish_final_answer_stream(self, turn: TurnState, *, request_index: int, content: str, status: FinalAnswerDeliveryStatus, public_delta_count: int, provider_completed: bool, usage: object, cost: object, error: str | None) -> FinalAnswerStreamResult:
    ...
    async def compact_prompt_async(self, prompt: AgentPrompt, turn: TurnState, *, require_plan: bool) -> AgentPrompt:
    ...
    def request_action(self, turn: TurnState, prompt: AgentPrompt, *, require_plan: bool) -> AgentAction | ProtocolFailure:
        ...
        try:
            ...
            self.release_accounting(turn, request_index=turn.model_request_count, reason='model_transport_failed')
            raise
    ...
    async def request_action_async(self, turn: TurnState, prompt: AgentPrompt, *, require_plan: bool) -> AgentAction | ProtocolFailure:
        ...
        try:
            ...
            await await_cleanup_after_error(self._release_accounting_async(turn, request_index=turn.model_request_count, reason='model_transport_failed'), exc)
            raise
    ...
    def reflect(self, proposed_answer: str, turn: TurnState) -> ReflectionResult:
        ...
        try:
            ...
            self.release_accounting(turn, request_index=request_index, reason='reflection_transport_failed')
            return self._fail_open_reflection(turn, proposed_answer, attempt=attempt, error=str(exc), raw_response=None, request_index=request_index)
        except BaseException:
            self.release_accounting(turn, request_index=request_index, reason='reflection_transport_failed')
            raise
        self._record_reflection_response(turn, request_index=request_index, attempt=attempt, raw_response=raw_response, response=response)
    ...
    async def reflect_async(self, proposed_answer: str, turn: TurnState) -> ReflectionResult:
        ...
        try:
            ...
            await self._release_accounting_async(turn, request_index=request_index, reason='reflection_transport_failed')
            return self._fail_open_reflection(turn, proposed_answer, attempt=attempt, error=str(exc), raw_response=None, request_index=request_index)
        except BaseException as exc:
            await await_cleanup_after_error(self._release_accounting_async(turn, request_index=request_index, reason='reflection_transport_failed'), exc)
            raise
    ...
    def _summarize(self, messages: list[dict[str, str]], turn: TurnState) -> ContextSummaryResult:
        ...
        try:
            ...
            self.release_accounting(turn, request_index=request_index, reason='context_summary_transport_failed')
            raise
    ...
    async def _summarize_async(self, messages: list[dict[str, str]], turn: TurnState) -> ContextSummaryResult:
        ...
        try:
            ...
            await await_cleanup_after_error(self._release_accounting_async(turn, request_index=request_index, reason='context_summary_transport_failed'), exc)
            raise
        return await self._finish_summary_async(messages, turn, request_index, response)

    def _summary_request_payload(self, summary_messages: list[dict[str, str]], turn: TurnState, request_index: int, source_message_count: int) -> dict:
        payload = format_model_request_trace(summary_messages, max_prompt_chars=self.trace_max_prompt_chars, request_index=request_index, turn_id=turn.turn_id, loaded_memory_ids=self.state.loaded_memory_ids, loaded_skill_names=self.state.loaded_skill_names, available_tool_names=turn.available_tool_names, context_report={'purpose': 'context_summary', 'source_message_count': source_message_count, 'existing_summary': self.memory.conversation_summary is not None})
        payload['purpose'] = 'context_summary'
        payload['summary_source_message_count'] = source_message_count
        return payload
    ...
    def _summary_failure(self, messages: list[dict[str, str]], turn: TurnState, request_index: int, exc: LLMError) -> ContextSummaryResult:
        self.release_accounting(turn, request_index=request_index, reason='context_summary_transport_failed')
        return self._summary_failure_result(messages, turn, request_index, exc)

    async def _summary_failure_async(self, messages: list[dict[str, str]], turn: TurnState, request_index: int, exc: LLMError) -> ContextSummaryResult:
        await self._release_accounting_async(turn, request_index=request_index, reason='context_summary_transport_failed')
        return self._summary_failure_result(messages, turn, request_index, exc)
    ...
    def _finish_summary(self, messages, turn, request_index, response):
        ...
        usage, cost = self.record_accounting(turn, request_index=request_index, usage=response.usage, cost=response.cost, fallback_attempts=fallback_attempts, purpose='context_summary')
        return self._summary_result(messages, turn, request_index, raw_summary, usage, cost)

    async def _finish_summary_async(self, messages: list[dict[str, str]], turn: TurnState, request_index: int, response: LLMResponse) -> ContextSummaryResult:
        raw_summary = response.content
        fallback_attempts = getattr(self.llm_client, 'last_attempts', None)
        self._record_model_selection_outcome(turn, fallback_attempts)
        usage, cost = await self._record_accounting_async(turn, request_index=request_index, usage=response.usage, cost=response.cost, fallback_attempts=fallback_attempts, purpose='context_summary')
        return self._summary_result(messages, turn, request_index, raw_summary, usage, cost)

    def _ensure_prompt_within_budget(self, turn: TurnState, prompt: AgentPrompt) -> None:
    ...
    def _reject_prompt_for_budget(self, turn: TurnState, context_report: dict) -> None:
        turn.context_reports.append(context_report)
        self.state.last_context_report = context_report
        self.trace(TraceEvent.CONTEXT_BUDGET_REJECTED, {'turn_id': turn.turn_id, 'context_report': context_report})
        raise ConfigurationError('Prompt exceeds the configured input token budget before a provider request.', details=ErrorDetails(failure_kind='context_budget_exceeded', conversation_id=self.state.conversation_id, turn_id=turn.turn_id, extensions={'input_token_budget': context_report['budget']['input_token_budget'], 'estimated_tokens': context_report['budget_estimated_tokens'], 'over_budget_tokens': context_report['over_budget_tokens']}))

    def _action_request_payload(self, turn: TurnState, prompt: AgentPrompt, messages: list[dict[str, str]], context_report: dict, hosted_mcp_enabled: bool) -> dict:
        payload = format_model_request_trace(messages, max_prompt_chars=self.trace_max_prompt_chars, request_index=turn.model_request_count, turn_id=turn.turn_id, loaded_memory_ids=self.state.loaded_memory_ids, loaded_skill_names=self.state.loaded_skill_names, available_tool_names=turn.available_tool_names, context_report=context_report)
        payload['action_transport'] = prompt.action_transport
        payload['hosted_mcp_enabled'] = hosted_mcp_enabled
        payload['hosted_mcp_server_labels'] = [server.label for server in self.mcp_servers] if hosted_mcp_enabled else []
        payload['native_tool_names'] = [str(declaration.get('name', '')) for declaration in prompt.native_tool_declarations]
        payload['native_tool_declarations'] = _bounded_native_tool_declarations(prompt.native_tool_declarations, max_chars=self.trace_max_prompt_chars)
        return payload

    def _record_model_request(self, turn: TurnState, prompt: AgentPrompt, *, hosted_mcp_enabled: bool) -> list[dict[str, str]]:
    ...
    def _record_protocol_failure(self, turn: TurnState, exc: LLMActionError) -> ProtocolFailure:
    ...
    def _prepare_action_result(self, turn: TurnState, result: LLMActionResult) -> object:
        ...
        if fallback_attempts:
            self.trace(TraceEvent.LLM_FALLBACK_ATTEMPTS, {'turn_id': turn.turn_id, 'request_index': turn.model_request_count, 'attempts': [attempt.to_dict() if hasattr(attempt, 'to_dict') else {'attempt': str(attempt)} for attempt in fallback_attempts]})
    ...
    def _publish_reflection_request(self, turn: TurnState, proposed_answer: str, messages: list[dict[str, str]], request_index: int, attempt: int) -> None:
        ...
        self.trace(TraceEvent.REFLECTION_STARTED, {'turn_id': turn.turn_id, 'reflection_attempt': attempt, 'proposed_answer': proposed_answer})
    ...
    async def _start_reflection_async(self, proposed_answer: str, turn: TurnState) -> tuple[int, list[dict[str, str]], int]:
        ...
        try:
            ...
            await await_cleanup_after_error(self._release_accounting_async(turn, request_index=request_index, reason='reflection_setup_failed'), exc)
            raise
        return (attempt, messages, request_index)
    ...
    async def _record_accounting_async(self, turn: TurnState, **kwargs: object) -> tuple[dict | None, dict | None]:
        callback = self.record_accounting_async
        if callback is not None:
            return await callback(turn, **kwargs)
        return await asyncio.to_thread(self.record_accounting, turn, **kwargs)

    async def _release_accounting_async(self, turn: TurnState, **kwargs: object) -> dict | None:
    ...
    def _record_model_selection_outcome(self, turn: TurnState, attempts: object) -> None:
        if not isinstance(attempts, (list, tuple)) or not attempts:
            return
        profile_attempts = [attempt for attempt in attempts if isinstance(getattr(attempt, 'model_profile_id', None), str)]
        if not profile_attempts:
            return
        serialized = [attempt.to_dict() if hasattr(attempt, 'to_dict') else {'attempt': str(attempt)} for attempt in profile_attempts]
        turn.extension_metadata['model_attempts'] = serialized
        selection = turn.extension_metadata.get('model_selection')
        if not isinstance(selection, dict):
            return
        successful = next((attempt for attempt in profile_attempts if getattr(attempt, 'success', False) and isinstance(getattr(attempt, 'model_profile_id', None), str)), None)
        ...
        if selected_id == previous_id:
            return
        failed_count = sum((1 for attempt in profile_attempts if not getattr(attempt, 'success', False)))
        reason = f'runtime fallback selected after {failed_count} failed or unavailable profile(s)'
        selection['reason'] = reason
        self.trace(TraceEvent.MODEL_PROFILE_SELECTED, {'turn_id': turn.turn_id, **selection, 'phase': 'runtime_fallback'})

    def _parse_reflection(self, turn: TurnState, proposed_answer: str, *, attempt: int, raw_response: str, request_index: int) -> ReflectionResult:
        try:
            reflection = parse_reflection_response(raw_response)
        except ReflectionParseError as exc:
            return self._fail_open_reflection(turn, proposed_answer, attempt=attempt, error=str(exc), raw_response=raw_response, request_index=request_index)
        record = {**reflection.to_dict(), 'attempt': attempt, 'proposed_answer': proposed_answer}
        turn.reflections.append(record)
        self.trace(TraceEvent.REFLECTION_COMPLETED, {'turn_id': turn.turn_id, **record})
        return reflection

    def _fail_open_reflection(self, turn: TurnState, proposed_answer: str, *, attempt: int, error: str, raw_response: str | None, request_index: int) -> ReflectionResult:
        reason = f'Reflection failed open: {error}'
        reflection = ReflectionResult(approved=True, reason=reason)
        record = {**reflection.to_dict(), 'attempt': attempt, 'proposed_answer': proposed_answer, 'error': error, 'raw_response': raw_response}
        turn.reflections.append(record)
        turn.errors.append(reason)
        self.trace(TraceEvent.REFLECTION_FAILED, {'turn_id': turn.turn_id, 'request_index': request_index, **record})
        return reflection
    ...
    def _action_schema(self, turn: TurnState, *, require_plan: bool) -> dict:
        action_types: list[str] = []
        if self._action_tools(require_plan=require_plan):
            action_types.append('tool_call')
        planning = self._planning_tool_availability(turn, require_plan=require_plan)
    ...
    @staticmethod
    def _planning_tool_availability(turn: TurnState, *, require_plan: bool) -> PlanningToolAvailability:
        active_plan = turn.active_plan
        return PlanningToolAvailability(propose_plan=require_plan and active_plan is None, update_plan_step=bool(turn.plan_approved and active_plan is not None and (active_plan.active_step() is not None)))

def _safe_hosted_mcp_declaration(server: MCPServerConfig) -> dict:
    declaration = {'type': 'mcp', 'name': f'mcp:{server.label}', 'server_label': server.label, 'server_url': server.server_url, 'require_approval': server.approval, 'authorization_configured': bool(server.authorization)}
...
def _bounded_native_tool_declarations(declarations: list[dict], *, max_chars: int) -> dict:
    serialized = json.dumps(declarations, ensure_ascii=False, sort_keys=True, separators=(',', ':'))
    limit = max(0, max_chars)
    truncated = len(serialized) > limit
    return {'items': None if truncated else declarations, 'json_preview': serialized[:limit] if truncated else None, 'char_count': len(serialized), 'returned_char_count': min(len(serialized), limit), 'truncated': truncated}
...
def _format_action_protocol_failure(error: str, raw_response: str | None) -> str:
    lines = ['Model response was not valid action JSON after repair.', 'I did not execute any tools from the invalid response.', '', f'Error: {error}']
    if raw_response:
        lines.extend(['', 'Unparsed model output:', '', _format_indented_preview(raw_response, MAX_UNPARSED_MODEL_OUTPUT_CHARS)])
    return '\n'.join(lines)
...
def _context_summary_messages(*, previous_summary: str | None, previous_checkpoint: dict[str, object] | None, messages: list[dict[str, str]]) -> list[dict[str, str]]:
    return [{'role': 'system', 'content': 'You update a compact, task-local checkpoint for an agent harness. Return only JSON with these keys: objective, constraints, decisions, completed, blocked, next_actions, evidence_hints. objective is a string; every other key is a list of concise strings. Preserve important decisions and results, but never secrets.'}, {'role': 'user', 'content': _format_context_summary_request(previous_summary=previous_summary, previous_checkpoint=previous_checkpoint, messages=messages)}]

def _format_context_summary_request(*, previous_summary: str | None, previous_checkpoint: dict[str, object] | None, messages: list[dict[str, str]]) -> str:
    sections: list[str] = []
    ...
    sections.extend(['New older messages to fold into the compact summary:', _format_messages_for_summary(messages), '', 'Return only the updated checkpoint JSON.'])
    return '\n'.join(sections)
...
def _fallback_context_checkpoint(previous_checkpoint: dict[str, object] | None, previous_summary: str | None, messages: list[dict[str, str]]) -> dict[str, object]:
    recent = []
...
_CHECKPOINT_KEYS = ('objective', 'constraints', 'decisions', 'completed', 'blocked', 'next_actions', 'evidence_hints')
...
def _parse_checkpoint(value: str) -> dict[str, object] | None:
    ...
    if not isinstance(objective, str) or not objective.strip():
        return None
    if any((key in payload and (not isinstance(payload[key], list) or any((not isinstance(entry, str) for entry in payload[key]))) for key in _CHECKPOINT_LIST_KEYS)):
        return None
...
def _normalize_checkpoint(value: dict[str, object]) -> dict[str, object]:
    ...
    for key in _CHECKPOINT_LIST_KEYS:
        ...
        checkpoint[key] = [_compact_summary_line(_clean_summary_source(str(entry)), limit=300) for entry in entries if str(entry).strip()][:6]
...
def _bound_checkpoint(checkpoint: dict[str, object]) -> None:
    for key in ('evidence_hints', 'completed', 'constraints', 'decisions', 'blocked', 'next_actions'):
        entries = _checkpoint_entries(checkpoint, key)
...
def _redact_summary_text(value: str) -> str:
    patterns = ['(?i)\\b(api[_-]?key|token|password|secret)\\s*[:=]\\s*[^\\s,;]+', '\\bsk-[A-Za-z0-9_-]{16,}\\b']
    redacted = value
    for pattern in patterns:
        redacted = re.sub(pattern, '[redacted secret]', redacted)
    return redacted

async def _next_stream_chunk(stream: AsyncIterator[LLMStreamChunk], *, timeout_seconds: float | None) -> LLMStreamChunk:
    if timeout_seconds is None:
        return await anext(stream)
    try:
        return await asyncio.wait_for(anext(stream), timeout=timeout_seconds)
    except TimeoutError as exc:
        timeout_error = FinalAnswerStreamIdleTimeout(f'Final answer stream stalled for {timeout_seconds:g} seconds without a provider chunk.')
...
def _format_runtime_status(turn: TurnState, *, max_tool_calls_per_turn: int) -> str:
    remaining = max(0, max_tool_calls_per_turn - turn.tool_call_count)
    lines = ['Harness-derived status (authoritative for this request):', f'tool calls: {turn.tool_call_count} used, {remaining} remaining']
    ...
    if failure_sequence is None:
        ...
        lines.append(f'unchanged non-retryable failures: {failure_sequence.count} for {failure_sequence.tool_name} ({failure_sequence.failure_kind}); change the tool or arguments before retrying')
    active_step = turn.active_plan.active_step() if turn.active_plan is not None and turn.plan_approved else None
    lines.append(f'active plan step: {active_step.id} - {active_step.title}' if active_step is not None else 'active plan step: none')
    return '\n'.join(lines)
